...
async function generateSetlistPdf() {
    ...
    for (const slot of setlistSlots) {
        if (slot.classList.contains('setlist-item')) {
            ...
            tableBody.push([
                ...
                songData.bpm || '', 
                chorusDisplay,
                specialEffectLabel // ここに特効のラベルを追加 (未選択なら空白)
            ]);

            if (isAlbum1) {
                simplePdfBody.push(`    ${titleText}${simpleEffectNote}`);
            } else {
                simplePdfBody.push(`${currentItemNoSimple++}   ${titleText}${simpleEffectNote}`);
            }

            if (isAlbum1) {
                shareableTextContent += `    ${titleText}${simpleEffectNote}\n`;
            } else {
                shareableTextContent += `${currentItemNoShareable++}   ${titleText}${simpleEffectNote}\n`;
            }
        } else if (slot.classList.contains('setlist-slot-text')) {
            const textContent = slot.textContent.trim();
            if (textContent) {
                // ★修正3: 詳細PDFの行に「特効」用の空白セルを忘れず追加★
    ...
    try {
        ...
        detailedPdf.autoTable({
            ...
            // ★★★ 修正4: 特効の列 (7番目の列) のスタイルを追加 ★★★
            columnStyles: {
                0: { cellWidth: 10, halign: 'center' },        // No. (fontSize: 9を継承)
                1: { cellWidth: 70, halign: 'left', fontSize: 11 }, // タイトル (fontSize: 11に据え置き)
                2: { cellWidth: 20, halign: 'center', fontSize: 8 },  // R.Gt (サイズを小さく)
                3: { cellWidth: 20, halign: 'center', fontSize: 8 },  // L.Gt (サイズを小さく)
                4: { cellWidth: 20, halign: 'center', fontSize: 8 },  // Ba (サイズを小さく)
                5: { cellWidth: 15, halign: 'center', fontSize: 8 },  // BPM (サイズを小さく)
                6: { cellWidth: 15, halign: 'center', fontSize: 8 },  // コーラス (サイズを小さく)
                7: { cellWidth: 18, halign: 'center', fontSize: 8 }   // 特効 (サイズを小さく)
            },
        ...
        if (numSimpleRows > 0) {
            ...
            simplePdfBody.forEach(line => {
                simplePdf.setFont('NotoSansJP', 'bold'); // 各行の描画前に太字に設定
