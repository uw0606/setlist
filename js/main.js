...
const DRAG_THRESHOLD = 30; // 20ピクセルから30ピクセルに増やしてみる (要調整)
...
function handleTouchMove(event) {
    ...
    if (!isDragging) {
        ...
        // X方向への移動よりもY方向への移動が優先され、かつDRAG_THRESHOLDを超えたらスクロールを許可しない
        // Y方向への大きな移動がない場合は、ブラウザのスクロールを一時的に許可する
        // ただし、既にtouch-action: none; が設定されているため、これは主に視覚的なフィードバックの調整
        if (deltaX < DRAG_THRESHOLD && deltaY < DRAG_THRESHOLD) {
             // ここでは event.preventDefault() を呼ばない。
             // 閾値以下の動きはスクロールとして扱うことを許可する（またはブラウザのデフォルト動作に任せる）
             // ただし、touch-action: none が効いている場合、ここではスクロールは発生しないので注意
            return;
        }
