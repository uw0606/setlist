...
function handleTouchStart(event) {
    ...
    if (isCheckboxClick) {
        ...
        // event.preventDefault(); // チェックボックスクリック時はデフォルト動作を許容するため、ここでも呼ばない
        return;
    }
    ...
    // ★★★ 修正点: ここで event.preventDefault() を呼ばない ★★★
    ...
    initialTouchedElement = touchedElement; // 最初にタッチされた要素を記録
    ...
    if (tapLength < 300 && tapLength > 0) {
        ...
        // ★★★ 修正点: ダブルタップ時のみ preventDefault() を呼ぶ ★★★
    ...
    if (event.touches.length === 1) {
        ...
        // ★★★ 修正点: ここでロングプレス開始タイマーをセットしない ★★★
        // handleTouchMove が移動距離でドラッグ開始を判断する
        if (touchTimeout) {
            clearTimeout(touchTimeout);
            touchTimeout = null;
        }
...
function handleTouchMove(event) {
    ...
    if (!isDragging) {
        ...
        // ★★★ 修正点: ドラッグが開始される瞬間のみ preventDefault() を呼ぶ ★★★
        ...
        // 最初にタッチされた要素 (initialTouchedElement) を使用してドラッグを開始
        if (initialTouchedElement && draggingItemId && document.body.contains(initialTouchedElement)) {
            ...
            return; // ドラッグを開始できない場合はここで終了
        }
    } else {
        // ★★★ 修正点: ドラッグ中は常に preventDefault() を呼ぶ ★★★
        event.preventDefault(); // ドラッグ中はスクロールを停止
    }
...
function handleTouchEnd(event) {
    ...
    // ★★★ 修正点: ドラッグが開始されていなかった場合の処理 ★★★
    if (!isDragging && initialTouchedElement) {
        // ドラッグではなく、単なるタップだった場合
        // ここで何か特定のタップ動作が必要なら追加できるが、
        // 現状はダブルタップで処理されるので、ここでは何もしない
        console.log("[handleTouchEnd] Not a drag, just a tap.");
    }
