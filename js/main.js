...
function handleTouchStart(event) {
    ...
    if (event.touches.length === 1) {
        ...
        // ★修正点1: ここではevent.preventDefault()を呼ばない。スクロールを許可する。
        // ドラッグが実際に開始されたらhandleTouchMoveでpreventDefaultする。
        ...
        touchTimeout = setTimeout(() => {
            // 長押しが確定し、かつ要素がまだ存在する場合にドラッグを開始
            if (draggingItemId && document.body.contains(touchedElement)) {
                // ドラッグ開始時に元のアイテムを隠す
                if (currentTouchDraggedOriginalElement) {
                    // セットリスト内のアイテムの場合
                    if (originalSetlistSlot) {
                        originalSetlistSlot.classList.add('placeholder-slot'); // placeholder-slotを追加
                        originalSetlistSlot.style.visibility = 'hidden'; // 非表示
...
function handleTouchEnd(event) {
    ...
    if (isDragging && (deltaX < dragThreshold && deltaY < dragThreshold)) {
        ...
        if (potentialDropTargetSlot && !potentialDropTargetSlot.classList.contains('setlist-item')) {
            ...
            isDragging = false; // ★isDragging を false にリセット
    ...
    finishDragging(); // ドロップが成功した場合はキャンセルフラグなし
}
