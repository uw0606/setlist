...
function handleTouchStart(event) {
    ...
    if (tapLength < 300 && tapLength > 0) {
        ...
        if (isInSetlist) { // セットリスト内のアイテムをダブルタップ -> メニューに戻す (希望通り)
        ...
        } else if (isInAlbumContent) { // アルバムからのダブルタップ -> セットリストに追加 (希望通り)
    ...
    // セットリスト内のアイテムは長押しでドラッグを開始しない（アルバムに戻らないようにする）
    // アルバムアイテムのみ長押しでドラッグを開始させる
    if (isInSetlist) {
        // セットリスト内のアイテムを長押ししてもドラッグを開始しない
        // ここでは event.preventDefault() を呼び出さないことで、スクロールを許可し、
        // 意図しない「長押し削除」を防ぐ
        console.log("[touchstart] Touched setlist item. No long press drag initiated for setlist items.");
        if (touchTimeout) { // 念のため既存のタイマーをクリア
            clearTimeout(touchTimeout);
            touchTimeout = null;
        }
        isDragging = false; // ドラッグではないのでフラグを確実にfalseに
        return; // セットリストアイテムの長押しではここで処理を終了
    }

    // ここに到達するのはアルバムアイテムがタッチされた場合のみ
    if (isInAlbumContent) {
        event.preventDefault(); // アルバムアイテムのタッチではデフォルト動作を防ぐ（長押しドラッグのため）
        console.log("[touchstart] Prevented default for album item touch (potential long press drag).");
        ...
        console.log(`[touchstart] Setting long press timer for album item drag.`);

        touchTimeout = setTimeout(() => {
            // タイマー発火時に、まだドラッグ中でなく、指が移動閾値を超えていない場合のみドラッグ開始
            // event.touches[0] は setTimeout 内では空になる可能性があるので、
            // handleTouchMove での deltaX/Y のチェックに任せる
            // ここでは単にドラッグ状態に移行する準備をする
            if (!isDragging && draggingItemId && document.body.contains(initialTouchElement)) {
                ...
                console.log("[touchstart] Dragging initiated after timeout and clone created for album item.");
                // 長押し開始時に元のアルバム要素を隠す
        ...
        console.warn("[touchstart] Touched element is not a recognized draggable album item for long press. Aborting.");
