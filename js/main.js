...
/**
 * アルバムの表示を切り替える。(既存のtoggleAlbum関数を新しいロジックで再定義)
 * @param {number} albumNumber - 切り替えるアルバムのインデックス
 */
function toggleAlbum(albumNumber) {
    const content = document.getElementById('album' + albumNumber);
    const tabs = document.querySelectorAll('#albumList li[class$="-tab"]');
    const contents = document.querySelectorAll('#albumList .album-content');
    ...
    if (content && content.style.display === 'none') {
        ...
        const tab = document.querySelector('.album' + albumNumber + '-tab');
        if (tab) tab.classList.add('active');
    }
}

/**
 * 新しく生成された文字順タブ用のトグル関数。
 * @param {string} groupKey - 切り替える文字順グループのキー (例: 'abc_a')
 */
function toggleNameOrderContent(groupKey) {
    const contents = document.querySelectorAll('#nameOrderList .album-content');
    const tabs = document.querySelectorAll('#nameOrderList .album-tab');
    const targetContent = document.getElementById('content-' + groupKey);
    const targetTab = document.getElementById('tab-' + groupKey);
...
/**
 * 文字順リストを生成する関数 (新しい分類ルールを適用)
 * * 💡 依存: NAME_ORDER_GROUPS (initialization.jsで定義), toggleNameOrderContent
 */
function generateNameOrderList() {
    const nameOrderList = document.getElementById('nameOrderList');
    // 全ての曲アイテムのコピーを取得
    const allItems = Array.from(document.querySelectorAll('#albumList .album-content .item'))
                        .map(item => item.cloneNode(true));
    
    nameOrderList.innerHTML = ''; // コンテナをクリア
    
    NAME_ORDER_GROUPS.forEach(group => { // NAME_ORDER_GROUPSは外部依存
        let groupItems = allItems.filter(item => {
            const kanaKey = item.getAttribute('data-kana-sort') || '';
            const alphaKey = item.getAttribute('data-alpha-sort') || '';
            
            const firstCharKana = kanaKey.charAt(0);
            const firstCharAlpha = alphaKey.charAt(0).toUpperCase();

            // === 優先順位 1: 日本語グループ (kana) の判定 ===
            if (group.type === 'kana') {
                 if (firstCharKana === '') return false;
                 // 日本語の範囲比較
                 return firstCharKana >= group.start && firstCharKana <= group.end;
            }
            
            // === 優先順位 2: アルファベットグループ の判定 ===
            if (group.type === 'alpha') {
                 // 日本語キーがあれば、日本語グループに譲る
                 if (kanaKey.length > 0) return false; 
                 
                 if (firstCharAlpha === '') return false;
                 // アルファベットの範囲比較
                 return firstCharAlpha >= group.start && firstCharAlpha <= group.end;
            }
            
            // === 優先順位 3: 数字・記号グループ の判定 (OTHER) ===
            if (group.type === 'other') {
                // 日本語キーがあれば除外
                if (kanaKey.length > 0) return false;

                // アルファベットキーがあれば除外
                if (alphaKey.length > 0) return false;
                
                // どちらのキーもなければ、このグループに含める
                return true; 
            }
            return false;
        });
        ...
        // 曲名順でソート
        groupItems.sort((a, b) => {
            const keyA = (group.type === 'kana') ? a.getAttribute('data-kana-sort') : a.getAttribute('data-alpha-sort');
            const keyB = (group.type === 'kana') ? b.getAttribute('data-kana-sort') : b.getAttribute('data-alpha-sort');
        ...
        // グループに曲が存在する場合のみDOMを生成
        if (groupItems.length > 0) {
            const groupKey = group.key.toLowerCase();
            
            // グループタブ（li）のDOM生成
            const tabLi = document.createElement('li');
            tabLi.textContent = group.label;
            tabLi.className = `album-tab ${groupKey}-tab`; 
            tabLi.id = `tab-${groupKey}`;
            tabLi.addEventListener('click', () => toggleNameOrderContent(groupKey)); // 依存

            // グループ内の曲リストコンテナ（ul.album-content）のDOM生成
            const contentUl = document.createElement('ul');
            contentUl.className = `album-content ${groupKey}`;
            contentUl.id = `content-${groupKey}`;
            contentUl.style.display = 'none'; 
            
            // 曲アイテムをコンテンツに追加
            groupItems.forEach(item => contentUl.appendChild(item));

            nameOrderList.appendChild(tabLi);
            nameOrderList.appendChild(contentUl);
        }
    });
    
    // 初回のタブを開く (例: 最初のグループ)
    if (nameOrderList.children.length > 0) {
        const firstTab = nameOrderList.querySelector('.album-tab');
        if (firstTab) firstTab.click(); // toggleNameOrderContent を呼び出す
    }
}
...
// --- 文字順ソート関連の定数 ---
const NAME_ORDER_GROUPS = [
    // ABC グループ (大文字 A-Z)
    { key: 'ABC_A', label: 'A・B・C・D・E', start: 'A', end: 'E', type: 'alpha' },
    { key: 'ABC_F', label: 'F・G・H・I・J', start: 'F', end: 'J', type: 'alpha' },
    { key: 'ABC_K', label: 'K・L・M・N・O', start: 'K', end: 'O', type: 'alpha' },
    { key: 'ABC_P', label: 'P・Q・R・S・T', start: 'P', end: 'T', type: 'alpha' },
    { key: 'ABC_U', label: 'U・V・W・X・Y・Z', start: 'U', end: 'Z', type: 'alpha' },
    
    // 日本語 (あ行〜わ行)
    { key: 'KANA_A', label: 'あいうえお', start: 'あ', end: 'お', type: 'kana' },
    { key: 'KANA_KA', label: 'かきくけこ', start: 'か', end: 'こ', type: 'kana' },
    { key: 'KANA_SA', label: 'さしすせそ', start: 'さ', end: 'そ', type: 'kana' },
    { key: 'KANA_TA', label: 'たちつてと', start: 'た', end: 'と', type: 'kana' },
    { key: 'KANA_NA', label: 'なにぬねの', start: 'な', end: 'の', type: 'kana' },
    { key: 'KANA_HA', label: 'はひふへほ', start: 'は', end: 'ほ', type: 'kana' },
    { key: 'KANA_MA', label: 'まみむめも', start: 'ま', end: 'も', type: 'kana' },
    { key: 'KANA_YA', label: 'やゆよ', start: 'や', end: 'よ', type: 'kana' },
    { key: 'KANA_RA', label: 'らりるれろ', start: 'ら', end: 'ろ', type: 'kana' },
    { key: 'KANA_WA', label: 'わおん', start: 'わ', end: 'ん', type: 'kana' },
    
    // 数字・記号
    { key: 'OTHER', label: '数字・記号', type: 'other' } 
];


// (依存: setlist, enableDragAndDrop, updateDays, openModalWithBodyClass, closeModalWithBodyClass, loadSetlistState, hideSetlistItemsInMenu)
// (★★★ 追加依存: generateNameOrderList, toggleAlbum, toggleNameOrderContent)

document.addEventListener('DOMContentLoaded', () => {
    console.log("[DOMContentLoaded] Page loaded. Initializing application.");

    // ★ グローバル変数 setlist の初期化 (外部で定義されていることを前提とする)
    ...
    if (setlist) {
        setlist.querySelectorAll(".setlist-slot").forEach((slot, index) => {
            ...
            enableDragAndDrop(slot); // 依存
        });
    } 
    
    // Global dragend listener (finishDragging は外部で定義されていることを前提とする)
    ...
    if (setlistYear) setlistYear.addEventListener('change', updateDays); // 依存
    if (setlistMonth) setlistMonth.addEventListener('change', updateDays); // 依存
    ...
    if (openPastSetlistsModalButton && pastSetlistsModal && closePastSetlistsModalButton) {
        openPastSetlistsModalButton.addEventListener('click', () => openModalWithBodyClass('pastSetlistsModal')); // 依存
        closePastSetlistsModalButton.addEventListener('click', () => closeModalWithBodyClass('pastSetlistsModal')); // 依存
        pastSetlistsModal.addEventListener('click', (event) => {
            if (event.target === pastSetlistsModal) closeModalWithBodyClass('pastSetlistsModal'); // 依存
        });
    }
    ...
    if (year2025DetailModal && close2025DetailModalButton) {
        if (open2025FromPastModalButton) {
            open2025FromPastModalButton.addEventListener('click', () => {
                closeModalWithBodyClass('pastSetlistsModal'); // 依存
                openModalWithBodyClass('year2025DetailModal'); // 依存
            });
        }
        close2025DetailModalButton.addEventListener('click', () => closeModalWithBodyClass('year2025DetailModal')); // 依存
        year2025DetailModal.addEventListener('click', (event) => {
            if (event.target === year2025DetailModal) closeModalWithBodyClass('year2025DetailModal'); // 依存
        });
    }
    ...
    if (year2024DetailModal && close2024DetailModalButton) {
        if (open2024FromPastModalButton) {
            open2024FromPastModalButton.addEventListener('click', () => {
                closeModalWithBodyClass('pastSetlistsModal'); // 依存
                openModalWithBodyClass('year2024DetailModal'); // 依存
            });
        }
        close2024DetailModalButton.addEventListener('click', () => closeModalWithBodyClass('year2024DetailModal')); // 依存
        year2024DetailModal.addEventListener('click', (event) => {
            if (event.target === year2024DetailModal) closeModalWithBodyClass('year2024DetailModal'); // 依存
        });
    }
    ...
    document.querySelectorAll('.setlist-link').forEach(link => {
        link.addEventListener('click', (event) => {
            ...
            if (shareIdMatch) {
                ...
                // loadSetlistState は外部で定義されていることを前提とする
                loadSetlistState().then(() => { 
                    console.log(`[setlist-link click] Setlist loaded from shareId: ${shareId}`);
                    // ロードが完了したら、両方のモーダルが閉じていることを確認
                    closeModalWithBodyClass('pastSetlistsModal'); // 依存
                    closeModalWithBodyClass('year2025DetailModal'); // 依存
                    closeModalWithBodyClass('year2024DetailModal'); // 依存
    ...
    if (sortSelect && albumList && nameOrderList) {
        
        // 初期化: nameOrderListを生成しておく (generateNameOrderListは setlist_item_management.js で定義)
        generateNameOrderList();

        // ソート順が変更されたときの処理
        sortSelect.addEventListener('change', (event) => {
            const selectedOrder = event.target.value;
            
            // 既存のアルバムコンテンツをすべて非アクティブにする（保険）
            document.querySelectorAll('#albumList .album-content').forEach(c => c.style.display = 'none');
            document.querySelectorAll('#albumList li[class$="-tab"]').forEach(t => t.classList.remove('active'));

            if (selectedOrder === 'album') {
                // アルバム順に戻す
                albumList.style.display = 'block';
                albumList.classList.add('active');
                nameOrderList.style.display = 'none';
                nameOrderList.classList.remove('active');
                
                // 最初のアルバムを開く（もしあれば）
                const firstAlbumTab = albumList.querySelector('li[class$="-tab"]');
                // toggleAlbumは setlist_item_management.js で定義
                // データ属性がない場合はデフォルトで1を使用
                if(firstAlbumTab) toggleAlbum(firstAlbumTab.dataset.albumNumber || 1); 

            } else if (selectedOrder === 'name_order') {
                // 文字順に切り替える
                albumList.style.display = 'none';
                albumList.classList.remove('active');
                nameOrderList.style.display = 'block';
                nameOrderList.classList.add('active');

                // 文字順のリストを表示し、最初のタブを開く
                if (nameOrderList.children.length === 0) {
                    generateNameOrderList(); // 何らかの理由でリストが空なら再生成
                }
                
                // 初回のタブを開く (toggleNameOrderContentは setlist_item_management.js で定義)
                const firstTab = nameOrderList.querySelector('.album-tab');
                if (firstTab) firstTab.click();
            }
        });

        // 初期状態で「アルバム順」が選ばれていれば、最初のアルバムを開く
    ...
    loadSetlistState().then(() => { 
        ...
        // hideSetlistItemsInMenu は外部で定義されていることを前提とする
        hideSetlistItemsInMenu(); 
