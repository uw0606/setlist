...
function processDrop(draggedItem, dropTargetSlot) {
  ...
  // ここを修正: draggedgedItem を draggedItem に変更
...
function getSlotItemData(slotElement) {
    // 修正: itemInSlot は slotElement 自身であるべきです
    // fillSlotWithItem は li.setlist-slot に直接 setlist-item クラスを付与しているため
    ...
    const itemInSlot = slotElement; // ここを修正

    const songInfo = itemInSlot.querySelector('.song-info');
    // songInfo の最初のテキストノードからテキストを取得（以前の修正をここにも適用）
    ...
    // albumClass は itemInSlot (li要素) のクラスから取得する
    ...
    return {
        ...
        itemId: itemInSlot.dataset.itemId,
        slotIndex: slotElement.dataset.slotIndex // スロットインデックスも保持
    };
}
 ...
 * 例: { itemId: "...", name: "曲名", albumClass: "...", short: true/false }
 */
function fillSlotWithItem(slotElement, songData) {
  ...
  const isCurrentlyChecked = songData.short; // songData.short は、移動前のチェック状態を示す

  // 元のアルバムアイテムが 'short' クラスを持つかを確認
  // これは、その曲が「ショートバージョン」であるという固定の属性情報
  const originalAlbumItem = document.querySelector(`.album-content .item[data-item-id="${itemId}"]`);
  const isOriginalAlbumShort = originalAlbumItem ? originalAlbumItem.classList.contains('short') : false;

  // その曲が「ショートバージョン」であるとみなすべきかどうか
  // -> アルバムに元々shortクラスがあるか、またはセットリストに存在しチェックボックスがあるか
  // ここでの 'hasShortAttribute' は、UI上のチェック状態に関わらず、
  // この曲が「ショートバージョン」のオプションを持つべきかを判断するものです。
  // 移動元のアイテムが元々ショートだった場合（アルバム由来）、または、
  // セットリスト内で一度でもショートとして認識されたことのあるアイテムの場合に true とします。
  // （今回は simple に isOriginalAlbumShort で判断します。複雑なロジックはバグの温床になります。）
  const hasShortAttribute = isOriginalAlbumShort;
  ...
  // ★ 修正点1: チェックボックスと (Short) ラベルを表示する条件を「その曲がショート属性を持つか」にする
  // これにより、チェック状態に関わらず、ショートバージョンであれば常にUI要素が表示される
  if (hasShortAttribute) {
    const checkbox = document.createElement('input');
    checkbox.type = 'checkbox';
    // チェックボックスの初期状態は、songData から受け取った移動前の状態 (isCurrentlyChecked) に基づいて設定
    ...
    checkbox.addEventListener('click', (e) => {
        e.stopPropagation();
        // チェックボックスの状態に応じてスロットに 'short' クラスを追加/削除する
  ...
  // ★ 修正点2: スロットに 'short' クラスを付与するロジックを変更
  // ユーザーがチェックボックスをチェックした状態（isCurrentlyChecked）を反映させる
  // これにより、CSSのスタイル（文字の打ち消し線、色など）が正確に適用される
  ...
  // イベントリスナーの削除と再設定はここで行われているので問題なし
...
function restoreToOriginalList(item) {
  ...
  // まず、アルバム内の非表示になっている元のアイテムを探して表示に戻す
  ...
  if (albumItemInMenu) {
    ...
    // アルバム内に元のアイテムが見つからない場合は、警告を出しつつ何もしない（通常は発生しないはず）
  ...
  if (isInsideSetlist) {
    ...
    if (!setlist.querySelector(`.setlist-slot.setlist-item[data-item-id="${item.dataset.itemId}"]`)) {
      ...
      const itemData = {
        ...
        // ★ 修正4: short の値は、アルバムの元々のshortクラスの状態をそのまま渡す
...
function shareSetlist() {
  ...
  setlistRef.set(currentState)
    .then(() => {
        ...
        // ★ 修正点: Web Share API を使用
        if (navigator.share) {
            navigator.share({
                title: '仮セトリ', // 共有ダイアログのタイトル
                text: `セットリストを共有します！\n\n${songListText}`, // 共有されるテキスト
                url: shareLink, // 共有されるURL
            })
            .then(() => {
                console.log('[shareSetlist] Web Share API Success');
                showMessageBox('セットリストを共有しました！'); // 共有成功時のメッセージ
            })
            .catch((error) => {
                console.error('[shareSetlist] Web Share API Failed:', error);
                // ユーザーが共有をキャンセルした場合もエラーになるため、メッセージは慎重に
                if (error.name !== 'AbortError') { // ユーザーキャンセルは通常エラーとして扱わない
                    showMessageBox('共有に失敗しました。');
                }
            });
        } else {
            // Web Share API が利用できない場合のフォールバック（以前のクリップボードコピー）
            const tempInput = document.createElement('textarea');
            tempInput.value = shareLink; // URLのみをコピー
        ...
        if (state && state.setlist) {
          ...
          state.setlist.forEach(itemData => {
            ...
            if (targetSlot) {
                ...
                // itemData はすでに必要なデータを持つオブジェクトなので、そのまま渡せる
