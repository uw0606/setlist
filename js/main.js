...
function processDrop(draggedElement, dropTargetSlot, originalSourceSlot) {
    ...
    if (originalSourceSlot) { // セットリスト内からの移動または入れ替え
        ...
        if (dropTargetSlot.classList.contains('setlist-item')) {
            ...
            // 元のスロットのデータを移動先のスロットに設定
            // ★修正: addSongToSlot の引数の順序を合わせる★
            addSongToSlot(
                targetSongData.name, // songName
                originalSourceSlot.dataset.slotIndex, // slotIndex (元のスロットのインデックスに、入れ替える曲を設定)
                targetSongData.albumClass, // albumClass
                targetSongData.itemId, // itemId
                {
                    isShortVersion: targetSongData.hasShortOption,
                    hasSeOption: targetSongData.hasSeOption,
                    drumsoloOption: targetSongData.hasDrumsoloOption, // ここは drumsoloOption で統一
            ...
            // ドロップされた曲のデータを移動先のスロットに設定
            // ★修正: addSongToSlot の引数の順序を合わせる★
            addSongToSlot(
                songData.name, // songName
                dropTargetSlot.dataset.slotIndex, // slotIndex (ドロップ先のスロットのインデックスに、ドロップされた曲を設定)
                finalAlbumClass, // albumClass (draggedElementのアルバムクラス)
                songData.itemId, // itemId
                {
                    isShortVersion: songData.hasShortOption,
                    hasSeOption: songData.hasSeOption,
                    drumsoloOption: songData.hasDrumsoloOption, // ここは drumsoloOption で統一
                    rGt: songData.rGt,
                    lGt: songData.lGt,
                    bass: songData.bass,
                    bpm: songData.bpm,
            ...
            addSongToSlot(
                songData.name, // songName
                dropTargetSlot.dataset.slotIndex, // slotIndex
                finalAlbumClass, // albumClass
                songData.itemId, // itemId
                {
                    isShortVersion: songData.hasShortOption,
                    hasSeOption: songData.hasSeOption,
            ...
            // 元のスロットをクリア (これは addSongToSlot の中で処理されるべきかもしれませんが、
            // 明示的にクリアするならば、ここで getSlotItemData を呼び出す必要はありません)
            // 単にコンテンツを空にするだけであれば clearSlotContent(originalSourceSlot) でOK
            clearSlotContent(originalSourceSlot); 

            showMessage("セットリスト内の曲を移動しました。", "success");
        }
    } else { // アルバムからの追加 (この部分は問題ないと見ています)
        ...
        if (dropTargetSlot.classList.contains('setlist-item')) {
            ...
            addSongToSlot(
                songData.name,
                dropTargetSlot.dataset.slotIndex,
                finalAlbumClass,
                songData.itemId,
                {
                    isShortVersion: songData.hasShortOption,
                    ...
                    bpm: songData.bpm,
                    chorus: songData.chorus,
                    short: false, // アルバムからの追加なので初期値はfalse
                    seChecked: false, // アルバムからの追加なので初期値はfalse
                    drumsoloChecked: false // アルバムからの追加なので初期値はfalse
                }
            );
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (setlist) {
        setlist.querySelectorAll(".setlist-slot").forEach((slot, index) => {
            ...
            // ★★★ この slot.addEventListener('click', ...) のブロックを削除します ★★★
