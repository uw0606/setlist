...
function getSlotItemData(slotElement) {
    if (!slotElement || !slotElement.dataset.itemId) {
        if (slotElement && slotElement.classList.contains('setlist-slot-text')) {
            return { type: 'text', textContent: slotElement.textContent.trim(), slotIndex: slotElement.dataset.slotIndex };
        }
        return null;
    }
    return {
        itemId: slotElement.dataset.itemId,
        name: slotElement.dataset.name,
        type: slotElement.dataset.type || 'song',
        short: slotElement.dataset.short === 'true',
        seChecked: slotElement.dataset.seChecked === 'true',
        drumsoloChecked: slotElement.dataset.drumsoloChecked === 'true',
        rGt: slotElement.dataset.rGt || '',
        lGt: slotElement.dataset.lGt || '',
        bass: slotElement.dataset.bass || '',
        bpm: slotElement.dataset.bpm || '',
        chorus: slotElement.dataset.chorus || '',
...
/**
 * 要素を元のリストに戻す関数 (アルバムに戻す)
 * @param {Element} elementOrSlot - セットリスト内のスロット要素、またはアルバム内のアイテム要素
 * @param {boolean} clearSlotContentFlag - elementOrSlotがセットリストスロットの場合、内容をクリアするかどうか
 */
function restoreToOriginalList(elementOrSlot, clearSlotContentFlag) {
    const itemId = elementOrSlot.dataset.itemId;
    if (!itemId) {
        console.warn("[restoreToOriginalList] No itemId found on element or slot to restore.");
        // スロットクリアフラグがあれば、スロットだけはクリア
        if (clearSlotContentFlag && elementOrSlot.classList.contains('setlist-slot')) {
            clearSlotContent(elementOrSlot);
        }
        return;
    }
    ...
    if (albumItem) {
        albumItem.style.visibility = ''; // アルバム内のアイテムを再表示
        console.log(`[restoreToOriginalList] Restored album item: ${itemId}`);
    } else {
        console.warn(`[restoreToOriginalList] Original album item for ${itemId} not found in menu.`);
    }

    if (clearSlotContentFlag && elementOrSlot.classList.contains('setlist-slot')) {
        clearSlotContent(elementOrSlot); // セットリストスロットの内容をクリア
        console.log(`[restoreToOriginalList] Cleared setlist slot: ${elementOrSlot.dataset.slotIndex || 'N/A'}`);
    } else if (elementOrSlot.classList.contains('setlist-item')) {
        // elementOrSlot が setlist-item だが clearSlotContentFlag が false の場合
        // これは double-click で削除された場合に呼ばれるので、スロットをクリアする
        clearSlotContent(elementOrSlot);
        console.log(`[restoreToOriginalList] Cleared setlist item (double-click delete): ${elementOrSlot.dataset.slotIndex || 'N/A'}`);
    }
}

// ==============================================
// 既存のDOMContentLoadedイベントリスナー内の変更点
// ==============================================

document.addEventListener('DOMContentLoaded', () => {
    // ... (既存の日付ドロップダウン初期化、その他の初期化コード) ...
    ...
    setlist.querySelectorAll(".setlist-slot").forEach((slot, index) => {
      ...
      slot.addEventListener('click', (e) => {
          // ドラッグがアクティブな場合は、クリックイベントを無視
          if (isDragging) {
              e.preventDefault();
              e.stopPropagation();
              console.log("[slotClick] Click ignored because drag is active.");
              return;
          }
          ...
          if (checkbox) {
              ...
              if (optionType === 'short') {
                  ...
                  if (itemData) itemData.drumsoloChecked = checkbox.checked;
              }
...
function fillSlotWithItem(slotElement, itemData) {
    if (!slotElement || !itemData) return;

    if (itemData.type === 'text') {
        slotElement.textContent = itemData.textContent;
        slotElement.classList.add('setlist-slot-text');
        slotElement.classList.remove('setlist-item'); // テキストなのでsetlist-itemを外す
        slotElement.innerHTML = `<span>${itemData.textContent}</span>`; // テキストスロットはシンプルに
        // オプション表示を消す
        slotElement.querySelector('.slot-options')?.remove();
        return;
    }

    slotElement.classList.remove('setlist-slot-text'); // テキストスロットクラスを外す
    slotElement.classList.add('setlist-item');
    slotElement.dataset.itemId = itemData.itemId;
    slotElement.dataset.name = itemData.name;
    slotElement.dataset.type = itemData.type || 'song';
    ...
    // スロットの表示内容を更新 (HTML構造を考慮)
    let contentHtml = `<span class="item-name">${itemData.name}</span>`;

    // オプションチェックボックスのコンテナを生成
    let optionsHtml = `
        <div class="slot-options">
            <label class="option-label">Short<input type="checkbox" data-option-type="short"${itemData.short ? ' checked' : ''}></label>
            <label class="option-label">SE有<input type="checkbox" data-option-type="se"${itemData.seChecked ? ' checked' : ''}></label>
            <label class="option-label">ドラムソロ有<input type="checkbox" data-option-type="drumsolo"${itemData.drumsoloChecked ? ' checked' : ''}></label>
            <button class="clear-slot-button" aria-label="Clear slot">×</button>
        </div>
    `;

    slotElement.innerHTML = contentHtml + optionsHtml;
    ...
    // クリアボタンにイベントリスナーを再アタッチ
    const clearButton = slotElement.querySelector('.clear-slot-button');
    if (clearButton) {
        clearButton.onclick = (e) => {
            e.stopPropagation(); // 親要素のクリックイベントが発火しないように
            console.log(`[clearButton] Clearing slot ${slotElement.dataset.slotIndex}.`);
            restoreToOriginalList(slotElement, true); // スロットをクリアし、アルバムに戻す
        };
    }
}
...
/**
 * ドロップ処理
 * @param {Element} draggedElement - ドラッグされた要素（モバイルクローン or PC元の要素）
...
function processDrop(draggedElement, dropTargetSlot, originalSetlistSlot = null) {
    ...
    if (!itemId) {
        console.error("[processDrop] draggedElement has no itemId. Aborting processDrop and cleaning up.");
        finishDragging(); 
        return;
    }

    let draggedItemData = null;
    // originalSetlistSlot があれば、そこから元のデータを取得 (セットリスト内移動用)
    if (originalSetlistSlot && originalSetlistSlot._originalItemData) {
        ...
        draggedItemData = getSlotItemData(document.querySelector(`.album-content .item[data-item-id="${itemId}"]`) || draggedElement);
        // draggedElementがクローンの場合、getSlotItemDataは空を返す可能性があるので、元のアルバムアイテムから取得
        if (!draggedItemData && draggedElement.dataset.name) { // クローンから直接取得できる場合
             draggedItemData = { ...draggedElement.dataset }; // データセットを直接コピー
             draggedItemData.name = draggedElement.dataset.name; // nameは必須
             draggedItemData.itemId = draggedElement.dataset.itemId;
             draggedItemData.type = draggedElement.dataset.type || 'song';
             draggedItemData.short = draggedElement.dataset.short === 'true';
             draggedItemData.seChecked = draggedElement.dataset.seChecked === 'true';
             draggedItemData.drumsoloChecked = draggedElement.dataset.drumsoloChecked === 'true';
             // その他のオプションも同様にコピー
             draggedItemData.rGt = draggedElement.dataset.rGt || '';
             draggedItemData.lGt = draggedElement.dataset.lGt || '';
             draggedItemData.bass = draggedElement.dataset.bass || '';
             draggedItemData.bpm = draggedElement.dataset.bpm || '';
             draggedItemData.chorus = draggedElement.dataset.chorus || '';

             console.log("[processDrop] Reconstructed item data from dragged clone dataset:", draggedItemData);

        } else if (!draggedItemData) {
            console.error("[processDrop] Could not retrieve dragged item data. Aborting processDrop.");
            finishDragging();
            return;
        }
    }
    ...
    if (!draggedItemData) {
        console.error("[processDrop] Final check: Item data is null. Aborting.");
        finishDragging();
        return;
    }

    const isDraggedFromSetlist = !!originalSetlistSlot; // originalSetlistSlotが存在すればセットリストから
    ...
    if (dropTargetSlot) {
        ...
        if (isDraggedFromSetlist && dropTargetSlot.dataset.slotIndex === originalSetlistSlot.dataset.slotIndex) {
            console.log("[processDrop] Dropped back to original slot. No change. Restoring original slot content.");
            // 元のスロットの内容を復元し、表示状態も戻す
            ...
            delete originalSetlistSlot._originalItemData; // 一時データは削除
            finishDragging();
            return;
        }

        if (isDraggedFromSetlist) {
            // セットリスト内での移動 (スワップ or 空きスロットへ移動)
            if (dropTargetSlot.classList.contains('setlist-item')) {
                ...
                fillSlotWithItem(originalSetlistSlot, targetSlotItemData); // 元のスロットにターゲットのアイテムを入れる
                ...
                clearSlotContent(originalSetlistSlot); // 元のスロットをクリア
                fillSlotWithItem(dropTargetSlot, draggedItemData); // ターゲットスロットにドラッグされたアイテムを入れる
            }
        } else {
            // アルバムからのドロップ
            ...
            // アルバムリスト内の元のアイテムは finishDragging で隠されるので、ここでは何もしない
        }
    } else {
        ...
        if (isDraggedFromSetlist) {
            ...
            console.log(`[processDrop] Item from setlist (${itemId}) dropped outside. Clearing original slot and restoring to album.`);
            restoreToOriginalList(originalSetlistSlot, true); // originalSetlistSlotをクリアし、アイテムをアルバムに戻す
        } else {
...
function handleTouchStart(event) {
    ...
    if (closestCheckbox) {
        ...
        isDragging = false; // ドラッグ状態を確実にリセット
        // チェックボックスの場合はデフォルト動作を許可し、イベントを終了
        return; 
    }
    ...
    // ドラッグ中の再タッチを無視
    if (isDragging) {
        console.log("[touchstart] Already dragging, ignoring new touchstart.");
        event.preventDefault(); // すでにドラッグ中なら、予期せぬ動作を防ぐためにデフォルトを阻止
        return;
    }
    ...
    if (tapLength < 300 && tapLength > 0) { // 300ms 以内の2回目のタップ
        ...
        if (touchTimeout) {
            clearTimeout(touchTimeout); // 長押しタイマーをキャンセル
            touchTimeout = null;
        }
        event.preventDefault(); // ダブルタップ時はデフォルトのズームなどを防ぐ
        event.stopPropagation(); // イベントの伝播は停止
        ...
        if (isInSetlist) { // セットリスト内のアイテムをダブルタップ -> メニューに戻す
            console.log("[touchstart] Double tap on setlist item. Restoring to original list.");
            restoreToOriginalList(touchedElement, true); // true を渡してスロットをクリア
            showMessageBox('曲をセットリストから削除しました。');
        } else if (isInAlbumContent) { // アルバムからのダブルタップ -> セットリストに追加
        ...
        lastTapTime = 0; // ダブルタップ処理が完了したのでリセット
        return; // ダブルタップ処理が実行されたら、ここで関数を終了
    }
    lastTapTime = currentTime; // 最初のタップ時刻を記録

    // --- 2. シングルタッチからの長押し（ドラッグ開始）処理 ---
    // ここではまだ event.preventDefault() を呼ばない。
    // スクロール可能な領域に触れていれば、この時点ではまだスクロールを許可する。
    // ドラッグが開始されると確定した時点で preventDefault を呼ぶ。
    ...
    // セットリストからのドラッグの場合、元のスロットを記憶し、データを保存
    const isInSetlist = setlist.contains(touchedElement) && touchedElement.classList.contains('setlist-item');
    if (isInSetlist) {
        originalSetlistSlot = touchedElement;
        originalSetlistSlot._originalItemData = getSlotItemData(originalSetlistSlot);
        console.log("[touchstart] Setlist item touched. Saving original data for drag.");
    } else {
        originalSetlistSlot = null; // アルバムからのドラッグ
    }
    initialTouchElement = touchedElement; // タッチ開始時の要素を保持
    ...
    touchTimeout = setTimeout(() => {
        // タイマー発火時、まだドラッグ中でなく、指の移動が閾値を超えていない場合のみドラッグ開始
        // handleTouchMoveで閾値を超えたらタイマーがクリアされるので、ここでは既に移動が閾値内であることを前提
        if (!isDragging && draggingItemId && document.body.contains(initialTouchElement)) {
            isDragging = true; // ドラッグ状態に移行
            
            // ★ここで preventDefault を呼び出し、ドラッグ中のデフォルト動作（スクロールなど）を抑制★
            // ただし、この preventDefault はタイマー発火時にしか呼び出されないため、
            // タイマーが発火するまでの短い間のスクロールは可能になる。
            // より厳密には、createTouchDraggedClone 内で preventDefault を呼ぶか、
            // handleTouchMove で isDragging が true の場合のみ preventDefault を呼ぶのが良い。
            // 今回は handleTouchMove の修正で対応します。

            createTouchDraggedClone(initialTouchElement, touchStartX, touchStartY, draggingItemId);
            
            // 長押し開始時に元の要素を隠す
            if (originalSetlistSlot) { // セットリストからのドラッグ
                originalSetlistSlot.classList.add('placeholder-slot');
                originalSetlistSlot.style.visibility = 'hidden';
                console.log(`[touchstart] Original setlist slot ${originalSetlistSlot.dataset.slotIndex} marked as placeholder and hidden.`);
            } else { // アルバムからのドラッグ
                initialTouchElement.style.visibility = 'hidden';
                console.log(`[touchstart] Original album item ${initialTouchElement.dataset.itemId} hidden.`);
            }
            console.log("[touchstart] Dragging initiated after timeout and clone created.");
        } else {
            console.warn("[touchstart] Dragging not initiated after timeout (condition unmet or moved too much).");
            finishDragging(); // 長押し失敗時はクリーンアップ
        }
        touchTimeout = null; // タイマーは役目を終えたのでクリア
    }, 600); // 600msの長押しでドラッグ開始
}
...
function handleTouchMove(event) {
    ...
    // ★ここを修正★ ドラッグがアクティブな場合のみ preventDefault を呼び出す
    if (isDragging) {
        event.preventDefault(); 
        event.stopPropagation(); // ドラッグ中はイベントの伝播も停止
    } else {
        // ドラッグ中でなければ、スクロールなどのデフォルト動作を許可する
        rafId = null; // リクエストフレームをクリアして終了
        return;
    }
    ...
    rafId = requestAnimationFrame(() => {
        ...
        // ここは isDragging が true のパスしか来ないはずなので、このブロックは不要になります。
        // もし残すのであれば、isDragging のチェックを改めて行う必要がありますが、
        // 上記の preventDefault の条件でほとんど制御されるため、現状のままでも動作は変わらないでしょう。
        // ただし、isDragging が false でここに到達するロジックは今後見直す可能性があります。
        
        if (!currentTouchDraggedClone) {
            console.warn("[handleTouchMove] Dragging clone is null, but isDragging is true. Aborting move.");
            finishDragging(); // クローンがないのにisDragging=trueはおかしいのでクリーンアップ
            rafId = null;
            return;
        }
        ...
        if (newDropZone !== currentDropZone) {
            ...
            if (newDropZone) {
                // 元のスロット自身をハイライトしない (セットリスト内移動の場合)
                if (originalSetlistSlot && newDropZone === originalSetlistSlot) {
                    newDropZone.classList.remove('drag-over'); // 元のスロットならハイライトしない
                } else {
                    newDropZone.classList.add('drag-over');
                }
            }
...
function handleTouchEnd(event) {
    ...
    if (isDragging && currentTouchDraggedClone) { 
        ...
        // ドラッグが開始されていなかった（＝単に短タップまたは移動が閾値未満で指を離した）場合のクリーンアップ
...
function createTouchDraggedClone(originalElement, initialX, initialY, itemIdToClone) {
    ...
    // ★元の要素を隠す処理はここから削除し、handleTouchStartのsetTimeout内に移動します。
    // if (setlist.contains(originalElement) && originalElement.classList.contains('setlist-item')) {
    //     originalSetlistSlot = originalElement;
...
function finishDragging() {
  ...
  if (draggedElement && draggedElement.classList.contains("dragging")) {
    ...
    console.log(`[finishDragging] Removed 'dragging' class from PC dragged element: ${draggedElement.dataset.itemId || 'N/A'}`);
  }
  ...
  // ここでは、アイテムが既に移動・削除されたかどうかの判断はしない
  // visibilityの復元のみに限定する (restoreToOriginalListはprocessDropで呼び出される)
  ...
  isDragging = false; 
  draggingItemId = null; 
  initialTouchElement = null;
  originalSetlistSlot = null; 
  currentDropZone = null;
  draggedElement = null; // PC用ドラッグ要素もリセット
  ...
  lastTapTime = 0; // タッチ終了時はダブルタップ判定をリセット

  // セットリストにあるアイテムをアルバムメニューから隠す最終処理
  // これはドラッグ＆ドロップ後、またはロード後に呼ばれるべき
  hideSetlistItemsInMenu();
...
function handleDoubleClick(event) {
    ...
    if (isMobileDevice) {
        // console.log("[handleDoubleClick] Ignoring dblclick on mobile, handled by touch events."); 
    ...
    event.preventDefault(); // デフォルト動作を防ぐ (PCでは必要)
...
/**
 * ドラッグ可能なアイテムとドロップターゲットにイベントリスナーを設定
 * @param {Element} element - ドラッグ元またはドロップ先になる要素
 */
function enableDragAndDrop(element) {
    // PC用ドラッグイベント
    element.addEventListener('dragstart', (e) => {
        if (!element.dataset.itemId && !element.classList.contains('setlist-item')) {
            e.preventDefault(); // アイテムIDがない、またはセットリストアイテムではないスロットからのドラッグは許可しない
            return;
        }

        isDragging = true;
        draggingItemId = element.dataset.itemId;
        draggedElement = element; // PCドラッグ元の要素を保存

        // セットリスト内のアイテムをドラッグする場合
        if (element.classList.contains('setlist-item')) {
            originalSetlistSlot = element; // 元のスロットを記憶
            // 元のスロットのデータを一時的に保存
            originalSetlistSlot._originalItemData = getSlotItemData(originalSetlistSlot);
            // 元のスロットを一時的に隠すか、プレースホルダーにする
            element.classList.add('placeholder-slot');
            element.style.visibility = 'hidden';
        } else {
            // アルバムからのドラッグの場合、元の要素を隠す
            element.style.visibility = 'hidden';
        }
        e.dataTransfer.effectAllowed = 'move';
        e.dataTransfer.setData('text/plain', element.dataset.itemId);
        element.classList.add('dragging');
        console.log(`[dragstart] Dragging started for itemId: ${draggingItemId}, from setlist: ${!!originalSetlistSlot}`);
    });

    element.addEventListener('dragend', () => {
        console.log("[dragend] Drag ended. Calling finishDragging.");
        // processDrop が呼ばれなかった場合にのみ finishDragging を呼ぶ
        // processDrop 内部で finishDragging を呼ぶように変更したので、
        // ここでの呼び出しは不要、または安全に呼び出すための条件が必要
        // 現状は processDrop が確実に呼ばれる前提なら不要
        // PCドラッグの場合は、ドロップされなかった時に備えてここでfinishDraggingを呼ぶのが適切
        if (isDragging) { // まだドラッグ状態がリセットされていない場合
             finishDragging(); // ドロップされなかったものとしてクリーンアップ
        }
    });
    ...
    element.addEventListener('dragover', (e) => {
        ...
        if (element.classList.contains('setlist-slot')) {
            if (currentDropZone && currentDropZone !== element) {
                currentDropZone.classList.remove('drag-over');
            }
    ...
    element.addEventListener('drop', (e) => {
        ...
        if (element.classList.contains('setlist-slot') && draggingItemId) {
            console.log(`[drop] Dropped itemId: ${draggingItemId} onto slot: ${element.dataset.slotIndex || 'N/A'}`);
            // PCドラッグでは draggedElement が元の要素
            processDrop(draggedElement, element, originalSetlistSlot);
        } else {
            console.warn("[drop] Invalid drop target or no dragging item. Calling finishDragging.");
            // ドロップターゲットがセットリストスロットでない場合、アイテムを元の場所に戻す
            if (draggedElement) {
                // アルバムアイテムからのドラッグの場合
                if (draggedElement.closest('.album-content')) {
                    draggedElement.style.visibility = ''; // 元のアルバムアイテムを表示に戻す
                }
                // セットリストアイテムからのドラッグの場合
                if (originalSetlistSlot) {
                    fillSlotWithItem(originalSetlistSlot, originalSetlistSlot._originalItemData);
                    originalSetlistSlot.style.visibility = '';
                    originalSetlistSlot.classList.remove('placeholder-slot');
    ...
    element.addEventListener('touchcancel', handleTouchEnd); // キャンセル時も終了処理
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setlist.querySelectorAll(".setlist-slot").forEach((slot, index) => {
      ...
      slot.addEventListener('click', (e) => {
          // ドラッグがアクティブな場合は、クリックイベントを無視
          // isDragging は touchstart/mousemove/touchend で更新されるグローバル変数
          if (isDragging) { // ドラッグ中なら、クリックイベントは無視
              e.preventDefault();
              e.stopPropagation();
              console.log("[slotClick] Click ignored because drag is active.");
              return;
