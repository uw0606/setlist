...
function handleTouchStart(event) {
    ...
    // そうでない場合（メニューの空白など）はスクロールを許可するため、ここでreturnする
    if (!touchedElement) {
    ...
    if (event.touches.length === 1) {
        ...
        // ★修正：event.preventDefault() をここに移動し、ドラッグ開始時にのみスクロールを抑制★
        touchTimeout = setTimeout(() => {
            // タップまたはダブルタップではないことを確認
            if (!isDragging && draggingItemId && document.body.contains(touchedElement)) {
                event.preventDefault(); // ★ここに追加★ ドラッグ開始時にスクロールを抑制
...
// finishDragging 関数 (前回の指示で既に修正済みですが、念のため完全版を再掲)
    ...
    const initializeSetlistItem = (itemElement) => {
        ...
        itemElement.addEventListener('touchstart', (e) => {
            if (e.touches.length === 1) { 
                ...
                const currentTime = new Date().getTime();
                // ダブルタップ候補の場合、ここでは preventDefault を削除し、
                // handleTouchStart の共通ロジックまたは handleDoubleClick に任せる
