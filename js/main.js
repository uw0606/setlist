...
function changeAlbumView() {
    ...
    // ★★★ 修正箇所: 変数名を IDと一致する pastSetlistsButtonContainer に統一 ★★★
    // 移動対象：過去セットリストボタンのコンテナ（ボタン単体）
    ...
    if (mode === 'name_order') {
        ...
        // ★★★ 修正の核心: 過去セットリストボタンを文字順ナビの下に移動 (ボタンのコンテナを移動) ★★★
        // pastSetlistsButton を pastSetlistsButtonContainer に置き換える
        ...
        // ★★★ 修正の核心: 過去セットリストボタンを元の親要素に戻す (ボタンのコンテナを戻す) ★★★
        // pastSetlistsButton を pastSetlistsButtonContainer に置き換える
        if (pastSetlistsButtonContainer && originalParent) {
            // 元の親要素（#menuFooter）の末尾にボタン単体 only を戻す
