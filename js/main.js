...
async function generateSetlistPdf() {
    ...
    try {
        ...
        const formattedTime = now.toLocaleTimeString('ja-JP', timeOptions).replace(/:\d{2}$/, ''); // HH:mm
        const generationTime = `${formattedDate} ${formattedTime}`;
        ...
        const tableWidth = pageWidth - (leftMargin + rightMargin); // 修正
        let detailedYPos = topMargin;

        // ★★★ 2. 生成日時表示の描画 (ヘッダーよりも右上) ★★★
        const timeFontSize = 8;
        detailedPdf.setFontSize(timeFontSize);
        detailedPdf.setFont('NotoSansJP', 'normal');
        detailedPdf.setTextColor(100, 100, 100); // グレー表示
        
        // ヘッダータイトルの少し上に表示
        const timeXPos = pageWidth - rightMargin;
        const timeYPos = topMargin - 5; // ヘッダーの上端 (topMargin) から 5mm 上
        ...
        // 詳細PDFの列幅を調整
        const detailedColumnStyles = {
            0: { cellWidth: 10, halign: 'center', fontSize: 11, fontStyle: 'normal' },   
            1: { cellWidth: 70, halign: 'left', fontSize: 11 },     
        ...
        detailedPdf.autoTable({
            ...
            margin: { top: topMargin, right: rightMargin, bottom: 10, left: leftMargin }, // 修正したマージンを使用
        });
        ...
        // フッターの描画ロジック
        const footerText = "Generated by UVERWORLD Setlist Builder"; 
