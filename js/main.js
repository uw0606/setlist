...
const menuButton = document.getElementById("menuButton"); // DOMContentLoadedでイベントリスナーを追加するため、グローバルで参照
...
// Firebase Realtime Database 参照 (!!!! ここで1回だけ宣言 !!!!)
...
function clearSlotContent(parentList, slotIndex) {
    ...
    if (slotToClear) {
        ...
        // イベントリスナーはfillSlotWithItemで再設定されるので、ここでは特に削除の必要はないが、厳密には再描画ごとに消すのが理想。
        // ただしパフォーマンスを考慮し、ここでは動的に追加・削除されるもののみに絞る。
        // fillSlotWithItem で設定されるイベントリスナーは、その関数内で古いものをremoveしてから追加しているため、ここでの明示的な削除は不要。
        // slotToClear.removeEventListener("dragstart", handleDragStart); // fillSlotWithItem で再設定される
        // slotToClear.removeEventListener("touchstart", handleTouchStart); // fillSlotWithItem で再設定される
        // slotToClear.removeEventListener("touchmove", handleTouchMove); // fillSlotWithItem で再設定される
        // slotToClear.removeEventListener("touchend", handleTouchEnd); // fillSlotWithItem で再設定される
        // slotToClear.removeEventListener("touchcancel", handleTouchEnd); // fillSlotWithItem で再設定される
        // slotToClear.removeEventListener("dblclick", handleDoubleClick); // fillSlotWithItem で再設定される

        slotToClear.draggable = false; // 空スロットはドラッグ不可にする
...
function fillSlotWithItem(slotElement, songData) {
  ...
  // 既存のイベントリスナーを削除してから再追加することで、重複登録を防ぐ
...
/**
 * ドロップ処理
 * @param {DragEvent} event - ドロップイベント (PC向け)
 */
function handleDrop(event) {
    event.preventDefault(); // デフォルト動作（リンクを開くなど）を防止
    ...
    // ドラッグ＆ドロップAPIからデータを取得
    const itemId = event.dataTransfer.getData("text/plain");
    const dropTargetSlot = event.target.closest('.setlist-slot');

    if (!itemId) {
        console.warn("[handleDrop] No item ID found in dataTransfer. Aborting drop.");
        finishDragging();
        return;
    }

    let draggedElement;
    // PCドラッグの場合、currentPcDraggedElementが存在するはず
    if (currentPcDraggedElement && currentPcDraggedElement.dataset.itemId === itemId) {
        draggedElement = currentPcDraggedElement;
    } else {
        // フォールバック（通常発生しないはずだが念のため）
        draggedElement = document.querySelector(`.album-content .item[data-item-id="${itemId}"]`) ||
                         document.querySelector(`.setlist-slot.setlist-item[data-item-id="${itemId}"]`);
    }

    if (!draggedElement) {
        console.error("[handleDrop] Could not find the dragged element for item ID:", itemId);
        finishDragging();
        return;
    }

    processDrop(draggedElement, dropTargetSlot, originalSetlistSlot);
    finishDragging(); // ドロップ操作後のクリーンアップ
}
...
function processDrop(draggedElement, dropTargetSlot, originalSetlistSlot = null) {
    ...
    if (dropTargetSlot) {
        ...
        if (isDraggedFromSetlist) {
            ...
            clearSlotContent(setlist, originalSetlistSlot.dataset.slotIndex);
            // セットリストから外にドロップされた場合、元のアルバムには戻さない (ダブルクリックで戻す仕様)
        } else {
...
function handleTouchStart(event) {
    ...
    // メニューボタンやチェックボックスはここでのドラッグ処理の対象外
    // これらは個別のクリックイベントリスナーで処理されます
    if (isCheckboxClick) {
    ...
    if (!touchedElement) {
        console.warn("[touchstart:Mobile] No draggable item found on touch start. Allowing default behavior (e.g., scroll).");
        return;
    }
    ...
    if (tapLength < 300 && tapLength > 0) {
        ...
        event.preventDefault(); // デフォルトの拡大などを阻害
...
function handleTouchMove(event) {
    ...
    if (!isDragging) {
        ...
        if (deltaX < DRAG_THRESHOLD && deltaY < DRAG_THRESHOLD) {
            return; // しきい値未満の移動中はデフォルトのスクロール動作を妨げない
        }

        // しきい値を超えた場合、ここでドラッグを開始
        event.preventDefault(); // ★ここが重要: ドラッグが始まったらスクロールを停止

        if (touchTimeout) { // touchTimeout がセットされている場合（シングルタップ判定用など）はクリア
            clearTimeout(touchTimeout);
            touchTimeout = null;
...
function handleTouchEnd(event) {
    ...
    // isDraggingがtrueであるか、またはcurrentTouchDraggedCloneが作成されていた場合
...
function finishDragging() {
    ...
    if (originalSetlistSlot) {
        ...
        if (setlist.contains(originalSetlistSlot) && (originalSetlistSlot.classList.contains('setlist-item') || originalSetlistSlot.classList.contains('setlist-slot-text'))) {
...
/**
 * 日本語フォントをjsPDFに登録する関数。
 * (これはjsPDFの仕様に合わせて、外部で定義されるものと仮定しています)
 * 例として、NotoSansJP-Regular.ttf が base64 形式で埋め込まれていることを想定。
 * 実際には、fontconverterツールなどで生成したコードをここに貼り付けます。
 */
function registerJapaneseFont(doc) {
    // ここに base64 エンコードされたフォントデータと、それを登録するコードを記述します。
    // 例:
    /*
    doc.addFileToVFS('NotoSansJP-Regular.ttf', 'YOUR_BASE64_FONT_DATA_HERE');
    doc.addFont('NotoSansJP-Regular.ttf', 'NotoSansJP', 'normal');
    doc.addFont('NotoSansJP-Regular.ttf', 'NotoSansJP', 'bold'); // 太字も必要なら
    */
    // 適切なフォントデータと addFont 呼び出しに置き換えてください。
    // 現状ではダミーのログを出力します。
    console.warn("[registerJapaneseFont] 日本語フォント登録関数が呼び出されました。適切なフォントデータの埋め込みと登録が必要です。");
    // jsPDF-AutoTableで日本語フォントを使用するためには、別途フォントファイルのbase64データと登録コードが必要です。
    // これは環境に依存するため、ここでは具体的なコードは含めません。
    // 開発中の環境に合わせてフォントを準備し、addFontメソッドで登録してください。
    // 例:
    // doc.addFont("data:font/ttf;base64,...", "NotoSansJP", "normal");
    // doc.addFont("data:font/ttf;base64,...", "NotoSansJP", "bold");
    // または外部ファイルからのロードなど。
}
