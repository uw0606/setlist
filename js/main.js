...
async function generateSetlistPdf() {
    ...
    try {
        ...
        const MAX_SONG_FONT_SIZE = 60; // 曲の最大フォントサイズ
        const MAX_HEADER_FONT_SIZE_ABS = 60; // ★修正1: タイトルの最大フォントサイズを60ptに設定
        ...
        if (headerText) {
            // ★修正2: ヘッダーサイズの初期値を最大値からスタート
            let headerFontSize = MAX_HEADER_FONT_SIZE_ABS; 
        ...
        if (numSimpleRows > 0) {
            ...
            // スムーズな行間推移ロジック
            // ★修正3: MIN_LINE_HEIGHT_FACTORを0.45に下げ、行間をより積極的に広げる
            const MIN_LINE_HEIGHT_FACTOR = 0.45; 
