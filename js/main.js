/**
 * グローバル変数
 * @type {HTMLElement|null} currentPcDraggedElement - PCドラッグ中に参照する元の要素（主にアルバムからドラッグする際のクローン）
 * @type {HTMLElement|null} currentTouchDraggedClone - タッチドラッグ中に動かすクローン要素
 * @type {string|null} draggingItemId - ドラッグ中のアイテムIDを保持 (PC/Mobile共通)
 * @type {number} touchStartX - タッチ開始時のX座標
 * @type {number} touchStartY - タッチ開始時のY座標
 * @type {number} lastTapTime - 最後のタップ時刻
 * @type {boolean} isDragging - 現在ドラッグ中かどうかのフラグ (タッチドラッグ用)
 * @type {number|null} touchTimeout - setTimeout のIDを保持する変数
 * @type {Map<string, string|null>} originalAlbumMap - 各アイテムの元のアルバムIDを保持するMap
 * @type {HTMLElement|null} originalSetlistSlot - PC/Mobile共通で、セットリスト内でドラッグ開始された「元のスロット要素」を指す
 * @type {HTMLElement} setlist - セットリスト要素
 * @type {HTMLElement} menu - メニュー要素
 * @type {HTMLElement} menuButton - メニューボタン要素
 * @type {HTMLElement} albumList - アルバムリスト要素
 * @type {number} maxSongs - セットリストに登録できる最大曲数
 * @type {HTMLElement|null} currentDropZone - 現在のドロップゾーン要素 (PCドラッグ用)
 * @type {HTMLElement|null} activeTouchSlot - モバイルでのドロップゾーンハイライト用
 * @type {number|null} rafId - requestAnimationFrame のID
 */
...
function clearSlotContent(parentList, slotIndex) {
  ...
  if (slotToClear) {
    ...
    slotToClear.style.cssText = ''; // スタイルをリセット
    slotToClear.style.visibility = ''; // 非表示を解除

    // イベントリスナーを削除（重複登録防止のため）
...
function restoreToOriginalList(elementToProcess, fromSetlistOnly = false) {
  ...
  // const wasOriginallyInAlbum = albumItemInMenu && albumItemInMenu.style.visibility === 'hidden'; // このフラグは現在未使用ですが、論理的には正しい
...
function hideSetlistItemsInMenu() {
  ...
  currentSetlistItems.forEach(slot => {
    ...
    if (itemId) {
      const albumItemInMenu = document.querySelector(`.album-content .item[data-item-id="${itemId}"]`);
      if (albumItemInMenu) {
        albumItemInMenu.style.visibility = 'hidden'; // アルバムメニューから非表示にする
...
function getSetlist() {
  const currentSetlist = Array.from(document.querySelectorAll("#setlist .setlist-slot.setlist-item"))
    .map((slot, index) => {
      ...
      if (!songData) {
        return `${index + 1}. (データなし)`; // データがない場合のフォールバック
      }
...
/**
 * 現在のセットリストとUIの状態を取得し、保存可能なオブジェクトとして返す。
 * @returns {object} 現在の状態を表すオブジェクト
 */
...
function fillSlotWithItem(slotElement, songData) {
  ...
  if (infoParts.length > 0) {
    ...
    additionalInfoDiv.style.display = ''; // 表示
  } else {
    additionalInfoDiv.style.display = 'none'; // 非表示
  }
  ...
  slotElement.addEventListener("touchstart", handleTouchStart, {
    passive: false
  });
  slotElement.addEventListener("touchmove", handleTouchMove, {
    passive: false
  });
...
function handleDragStart(event) {
  ...
  if (setlist.contains(originalElement) && originalElement.classList.contains('setlist-item')) {
    ...
    originalSetlistSlot.style.visibility = 'hidden'; // 元の要素を非表示にする
    originalSetlistSlot.classList.add('placeholder-slot'); // プレースホルダーとして表示
...
function handleDragOver(event) {
  ...
  if (!draggingItemId) return; // draggingItemIdがない場合は何もしない
...
function processDrop(draggedElement, dropTargetSlot, originalSetlistSlot = null) {
  ...
  // セットリストからのドラッグで、_originalItemDataが保存されていればそれを使用
  if (originalSetlistSlot && originalSetlistSlot._originalItemData) {
    ...
    // アルバムからのドラッグの場合、または_originalItemDataがない場合
  ...
  if (dropTargetSlot) {
    ...
    if (isDraggedFromSetlist) {
      // セットリスト内での移動、またはセットリスト内のアイテムを空きスロットへ移動
    ...
    if (isDraggedFromSetlist) {
      ...
      restoreToOriginalList(draggedElement, true); // アルバムアイテムは表示しない (セットリストからの移動なので)
    } else {
...
function handleDrop(event) {
  ...
  if (currentPcDraggedElement && currentPcDraggedElement.dataset.itemId === droppedItemId) {
    ...
    // フォールバック: アルバムリスト内から元の要素を探す（例えば、ドラッグ開始からドロップまでの間にDOMが変更された場合など）
...
function handleTouchStart(event) {
  ...
  // `event.preventDefault()` をここに追加して、スクロールやデフォルトのブラウザ動作を早めに阻止
  event.preventDefault();
  ...
  // ダブルタップ検出ロジックは preventDefault の後に
  ...
  if (event.touches.length === 1) {
    ...
    touchTimeout = setTimeout(() => {
      ...
      if (!isDragging && draggingItemId && document.body.contains(touchedElement)) {
        ...
        console.warn("[touchstart:Mobile] Dragging not initiated after timeout (isDragging or touchedElement issues).");
      }
      touchTimeout = null;
...
function handleTouchMove(event) {
  // ドラッグが開始されていない、またはクローン要素がない場合は何もしない
  if (!isDragging || !currentTouchDraggedClone) {
    // まだドラッグが開始されていないが、ロングプレス判定のためのタッチムーブが発生している場合は
    // デフォルトのスクロール動作を阻止
    if (touchTimeout) {
      // スクロールを禁止するため、予防的に preventDefault を呼ぶ
      event.preventDefault();
    }
    return;
  }
...
function handleTouchEnd(event) {
  // ロングプレス待ちのタイムアウトがまだアクティブな場合（ドラッグが開始されなかった場合）
  if (touchTimeout) {
    clearTimeout(touchTimeout);
    touchTimeout = null;
  ...
  if (touch) {
    // タッチが終了した座標の要素を取得し、セットリストスロットを探す
...
function createTouchDraggedClone(originalElement, initialX, initialY, itemIdToClone) {
  ...
  currentTouchDraggedClone = originalElement.cloneNode(true); // 子要素も含めてクローン
  ...
  if (setlist.contains(originalElement) && originalElement.classList.contains('setlist-item')) {
    ...
    originalElement.style.visibility = 'hidden'; // アルバムアイテムの場合も隠す
  ...
  currentTouchDraggedClone.style.zIndex = '10000'; // 他の要素より手前に表示
...
function finishDragging() {
  ...
  setlist.querySelectorAll('.setlist-slot.drag-over')
    .forEach(slot => {
      ...
      // slot.classList.remove('active-drop-target'); // コメントアウト: このクラスは使われていない模様
      // slot.style.opacity = ''; // コメントアウト: このスタイルはここでリセットする必要がない可能性あり
    });
  console.log("[finishDragging] Removed drag-related classes from all setlist slots.");

  // グローバル変数をリセット
  currentDropZone = null;
  activeTouchSlot = null; // モバイル用ハイライトのリセット
  currentPcDraggedElement = null;
  draggingItemId = null;
  isDragging = false; // これが重要: ドラッグ状態を終了
...
function shareSetlist() {
  ...
  setlistRef.set(currentState) // 現在の状態をデータベースに保存
    .then(() => {
      const shareId = setlistRef.key; // 生成されたキーを共有IDとして取得
      const shareLink = `${window.location.origin}${window.location.pathname}?shareId=${shareId}`; // 共有リンクを作成

      // 特定のアルバム（album1）のアイテムIDリスト
      ...
      if (navigator.share) {
        ...
        // Web Share APIが利用できない場合のフォールバック（リンクをクリップボードにコピー）
        const tempInput = document.createElement('textarea');
        tempInput.value = shareLink; // リンクのみをコピー
        document.body.appendChild(tempInput);
        tempInput.select();
        document.execCommand('copy'); // 非推奨だが広くサポートされている
 ...
 * @async
 * @returns {Promise<void>} PDF生成が完了すると解決されるPromise
 */
async function generateSetlistPdf() {
  ...
  if (setlistVenue) {
    if (headerText) headerText += ' '; // 日付がある場合のみスペースを追加
  ...
  const tableBodyDetailed = []; // 詳細PDF用データ
  const simplePdfLines = []; // シンプルPDF用テキスト行データ

  let detailedItemNo = 1; // 詳細PDFの連番用
  ...
  for (const slot of setlistSlots) {
    if (slot.classList.contains('setlist-item')) {
      ...
      if (songData) {
        ...
        tableBodyDetailed.push(detailedRow);

        // --- シンプルPDF用の行の生成 ---
        if (isAlbum1) {
          simplePdfLines.push(`    ${titleText}`); // インデント
        } else {
          simplePdfLines.push(`${simpleItemNoForNumberedList}. ${titleText}`);
      ...
      if (textContent) {
        ...
        tableBodyDetailed.push([textContent, '', '', '', '', '', '']);
        // シンプルPDF用の行 (テキストスロットはNo.なし、そのまま追加)
        simplePdfLines.push(textContent);
      }
    }
  }
  ...
  try {
    ...
    // --- 1. 詳細なセットリストPDFの生成 ---
    console.log("[generateSetlistPdf] Generating detailed PDF...");
    ...
    let detailedYPosition = MARGIN_TOP;

    // ヘッダー情報の描画
    if (headerText) {
      detailedPdf.setFillColor(220, 220, 220); // グレー背景
      detailedPdf.setDrawColor(0, 0, 0); // 黒線
      detailedPdf.setLineWidth(0.3);
      detailedPdf.rect(MARGIN_LEFT, detailedYPosition, tableWidth, headerCellHeight, 'FD'); // 背景と枠線

      detailedPdf.setFontSize(14);
      detailedPdf.setFont('NotoSansJP', 'bold');
      detailedPdf.setTextColor(0, 0, 0);
      detailedPdf.text(headerText, A4_WIDTH / 2, detailedYPosition + headerCellHeight / 2 + 0.5, {
        align: 'center',
        baseline: 'middle'
      });

      detailedYPosition += headerCellHeight; // ヘッダーの高さ分Y座標を移動
    }

    // autoTableでセットリストのテーブルを描画
    detailedPdf.autoTable({
      head: [tableHeaders],
      body: tableBodyDetailed,
      startY: detailedYPosition, // テーブルの開始Y座標
      theme: 'grid',
      styles: {
        font: 'NotoSansJP',
        fontStyle: 'normal',
      ...
      didDrawPage: function(data) {
        const pageNumber = 'Page ' + detailedPdf.internal.getNumberOfPages();
        detailedPdf.setFontSize(10);
        detailedPdf.setFont('NotoSansJP', 'normal');
        detailedPdf.text(pageNumber, detailedPdf.internal.pageSize.getWidth() - 10, detailedPdf.internal.pageSize.getHeight() - 10, {
          align: 'right'
        });
      },
    });
    ...
    await new Promise(resolve => setTimeout(resolve, 500)); // 少し待ってから次のPDFを生成

    // --- 2. シンプルなセットリストPDFの生成 ---
    console.log("[generateSetlistPdf] Generating simple PDF...");
    const simplePdf = new jsPDF('p', 'mm', 'a4');
    registerJapaneseFont(simplePdf); // フォント登録

    // シンプルPDFのフォントスタイルとサイズ
    const SIMPLE_FONT_SIZE = 19;
    const SIMPLE_MARGIN_TOP = 25;
    const SIMPLE_MARGIN_LEFT = 25;
    const SIMPLE_LINE_HEIGHT_RATIO = 0.5; // 行の高さ調整
    const SIMPLE_HEADER_FONT_SIZE_RATIO = 1.2; // ヘッダーのフォントサイズ比

    simplePdf.setFont('NotoSansJP', 'bold'); // シンプルは常に太字
    simplePdf.setFontSize(SIMPLE_FONT_SIZE);

    let simpleYPosition = SIMPLE_MARGIN_TOP;

    // ヘッダーテキスト（日付と会場）
    if (headerText) {
      simplePdf.setFontSize(SIMPLE_FONT_SIZE * SIMPLE_HEADER_FONT_SIZE_RATIO);
      simplePdf.text(headerText, SIMPLE_MARGIN_LEFT, simpleYPosition);
      simpleYPosition += SIMPLE_FONT_SIZE * SIMPLE_LINE_HEIGHT_RATIO * 2; // ヘッダーと曲リストの間の余白を調整
      simplePdf.setFontSize(SIMPLE_FONT_SIZE); // 曲リスト用に戻す
    }

    // 各曲目をテキストとして追加
    simplePdfLines.forEach(line => {
      simplePdf.text(line, SIMPLE_MARGIN_LEFT, simpleYPosition);
      simpleYPosition += SIMPLE_FONT_SIZE * SIMPLE_LINE_HEIGHT_RATIO; // 行の高さを調整

      // ページ下部に近づいたら新しいページを追加
      const bottomMarginThreshold = SIMPLE_MARGIN_TOP - 10;
      if (simpleYPosition > A4_HEIGHT - bottomMarginThreshold) {
        simplePdf.addPage();
        simpleYPosition = SIMPLE_MARGIN_TOP;
        simplePdf.setFont('NotoSansJP', 'bold'); // 新しいページでもフォント設定を維持
        simplePdf.setFontSize(SIMPLE_FONT_SIZE);
      }
    });
