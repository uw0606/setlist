...
function handleTouchStart(event) {
    ...
    if (tapLength < 300 && tapLength > 0) {
        ...
        // ★重要: ダブルタップの場合はpreventDefault() を呼ばないことで、
        // 以下の処理が正常に実行され、ブラウザのクリックイベントも妨害しないようにする
        // event.preventDefault(); // この行を削除またはコメントアウト
    ...
    // ダブルタップでなければ、ここで初めてpreventDefault()を呼ぶ
    // これにより、長押し中に指が動いてもスクロールと誤認識されなくなる
    event.preventDefault(); 
    console.log("[touchstart] Prevented default for single touch (potential drag).");
