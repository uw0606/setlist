 ...
 * ドラッグ終了時のクリーンアップ処理
 */
function finishDragging(droppedSuccessfully = false) { // 引数を追加
    if (rafId !== null) { cancelAnimationFrame(rafId); rafId = null; }

    if (currentTouchDraggedClone) {
        currentTouchDraggedClone.remove();
        currentTouchDraggedClone = null;
        console.log("[finishDragging] Dragged clone removed.");
    }

    if (currentDropZone) {
        currentDropZone.classList.remove('drag-over');
        currentDropZone = null;
        console.log("[finishDragging] Drop zone highlight removed.");
    }

    if (originalSetlistSlot) {
        // ドラッグが成功しなかった場合（またはセットリスト外にドロップされた場合）のみ、
        // 元のセットリストスロットのアイテムをアルバムに戻す
        // droppedSuccessfully が true の場合は、移動が成功したと判断し、何もしない
        if (!droppedSuccessfully) {
            console.log("[finishDragging] Drag was not successful or dropped outside setlist. Restoring original setlist slot item.");
            // ここで originalSetlistSlot の中身をクリアし、アルバムに戻す
            // originalSetlistSlot には _originalItemData が保存されているはず
            if (originalSetlistSlot._originalItemData) {
                restoreToOriginalList(originalSetlistSlot, true); // true を渡してスロットをクリア
            } else {
                // _originalItemData がない場合は、単にスロットをクリア
                clearSlotContent(originalSetlistSlot, originalSetlistSlot.dataset.slotIndex);
            }
        } else {
            console.log("[finishDragging] Drag was successful. Not restoring original setlist slot item.");
        }
        originalSetlistSlot.classList.remove('placeholder-slot');
        originalSetlistSlot.style.visibility = ''; // プレースホルダーを元に戻す
        originalSetlistSlot = null;
    } else if (initialTouchElement && initialTouchElement.closest('.album-content')) {
        // アルバムからのドラッグで、ドロップが成功しなかった場合のみ、元のアルバムアイテムを表示に戻す
        if (!droppedSuccessfully) {
            initialTouchElement.style.visibility = '';
            console.log("[finishDragging] Album item drag not successful. Showing original album item.");
        } else {
            console.log("[finishDragging] Album item drag successful. Keeping original album item hidden.");
        }
    }
    ...
    console.log("[finishDragging] All drag-related flags and elements reset.");
}
