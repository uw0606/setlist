...
function handleTouchStart(event) {
    ...
    if (tapLength < 300 && tapLength > 0) {
        ...
        lastTapTime = 0;
        isDragging = false; // ダブルタップの場合、ドラッグは発生しないのでisDraggingをfalseにする
    ...
    if (event.touches.length === 1) {
        // タッチ開始時点ではドラッグ中ではないと明確に設定
        isDragging = false;
        draggingItemId = touchedElement.dataset.itemId;

        const isTouchedFromSetlist = setlist.contains(touchedElement) && touchedElement.classList.contains('setlist-item');

        if (isTouchedFromSetlist) {
        ...
        // アルバムアイテムの場合のみロングプレスでドラッグ開始
        if (!isTouchedFromSetlist) {
            touchTimeout = setTimeout(() => {
                // setTimeout内でも再度、isDraggingとタッチされた要素がまだ存在するか、そしてセットリストのアイテムではないことを確認
                if (!isDragging && draggingItemId && document.body.contains(touchedElement) && !setlist.contains(touchedElement)) {
                    createTouchDraggedClone(touchedElement, touchStartX, touchStartY, draggingItemId);
                    isDragging = true;
                    console.log("[touchstart:Mobile] Dragging initiated after timeout for album item.");
                } else {
                    console.warn("[touchstart:Mobile] Long press timeout for setlist item or drag already initiated/element removed.");
                }
                touchTimeout = null;
            }, 600); // 600ミリ秒のロングプレスでドラッグ開始
        } else {
            console.log("[touchstart:Mobile] Setlist item touched. Long press drag is explicitly disabled for this type of item.");
            // セットリストアイテムの場合、ここで`touchTimeout`を設定しない
            // この場合、短いタップか、または動かない限り何も起きない
        }
    }
}
