...
function handleTouchStart(event) {
    ...
    // ダブルタップの場合はpreventDefault()を呼ばず、ブラウザのクリック/dblclickイベントに任せる
    ...
    // ここでevent.preventDefault()を呼ぶことで、長押しが開始されたと判断された時点で
    // その後のクリックイベントやデフォルトのスクロールを全て阻止する
    event.preventDefault(); 
    console.log("[touchstart] Prevented default for single touch (potential drag or long press).");
    ...
    if (event.touches.length === 1) {
        ...
        if (isInSetlist || isInAlbumContent) {
            ...
            touchTimeout = setTimeout(() => {
                // タイマー発火時にもう一度isDraggingをチェックし、
                // ドラッグ開始の条件が満たされていることを確認
                // (例えば、指が閾値を超えて動いていないかなど)
                // ★この時点でクローンを作成し、isDraggingをtrueにする
                if (!isDragging && draggingItemId && document.body.contains(initialTouchElement)) {
                    ...
                    // ここでドラッグ開始に至らなかった場合のクリーンアップを行う
                    // (handleTouchEndがisDraggingがfalseの場合にこれを処理する)
                }
            ...
            // ドラッグ対象外の要素なら処理を終了
            // ここでfinishDragging()を呼ぶ必要はない。handleTouchEndで処理される。
            return;
        }
    }
}
...
function handleTouchEnd(event) {
    ...
    if (touchTimeout) { // 長押しタイマーがまだアクティブならクリア
        ...
        // 長押しタイマーがキャンセルされた状態で指が離れた場合、
        // isDraggingはfalseなので、そのままfinishDragging()が呼ばれる
    }
