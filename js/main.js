...
function handleTouchStart(event) {
    ...
    if (isDragging || currentTouchDraggedClone || currentDropZone || draggingItemId || originalSetlistSlot || touchTimeout || isLongPressReady) {
        ...
        isLongPressReady = false; // ★追加: フラグをリセット
    }
    // --- リセット処理ここまで ---
    ...
    if (event.touches.length === 1) {
        ...
        // ★修正: 長押し判定用のタイマーを開始。タイマー発火で isLongPressReady を true にするだけ★
        touchTimeout = setTimeout(() => {
            if (!isDragging && draggingItemId) { // まだドラッグが開始されておらず、有効なアイテムの場合
                isLongPressReady = true; // 長押しが確定したことを示すフラグを立てる
                console.log("[touchstart:Mobile] Long press ready. Waiting for movement to start drag.");
            } else {
...
function handleTouchMove(event) {
    if (event.touches.length !== 1) {
        ...
        isLongPressReady = false; // ★追加: フラグもリセット
        return;
    }
    ...
    if (!isDragging) { // まだドラッグが開始されていない場合
        if (touchTimeout) { // 長押しタイマーがまだ動いている場合
            if (distance > DRAG_THRESHOLD) { // 長押し前に移動しすぎたらキャンセル
                ...
                touchTimeout = null;
                isLongPressReady = false; // ★追加: フラグもリセット
                console.log("[handleTouchMove] Cleared long press timeout due to movement exceeding threshold.");
                return; // ドラッグは開始しない（スクロールを許可）
            }
        }

        // 長押しが確定しており (isLongPressReady)、かつ DRAG_THRESHOLD を超えたらドラッグ開始
        if (isLongPressReady && distance > DRAG_THRESHOLD) {
            if (!draggingItemId) {
                 console.warn("[handleTouchMove] Drag threshold met, but draggingItemId is null. Aborting drag start.");
                 isLongPressReady = false; // 無効な状態なのでフラグもクリア
                 return;
            }
            ...
            // ここからドラッグ開始と判断
            isDragging = true;
            // クローン要素作成時に、現在のタッチ座標を渡す（初期座標ではない）
            createTouchDraggedClone(elementToClone, currentX, currentY, draggingItemId);
            console.log("[handleTouchMove] Dragging started due to long press and movement threshold.");
            event.preventDefault(); // ドラッグ開始したら、それ以降のスクロールをブロック
            return; // ドラッグ開始したので、これ以上処理しない
        } else {
            // 長押し準備ができていない、または移動距離が足りない場合はスクロールを許可
            return;
        }
    }
