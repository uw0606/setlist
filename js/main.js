...
function handleTouchStart(event) {
    ...
    // --- ★修正点: ドラッグ関連のグローバル変数を関数の冒頭で確実にリセット★ ---
    // いずれかの条件で早期リターンする場合（またはドラッグを開始しない場合）は、
    // ドラッグ関連の状態をリセットすることで、前の操作の状態が残るのを防ぐ
    if (isDragging || currentTouchDraggedClone || currentDropZone || draggingItemId || originalSetlistSlot) {
...
function handleTouchMove(event) {
    if (event.touches.length !== 1) {
        // 複数指の場合はドラッグを中止し、状態をリセット
        if (isDragging) { // ドラッグ中だった場合のみfinishDraggingを呼ぶ
    ...
    if (!isDragging) {
        ...
        if (distance > DRAG_THRESHOLD) {
            // --- ★修正点: draggingItemId が存在し、かつ有効な要素が見つかるかを確認★ ---
            if (!draggingItemId) {
                 console.warn("[handleTouchMove] Drag threshold met, but draggingItemId is null. Aborting drag start.");
                 // draggingItemId がない場合はドラッグを開始しない
                 return;
            }

            let elementToClone = null;
            // originalSetlistSlot が設定されており、それが draggingItemId と一致する場合、それを優先
            if (originalSetlistSlot && originalSetlistSlot.dataset.itemId === draggingItemId) {
                elementToClone = originalSetlistSlot;
            } else {
                // そうでなければ、アルバムコンテンツから itemId に一致する要素を探す
                elementToClone = document.querySelector(`.album-content .item[data-item-id="${draggingItemId}"]`);
            }

            // elementToClone が見つからない場合は警告を出して中断
            if (!elementToClone || !document.body.contains(elementToClone)) {
                 console.warn("[handleTouchMove] Drag threshold met, but elementToClone (original item) not found or not in DOM. Aborting drag start.");
                 draggingItemId = null; // 無効な状態なのでクリア
                 originalSetlistSlot = null; // 無効な状態なのでクリア
                 return;
            }
            // --- ★修正点ここまで★ ---
            ...
            console.log("[handleTouchMove] Dragging started due to movement threshold. Element:", elementToClone);
...
function handleTouchEnd(event) {
    // --- ★修正点: touchTimeoutのクリアを冒頭のリセット処理に統合、checkboxClickの処理をよりシンプルに★ ---
    // handleTouchStart の冒頭で全てのリセットが行われているため、ここではisDraggingがfalseの場合のみチェック
    if (!isDragging) { // ドラッグしていなかった場合
        ...
        if (closestCheckbox) { 
            ...
            // ドラッグしていなければ、finishDragging() を呼ばない。
            // これにより、スクロールのブロックが不要な場合に誤ってクリーンアップされることを防ぐ。
        }
    ...
    if (!currentTouchDraggedClone) {
        ...
        finishDragging(); // 問題がある状態なので、クリーンアップだけは試みる
        return;
    }
    ...
    // ドロップ処理を実行
    // currentTouchDraggedClone はクローンなので、実際のドロップ処理には draggingItemId と originalSetlistSlot を使用
...
function enableDragAndDrop(element) {
    if (element.classList.contains('item')) {
        ...
        // タッチ操作中にブラウザのデフォルトのスクロールを無効にする
        // これにより、ドラッグ開始時のイベント競合を防ぎます。
        element.style.touchAction = 'none';
        console.log(`[enableDragAndDrop] Set touch-action: none for album item: ${element.dataset.itemId || 'N/A'}`);
        ...
        // セットリストのスロットでも同様にタッチ操作のデフォルト動作を無効にする
        element.style.touchAction = 'none';
        console.log(`[enableDragAndDrop] Set touch-action: none for setlist slot: ${element.dataset.slotIndex || 'N/A'}`);
