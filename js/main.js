...
async function generateSetlistPdf() {
    ...
    try {
        ...
        // --- 2. シンプルなセットリストPDFの生成 (曲間優先ロジックに修正) ---

        const simplePdf = new jsPDF('p', 'mm', 'a4');
        registerJapaneseFont(simplePdf);
        simplePdf.setFont('NotoSansJP', 'normal');

        // ★修正ポイント1: フォントサイズの上限を事実上撤廃（非常に大きな値に設定）
        const BASE_FONT_SIZE = 30; // 曲間を最大限優先するための設定
        ...
        const simpleBottomMargin = 15; 
        const SPACE_BETWEEN_HEADER_AND_BODY = 10; // ヘッダーと曲の間のマージン
        ...
        // ヘッダーの描画と高さの計算
        if (headerText) {
            // ヘッダーの初期フォントサイズは、曲の最小サイズ MIN_FONT_SIZE に基づく
            let headerFontSize = MIN_FONT_SIZE + 4; 
            ...
            if (headerTextWidth > availableWidth) {
                ...
                headerFontSize = Math.min(28, headerFontSize); // 24ptを最大とする
            }
        ...
        if (numSimpleRows > 0) {
            ...
            // ★修正ポイント2: スムーズな行間推移ロジックに置き換え (6曲〜15曲で調整)
            const MIN_LINE_HEIGHT_FACTOR = 1.0; // 6曲で最大の広げ幅 (40%増)
            const MAX_LINE_HEIGHT_FACTOR = 1.0; // 15曲以上で最小の広げ幅 (0%増)
            const LINE_COUNT_START = 6;  // スムーズな調整を始める曲数
            const LINE_COUNT_END = 15;   // スムーズな調整を終える曲数
            ...
            if (numSimpleRows <= LINE_COUNT_START) {
                ...
                // 15曲以上の場合、通常の均等割り（広げ幅なし）を適用
                factor = MAX_LINE_HEIGHT_FACTOR; 
            } else {
                // 6曲から15曲の間で、係数を0.6から1.0へ線形補間する
            ...
            // 仮想行数（除数）を計算：曲数が少ないほど、実質的な除数が小さくなり、行間が広がる
            ...
            // 2. 行間を IDEAL_LINE_HEIGHT に設定 (論理的上限 300mm)
            ...
            // 4. 逆算したフォントサイズが大きすぎる場合は、文字の縮小を行う
