...
function handleTouchStart(event) {
    ...
    if (tapLength < 300 && tapLength > 0) {
        ...
        handleDoubleClick(event); // ダブルクリックハンドラを呼び出す
        lastTapTime = 0;
        console.log("[touchstart] Double tap detected. Handled by handleDoubleClick.");
        
        // ダブルクリックで移動した場合、その後のドラッグを防ぐために状態をリセット
        isDragging = false; 
        draggingItemId = null;
    ...
    if (event.touches.length === 1) {
        ...
        const isInAlbumContent = touchedElement.closest('.album-content'); // または album-list など、ハンバーガーメニュー内の要素を特定する適切なセレクタ
        ...
        if (isInSetlist && touchedElement.classList.contains('setlist-item')) {
            ...
            console.warn("[touchstart:Mobile] Touched element is neither a setlist item nor an album item for dragging.");
        ...
        // ここが重要: どの状況でロングプレスによるドラッグを許可するかを制御
        // セットリストからハンバーガーメニューに戻す場合はダブルクリックのみなので、
        // セットリスト内のアイテム（setlist-item）からのロングプレスドラッグは禁止する。
        // ハンバーガーメニューからのドラッグは許可。セットリスト内でのドラッグも許可。
        if (isInSetlist) {
            // セットリスト内のアイテムであれば、ロングプレスドラッグは許可する (セットリスト内移動のため)
            // ただし、もしセットリストからメニューへ戻すドラッグを完全に禁止するなら、以下のsetTimeout全体を if (!isInAlbumContent) で囲むなど調整が必要
            console.log("[touchstart:Mobile] Touch started on a setlist item. Allowing long press for drag within setlist.");
            touchTimeout = setTimeout(() => {
                if (!isDragging && draggingItemId && document.body.contains(touchedElement)) {
                    // セットリストからのドラッグ（セットリスト内移動）
                    createTouchDraggedClone(touchedElement, touchStartX, touchStartY, draggingItemId);
                    isDragging = true; // ドラッグ開始！
                    console.log("[touchstart:Mobile] Dragging initiated from setlist item after timeout.");
                } else {
                    console.warn("[touchstart:Mobile] Dragging not initiated from setlist item after timeout (isDragging or touchedElement issues).");
                }
                touchTimeout = null;
            }, 600);
        } else if (isInAlbumContent) {
            // ハンバーガーメニュー内のアイテムであれば、ロングプレスドラッグを許可する
            console.log("[touchstart:Mobile] Touch started on an album item. Allowing long press for drag to setlist.");
            touchTimeout = setTimeout(() => {
                if (!isDragging && draggingItemId && document.body.contains(touchedElement)) {
                    // ハンバーガーメニューからのドラッグ（セットリストへ移動）
                    createTouchDraggedClone(touchedElement, touchStartX, touchStartY, draggingItemId);
                    isDragging = true; // ドラッグ開始！
                    console.log("[touchstart:Mobile] Dragging initiated from album item after timeout.");
                } else {
                    console.warn("[touchstart:Mobile] Dragging not initiated from album item after timeout (isDragging or touchedElement issues).");
                }
                touchTimeout = null;
            }, 600);
        } else {
            // ここには到達しないはずだが、念のため
            console.warn("[touchstart:Mobile] Unexpected touch start location. No long press timer set.");
            if (touchTimeout) {
                clearTimeout(touchTimeout);
                touchTimeout = null;
            }
