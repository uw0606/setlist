...
function handleTouchStart(event) {
    ...
    if (event.touches.length === 1) {
        ...
        draggingElement = touchedElement; // 元の要素を保存
        ...
        // ロングプレスのタイムアウトはここでは不要 (長押しドラッグを避けるため)
        // touchTimeout = setTimeout(() => { ... }, 600); // 削除
...
const VERTICAL_SCROLL_SENSITIVITY = 2; // 垂直方向の動きが水平方向のX倍以上ならスクロールと見なす
...
function handleTouchMove(event) {
    ...
    if (!touch) {
        if (isDragging) {
            // ドラッグ中に指が離れたがtouchendが呼ばれなかったケース
            finishDragging();
            isDragging = false;
            draggingElement = null;
        }
    ...
    // まだドラッグが開始されていない場合
    if (!isDragging) {
        // DRAG_THRESHOLD に達していない場合は、スクロールを許可し、何もしない
        if (deltaX < DRAG_THRESHOLD && deltaY < DRAG_THRESHOLD) {
            return;
        }

        // DRAG_THRESHOLD に達した場合、スクロールかドラッグかを判定
        // 垂直方向の動きが水平方向より圧倒的に大きい場合はスクロールと見なす
        if (deltaY > deltaX * VERTICAL_SCROLL_SENSITIVITY) {
            console.log("[handleTouchMove] Dominant vertical movement. Assuming scroll.");
            // ここでは event.preventDefault() を呼ばず、スクロールを許可
            return;
        }
        ...
        event.preventDefault(); // これ以降のスクロールを停止
    }
...
function handleTouchEnd(event) {
    // touchTimeout は long press に関連するため、ここでは不要
