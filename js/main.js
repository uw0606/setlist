...
function handleTouchStart(event) {
    ...
    if (!touchedElement) {
        console.log("[touchstart:Mobile] Non-draggable element touched. Allowing default behavior (e.g., scrolling).");
        return;
    }
    ...
    if (tapLength < DOUBLE_TAP_DELAY && tapLength > 0) { 
        ...
        handleDoubleClick(event); // ★修正: ここで handleDoubleClick を呼び出す★
        lastTapTime = 0;
    ...
    if (event.touches.length === 1) {
        ...
        // ロングプレスによるドラッグ開始の setTimeout は削除されていることを前提とします
    }
}
    ...
    if (event.touches.length === 1) {
        ...
        if (deltaX > TAP_MOVE_THRESHOLD || deltaY > TAP_MOVE_THRESHOLD) {
            if (touchTimeout) { 
                clearTimeout(touchTimeout);
                touchTimeout = null;
                console.log("[handleTouchMove] Cleared touchTimeout due to significant movement.");
            }
            if (!isDragging) { 
                const touchedElement = event.target.closest(".setlist-slot.setlist-item") || event.target.closest(".item");
                if (touchedElement && draggingItemId === touchedElement.dataset.itemId) {
                    event.preventDefault(); 
                    ...
                    console.log("[handleTouchMove] Movement detected, but not initiating drag (element mismatch or no draggingItemId).");
                }
            }
        }
        if (isDragging && currentTouchDraggedClone) {
            updateTouchDraggedClonePosition(touch.clientX, touch.clientY);
...
function handleDoubleClick(event) {
    ...
    if (isInsideSetlist) {
        ...
        // アルバムリスト（ハンバーガーメニュー）内のアイテムをダブルクリックした場合
        ...
        if (!emptySlot) {
            showMessageBox('セットリストは最大曲数に達しています。');
            console.log("[handleDoubleClick] Setlist is full. Cannot add item.");
            finishDragging();
            return;
        }

        // 既にセットリストに同じアイテムがないか確認
        if (!setlist.querySelector(`.setlist-slot.setlist-item[data-item-id="${item.dataset.itemId}"]`)) {
            // 元のアルバムリストのIDを保存（restoreToOriginalListで使うため）
    ...
    finishDragging(); // 全ての handleDoubleClick 処理の最後に必ず呼び出す
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const initializeSetlistItem = (itemElement) => {
        ...
        itemElement.addEventListener("dblclick", (e) => {
            // ドラッグハンドル上でない場合のみ実行 (曲の移動・復帰はドラッグハンドルでは行わない)
            if (!e.target.closest('.drag-handle')) {
                console.log(`[dblclick] PC double-click detected for item: ${itemElement.dataset.itemId}. Calling handleDoubleClick.`);
                handleDoubleClick(e); // ★修正: handleDoubleClick を呼び出すように変更★
                // handleDoubleClick 内で stopPropagation と preventDefault が処理されるため、ここでは不要だが、念のため維持
                e.stopPropagation(); 
                e.preventDefault(); 
            }
        }); 
        ...
        // スマートフォンでのダブルタップ検出ロジックを改善
        itemElement.addEventListener('touchstart', (e) => {
            // チェックボックスがタッチされた場合は、デフォルトの動作を許可し、ダブルタップ処理は行わない
            if (e.target.closest('input[type="checkbox"]')) {
                console.log("[touchstart] Checkbox within item touched. Allowing default.");
                return;
            }
            ...
            if (e.touches.length === 1) { 
                ...
                const currentTime = new Date().getTime();
                // ダブルタップ候補の場合、ここで preventDefault は呼ばない
                // handleTouchStart の共通ロジックがタッチ開始時のスクロール制御を行う
                lastTapTime = currentTime;
            }
        }, { passive: false }); 

        // itemElement.addEventListener('touchmove', ...) はここでは修正なし。handleTouchMove全体が修正対象。

        itemElement.addEventListener('touchend', (e) => {
            // チェックボックスがタッチされた場合は、デフォルトの動作を許可し、ダブルタップ処理は行わない
            if (e.target.closest('input[type="checkbox"]')) {
                console.log("[touchend] Checkbox within item touched. Allowing default.");
                return;
            }

            const currentTime = new Date().getTime();

            // isDraggingTouch が false (指がほとんど動いていない) かつ
            // currentDraggedItem が null (現在ドラッグ中のアイテムがない) かつ
            // ダブルタップと認識する時間範囲内である場合
            if (!isDraggingTouch && !currentDraggedItem && (currentTime - lastTapTime < DOUBLE_TAP_DELAY && currentTime - lastTapTime > 0)) {
                ...
                if (!e.target.closest('.drag-handle')) {
                    console.log(`[touchend] Confirmed double tap for item: ${itemElement.dataset.itemId}. Calling handleDoubleClick.`);
                    handleDoubleClick(e); // ★修正: handleDoubleClick を呼び出すように変更★
                    e.stopPropagation(); 
                    e.preventDefault(); 
                    // finishDragging(); // handleDoubleClick内で既に呼ばれるためここでは不要
                } else {
                    console.log("[touchend] Double tap on drag handle. Ignoring restore.");
                }
            }
            lastTapTime = 0; 
            initialTouchX = 0; 
        ...
        // setlist-item 内のチェックボックスにもイベントリスナーを設定 (変更なし)
    ...
    // セットリストのスロットにドロップターゲットとしてのイベントを設定 (スクロール関連の修正はここには含めない)
    setlist.querySelectorAll(".setlist-slot").forEach((slot, index) => {
        ...
        // スロット自体のタッチイベントも強化 (スクロール関連の修正はここには含めない)
        slot.addEventListener('touchstart', (e) => {
            if (!e.target.closest('.setlist-item') && !currentDraggedItem) {
                ...
                initialTouchY = touch.clientY;
                isDraggingTouch = false; 
                // ★修正: 空のスロットでのタッチはpreventDefaultを呼ばない (スクロールを許可)★
                // e.preventDefault(); 削除済
                console.log("[slot touchstart] Touch initiated on empty slot. Allowing default behavior.");
            } else if (e.target.closest('input[type="checkbox"]')) {
                console.log("[slot touchstart] Checkbox within slot touched. Allowing default.");
                return;
            }
        }, { passive: false });
    ...
    // MutationObserver: セットリストへのアイテム追加を監視し、イベントリスナーを設定 (変更なし)
