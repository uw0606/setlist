...
function handleTouchStart(event) {
    ...
    // これにより、長押し意図でタップしたものが、中途半端な長押しで離したときにクリックやダブルタップとして
    // 誤認識されることを防ぎます。
    event.preventDefault(); 
    ...
    if (event.touches.length === 1) {
        ...
        if (isInSetlist || isInAlbumContent) {
            ...
            touchTimeout = setTimeout(() => {
                // タイマー発火時にisDraggingがまだfalseであることを確認し、
                // ドラッグ開始の条件が満たされていることを再確認
                if (!isDragging && draggingItemId && document.body.contains(initialTouchElement)) {
                    ...
                    // ここでドラッグ開始に至らなかった場合（例えば、既に指が離れてfinishDraggingが呼ばれた後）
                    // は、追加のクリーンアップは不要だが、状態によっては必要になる可能性もあるのでログを残す
                }
