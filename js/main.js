...
function handleTouchMove(event) {
    ...
    if (!isDragging) {
        ...
        if (distance > DRAG_THRESHOLD) {
            ...
            // ドラッグが開始されたら、ここで一度preventDefault()を呼ぶ
            // 以降のRAF内での移動中は、RAFが実行されるたびにpreventDefault()が呼ばれ続ける
            event.preventDefault();
        } else {
            // しきい値に達していない場合は何もしない = スクロールを許可
            return;
        }
    }
    ...
    if (isDragging && currentTouchDraggedClone) {
        ...
        event.preventDefault(); // ここで毎回呼ぶのは変わらず

        // ... (以降の requestAnimationFrame ロジックは変更なし)
        if (rafId !== null) {
