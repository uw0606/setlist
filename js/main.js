...
function handleDragEnter(event) {
    ...
    if (targetSlot && !(originalSetlistSlot && targetSlot.dataset.slotIndex === originalSetlistSlot.dataset.slotIndex)) {
        ...
        // ドロップターゲットとして有効にする
        targetSlot.style.pointerEvents = 'auto'; // ★追加：ドラッグオーバー中にpointer-eventsをautoにする
...
function handleDragOver(event) {
    ...
    if (newDropZone && !(originalSetlistSlot && newDropZone.dataset.slotIndex === originalSetlistSlot.dataset.slotIndex)) {
        ...
        // ドロップターゲットとして有効にする
        newDropZone.style.pointerEvents = 'auto'; // ★追加：ドラッグオーバー中にpointer-eventsをautoにする
    ...
    if (event.touches.length === 1) {
        ...
        touchTimeout = setTimeout(() => {
            if (draggingItemId && document.body.contains(touchedElement)) {
                ...
                // ドラッグ開始時にすべてのセットリストスロットをドロップ可能にする（pointer-events: auto;）
                document.querySelectorAll('.setlist-slot').forEach(slot => {
                    slot.style.pointerEvents = 'auto'; // ★修正：ドラッグ開始時にすべてのスロットを有効にする
                    slot.style.touchAction = 'pan-y'; // ★追加：タッチアクションも有効にする
                });
            } else {
...
function handleTouchMove(event) {
    ...
    document.querySelectorAll('.setlist-slot').forEach(slot => { // ★修正：すべてのスロットを対象に
        ...
        slot.style.pointerEvents = 'auto'; // ★追加：タッチ移動中もpointer-eventsをautoにする
    });
