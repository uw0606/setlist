...
function handleDragStart(event) {
  // Chromeなどでドラッグイメージが出ない場合があるため、setDataを呼ぶ前に少し遅延させる
  // setTimeoutを削除することで、dragstart時に確実にデータが設定されるようにする
...
function fillSlotWithItem(slotElement, itemToFill) {
  ...
  console.log(`[fillSlotWithItem]   itemToFill element received:`, itemToFill);
  ...
  let songName = itemToFill.dataset.songName; // 常にdataset.songNameを優先する

  // もしdataset.songNameが何らかの理由で取得できない場合のみ、textContentから取得を試みる
  if (!songName) {
      if (itemToFill.classList.contains('setlist-item')) {
          // セットリスト内のアイテムの場合、song-info divのテキストノードから取得を試みる
          const existingSongInfoDiv = itemToFill.querySelector('.song-info');
          if (existingSongInfoDiv && existingSongInfoDiv.childNodes[0] && existingSongInfoDiv.childNodes[0].nodeType === Node.TEXT_NODE) {
              songName = existingSongInfoDiv.childNodes[0].textContent.trim();
          } else {
              songName = itemToFill.textContent.trim(); // 最後の手段
          }
      } else {
          // ハンバーガーメニューからの通常のアイテムの場合
          songName = itemToFill.textContent.trim();
      }
      console.warn(`[fillSlotWithItem] songName was empty for itemId ${itemId}. Derived from textContent: "${songName}"`);
  }
  ...
  console.log(`[fillSlotWithItem]   Resolved songName to use: "${songName}"`);

  // スロットに曲情報のためのdivを追加
  const songInfoDiv = document.createElement('div');
  songInfoDiv.classList.add('song-info');
  songInfoDiv.textContent = songName; // ここで設定されるテキストはクリーンな曲名であるべき
  ...
  slotElement.dataset.songName = songName; // ここにクリーンな曲名を保存
  // スロット自身が元のスロットインデックスを持つのは、スロット内移動時のために必要
  // アルバムからの追加の場合は originalSlotIndex は不要だが、既存のロジックに合わせて保持
  if (slotElement.dataset.slotIndex) { // slotIndex が存在する場合のみ originalSlotIndex を設定
  ...
  if (originalList) {
    ...
    if (albumItem) {
      ...
      console.log(`[restoreToOriginalList] Original album item found and displayed: ${itemId}`);
        ...
        // 元のアイテムのクラスをコピー（アルバムクラスやshortクラスなど）
        ...
        newAlbumItem.dataset.songName = item.dataset.songName; // dataset.songNameを使用
        newAlbumItem.textContent = item.dataset.songName; // テキスト内容もdatasetから設定
