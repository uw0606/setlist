...
function processDrop(dropZone) {
    ...
    if (dropZone.id === 'trashCan') {
        ...
        if (originalSetlistSlot) {
            ...
            // originalSetlistSlot.style.visibility = ''; // finishDraggingで確実に可視に戻されるため不要
        } else {
    ...
    if (originalSetlistSlot === null) { // アルバムリストからドラッグされた場合
        ...
        if (originalSlotIndex === targetSlotIndex) {
            ...
            // ここでの復元はfinishDraggingに任せるか、あるいはvisibilityのみ戻す
            // originalSetlistSlot.style.visibility = ''; // finishDraggingで確実に可視に戻されるため不要
            // originalSetlistSlot.classList.remove('placeholder-slot'); // finishDraggingで確実に除去されるため不要
            // _originalItemDataはfinishDraggingでクリアされるか復元される
...
function handleTouchStart(e) {
    ...
    // ここで一旦 preventDefault を呼ぶことで、スクロールを抑制し、ドラッグ開始を優先
    // ただし、これが通常のスクロールを妨げる可能性もあるため、注意が必要
    // e.preventDefault(); // ★後続のmoveイベントで制御するために、ここでは一旦コメントアウト
    ...
    touchTimeout = setTimeout(() => {
        ...
        e.preventDefault(); // 長押しでドラッグ開始が確定したら、ここで preventDefault を呼ぶ
...
function finishDragging() {
  ...
  if (currentPcDraggedElement && setlist.contains(currentPcDraggedElement)) {
    ...
    // if (!isDragging) { // ドラッグが成功しなかった場合のみ表示を戻す
  ...
  if (originalSetlistSlot) { // originalSetlistSlot が存在する場合に処理
    ...
    // isDraggingがfalse (つまりドロップが成功していない、またはキャンセルされた) か、
    // processDropで元のスロットから移動しているのにvisibilityが戻っていない場合をカバー
    ...
    if (originalSetlistSlot._originalItemData) {
        ...
        if (!isDragging && currentDropZone !== originalSetlistSlot) { // ドロップ失敗時
            console.log(`[finishDragging] Drag failed. Restoring original item to slot ${originalSetlistSlot.dataset.slotIndex}.`);
            fillSlotWithItem(originalSetlistSlot.dataset.slotIndex, originalSetlistSlot._originalItemData);
        } else if (currentDropZone === originalSetlistSlot) { // 同じスロットに戻された場合
             // processDrop内で既にrestoreされているはずだが、念のため二重に呼ぶ
             console.log(`[finishDragging] Item dropped back to original slot ${originalSetlistSlot.dataset.slotIndex}. Ensuring restore.`);
             fillSlotWithItem(originalSetlistSlot.dataset.slotIndex, originalSetlistSlot._originalItemData);
        }
