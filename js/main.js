...
document.addEventListener("dblclick", (event) => {
  ...
  if (!!event.target.closest("#setlist")) {
    // 共有されたセットリストのアイテムをハンバーガーメニューに戻す処理 (曲名検索による複製)
    ...
    // ハンバーガーメニューのアイテムをセットリストに追加する処理 (既存の処理)
    if (setlist.children.length < maxSongs) {
      ...
      .then((snapshot) => {
        ...
        if (state && state.setlist) {
          ...
          state.setlist.forEach(itemData => {
            ...
            li.addEventListener("dragstart", (event) => { // ← 追加
                draggingItem = li;
                li.classList.add("dragging");
                event.dataTransfer.setData("text/plain", "");
            });
            li.addEventListener("dragend", finishDragging); // ← 追加
