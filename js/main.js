...
let draggingElement = null; // 新しく追加: ドラッグ中の元の要素を保持
...
function handleTouchStart(event) {
    ...
    if (isCheckboxClick) {
        ...
        draggingElement = null; // ドラッグ要素もリセット
        return; 
    }
    ...
    // ここで event.preventDefault() を呼ぶことで、以降のデフォルト動作（スクロールなど）を停止
    // ただし、ダブルタップやチェックボックスクリックの直後は呼ばない
    // ダブルタップの判定が優先されるため、ここではまだ呼ばないでおく。
    // ロングプレスでのドラッグ開始時に preventDefault() を呼ぶ。
    // もしくは、touchmoveで閾値を超えたらpreventDefault()を呼ぶ。
    ...
    if (event.touches.length === 1) {
        ...
        draggingElement = touchedElement; // ここで元の要素を保存
        ...
        touchTimeout = setTimeout(() => {
            ...
            if (!isDragging && draggingItemId && document.body.contains(touchedElement)) {
                // ここで event.preventDefault() を呼び、スクロールを停止する
                // preventDefault() は handleTouchMove ではなく、ドラッグ開始時にのみ呼ばれる
                // ただし、touchstartで呼んでしまうと、その後のスクロールもブロックされるため、
                // touchmoveでクローン作成直前に呼ぶのが望ましいが、ロングプレスの場合はtouchstartのsetTimeout内で呼ぶ。
                // 複数のイベントハンドラでpreventDefault()が呼ばれると問題が起きる可能性があるため、
                // touchmoveのpreventDefault()はisDraggingがtrueの場合に限定する。
                event.preventDefault(); // ここでスクロールなどのデフォルト動作を防止
                createTouchDraggedClone(touchedElement, touchStartX, touchStartY, draggingItemId);
                isDragging = true;
                console.log("[touchstart:Mobile] Dragging initiated after timeout (long press).");
            } else {
        ...
        console.log("[touchstart:Mobile] Touch detected. Waiting for long press or double tap.");
    }
}
...
const DRAG_THRESHOLD = 20; // 10ピクセル以上動いたらドラッグと判定
...
function handleTouchMove(event) {
    // ドラッグが開始されていない場合は何もしない（スクロールを許可）
    if (!isDragging) {
        // まだドラッグが開始されていないが、ロングプレス判定のためのタッチムーブが発生している場合は
        // デフォルトのスクロール動作を阻止しない
        if (touchTimeout) {
            // ロングプレス判定中のわずかな動きはスクロールとして扱う。
            // ここで preventDefault を呼ぶとスクロールもブロックされてしまう。
            // event.preventDefault(); // これは削除
        }
        return;
    }

    // ドラッグが既に開始されている場合のみ preventDefault() を呼び出す
...
function handleTouchEnd(event) {
    // touchTimeout が残っている場合（ロングプレスにならなかった場合）
    // これは短いタップまたはスクロールの意図だったと判断
    if (touchTimeout) {
        clearTimeout(touchTimeout);
        touchTimeout = null;
