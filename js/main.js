...
async function generateSetlistPdf() {
    ...
    try {
        ...
        const MAX_SONG_FONT_SIZE = 60; // 曲の最大フォントサイズ (変更なし)
        const MAX_HEADER_FONT_SIZE = 40; // ★修正1: タイトルの最大フォントサイズを40ptに設定
        ...
        // --- 1. ヘッダーの描画と高さの計算 (曲のサイズに依存しない独立ロジック) ---
        let simpleHeaderHeight = 0;
        if (headerText) {
            // ★修正2: タイトルサイズを独立計算（MIN_FONT_SIZE + 4 からスタート）
            ...
            // ★修正3: MAX_HEADER_FONT_SIZE (40pt) を上限として適用
