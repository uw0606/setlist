...
function shareSetlist() {
    ...
    setlistRef.set(currentState)
        .then(() => {
            ...
            // 共有する最終的なテキスト (セットリスト内容 + リンク)
            const finalShareContent = `${shareText}\n共有リンク: ${shareLink}`; 

            if (navigator.share) {
                // ★修正箇所: Web Share API の text プロパティにリンクも追加し、url プロパティはそのまま残す
                navigator.share({
                    title: 'セットリスト共有',
                    // text にセットリスト内容とリンクを含めることで、アプリが text のみを扱う場合に対応
                    text: finalShareContent, 
                    // url も残すことで、リンクとして認識する必要があるアプリに対応
                    url: shareLink,
                })
                ...
                // PC (Web Share API非対応): クリップボードへのコピーは既に finalShareContent を使用
                const tempInput = document.createElement('textarea');
                tempInput.value = finalShareContent; 
