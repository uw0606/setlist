...
function handleTouchStart(e) {
    ...
    // ★修正点1: タッチされた要素がアルバムアイテムかセットリストスロットかを正確に判断
    ...
    if (itemId) {
        ...
        // itemIdがない場合はドラッグを開始しない
        return;
    }

    // ★修正点2: ドラッグ元がアルバムアイテムか、セットリストスロットかを明確に区別するロジック
    // targetElement が .album-content 内の .item であることを確認する
    const isAlbumItem = targetElement.classList.contains('item') && targetElement.closest('.album-content');
    const isSetlistItem = targetElement.classList.contains('setlist-item'); // セットリストに既にあるアイテム
    const isEmptySetlistSlot = targetElement.classList.contains('setlist-slot') && !targetElement.classList.contains('setlist-item'); // 空のセットリストスロット
    ...
    touchTimeout = setTimeout(() => {
        ...
        if (isAlbumItem) {
            ...
            currentPcDraggedElement = targetElement;
        } else if (isSetlistItem) {
            targetElement.style.visibility = 'hidden';
