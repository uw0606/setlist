...
function clearSlotContent(slotIndex) {
    // 修正点: `document.querySelector` でスロット要素を取得
    const slot = document.querySelector(`.setlist-slot[data-slot-index="${slotIndex}"]`);
    if (slot) {
        // スロットから全てのアイテム関連のクラスとデータを削除
        slot.classList.remove('setlist-item', 'item', 'album'); // 必要に応じて他のクラスも削除
        for (const key in slot.dataset) {
            if (key !== 'slotIndex') { // slotIndexは残す
                delete slot.dataset[key];
            }
        }
        slot.innerHTML = `<div class="index">${slotIndex + 1}</div><div class="song-name-and-option"></div>`; // 初期状態に戻す
        // 他の要素（チェックボックスなど）も削除または初期化するロジックを追加

        // 必要に応じて、内部のデータ構造（例：保存されたセットリスト配列）からも削除する
        // 例: setlistData[slotIndex] = null; // または適切な初期値

        console.log(`[clearSlotContent] Slot ${slotIndex} content cleared.`);
        return true;
    }
...
function handleTouchMove(e) {
    if (touchTimeout) {
        // 長押しタイマーが動いている間に指が動いたら、ドラッグではなくスクロールと判断
        const currentX = e.touches[0].clientX;
        const currentY = e.touches[0].clientY;
        const deltaX = Math.abs(currentX - startTouchPos.x); // 修正点: startTouchPos.x を使用
        const deltaY = Math.abs(currentY - startTouchPos.y); // 修正点: startTouchPos.y を使用

        // ある程度の移動量を超えたら長押しをキャンセルし、スクロールを許可
        if (deltaX > DRAG_THRESHOLD || deltaY > DRAG_THRESHOLD) {
            clearTimeout(touchTimeout);
            touchTimeout = null;
            console.log("[touchmove] Moved beyond threshold before long press. Cancelling drag timeout. Allowing scroll.");
            // ここで return することで、e.preventDefault() を呼ばずにスクロールを許可
            return;
        }
    }

    // ドラッグ中の場合のみ preventDefault を呼び出し、スクロールを抑制
    if (isDragging) {
        e.preventDefault(); // これがスクロールを抑制する主な役割
        updateTouchDraggedClonePosition(e); // 修正点: イベントオブジェクト全体を渡す
        // ドロップゾーンの判定ロジック
        const targetElement = document.elementFromPoint(e.touches[0].clientX, e.touches[0].clientY);
        handleDragOver(targetElement); // ドロップゾーンのハイライトなどを処理
    }
}
...
function finishDragging() {
  ...
  if (originalSetlistSlot) { // originalSetlistSlot が存在する場合に処理
    ...
    if (originalSetlistSlot._originalItemData) {
        // ドロップが成功しなかった場合、または同じスロットに戻された場合
        if (!isDragging || currentDropZone === originalSetlistSlot) {
            ...
            // 修正点: fillSlotWithItem の第1引数をスロット要素自体に変更
 ...
 */
function processDrop(droppedItemId, dropZone) { // 修正点: droppedItemId を引数に追加
    ...
    if (dropZone.id === 'trashCan') {
        ...
        if (originalSetlistSlot) {
            ...
            clearSlotContent(parseInt(originalSetlistSlot.dataset.slotIndex)); // 修正点: 引数修正
            restoreToOriginalList({dataset: {itemId: droppedItemId}}); // restoreToOriginalList には要素が必要
        } else {
            // アルバムからのアイテムをゴミ箱に捨てた場合
            // 何もしない（アルバムアイテムは常にメニューにあるため、見えないだけ）
            console.log(`[processDrop] Album item ${droppedItemId} dropped on trash can. No action needed for album item.`);
            restoreToOriginalList({dataset: {itemId: droppedItemId}}); // アルバムアイテムをゴミ箱に捨てた場合、非表示になっているものを表示に戻す
        }
        showMessageBox('アイテムを削除しました。'); // 修正点: showMessage を showMessageBox に変更
        finishDragging();
        return;
    }

    // ドロップ先がセットリストスロットでない場合（通常は発生しないはずだが念のため）
    ...
    if (originalSetlistSlot === null) { // アルバムリストからドラッグされた場合
        ...
        if (originalSlotIndex === targetSlotIndex) {
            ...
            showMessageBox('元の位置に戻しました。'); // 修正点: showMessage を showMessageBox に変更
        } else {
            // スロット間で移動またはスワップ
            ...
            if (targetItemData) {
                ...
                showMessageBox('アイテムを入れ替えました。'); // 修正点: showMessage を showMessageBox に変更
            } else {
                // 移動 (空のスロットへ)
                ...
                showMessageBox('アイテムを移動しました。'); // 修正点: showMessage を showMessageBox に変更
            }
        }
    }

    // ドロップ処理が完了したら、ドラッグ状態をクリーンアップ
    // finishDragging() は processDrop の呼び出し元 (handleDrop, handleTouchEnd) で呼ばれるため、ここでは不要。
    // しかし、明示的に呼んでも問題ない場合が多い。
    // 今回は呼び出し元に任せるためコメントアウトしますが、必要に応じて戻してください。
    // finishDragging();
}
