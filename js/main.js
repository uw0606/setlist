...
function handleDrop(event) {
  ...
  if (setlist.children.length < maxSongs) {
    ...
    const originalSongInfo = draggingItem.querySelector(".song-info");
    const songInfo = document.createElement("div");
    songInfo.classList.add("song-info");
    songInfo.textContent = draggingItem.textContent;

    const hasExistingShortLabel = songInfo.querySelector("span") !== null;
    const hasExistingCheckbox = songInfo.querySelector("input[type='checkbox']") !== null;

    // 元の song-info が存在する場合は、チェックボックスと "(Short)" ラベルを引き継ぐ
    if (originalSongInfo) {
      const originalCheckbox = originalSongInfo.querySelector("input[type='checkbox']");
      const originalShortLabel = originalSongInfo.querySelector("span");
      if (originalCheckbox && !hasExistingCheckbox) {
        songInfo.appendChild(originalCheckbox.cloneNode(true)); // チェックボックスをコピー
      }
      if (originalShortLabel && !hasExistingShortLabel) {
        songInfo.appendChild(originalShortLabel.cloneNode(true)); // "(Short)" ラベルをコピー
      }
    } else if (draggingItem.classList.contains("short") && !hasExistingShortLabel) {
      // 元の song-info が存在せず、".short" クラスがあり、"(Short)" ラベルがない場合のみ新規作成
      ...
      // ".short" クラスがなく、"(Short)" ラベルが存在する場合は削除
      songInfo.querySelector("span").remove();
