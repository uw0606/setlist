...
function handleDoubleClick(event) {
    ...
    // ★★★ STEP 2: 要素とデータ属性の確認ログ ★★★
    if (albumItemElement) {
        console.log(`[handleDoubleClick] Found .item element: ID=${albumItemElement.dataset.itemId}`);
        const drumsoloOptionValue = albumItemElement.dataset.hasDrumsoloOption;
        console.log(`[handleDoubleClick] hasDrumsoloOption attribute value: ${drumsoloOptionValue}`);
    } else {
        console.log("[handleDoubleClick] No .item element found via closest().");
    }
    // ★★★ デバッグログここまで ★★★
    ...
    if (albumItemElement && !albumItemElement.classList.contains('setlist-slot')) {
        ...
        if (firstEmptySlot) {
            ...
            addSongToSlot(firstEmptySlot, songData.itemId, songData.name, {
                ...
                // ★★★ データの引き渡し（論理的に正しいコード）★★★
                // 「data-has-drumsolo-option」属性が存在し、その値が 'true' なら true を返す
