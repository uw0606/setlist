...
function handleTouchStart(event) {
    ...
    if (tapLength < 300 && tapLength > 0) {
        ...
        // ここで再度 event.preventDefault() を呼ぶ必要はないが、stopPropagation() は重要
...
function handleTouchMove(event) {
    ...
    // ★重要: handleTouchStartで既にpreventDefault()を呼んでいるが、
    // touchmoveでも継続して呼ぶことで、ドラッグ中のスクロールを確実に抑制する。
    event.preventDefault(); 
    event.stopPropagation(); // イベント伝播も停止
    ...
    rafId = requestAnimationFrame(() => {
        ...
        if (!isDragging) {
            ...
            // 移動距離が閾値を超えていたら、長押しタイマーをキャンセルする
            // ここでのキャンセルは、ドラッグ開始前にユーザーが意図せず指を動かしてしまい、
            // それがスクロールと見なされるのを防ぐため。
            // event.preventDefault()はhandleTouchStartで既に呼ばれているので、スクロールは発生しない。
            if (deltaX > DRAG_THRESHOLD || deltaY > DRAG_THRESHOLD) {
                if (touchTimeout) {
                    clearTimeout(touchTimeout);
                    touchTimeout = null; 
                    console.log("[handleTouchMove] Movement beyond threshold before drag started, cancelling long press timer.");
                }
            }
            rafId = null;
        ...
        // ドラッグが開始された後 (isDraggingがtrue) の処理
...
function handleTouchEnd(event) {
    ...
    if (isDragging && currentTouchDraggedClone) { 
        ...
        console.log("[touchEnd] No active drag or drag not initiated, cleaning up.");
        if (currentTouchDraggedClone) {
            currentTouchDraggedClone.remove(); // 生成されたクローンを削除
            currentTouchDraggedClone = null;
        }
        if (originalSetlistSlot) {
