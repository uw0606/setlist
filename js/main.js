...
// main.js の getSlotItemData 関数 (例)
/**
 * スロット要素からアイテムデータを抽出するヘルパー関数
 * @param {HTMLElement} slotElement - セットリストスロットまたはアルバムアイテム要素
 * @returns {Object|null} アイテムデータオブジェクト、またはデータが見つからない場合はnull
 */
function getSlotItemData(slotElement) {
    ...
    return {
        ...
        name: slotElement.dataset.songName, // song-name -> songName
        short: slotElement.dataset.short === 'true', // 'true'/'false' 文字列をブール値に変換
        seChecked: slotElement.dataset.seChecked === 'true',
        drumsoloChecked: slotElement.dataset.drumsoloChecked === 'true',
        isShortVersion: slotElement.dataset.isShortVersion === 'true',
        hasSeOption: slotElement.dataset.hasSeOption === 'true',
        hasDrumsoloOption: slotElement.dataset.hasDrumsoloOption === 'true',
        rGt: slotElement.dataset.rGt || '',
        lGt: slotElement.dataset.lGt || '',
        bass: slotElement.dataset.bass || '',
        bpm: slotElement.dataset.bpm ? parseInt(slotElement.dataset.bpm) : 0,
        chorus: slotElement.dataset.chorus || '',
        // その他の必要なデータ属性があればここに追加
    };
}
...
// main.js の handleTouchStart 関数 (一部抜粋、関連部分のみ)
...
function handleTouchStart(e) {
    ...
    const targetElement = e.target.closest('.item, .setlist-slot'); // .item (アルバムアイテム) または .setlist-slot を取得
    if (!targetElement) {
    ...
    if (e.target.classList.contains('checkbox')) {
        console.log("[touchstart:Mobile] Touched checkbox. Preventing drag.");
        return;
    }

    // ドラッグ中の要素が既に存在する場合は何もしない
    if (isDragging) {
        console.log("[touchstart:Mobile] Already dragging. Ignoring new touch start.");
        return;
    }

    // グローバル変数 activeTouchSlot を設定 (touchmove/touchend で使用)
    ...
    // アルバムアイテムからのドラッグか、セットリストからのドラッグかを判定
    if (targetElement.classList.contains('album')) {
        originalSetlistSlot = null; // アルバムからのドラッグなので元のセットリストスロットはなし
        console.log("[touchstart:Mobile] Dragging from album list:", itemId);
    } else if (targetElement.classList.contains('setlist-slot') && targetElement.classList.contains('setlist-item')) {
        originalSetlistSlot = targetElement; // セットリストからのドラッグなので元のスロットを保存
        // セットリスト内での移動の場合、_originalItemData に元のスロットのデータを保存
        originalSetlistSlot._originalItemData = getSlotItemData(originalSetlistSlot);
        console.log("[touchstart:Mobile] Dragging from setlist slot:", itemId, "Original data saved.");
    } else {
        // 空のスロットを触った場合はドラッグを開始しない
    ...
    touchTimeout = setTimeout(() => {
        ...
        isDragging = true;
        createTouchDraggedClone(targetElement); // クローンを作成
        // dragstartイベントのように、元の要素を非表示にする (モバイルドラッグの場合)
        // アルバムアイテムの場合
        if (targetElement.classList.contains('album')) {
            targetElement.style.visibility = 'hidden';
            currentPcDraggedElement = targetElement; // finishDraggingで元の表示に戻すために一時的に保持
        } else if (targetElement.classList.contains('setlist-item')) {
            // セットリスト内のアイテムの場合
            targetElement.style.visibility = 'hidden';
            targetElement.classList.add('placeholder-slot'); // プレースホルダーとしてマーク
            currentPcDraggedElement = targetElement; // finishDraggingで元の表示に戻すために一時的に保持
        }
        e.preventDefault(); // デフォルトのスクロール動作などを防ぐ
    }, LONG_PRESS_THRESHOLD);

    // デフォルトのスクロール動作を防ぐためにタッチイベントを抑制
    // ただし、長押しに達しなかった場合は touchend で解除される
    // e.preventDefault(); // これをコメントアウトして、タッチスクロールを許可し、閾値を超えたらドラッグを開始するようにする
}
...
// main.js の processDrop 関数 (一部抜粋、関連部分のみ)

/**
 * ドロップされたアイテムの処理
 * @param {string} droppedItemId - ドロップされたアイテムのID (実際には draggingItemId を使用)
 * @param {HTMLElement} dropZone - ドロップされたターゲット要素（セットリストのスロットまたはゴミ箱）
 */
function processDrop(droppedItemIdParam, dropZone) { // droppedItemIdParam はここでは使わず、draggingItemId を使う
    console.log(`[processDrop] Dropping item (global) ${draggingItemId} onto`, dropZone);

    if (!draggingItemId) { // グローバルな draggingItemId を確認
    ...
    if (originalSetlistSlot) {
        ...
        // アルバムリストからドラッグされた場合
        // ★ここを修正★
        // albumItemsData という全体データ配列から、draggingItemId に一致するアイテムを探す
        // albumItemsData はグローバルに定義されていることを前提とする
        draggedItemData = albumItemsData.find(item => item.itemId === draggingItemId);
        if (draggedItemData) {
            // 見つかったアイテムデータは元のオブジェクトへの参照なので、コピーを作成して変更に備える
            draggedItemData = { ...draggedItemData };
            console.log("[processDrop] Dragged item data from albumItemsData:", draggedItemData);
        } else {
            console.error(`[processDrop] Critical: dragged item data not found in albumItemsData for ID: ${draggingItemId}.`);
            return;
        }
    }

    if (!draggedItemData) {
        console.error("[processDrop] Critical: draggedItemData is null after all attempts. Cannot proceed with drop.");
        return;
    }
    ...
    if (originalSetlistSlot === null) {
        ...
        if (targetSlotHasItem) {
            ...
            if (targetSlotCurrentItemData) {
                ...
                restoreToOriginalList(targetSlotCurrentItemData); // 修正: itemElement ではなく itemData を渡す
            }
            // ドロップ先のスロットをドラッグしたアイテムで埋める
            draggedItemData.slotIndex = targetSlotIndex;
            fillSlotWithItem(dropZone, draggedItemData);
            // 元のアルバムアイテムを非表示にする
            const albumItemToHide = document.querySelector(`.album-content .item[data-item-id="${draggingItemId}"]`);
...
let albumItemsData = []; // この配列にすべてのアルバムアイテムデータがロードされている必要があります

// 例えば、JSONファイルからロードする関数内で
async function loadAlbumData() {
    const response = await fetch('data/album_data.json'); // 仮のパス
    albumItemsData = await response.json();
    // ロード後、DOMのアルバムアイテムに data-item-id などを設定する処理を呼ぶ
