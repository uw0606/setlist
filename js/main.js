...
function handleTouchStart(event) {
    ...
    if (event.touches.length === 1) {
        ...
        // ここでの setTimeout は、ロングプレスを検出して createTouchDraggedClone を呼ぶためのものなので維持
        // ただし、この中で event.preventDefault() を呼ばないようにする
        touchTimeout = setTimeout(() => {
            if (!isDragging && draggingItemId && document.body.contains(touchedElement)) {
                // ★修正: ここで event.preventDefault() を呼ばないようにする★
                // event.preventDefault(); // この行を削除
                createTouchDraggedClone(touchedElement, touchStartX, touchStartY, draggingItemId);
                isDragging = true;
                // ここでドラッグ開始を検知した場合は、スクロールを止めるためのpreventDefaultを
                // handleTouchMoveの初回で確実に行う必要がある。
            } else {
...
function handleTouchMove(event) {
    // ★ここから修正点★
    // まだドラッグが始まっていない場合のみ、ドラッグ開始を検出し、preventDefaultを呼ぶ
    if (!isDragging && draggingItemId && event.touches.length === 1) {
        const currentX = event.touches[0].clientX;
        const currentY = event.touches[0].clientY;
        const dx = Math.abs(currentX - touchStartX);
        const dy = Math.abs(currentY - touchStartY);
        const dragThreshold = 10; // 例: 10ピクセル以上移動したらドラッグ開始とみなす

        if (dx > dragThreshold || dy > dragThreshold) {
            // ロングプレス待ちのsetTimeoutがあればクリア
            if (touchTimeout) {
                clearTimeout(touchTimeout);
                touchTimeout = null;
            }
            ...
            if (touchedElement) {
                event.preventDefault(); // ブラウザのスクロールを停止！
                createTouchDraggedClone(touchedElement, touchStartX, touchStartY, draggingItemId);
                isDragging = true;
...
function handleTouchEnd(event) {
    ...
    // ★ここから修正点★
    // dropTargetSlot が見つからなかった場合、セットリスト内であれば最も近いスロットを探す
    if (!dropTargetSlot) {
        ...
        if (touch.clientX >= setlistRect.left && touch.clientX <= setlistRect.right &&
            ...
            setlist.querySelectorAll('.setlist-slot').forEach(slot => {
                ...
                // 有効なスロット（空または既存のアイテムがあるスロット）のみを対象とする
                // 自分自身のスロットを除外することで、無限に自分に戻るのを避ける
    ...
    if (dropTargetSlot) {
        ...
        // ★ここから重要な修正: dropTargetSlot が null の場合に、必ず元の場所に戻すことを保証する★
