...
// ★追加: ドラッグ開始とみなす最小移動距離 (ピクセル)
const DRAG_THRESHOLD = 100; // 10ピクセル以上移動したらドラッグと判断
...
function handleTouchStart(event) {
    ...
    if (!touchedElement) {
        ...
        // 非ドラッグ可能な要素であれば、ここで絶対にpreventDefaultを呼ばない
        return;
    }
    ...
    if (tapLength < 300 && tapLength > 0) {
        ...
        // ダブルタップ時はデフォルト動作を防止するためにpreventDefaultを呼ぶ
        // しかし、これが通常のスクロールを阻害している可能性もある
        event.preventDefault(); // ★ここで呼ばれるpreventDefaultが原因の可能性あり★
        return;
    }
    lastTapTime = currentTime;

    // ここに到達したということは、draggable要素にシングルタッチされた、かつダブルタップではないということ
    if (event.touches.length === 1) {
        ...
        // ★重要: ここでは絶対に event.preventDefault() を呼ばない★
        // preventDefaultはhandleTouchMoveでドラッグが確定したときにのみ呼ぶ
    }
}
/*------------------------------------------------------------------------------------------------------------*/
