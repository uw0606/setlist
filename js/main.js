let touchTimeout = null; // 長押し判定用
let startTouchPos = { x: 0, y: 0 }; // タッチ開始位置
const DRAG_THRESHOLD = 5; // ドラッグ開始と判断する移動距離の閾値 (ピクセル)
...
// 他のグローバル変数やユーティリティ関数（showMessage, getSlotItemData, fillSlotWithItem,
// clearSlotContent, swapItems, swapSetlistItemSlots, moveSetlistItemToEmptySlot,
// findNextEmptySetlistSlot, hideSetlistItemsInMenu など）は、
// このコードブロックの上部または別途定義されているものとしています。
// ここでは提供されたコードと修正が必要な関数のみを記述します。

/**
 * タッチ開始時の処理 (モバイル用)
 */
function handleTouchStart(e) {
    console.log("[touchstart] Touch start detected.");

    // 多点タッチは無視
    if (e.touches.length > 1) {
        console.log("[touchstart] Multiple touches detected. Ignoring.");
        return;
    }
    ...
    // 前回のタップからの時間差でダブルタップを判定
    if (tapLength < 500 && tapLength > 0) { // 500ms以内
        console.log("[touchstart] Double tap detected. Handled by handleDoubleClick.");
        handleDoubleClick(e.target); // ★修正点: e.target を直接渡す
        lastTapTime = 0; // ダブルタップとして処理済み
        e.preventDefault(); // ダブルタップの場合はデフォルトの動作をキャンセル
        return; // ダブルタップの場合はドラッグ処理に進まない
    }
    lastTapTime = currentTime; // 今回のタップ時間を記録

    // チェックボックスに対するタッチは、通常通りに処理させる（ドラッグ開始させない）
    if (e.target.closest('input[type="checkbox"]')) {
        console.log("[touchstart] Touched checkbox. Allowing default behavior for checkbox.");
        return;
    }

    // ドラッグハンドル、または.item要素自体が対象
    const targetItem = e.target.closest('.drag-handle, .item');
    if (!targetItem) {
        console.log("[touchstart] Touched element is not a drag handle or an item. Allowing default behavior.");
        return;
    }

    isDragging = false; // ドラッグ開始前はfalseにリセット
    draggingItemId = targetItem.dataset.itemId;
    ...
    touchTimeout = setTimeout(() => {
        ...
        e.preventDefault(); // ★修正点: 長押しでドラッグ開始が確定したら、ここで preventDefault を呼ぶ

        if (targetItem.classList.contains('setlist-item')) {
            // セットリスト内のアイテムをドラッグ
            console.log(`[touchstart:Mobile] Dragging from setlist slot: ${targetItem.dataset.slotIndex}, data:`, getSlotItemData(targetItem));
            originalSetlistSlot = targetItem;
            createTouchDraggedClone(targetItem, e.touches[0].clientX, e.touches[0].clientY, draggingItemId);
        } else {
            // アルバムリストからのアイテムをドラッグ
            console.log(`[touchstart:Mobile] Dragging from album list: ${draggingItemId}`);
            currentPcDraggedElement = targetItem; // モバイルでも元の要素を保持
            createTouchDraggedClone(targetItem, e.touches[0].clientX, e.touches[0].clientY, draggingItemId);
            // 元のアルバムアイテムは createTouchDraggedClone で visibility: hidden にされる
        }

        // クローン要素の位置を追従させるための requestAnimationFrame を開始
        rafId = requestAnimationFrame(() => updateTouchDraggedClonePosition(e)); // ★修正点: 新しく追加した関数を呼び出す

    }, 300); // 300ms 長押しでドラッグ開始

    console.log("[touchstart:Mobile] Touched element (non-checkbox):", targetItem, "itemId:", draggingItemId);
}
...
function handleTouchMove(e) {
    // isDragging が true になる前に move イベントが発生した場合、通常のスクロールと判断し何もしない
    if (!isDragging) {
        // 長押しタイマーが設定されていて、かつ閾値を超えて移動した場合、ドラッグをキャンセルしスクロールを許可
        const currentTouchX = e.touches[0].clientX;
        const currentTouchY = e.touches[0].clientY;
        const dx = Math.abs(currentTouchX - startTouchPos.x);
        const dy = Math.abs(currentTouchY - startTouchPos.y);

        if ((dx > DRAG_THRESHOLD || dy > DRAG_THRESHOLD) && touchTimeout) {
            clearTimeout(touchTimeout); // 長押しをキャンセル
            touchTimeout = null;
            console.log("[touchmove] Moved beyond threshold before long press. Cancelling drag timeout. Allowing scroll.");
            return; // スクロールを許可
        }
        return; // ドラッグ中でない場合はこれ以上何もしない
    }

    e.preventDefault(); // ドラッグ中はスクロールなどのデフォルト動作をキャンセル

    if (currentTouchDraggedClone) {
        // クローン要素の位置を更新 (requestAnimationFrameで制御されるため、ここでは不要かもしれないが念のため)
        currentTouchDraggedClone.style.left = e.touches[0].clientX - currentTouchDraggedClone.offsetWidth / 2 + 'px';
        currentTouchDraggedClone.style.top = e.touches[0].clientY - currentTouchDraggedClone.offsetHeight / 2 + 'px';
    }
    ...
    if (targetElement) {
        newDropZone = targetElement.closest('.setlist-slot, #trashCan');
    }

    if (newDropZone && newDropZone !== currentDropZone) {
        // 既存のハイライトを解除
        if (currentDropZone) {
            currentDropZone.classList.remove('drag-over', 'active-drop-target');
        }
        // 新しいドロップゾーンをハイライト
        newDropZone.classList.add('drag-over', 'active-drop-target');
        currentDropZone = newDropZone;
        console.log(`[touchmove] Active drop zone changed to: ${currentDropZone.id || currentDropZone.dataset.slotIndex}`);
    } else if (!newDropZone && currentDropZone) {
        // ドロップゾーンから外れた場合
        currentDropZone.classList.remove('drag-over', 'active-drop-target');
        currentDropZone = null;
        console.log("[touchmove] Left active drop zone. currentDropZone is now null.");
    }
}
...
function handleTouchEnd(e) {
    ...
    if (currentDropZone) {
        ...
        processDrop(currentDropZone); // ★成功時はprocessDropがfinishDraggingを呼ぶ★
    } else {
...
function finishDragging() {
  ...
  if (currentPcDraggedElement && setlist.contains(currentPcDraggedElement)) {
    // currentPcDraggedElement はPCドラッグまたはアルバムアイテムのドラッグ用
    currentPcDraggedElement.classList.remove("dragging");
    // アルバムからのドラッグアイテムだった場合は、元の要素を可視に戻す
    if (!originalSetlistSlot && currentPcDraggedElement.style.visibility === 'hidden') {
  ...
  if (originalSetlistSlot) { // originalSetlistSlot が存在する場合に処理
    ...
    // ★重要: ドロップが成功しなかった場合、または同じスロットに戻された場合、元のデータを復元★
    // originalSetlistSlot._originalItemData が残っている = 移動またはスワップが未完了 or キャンセルされた状態
    // currentDropZone が originalSetlistSlot 自身だった場合もカバー
    if (originalSetlistSlot._originalItemData) {
        // ドロップが成功しなかった場合、または同じスロットに戻った場合に、_originalItemDataを使ってスロットを再構築
        // この処理は `processDrop` の「同じスロットに戻された場合」とも連携
        if (!isDragging || currentDropZone === originalSetlistSlot) { // ドロップ失敗時、または同じスロットに戻された場合
            console.log(`[finishDragging] Drag failed or returned to original slot. Restoring original item to slot ${originalSetlistSlot.dataset.slotIndex}.`);
            // ここで fillSlotWithItem が呼び出されることを確認
  ...
  document.querySelectorAll('.setlist-slot').forEach(slot => { // setlistからdocument.querySelectorAllに変更
...
function processDrop(dropZone) {
    ...
    if (!draggingItemId) {
        ...
        finishDragging(); // 何もドラッグしていなければクリーンアップして終了
        return;
    }
    ...
    if (originalSetlistSlot === null) { // アルバムリストからドラッグされた場合
        ...
        if (getSlotItemData(dropZone)) {
            ...
            addItemToSetlist(draggingItemId, targetSlotIndex); // ★修正点: 新しく追加した関数を呼び出す
        }
 ...
 */
function addItemToSetlist(itemId, slotIndex) {
    console.log(`[addItemToSetlist] Attempting to add item ${itemId} to slot ${slotIndex}.`);

    const setlistSlot = document.querySelector(`.setlist-slot[data-slot-index="${slotIndex}"]`);
    if (!setlistSlot) {
        console.error(`[addItemToSetlist] Target setlist slot ${slotIndex} not found.`);
        showMessage('エラー: スロットが見つかりませんでした。', 'error');
        return;
    }

    const itemData = getAlbumItemData(itemId); // アルバムアイテムデータ取得関数を呼び出す
    if (!itemData) {
        console.error(`[addItemToSetlist] Item data for ${itemId} not found.`);
        showMessage('エラー: アイテムデータが見つかりませんでした。', 'error');
        return;
    }

    fillSlotWithItem(slotIndex, itemData);
    showMessage(`${itemData.name} をスロット ${slotIndex + 1} に追加しました。`);
    
    // アルバムリストから移動したアイテムを非表示にする
    hideSetlistItemsInMenu(); // これでセットリストにあるアイテムをメニューから隠す
}

/**
 * ダブルクリック（モバイルではダブルタップ）時の処理
 * ★修正関数★
 * @param {HTMLElement} targetElement ダブルクリックされた要素
 */
function handleDoubleClick(targetElement) {
    console.log("[handleDoubleClick] Double click detected on element:", targetElement);

    // 有効なターゲット要素か確認
    if (!targetElement) {
        console.warn("[handleDoubleClick] No item found for double click.");
        return;
    }

    const itemElement = targetElement.closest('.item');
    if (!itemElement) {
        console.warn("[handleDoubleClick] Clicked element is not an item:", targetElement);
        return;
    }

    const itemId = itemElement.dataset.itemId;
    if (!itemId) {
        console.warn("[handleDoubleClick] No item ID found for double clicked item.");
        return;
    }

    console.log(`[handleDoubleClick] Double click on item ID: ${itemId}`);

    if (itemElement.classList.contains('setlist-item')) {
        // セットリスト内のアイテムがダブルクリックされた場合（例: 削除、編集など）
        console.log(`[handleDoubleClick] Item ${itemId} is in setlist. No action defined for double click.`);
        // 例: removeSetlistItem(itemElement.dataset.slotIndex);
    } else {
        // アルバムリスト内のアイテムがダブルクリックされた場合
        // 空いているセットリストスロットに自動的に追加
        console.log(`[handleDoubleClick] Item is in album list. Attempting to add to setlist.`);
        const emptySlotIndex = findNextEmptySetlistSlot(); // 空のスロットを探す関数を呼び出す
        if (emptySlotIndex !== -1) {
            addItemToSetlist(itemId, emptySlotIndex); // 新しく追加する関数を呼び出す
            // originalAlbumMap に元のリストIDを記録（既に createTouchDraggedClone で行われているはずだが念のため）
            const albumListElement = itemElement.closest('.album-list');
            if (albumListElement && !originalAlbumMap.has(itemId)) {
                originalAlbumMap.set(itemId, albumListElement.id);
                console.log(`[handleDoubleClick] Original list for ${itemId} set to: ${albumListElement.id}`);
            }
            // 元のアルバムアイテムを非表示にする
            itemElement.style.visibility = 'hidden';
            console.log(`[handleDoubleClick] Hiding original album item: ${itemId}`);
        } else {
            showMessage('セットリストに空きがありません。', 'warning');
        }
    }
    // ダブルクリック後に不要なドラッグ状態をリセット
    finishDragging(); // ドラッグ状態のリセットを確実にする
}
...
function findNextEmptySetlistSlot() {
    ...
    for (let i = 0; i < setlistSlots.length; i++) {
        ...
        if (!slot.querySelector('.item') && !slot.dataset.itemId) {
