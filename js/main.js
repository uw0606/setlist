...
function getSlotItemData(element) {
    ...
    let hasShortOption = false; // デフォルトはfalse
    let hasSeOption = false;    // デフォルトはfalse
    let hasDrumsoloOption = false; // デフォルトはfalse
    ...
    if (isSetlistItem) {
        ...
        // セットリストアイテムの場合、datasetからオプションの有無を文字列として取得し、Booleanに変換
        ...
        // アルバムアイテムの場合、datasetからオプションの有無を文字列として取得し、Booleanに変換
        // ★ここが重要な修正点★
        hasShortOption = element.dataset.isShortVersion === 'true';
        hasSeOption = element.dataset.hasSeOption === 'true';
        ...
        // クローン要素の場合もdatasetからオプションの有無を文字列として取得し、Booleanに変換
        hasShortOption = element.dataset.isShortVersion === 'true';
        hasSeOption = element.dataset.hasSeOption === 'true';
        hasDrumsoloOption = element.dataset.drumsoloOption === 'true';
    } else {
...
function updateSlotContent(slotElement, songName, options) {
    ...
    if (options.isShortVersion) { // isShortVersionは、そのオプションが「存在しうるか」
        ...
        const shortVersionCheckboxWrapper = createCheckboxWrapper('Short', options.short, (e) => { // options.shortが現在のチェック状態
            ...
            // ここで options オブジェクトを正確にコピーし、チェック状態を更新して渡す
            const newOptions = { ...options, short: e.target.checked };
    ...
    if (options.hasSeOption) { // hasSeOptionは、そのオプションが「存在しうるか」
        ...
        const seOptionCheckboxWrapper = createCheckboxWrapper('SE有り', options.seChecked, (e) => { // options.seCheckedが現在のチェック状態
            ...
            // ここで options オブジェクトを正確にコピーし、チェック状態を更新して渡す
            const newOptions = { ...options, seChecked: e.target.checked };
            updateSlotContent(slotElement, songName, newOptions);
        });
    ...
    // ドラムソロ有無
    // ここでドラムソロが表示されない問題の修正: options.drumsoloOption が正しいBoolean値であることを確認
    if (options.hasDrumsoloOption) { // hasDrumsoloOptionは、そのオプションが「存在しうるか」
        hasAnyCheckboxOption = true;
        const drumsoloOptionCheckboxWrapper = createCheckboxWrapper('ドラムソロ有り', options.drumsoloChecked, (e) => { // options.drumsoloCheckedが現在のチェック状態
            ...
            // ここで options オブジェクトを正確にコピーし、チェック状態を更新して渡す
            const newOptions = { ...options, drumsoloChecked: e.target.checked };
            updateSlotContent(slotElement, songName, newOptions);
        });
