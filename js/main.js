...
const LONG_PRESS_DELAY = 2000; // 長押しと判断する時間（ミリ秒） - 例: 300ms
...
function handleTouchMove(event) {
    if (draggingItemId === null) {
        // ドラッグするアイテムが設定されていない場合は何もしない
        // これは handleTouchStart で draggable element 以外を触った場合に起こりうる
        return;
    }
    ...
    if (!isDragging) {
        // まだドラッグ開始と判定されていない場合、閾値を超えたかチェック
        ...
        if (deltaX > DRAG_THRESHOLD || deltaY > DRAG_THRESHOLD) {
            ...
            if (touchedElement) {
                ...
                touchStartX = currentX - rect.left;
                touchStartY = currentY - rect.top;

                createTouchDraggedClone(touchedElement, currentX, currentY, draggingItemId);
                // ドラッグ開始時の preventDefault はここで呼ぶ
                event.preventDefault(); // ★ここで preventDefault を呼び出し、スクロールを抑制
            } else {
                // 念のため、touchedElementが見つからない場合はドラッグをキャンセル
    ...
    if (isDragging && currentTouchDraggedClone) {
        ...
        rafId = requestAnimationFrame(() => {
            ...
            // ★修正: クローンの位置計算に touchStartX/Y オフセットを使用
            currentTouchDraggedClone.style.left = (currentX - touchStartX) + 'px';
            currentTouchDraggedClone.style.top = (currentY - touchStartY) + 'px';
