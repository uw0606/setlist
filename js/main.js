...
function handleDoubleClick(event) {
    ...
    // 最初に、ダブルクリックされた要素がアルバムまたは文字順リスト内の曲かチェック
    // ★★★ 修正箇所: セレクタを .item に変更し、両方のビューの曲に対応させる ★★★
    let albumItemElement = event.target.closest('.item'); 
    
    // ただし、この要素がセットリストのスロットでないことを確認 (セットリストアイテムは .setlist-slot.setlist-item クラスを持つ)
    if (albumItemElement && !albumItemElement.classList.contains('setlist-slot')) {
        
        // 自由入力曲のプレースホルダーを誤って追加しないようにする（ただし、data-allow-multipleで処理されている可能性が高い）
        ...
        if (firstEmptySlot) {
            ...
            // この関数が .item 要素からデータを正しく取得できることを前提とします。
        ...
        return; // メニューアイテムの処理が完了したので終了
    }

    // 次に、セットリストの曲がダブルクリックされたかチェック (処理に変更なし)
...
function changeAlbumView() {
    ...
    // ★★★ 過去セットリストボタンと移動先/元の親要素の取得 ★★★
    // 💡 実際のIDに合わせて修正してください
    const pastSetlistsButton = document.getElementById('pastSetlistsButtonContainer'); 
    const nameOrderNav = document.getElementById('nameOrderNav'); 
    const albumViewFooter = document.getElementById('albumViewFooter'); // 💡 元の親要素のID（仮）
    // ★★★ /追加要素の取得 ★★★
    ...
    if (mode === 'name_order') {
        ...
        // ★★★ 修正の核心: 過去セットリストボタンを文字順ナビの下に移動 ★★★
        if (pastSetlistsButton && nameOrderNav) {
            // #nameOrderNav の親要素の中に、#nameOrderNav の直後の兄弟として挿入
            nameOrderNav.parentNode.insertBefore(pastSetlistsButton, nameOrderNav.nextSibling);
            
            // 必要に応じて文字順用のスタイルを適用
            pastSetlistsButton.classList.add('name-order-footer-style'); 
            console.log("[changeAlbumView] Past setlists button moved below name order nav.");
        }
        ...
        // ★★★ 修正の核心: 過去セットリストボタンを元の位置に戻す ★★★
        if (pastSetlistsButton && albumViewFooter) {
            // 元の親要素の末尾に戻す
            albumViewFooter.appendChild(pastSetlistsButton);
            
            // 文字順用のスタイルを削除
            pastSetlistsButton.classList.remove('name-order-footer-style'); 
            console.log("[changeAlbumView] Past setlists button returned to album view footer.");
        }
