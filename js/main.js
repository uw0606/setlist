...
function handleTouchMove(event) {
    // isDragging が false の場合は、autoScrollSetlist(0) は handleTouchEnd で確実に呼ばれるため、ここでは不要。
    ...
    if (!touch) {
        ...
        // タッチ情報がない場合も、ドラッグ中であったならアニメーションを停止
    ...
    // --- ★ここから自動スクロールロジックの実行★ ---
    // autoScrollSetlist, SCROLL_AREA_HEIGHT, rafId はグローバルに定義されている前提で実行
    if (typeof autoScrollSetlist === 'function' && typeof SCROLL_AREA_HEIGHT !== 'undefined') {
        ...
        // rafId はグローバル変数として定義されている必要があります。
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // setlist 変数（グローバルスコープで宣言済み）に DOM 要素を代入します
    // 💡 グローバルスコープで 'let setlist = null;' と宣言されている必要があります
    setlist = document.getElementById('setlist'); // ★修正点：const/let を削除し、グローバル変数に代入
