...
function buildNameOrderList() {
    ...
    if (allSongData.length === 0) {
        ...
        document.querySelectorAll('.album-content-list .item').forEach(item => {
            ...
            if (itemId && songName) {
                // getSlotItemDataの代わりに、データ属性からオプションを直接抽出
                const data = {
                    itemId: itemId,
                    songName: songName,
    ...
    // --- 4. ナビゲーションタブの生成とコンテンツの構築 ---
    
    // ナビゲーションの定義 (groupSongsで定義したキーと対応させる)
    const navigationGroups = [
        { label: "A・B・C・D・E", key: "ABCDE" },
        { label: "F・G・H・I・J", key: "FGHIJ" },
        { label: "K・L・M・N・O", key: "KLMNO" },
        { label: "P・Q・R・S・T", key: "PQRST" },
        { label: "U・V・W・X・Y・Z", key: "UVWXYZ" },
        { label: "あいうえお", key: "AIUEO" },
        { label: "かきくけこ", key: "KAKIKUKEO" },
        { label: "さしすせそ", key: "SASISHISO" },
        { label: "たちつてと", key: "TATITUTETO" },
        { label: "なにぬねの", key: "NANINUNENO" },
        { label: "はひふへほ", key: "HAHIHUHEHO" },
        { label: "まみむめも", key: "MAMIMUMEMO" },
        { label: "やゆよ", key: "YAYUYO" },
        { label: "らりるれろ", key: "RARIRURERO" },
        { label: "わおん", key: "WAWOUN" },
        { label: "数字・記号", key: "ETC_NONE" },
    ];
    
    // 古いナビゲーションをクリア
    nameOrderNav.innerHTML = ''; 

    // ナビゲーションタブの生成
    let firstNav = null;
    navigationGroups.forEach(group => {
        // 曲が存在しないグループのタブは表示しない (ここではすべてのタブを表示します)

        const navItem = document.createElement('li');
        navItem.classList.add('name-order-nav-item');
        navItem.textContent = group.label;
        // toggleNameOrderGroup(this, groupKey) の形式でイベントを設定
        navItem.setAttribute('onclick', `toggleNameOrderGroup(this, '${group.key}')`);
        navItem.dataset.group = group.key; 
        
        nameOrderNav.appendChild(navItem);
        
        if (!firstNav) {
            firstNav = navItem; // 最初のナビゲーション要素を記憶
        }
    });
    ...
    if (firstNav) {
        ...
        console.warn("[buildNameOrderList] No navigation tabs were generated.");
    }
