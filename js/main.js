...
function handleTouchStart(event) {
    ...
    // チェックボックスの直接クリックはネイティブ動作を優先し、ドラッグ処理をスキップ
    if (isCheckboxClick) {
    ...
    // ⭐ 最も重要な修正点: ドラッグ対象要素の特定を極めて厳密にする ⭐
    // この関数でドラッグを開始するのは、以下のいずれかの条件を満たす要素のみです。
    // 1. アルバムメニュー内の `.item` (曲) 要素
    // 2. セットリスト内の、すでに曲が入っている `.setlist-slot.setlist-item` 要素
    let touchedElement = null;

    // まず、アルバムの曲 (.item) をチェック
    const albumItem = event.target.closest(".album-content .item"); // album-content 内の item に限定
    if (albumItem) {
        touchedElement = albumItem;
    } else {
        // アルバムの曲でなければ、セットリストのスロットをチェック
        const setlistSlot = event.target.closest(".setlist-slot");
        // セットリストのスロットの場合、それが 'setlist-item' クラスを持っているか（=曲が入っているか）を確認
        if (setlistSlot && setlistSlot.classList.contains('setlist-item')) {
            touchedElement = setlistSlot;
        }
    }

    // ドラッグ対象ではない要素 (例: 空の setlist-slot、メニューの空白部分、ヘッダーなど) をタッチした場合は、
    // ドラッグ処理をせずに、イベントのデフォルト動作 (スクロールなど) を妨げません。
    if (!touchedElement) {
        console.log("[touchstart:Mobile] Touched non-draggable/empty element. Allowing default behavior (e.g., scrolling).");
        return; // ここで return すると event.preventDefault() が呼ばれないため、スクロールが継続されます
    }
    ...
    if (event.touches.length === 1) { // シングルタッチのみを処理
        ...
        draggingItemId = touchedElement.dataset.itemId; // ドラッグするアイテムのIDを設定

        // originalSetlistSlot の設定（セットリストからのドラッグの場合のみ）
        if (touchedElement.classList.contains('setlist-item')) { // 'setlist-item' クラスがあるか確認
            ...
            originalSetlistSlot = null; // アルバムからのドラッグの場合
        }

        // タッチ開始地点の絶対座標と、クローン生成時のオフセットを記録
        ...
        touchTimeout = setTimeout(() => {
            ...
            // `draggingItemId` が `undefined` や `null` の場合はクローンを作成しない
            // これが重要: 空のスロットを間違ってドラッグ対象とした場合でも、ここで止める
            if (elementToClone && draggingItemId) {
                createTouchDraggedClone(elementToClone, event.touches[0].clientX, event.touches[0].clientY, draggingItemId);
            } else {
                console.warn("[touchstart:Mobile] Cannot create clone: elementToClone or draggingItemId is missing. Aborting drag.");
                isDragging = false; // クローン作成できない場合はドラッグをキャンセル
                // ここで元のスロットが表示状態に戻るべきだが、それができていない可能性も考慮
                // restoreOriginalSlotVisibility(); // 念のため、このような関数があれば呼び出す
            }
        }, 300);
    }
}
...
function handleTouchEnd(event) {
    ...
    if (isDragging) {
        // ⭐ 修正箇所: ドロップターゲットとして `currentDropZone` を使用する ⭐
        // `currentDropZone` は `handleTouchMove` で最後に指の下にあった有効なスロットを保持しているはず
        const dropTargetSlot = currentDropZone; // ここで currentDropZone を使用
        console.log("[touchend] Drop target slot:", dropTargetSlot ? dropTargetSlot.dataset.slotIndex : "none");

        // processDrop 関数に現在の情報（ドラッグ中の要素とドロップターゲット）を渡す
        processDrop(currentTouchDraggedClone, dropTargetSlot, originalSetlistSlot);

        finishDragging(); // ドラッグ状態のリセットとクリーンアップ
    } else {
        console.log("[touchend] Not dragging, no drop to process.");
    }

    // ドラッグが開始されていなかった場合でも、isDraggingはfalseにリセットされるべき
    // ただし、handleTouchStart/Moveのロジックで既にfalseになっているはず
    // ここで強制的にisDragging = false; を行う必要はないが、念のため明示的にしても良い
    isDragging = false; // 確実にリセット
}
