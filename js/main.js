...
function clearSlotContent(slotElement) {
    ...
    slotElement.classList.remove('setlist-item', 'item', 'album1', 'album2', 'album3', 'album4', 'album5', 'album6', 'album7', 'album8', 'album9', 'album10', 'album11', 'album12', 'album13', 'album14'); // アイテムを示すクラスを全て削除
    ...
    slotElement.removeAttribute('data-drumsolo-option');
    ...
    slotElement.removeAttribute('data-chorus');
    ...
    // ★JS修正の核その6: スロットをクリアしたら、pointer-eventsを 'none' に設定する★
    // これにより、クリアされたスロットがすぐに「隙間」としての動作に戻る
...
function restoreToOriginalList(setlistItem) {
    ...
    // （必要に応じて、アルバムリストに「戻った」ことを示すUI変更をここに追加）
    // 現状では、単にセットリストから削除されることで「アルバムに戻った」とみなす
...
function processDrop(draggedElement, dropTargetSlot, originalSourceSlot) {
    ...
    const options = {
        ...
        rGt: draggedElement.dataset.rGt === 'true',
        lGt: draggedElement.dataset.lGt === 'true',
        bass: draggedElement.dataset.bass === 'true',
        bpm: draggedElement.dataset.bpm === 'true',
        chorus: draggedElement.dataset.chorus === 'true'
    };
    ...
    if (originalSourceSlot) {
        ...
        if (dropTargetSlot.classList.contains('setlist-item')) {
            ...
            // ドロップ先アイテムのデータを取得
            const targetItemId = dropTargetSlot.dataset.itemId;
            const targetSongName = dropTargetSlot.dataset.songName;
            const targetOptions = {
                isShortVersion: dropTargetSlot.dataset.isShortVersion === 'true',
                hasSeOption: dropTargetSlot.dataset.hasSeOption === 'true',
                drumsoloOption: dropTargetSlot.dataset.drumsoloOption === 'true',
                rGt: dropTargetSlot.dataset.rGt === 'true',
                lGt: dropTargetSlot.dataset.lGt === 'true',
                bass: dropTargetSlot.dataset.bass === 'true',
                bpm: dropTargetSlot.dataset.bpm === 'true',
                chorus: dropTargetSlot.dataset.chorus === 'true'
            };
            const targetAlbumClass = Array.from(dropTargetSlot.classList).find(cls => cls.startsWith('album') && cls !== 'item');

            // 元のスロットにドロップ先アイテムを再配置
            clearSlotContent(originalSourceSlot); // 元のスロットをクリア
            addSongToSlot(originalSourceSlot, targetItemId, targetSongName, targetOptions, targetAlbumClass);
    ...
    hideSetlistItemsInMenu(); // メニュー内の表示を更新
}
...
function handleTouchStart(event) {
    ...
    if (event.touches.length === 1) {
        ...
        // ★JS修正の核その1: 空のセットリストスロットはCSSでpointer-events: none; なのでここでは対象外
        // 空のスロットはイベントを拾わないので、touchedElement は null になるはず。
        // もし空スロットをタップしてここに来るなら、CSSかHTML構造に問題がある。
        ...
        if (!touchedElement) {
            ...
            // ここで return することで、空スロットや背景のタップはブラウザのデフォルト動作（スクロールなど）を許可し、
            // ドラッグ開始ロジックには進まない。
            return; 
        }
        ...
        touchTimeout = setTimeout(() => {
            if (draggingItemId && document.body.contains(touchedElement)) {
                ...
                // ★JS修正の核その2: ドラッグ開始時にすべてのセットリストスロットをドロップ可能にする（pointer-events: auto;）★
...
function handleTouchMove(event) {
    ...
    // クローン要素の位置を更新
    currentTouchDraggedClone.style.left = `${currentX - currentTouchDraggedClone.offsetX}px`;
    currentTouchDraggedClone.style.top = `${currentY - currentTouchDraggedClone.offsetY}px`;
    ...
    if (targetSlot) {
        ...
        // (元のスロットがnullでない = セットリスト内ドラッグ、または、元のスロットがnull = アルバムからのドラッグ)
        const isSelfSlot = originalSetlistSlot && targetSlot.dataset.slotIndex === originalSetlistSlot.dataset.slotIndex;
        
        // ★修正ポイント★
        // ここで、セットリスト内で空のスロットにドロップする場合と、既存のアイテムと入れ替わる場合を考慮
        // 空のスロットへのドロップ、またはアイテムが入っているスロットへのドロップ
        if (!isSelfSlot) { // 自分自身のスロットにはハイライトしない
            targetSlot.classList.add('drag-over');
            // 必要に応じて、ここでプレースホルダーなどのUIを更新
        }
    }
}
...
function handleTouchEnd(event) {
    ...
    // ★JS修正の核その3: ドラッグが開始されていて、かつ指の移動が最小限だった場合（長押しタップ） ★
    if (isDragging && (deltaX < dragThreshold && deltaY < dragThreshold)) {
        ...
        // finishDragging 内部で pointer-events がリセットされる
        finishDragging(true); // キャンセルされたドラッグとしてクリーンアップ
        event.preventDefault(); // デフォルト動作を防止
        return; // ここで処理を完全に中断
    }

    // isDragging が false の場合（ロングプレスにならなかった単なるタップなど）
    if (!isDragging) {
        ...
        // ここで event.preventDefault() を呼ばないことで、ブラウザのデフォルト動作を許可
        return;
    }

    // ★JS修正の核その4: ここから下は、実際に「ドラッグ（指の移動あり）」が検出された場合の処理 ★
...
function createTouchDraggedClone(originalElement, initialX, initialY, itemIdToClone) {
    ...
    if (setlist.contains(originalElement) && originalElement.classList.contains('setlist-item')) {
        ...
        // currentPcDraggedElement に設定されている元のアルバムアイテムを隠す
        // handleTouchStart で既に currentPcDraggedElement に設定済み
        if (currentPcDraggedElement && currentPcDraggedElement === originalElement) {
             currentPcDraggedElement.style.visibility = 'hidden'; // ★修正点3: アルバムアイテムをここで隠す
             console.log(`[createTouchDraggedClone] Original album item ${originalElement.dataset.itemId} hidden.`);
        } else {
            console.warn("[createTouchDraggedClone] Album item not found as currentPcDraggedElement or mismatch. Not hiding.");
            originalElement.style.visibility = 'hidden'; // 念のため直接隠す
        }
...
function finishDragging(isCancelled = false) {
    ...
    if (originalSetlistSlot) {
        ...
        originalSetlistSlot.style.visibility = ''; // CSSで設定したvisibilityに戻す（またはデフォルト）
        originalSetlistSlot = null;
    ...
    // ★JS修正の核その5: ドラッグ終了時にすべてのセットリストスロットのpointer-eventsをリセットする★
    ...
    document.querySelectorAll('.setlist-slot').forEach(slot => {
        ...
        slot.style.touchAction = ''; // touch-actionもCSS定義に戻す（空文字列でインラインスタイルをクリア）
    });
...
function handleDoubleClick(event) {
    ...
    // .album-content はアルバムの曲リストのコンテナ、.item は個々の曲要素
    ...
    if (albumItemElement) {
        ...
        if (firstEmptySlot) {
            // albumItemElement からアイテムデータを取得
            const itemId = albumItemElement.dataset.itemId;
            const songName = albumItemElement.dataset.songName;
            const isShortVersion = albumItemElement.dataset.isShortVersion === 'true';
            const hasSeOption = albumItemElement.dataset.hasSeOption === 'true';
            const drumsoloOption = albumItemElement.dataset.drumsoloOption === 'true';
            const hasRgt = albumItemElement.dataset.rGt === 'true';
            const hasLgt = albumItemElement.dataset.lGt === 'true';
            const hasBass = albumItemElement.dataset.bass === 'true';
            const hasBpm = albumItemElement.dataset.bpm === 'true';
            const hasChorus = albumItemElement.dataset.chorus === 'true';
            
            // アルバムクラスを正確に取得
            const albumClass = Array.from(albumItemElement.classList).find(cls => cls.startsWith('album') && cls !== 'item');
            if (!albumClass) {
                console.warn("[handleDoubleClick] Could not find album class for item:", albumItemElement);
                // albumClass が見つからなければ、デフォルト値を設定するか、処理を中断するか検討
                // ここでは空文字列で続行し、addSongToSlotで処理させる
            }

            // スロットにアイテムを追加
            addSongToSlot(firstEmptySlot, itemId, songName, {
                isShortVersion, hasSeOption, drumsoloOption, hasRgt, hasLgt, hasBass, hasBpm, hasChorus
            }, albumClass);
            showMessage("セットリストに曲を追加しました。", "success");
    ...
    if (!setlistItemElement) {
        ...
        return; // 有効なアイテムでなければ処理を中断
    }

    const slotIndex = setlistItemElement.dataset.slotIndex;
    const itemId = setlistItemElement.dataset.itemId;
    console.log(`[handleDoubleClick] Double-clicked setlist item: ID=${itemId}, Slot Index=${slotIndex}`);

    if (itemId) {
        // 現在の曲のデータを取得
        const songName = setlistItemElement.dataset.songName;
        const isShortVersion = setlistItemElement.dataset.isShortVersion === 'true';
        const hasSeOption = setlistItemElement.dataset.hasSeOption === 'true';
        const drumsoloOption = setlistItemElement.dataset.drumsoloOption === 'true';
        const hasRgt = setlistItemElement.dataset.rGt === 'true';
        const hasLgt = setlistItemElement.dataset.lGt === 'true';
        const hasBass = setlistItemElement.dataset.bass === 'true';
        const hasBpm = setlistItemElement.dataset.bpm === 'true';
        const hasChorus = setlistItemElement.dataset.chorus === 'true';

        // オプションがない場合はアルバムに戻す
        // (event.ctrlKey || event.metaKey) はPCのみの動作なのでスマホでは考慮しない
        if (!(isShortVersion || hasSeOption || drumsoloOption || hasRgt || hasLgt || hasBass || hasBpm || hasChorus)) {
            console.log("[handleDoubleClick] No options to toggle. Restoring to original list.");
            restoreToOriginalList(setlistItemElement);
        } else {
            console.log("[handleDoubleClick] Toggling options.");
            // ここでオプションをトグルするロジックを実装
            if (setlistItemElement.dataset.isShortVersion !== undefined) {
                setlistItemElement.dataset.isShortVersion = (!isShortVersion).toString();
                console.log(`[handleDoubleClick] Toggled short version to ${setlistItemElement.dataset.isShortVersion}`);
            }
            if (setlistItemElement.dataset.hasSeOption !== undefined) {
                setlistItemElement.dataset.hasSeOption = (!hasSeOption).toString();
                console.log(`[handleDoubleClick] Toggled SE option to ${setlistItemElement.dataset.hasSeOption}`);
            }
            if (setlistItemElement.dataset.drumsoloOption !== undefined) {
                setlistItemElement.dataset.drumsoloOption = (!drumsoloOption).toString();
                console.log(`[handleDoubleClick] Toggled drum solo option to ${setlistItemElement.dataset.drumsoloOption}`);
            }
            if (setlistItemElement.dataset.rGt !== undefined) {
                setlistItemElement.dataset.rGt = (!hasRgt).toString();
                console.log(`[handleDoubleClick] Toggled R-GT option to ${setlistItemElement.dataset.rGt}`);
            }
            if (setlistItemElement.dataset.lGt !== undefined) {
                setlistItemElement.dataset.lGt = (!hasLgt).toString();
                console.log(`[handleDoubleClick] Toggled L-GT option to ${setlistItemElement.dataset.lGt}`);
            }
            if (setlistItemElement.dataset.bass !== undefined) {
                setlistItemElement.dataset.bass = (!hasBass).toString();
                console.log(`[handleDoubleClick] Toggled Bass option to ${setlistItemElement.dataset.bass}`);
            }
            if (setlistItemElement.dataset.bpm !== undefined) {
                setlistItemElement.dataset.bpm = (!hasBpm).toString();
                console.log(`[handleDoubleClick] Toggled BPM option to ${setlistItemElement.dataset.bpm}`);
            }
            if (setlistItemElement.dataset.chorus !== undefined) {
                setlistItemElement.dataset.chorus = (!hasChorus).toString();
                console.log(`[handleDoubleClick] Toggled Chorus option to ${setlistItemElement.dataset.chorus}`);
            }

            // スロットを再描画して変更を反映
            updateSlotContent(setlistItemElement, songName, {
                isShortVersion: setlistItemElement.dataset.isShortVersion === 'true',
                hasSeOption: setlistItemElement.dataset.hasSeOption === 'true',
                drumsoloOption: setlistItemElement.dataset.drumsoloOption === 'true',
                rGt: setlistItemElement.dataset.rGt === 'true',
                lGt: setlistItemElement.dataset.lGt === 'true',
                bass: setlistItemElement.dataset.bass === 'true',
                bpm: setlistItemElement.dataset.bpm === 'true',
                chorus: setlistItemElement.dataset.chorus === 'true'
            });
        }
    } else {
        console.warn("[handleDoubleClick] Item ID is missing for the double-clicked element. No action taken.");
    }
}
 ...
 * @param {Object} options - 曲のオプション（isShortVersion, hasSeOptionなど）。
...
function addSongToSlot(slotElement, itemId, songName, options, albumClass) {
    ...
    slotElement.dataset.isShortVersion = options.isShortVersion || 'false';
    slotElement.dataset.hasSeOption = options.hasSeOption || 'false';
    slotElement.dataset.drumsoloOption = options.drumsoloOption || 'false';
    slotElement.dataset.rGt = options.rGt || ''; // デフォルト値を空文字列に
    slotElement.dataset.lGt = options.lGt || ''; // デフォルト値を空文字列に
    slotElement.dataset.bass = options.bass || ''; // デフォルト値を空文字列に
    slotElement.dataset.bpm = options.bpm || '';   // デフォルト値を空文字列に
    slotElement.dataset.chorus = options.chorus || 'false';

    // クラスを追加してスタイルを適用
    slotElement.classList.add('setlist-item', 'item', albumClass); // setlist-item, item, albumクラスを追加
    ...
    // スロットにイベントリスナーを再設定 (既存の addSongToSlot の中で行われるべき)
    // 注意: 同じイベントリスナーが複数回追加されないように注意が必要
    // attachSlotEvents 関数がある場合はそれを呼び出すのが理想的
    // ここでは念のため、手動でイベントリスナーを追加（重複防止のため remove -> add）
    slotElement.removeEventListener('click', handleSlotClick);
    slotElement.removeEventListener('dblclick', handleDoubleClick);
    slotElement.removeEventListener('touchstart', handleTouchStart);
    slotElement.removeEventListener('touchend', handleTouchEnd);
    ...
    slotElement.addEventListener('touchstart', handleTouchStart, { passive: false }); // touchstartはpreventDefaultのためpassive: false
    slotElement.addEventListener('touchend', handleTouchEnd, { passive: false });
    slotElement.addEventListener('touchmove', handleTouchMove, { passive: false });
...
function updateSlotContent(slotElement, songName, options) {
    ...
    // オプション要素 (チェックボックスやその他)
    if (options.isShortVersion || options.hasSeOption || options.drumsoloOption || options.rGt || options.lGt || options.bass || options.bpm || options.chorus) {
        const itemOptions = document.createElement('div');
        itemOptions.classList.add('item-options');

        // 短縮版
        if (options.isShortVersion) {
            const shortVersionCheckboxWrapper = createCheckboxWrapper('短縮', options.isShortVersion, (e) => {
                slotElement.dataset.isShortVersion = e.target.checked.toString();
                updateSlotContent(slotElement, songName, { ...options, isShortVersion: e.target.checked });
            });
        ...
        if (options.hasSeOption) {
            const seOptionCheckboxWrapper = createCheckboxWrapper('SE', options.hasSeOption, (e) => {
                slotElement.dataset.hasSeOption = e.target.checked.toString();
                updateSlotContent(slotElement, songName, { ...options, hasSeOption: e.target.checked });
            });
        ...
        if (options.drumsoloOption) {
            const drumsoloOptionCheckboxWrapper = createCheckboxWrapper('ドラムソロ', options.drumsoloOption, (e) => {
                slotElement.dataset.drumsoloOption = e.target.checked.toString();
                updateSlotContent(slotElement, songName, { ...options, drumsoloOption: e.target.checked });
            });
    ...
    if (options.chorus) infoParts.push(`コーラス`); // コーラスは単に表示
