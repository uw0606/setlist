...
function handleTouchEnd(event) {
    ...
    // ⭐ 修正ポイント1: ドロップゾーンのハイライト解除は、finishDraggingで一括して行う ⭐
    // ここで個別に解除するのではなく、終了処理に任せることで一貫性を保つ。
    // document.querySelectorAll('.setlist-slot.drag-over').forEach(slot => slot.classList.remove('drag-over'));
    
    // ⭐ 修正ポイント2: ドロップターゲットは currentDropZone を使用する ⭐
    // handleTouchMove で常に更新されている currentDropZone を信頼する。
    // これにより、touchend の瞬間に指の下に正確な要素がなくても、最後に有効だったドロップゾーンが使われる。
    const dropTargetSlot = currentDropZone; 
