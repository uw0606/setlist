...
function handleTouchStart(e) {
    ...
    if (targetElement.classList.contains('album')) {
        ...
        // ★確認点：_originalItemDataが正しく保存されているか
        originalSetlistSlot._originalItemData = getSlotItemData(originalSetlistSlot);
    ...
    // ★重要: 既存の touchTimeout がある場合はクリアしてから新しいタイマーを設定
    if (touchTimeout) {
        clearTimeout(touchTimeout);
    }
    ...
    touchTimeout = setTimeout(() => {
        ...
        // クローン要素の作成と元の要素の非表示
        createTouchDraggedClone(targetElement);

        // 元の要素を非表示にする処理
        if (targetElement.classList.contains('album')) {
            targetElement.style.visibility = 'hidden';
            currentPcDraggedElement = targetElement; // finishDragging で元の表示に戻すために保持
    ...
    }, LONG_PRESS_THRESHOLD); // LONG_PRESS_THRESHOLD の値も確認 (例: 500ms)

    // ここで e.preventDefault() を呼ばないこと。
    // 長押し検出前に呼ぶと、タップやスクロールが全くできなくなる。
    // e.preventDefault(); // これがコメントアウトされているか、削除されていることを確認
}
