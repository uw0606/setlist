...
function changeAlbumView() {
    ...
    const albumViewContainer = document.getElementById('albumViewContainer'); // 既存のアルバムリストの親
    const nameOrderContainer = document.getElementById('nameOrderContainer'); // 文字順リストの親
    ...
    if (mode === 'name_order') {
        ...
        // アルバム順に戻したとき、最初のアルバムを開く（通常はtoggleAlbumが呼ばれる）
        const firstAlbumNav = document.querySelector('.album-nav-item');
        if (firstAlbumNav) {
            // ここで最初のアルバムのリストを強制的に開くロジックを追加（例: Album 1のナビアイテムをクリック）
            // 既存の toggleAlbum(1) が実行されることが理想ですが、ここではアルバムナビの最初の要素をクリックする
            const firstAlbumNavButton = document.querySelector('#albumNav li[onclick^="toggleAlbum"]');
...
function buildNameOrderList() {
        ...
        document.querySelectorAll('.album-content-list .item').forEach(item => {
            ...
            if (itemId && songName) {
                ...
                const data = {
                    ...
                    // チューニング/BPM (文字順リストでは基本的に使用されないがデータは保持)
    ...
    // 初期状態で最初のグループを開く（例: A・B・C・D・E）
    const initialNav = document.querySelector('.name-order-nav-item[data-group="ABCDE"]');
    if (initialNav) {
        toggleNameOrderGroup(initialNav, 'ABCDE');
    }
...
function clearAndDrawNameOrderList() {
    ...
    nameOrderContent.innerHTML = ''; // コンテンツをクリア
    ...
    for (const groupKey in nameOrderGroups) {
        ...
        groupSongs.forEach(songData => {
            const itemElement = createAlbumItemElement(songData); // ヘルパー関数を呼び出し
            
            // Drag and Drop イベントを有効化
