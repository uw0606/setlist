...
function handleTouchEnd(event) {
    ...
    if (isDragging && (deltaX < dragThreshold && deltaY < dragThreshold)) {
        ...
        if (potentialDropTargetSlot && !potentialDropTargetSlot.classList.contains('setlist-item')) {
            ...
            return; // ここで処理を中断し、曲が消えるのを防ぐ
        }

        // それ以外の長押しタップ（例: 曲が入っているスロット上での長押しタップ）は、
        // 意図しないドロップ処理を避けるために、ここではドロップ処理を行わない
        // セットリスト内の長押しタップによる削除は、この後のprocessDropで別途処理されるべき
        console.log("[touchend] Long-press tap on a FILLED setlist slot or album item. Performing cleanup only.");
        finishDragging(); // クリーンアップのみ
        return;
    }

    // isDragging が false の場合（ロングプレスにならなかった場合、単なるタップなど）
    if (!isDragging) {
    ...
    // potentialDropTargetSlot をそのまま利用
...
function finishDragging(cancelled = false) {
    ...
    // PCドラッグの元の要素のvisibilityを戻す
    if (currentPcDraggedElement && !currentPcDraggedElement.closest('.setlist-slot')) {
    ...
    // ★★★ 重要な修正点：originalSetlistSlot の処理を強化 ★★★
    // セットリストからのドラッグだった場合、元のスロットの状態を適切に戻す
    if (originalSetlistSlot) {
        // 元のスロットがまだDOMに存在し、かつplaceholder-slotクラスを持っている場合のみ処理
        // これにより、スロットが「消える」問題を修正します。
        if (setlist.contains(originalSetlistSlot) && originalSetlistSlot.classList.contains('placeholder-slot')) {
            // スロットの表示を元に戻す（placeholder-slotクラスを削除）
            ...
            console.log(`[finishDragging] originalSetlistSlot was not a placeholder or no longer in DOM. Skipping restore. Slot Index: ${originalSetlistSlot.dataset.slotIndex}`);
        }
        originalSetlistSlot = null; // 処理後リセット
    }
