...
function handleTouchStart(event) {
    ...
    // 空のスロットはdata-item-idを持たないので、ここで弾かれる
    if (!touchedElement || !touchedElement.dataset.itemId) { // ここで .dataset.itemId を追加チェック
    ...
    // 以降の処理は、touchedElement が有効な曲アイテムである場合にのみ実行される
    ...
    if (event.touches.length === 1) {
        ...
        isDragging = false;
        draggingItemId = touchedElement.dataset.itemId; // ここで draggingItemId を設定 (有効なIDであることが保証されている)
        ...
        touchTimeout = setTimeout(() => {
            ...
            // この時点では draggingItemId は確実に有効な値なので、追加のチェックは不要だが、あっても害はない
 ...
 * @param {string} itemId - ドラッグ中のアイテムのID
 */
function createTouchDraggedClone(originalElement, clientX, clientY, itemId) {
    // ⭐ 最終チェック: originalElement が有効なアイテムであり、itemId が存在すること ⭐
    // このチェックを追加することで、空のスロットがここまで到達した場合でも、
    // クローン作成や元の要素の非表示処理を中止できます。
    if (!originalElement || !itemId) {
        console.warn("[createTouchDraggedClone] Aborting clone creation: originalElement or itemId is missing/invalid.");
        // isDragging が true になってしまっている可能性があるので、ここでリセット
        isDragging = false;
        // もし originalSetlistSlot が設定されていたら、そのplaceholderクラスを解除し、表示状態に戻す
        if (originalSetlistSlot) {
            originalSetlistSlot.classList.remove('placeholder-slot');
            originalSetlistSlot.style.visibility = ''; // 非表示になっていたら戻す
            console.log("[createTouchDraggedClone] Restored visibility of potentially hidden originalSetlistSlot.");
        }
        return null; // クローンが作成されなかったことを示す
    }
    ...
    // initialTouchX, initialTouchY は handleTouchStart でタッチ開始位置
    // touchStartX, touchStartY は handleTouchStart での、要素左上から指までのオフセット
    // クローンの左上を指の真下ではなく、元の要素に対する相対位置になるように調整
    ...
    if (originalSetlistSlot && setlist.contains(originalElement)) {
        originalSetlistSlot.classList.add('placeholder-slot');
        originalSetlistSlot.style.visibility = 'hidden';
        ...
        // _originalItemData は handleTouchStart で設定済みなので、ここでは不要
    }
