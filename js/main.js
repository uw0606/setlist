...
async function generateSetlistPdf() {
    ...
    for (const slot of setlistSlots) {
        if (slot.classList.contains('setlist-item')) {
            ...
            if (isAlbum1) {
                // No.なしの曲。2桁番号（例: '12.'）とスペース分を確保する
                const paddingSpace = '   '; 
                simplePdfBody.push(`${paddingSpace}${titleText}${simpleEffectNote}`);
            } else {
                // No.ありの曲。番号の桁数に応じてスペースを調整
                const currentNo = currentItemNoSimple++;
                const noString = `${currentNo}.`;
                // 1桁の番号ならスペース2つ、2桁の番号ならスペース1つを加えて曲名始まりを揃える
    ...
    try {
        ...
        detailedPdf.autoTable({
            ...
            styles: {
                font: 'NotoSansJP',
                // ★★★ 修正5: 全体のデフォルトサイズを8にする（その他情報はすべて8） ★★★
                fontSize: 8, 
                cellPadding: 2,
                lineColor: [0, 0, 0],
                lineWidth: 0.3,
