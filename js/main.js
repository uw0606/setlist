...
/**
 * ドロップ処理
 * @param {HTMLElement} draggedElement - ドラッグされた要素（PCの場合はcurrentPcDraggedElement、モバイルの場合はcurrentTouchDraggedClone）
 * @param {HTMLElement|null} dropTargetSlot - ドロップされた先のセットリストスロット要素、またはnull
 * @param {HTMLElement|null} originalSetlistSlot - ドラッグ元のセットリストスロット要素（セットリスト内移動の場合）、またはnull
 */
function processDrop(draggedElement, dropTargetSlot, originalSetlistSlot = null) {
    ...
    if (!itemId) {
        ...
        // アイテムIDがない場合、PCドラッグであれば元のスロットを復元する処理
        if (originalSetlistSlot && originalSetlistSlot.classList.contains('placeholder-slot')) {
            ...
            // originalSetlistSlot._originalItemData があれば再表示
    ...
    if (originalSetlistSlot && originalSetlistSlot._originalItemData) {
        ...
        if (originalAlbumItem) {
            // albumItems からデータを取得し、オプション情報で強化
            const albumData = albumItems.find(item => item.id === itemId); // albumItemsがグローバルに利用可能と仮定
            if (albumData) {
                // draggedItemData にアルバムからのオプション情報を追加/上書き
                draggedItemData.hasShortOption = albumData.hasShortOption === true;
                draggedItemData.hasSeOption = albumData.hasSeOption === true;
                draggedItemData.hasDrumsoloOption = albumData.hasDrumsoloOption === true; 
                // その他のプロパティもalbumDataからコピーしたい場合はここに追加
                draggedItemData.rGt = albumData.rGt || '';
                draggedItemData.lGt = albumData.lGt || '';
                draggedItemData.bass = albumData.bass || '';
                draggedItemData.bpm = albumData.bpm || '';
                draggedItemData.chorus = albumData.chorus || '';
                draggedItemData.albumClass = albumData.albumClass || '';
            } else {
                console.warn(`[processDrop] Album data not found for ID: ${itemId}`);
            }
        }
    ...
    // ドロップターゲットがない場合 (セットリスト外にドロップされた場合)
    if (!dropTargetSlot || !setlist.contains(dropTargetSlot)) {
        console.log("[processDrop] Dropped outside setlist.");
        if (isDraggedFromSetlist) {
            // セットリストからドラッグされた場合、元のスロットに戻す
            console.log("[processDrop] Returning item to original setlist slot as dropped outside.");
            if (originalSetlistSlot && originalSetlistSlot._originalItemData) {
                fillSlotWithItem(originalSetlistSlot, originalSetlistSlot._originalItemData);
            }
            // PCドラッグで非表示にした元のスロットの表示を戻す
            if (originalSetlistSlot && originalSetlistSlot.classList.contains('placeholder-slot')) {
                originalSetlistSlot.classList.remove('placeholder-slot');
                originalSetlistSlot.style.visibility = '';
            }
        } else {
            // アルバムからドラッグされ、セットリスト外にドロップされた場合は、アルバム内の表示を戻す
            console.log("[processDrop] Album item dropped outside setlist. Restoring to original list.");
            restoreToOriginalList(draggedElement); 
        }
        return; // ここで処理を終了
    }

    // ドロップターゲットがセットリストスロットの場合
    const dropTargetSlotIndex = dropTargetSlot.dataset.slotIndex;
    const currentItemInTargetSlot = getSlotItemData(dropTargetSlot); // ドロップ先に元々あったアイテムのデータ

    if (isDraggedFromSetlist) {
        // ★セットリスト内でのドラッグ＆ドロップのロジックです★
        const originalSlotIndex = originalSetlistSlot.dataset.slotIndex;

        if (originalSlotIndex === dropTargetSlotIndex) {
            // 同じスロットに戻した場合は、単に元のスロットを復元
            console.log("[processDrop] Dropped item back to its original slot. Restoring original appearance.");
            if (originalSetlistSlot && originalSetlistSlot._originalItemData) {
                fillSlotWithItem(originalSetlistSlot, originalSetlistSlot._originalItemData);
            }
        } else {
            // 別のスロットに移動（スワップまたは空スロットへの移動）
            console.log(`[processDrop] Moving/Swapping within setlist. Original slot: ${originalSlotIndex}, Target slot: ${dropTargetSlotIndex}`);

            // 1. ドロップターゲットにドラッグされたアイテムをセット
            fillSlotWithItem(dropTargetSlot, draggedItemData);

            // 2. 元のスロットの処理
            if (currentItemInTargetSlot) {
                // ドロップ先に元々アイテムがあった場合、元のスロットにそのアイテムをセット
                console.log(`[processDrop] Swapping: Target's item (${currentItemInTargetSlot.name}) moves to original slot.`);
                fillSlotWithItem(originalSetlistSlot, currentItemInTargetSlot);
            } else {
                // ドロップ先が空だった場合、元のスロットをクリア
                console.log("[processDrop] Moving: Target was empty. Clearing original slot.");
        ...
        if (originalSetlistSlot && originalSetlistSlot.classList.contains('placeholder-slot')) {
            originalSetlistSlot.classList.remove('placeholder-slot');
            originalSetlistSlot.style.visibility = '';
            console.log("[processDrop] Original setlist slot placeholder removed and visibility restored.");
        }

    } else {
        // ★アルバムからのドロップ処理★ (変更なしで問題ないはずですが、念のため構造を維持)
        if (currentItemInTargetSlot) {
            // ドロップ先がすでに埋まっている場合
        ...
        console.log(`[processDrop] Adding item from album. Target slot: ${dropTargetSlotIndex}`);
        fillSlotWithItem(dropTargetSlot, draggedItemData);
        ...
        if (albumItem) {
            ...
            console.log(`[processDrop] Album item ${itemId} hidden.`);
        }
    }

    // ドロップ処理後に、アルバム内の表示状態を再同期する (念のため)
    hideSetlistItemsInMenu(); 
    console.log("[processDrop] Drop process finished. hideSetlistItemsInMenu called.");
}
...
let initialTouchX = 0; // タッチ開始時のX座標
let initialTouchY = 0; // タッチ開始時のY座標
const DRAG_THRESHOLD = 30; // ドラッグと判断する移動のしきい値（ピクセル）
const LONG_PRESS_DELAY = 600; // 長押しと判断するまでの時間（ミリ秒）
let isLongPressReady = false; // 長押しが確定したかどうかを示すフラグ
...
function handleTouchStart(event) {
    ...
    // --- 既存のリセット処理に isLongPressReady も追加 ---
    if (isDragging || currentTouchDraggedClone || currentDropZone || draggingItemId || originalSetlistSlot || touchTimeout || isLongPressReady) {
        console.log("[touchstart:Mobile] Resetting previous drag state.");
        isDragging = false;
        draggingItemId = null;
        originalSetlistSlot = null;
        ...
        isLongPressReady = false; // フラグをリセット
    }
    // --- リセット処理ここまで ---
    ...
    if (event.touches.length === 1) {
        ...
        if (!touchedElement) {
            console.warn("[touchstart:Mobile] No draggable item found on touch start. Allowing scroll.");
            return;
        }
        ...
        // 長押し判定用のタイマーを開始。タイマー発火で isLongPressReady を true にするだけ
        touchTimeout = setTimeout(() => {
            if (!isDragging && draggingItemId) { // まだドラッグが開始されておらず、有効なアイテムの場合
                isLongPressReady = true; // 長押しが確定したことを示すフラグを立てる
                console.log("[touchstart:Mobile] Long press ready. Drag will start on first movement.");

                // ★追加: 長押しが確定したら、その時点でクローンを作成し、ドラッグを開始する★
                // handleTouchMoveで改めて isDragging=true となりますが、
                // ここでクローンを作成し、イベントのデフォルト動作をブロックすることで、
                // ユーザーは指を動かし始めた瞬間にドラッグが開始されたと感じられます。
                let elementToClone = null;
                if (originalSetlistSlot && originalSetlistSlot.dataset.itemId === draggingItemId) {
                    elementToClone = originalSetlistSlot;
                } else {
                    elementToClone = document.querySelector(`.album-content .item[data-item-id="${draggingItemId}"]`);
                }

                if (elementToClone && document.body.contains(elementToClone)) {
                    isDragging = true; // ここで isDragging を true に設定
                    createTouchDraggedClone(elementToClone, initialTouchX, initialTouchY, draggingItemId);
                    // ここで event.preventDefault() を呼びたいが、setTimeout 内では元のイベントにアクセスできない。
                    // そのため、handleTouchMove で確実に preventDefault を呼ぶ必要がある。
                    // または、touchstart 時に `touch-action: none;` が設定されていれば、
                    // この問題は発生しにくい。
                    // 今回は handleTouchMove での呼び出しを継続する。
                    console.log("[touchstart:Mobile] Clone created and drag state set to true after long press.");
                } else {
                    console.warn("[touchstart:Mobile] Long press timer fired but elementToClone not found. Aborting potential drag.");
                    draggingItemId = null;
                    originalSetlistSlot = null;
                ...
                console.warn("[touchstart:Mobile] Long press timer fired but drag already started or item invalid.");
            }
            touchTimeout = null; // タイマーは一度発火したらクリア
        }, LONG_PRESS_DELAY);

    } else {
        console.log("[touchstart:Mobile] Multiple touches detected. State reset done above.");
    }
}
...
function handleTouchMove(event) {
    if (event.touches.length !== 1) {
        if (isDragging) {
            console.log("[handleTouchMove] Multiple touches during drag. Finishing dragging.");
            finishDragging();
        }
        if (touchTimeout) {
            clearTimeout(touchTimeout);
            touchTimeout = null;
            console.log("[handleTouchMove] Cleared long press timeout due to multiple touches.");
        }
        isLongPressReady = false;
        return;
    }
    ...
    if (!isDragging) {
        if (touchTimeout) {
            if (distance > DRAG_THRESHOLD) {
                ...
                touchTimeout = null;
                isLongPressReady = false;
                console.log("[handleTouchMove] Cleared long press timeout due to movement exceeding threshold before long press.");
                return;
            }
        }

        if (isLongPressReady) {
            // 長押しが確定したら、最初の動きですぐにドラッグを開始
            if (!draggingItemId) {
                console.warn("[handleTouchMove] Long press ready but draggingItemId is null. Aborting drag start.");
                isLongPressReady = false;
                return;
            }
            ...
            if (!elementToClone || !document.body.contains(elementToClone)) {
                console.warn("[handleTouchMove] Long press ready but elementToClone not found or not in DOM. Aborting drag start.");
                draggingItemId = null;
                originalSetlistSlot = null;
                isLongPressReady = false;
                return;
            ...
            isDragging = true;
            createTouchDraggedClone(elementToClone, currentX, currentY, draggingItemId); // ★ 現在のタッチ位置で作成
            console.log("[handleTouchMove] Dragging started after long press.");
            event.preventDefault(); // ドラッグ開始と同時にスクロールをブロック
            return;
        }
        return;
    }

    // ドラッグが既に開始されている場合のみ、クローン要素の移動とドロップゾーンのハイライトを行う
    if (isDragging && currentTouchDraggedClone) {
        event.preventDefault(); // ドラッグ中は常にスクロールをブロック
...
function handleTouchEnd(event) {
    ...
    if (touchTimeout) {
        clearTimeout(touchTimeout);
        touchTimeout = null;
        console.log("[touchend] Cleared long press timeout on touch end.");
    }
    // 長押しフラグもクリア
    isLongPressReady = false; 
    ...
    if (!isDragging) {
        const closestCheckbox = event.target.closest('input[type="checkbox"]');
        if (closestCheckbox) { 
            console.log("[touchend] Not dragging, it's a checkbox click. Skipping finishDragging.");
        } else {
...
function createTouchDraggedClone(originalElement, initialX, initialY, itemIdToClone) {
    ...
    // 特定データ属性も手動で確実にコピー（念のため） (既存ロジックは変更なし)
    ...
    // ✅ セットリスト内のアイテムだった場合
    if (setlist.contains(originalElement) && originalElement.classList.contains('setlist-item')) {
        originalSetlistSlot = originalElement; // 元の setlist-slot 要素を保持
        ...
        // ここで元の要素を非表示にする代わりに、元のスロットを「空」の状態にする
        // originalElement (これは .setlist-slot です) の中身を一時的にクリア
        while (originalElement.firstChild) {
            originalElement.removeChild(originalElement.firstChild);
        }
        originalElement.classList.add('placeholder-slot'); // プレースホルダー用のクラスを追加
        // 曲としてのクラスとデータを削除して、スロットを「空」の状態に戻す
        originalElement.classList.remove('setlist-item', 'short', 'se-active', 'drumsolo-active'); 
        delete originalElement.dataset.itemId; // スロットのitemIdも削除

        console.log(`[createTouchDraggedClone] Original setlist slot ${originalSetlistSlot.dataset.slotIndex} cleared and marked as placeholder.`);
    } else {
        // アルバムリストからのアイテムの場合
        originalElement.style.visibility = 'hidden'; // アルバムアイテムは非表示のまま
