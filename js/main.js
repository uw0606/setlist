...
function getSlotItemData(element) {
    ...
    if (isSetlistItem) {
        ...
        // ★ここから追加・変更★
        // セットリストアイテムの場合も、常にデータ属性から情報を取得する
 ...
 * 例: { itemId: "...", name: "曲名", albumClass: "...", short: true/false, seChecked: true/false, hasShortOption: true/false, hasSeOption: true/false, rGt: "D", lGt: "D", bass: "D", bpm: "180", chorus: "あり" }
 */
...
async function generateSetlistPdf() {
    ...
    for (const slot of setlistSlots) {
        if (slot.classList.contains('setlist-item')) {
            ...
            if (songData) {
                ...
                const isAlbum1 = slot.classList.contains('album1'); 
    ...
    try {
        ...
        // ヘッダーセルの高さとマージンを定義
        const headerCellHeight = 10; // 日付と会場名を表示するセルの高さ
        const topMargin = 20; // ★上部の固定余白を増やします (例: 10mm -> 20mm)
        const leftMargin = 10;
        const pageWidth = detailedPdf.internal.pageSize.getWidth();
        const tableWidth = pageWidth - (leftMargin * 2); 

        // autoTableの開始Y座標を調整
        let detailedYPos = topMargin; // ★計算されたstartYではなく、固定のtopMarginから開始

        if (headerText) {
            // 背景色付きのセルを描画
            detailedPdf.setFillColor(220, 220, 220); // ヘッダーと同じ背景色
            detailedPdf.setDrawColor(0, 0, 0); // 線を黒に
            detailedPdf.setLineWidth(0.3);    // 線を太く
            detailedPdf.rect(leftMargin, detailedYPos, tableWidth, headerCellHeight, 'FD'); // 塗りつぶしと線を描画

            // 文字を中央揃えで配置
            detailedPdf.setFontSize(14);
            detailedPdf.setFont('NotoSansJP', 'bold');
            detailedPdf.setTextColor(0, 0, 0); // 文字色を黒に
            detailedPdf.text(headerText, pageWidth / 2, detailedYPos + headerCellHeight / 2 + 0.5, { align: 'center', baseline: 'middle' }); // 0.5は微調整

            detailedYPos += headerCellHeight; // セルの高さ分だけY座標を移動
        }

        detailedPdf.autoTable({
            head: [tableHeaders],
            body: tableBody,
            startY: detailedYPos, // ★調整済みのY座標を使用
            theme: 'grid',
            styles: {
                font: 'NotoSansJP',
                fontStyle: 'bold',      // 本文を太字に
                fontSize: 11,           // 少し大きく
                cellPadding: 2,
                lineColor: [0, 0, 0],   // 線を黒に
                lineWidth: 0.2,         // 線を太く
                textColor: [0, 0, 0]    // 文字を黒に
            },
            headStyles: {
                fillColor: [220, 220, 220], // 少し濃いめのヘッダー背景
                textColor: [0, 0, 0],
                font: 'NotoSansJP',
                fontStyle: 'bold',
                halign: 'center'
            ...
            margin: { top: topMargin, right: 10, bottom: 10, left: leftMargin }, // ★マージンも調整
            didDrawPage: function (data) {
                let str = 'Page ' + detailedPdf.internal.getNumberOfPages();
                detailedPdf.setFontSize(10);
                detailedPdf.setFont('NotoSansJP', 'normal'); // ページ番号はnormalで
        ...
        // シンプルPDF全体のフォントを太字に設定
        simplePdf.setFont('NotoSansJP', 'bold');
        simplePdf.setFontSize(12); // シンプルなPDFは少し大きめのフォントサイズに

        let simpleYPos = 10;

        // ヘッダーテキスト（日付と会場）
        if (headerText) {
            simplePdf.setFontSize(14); // ヘッダーは少し大きく
            simplePdf.text(headerText, 10, simpleYPos);
            simpleYPos += 10;
            simplePdf.setFontSize(12); // サイズを戻す
        }

        // 各曲目をテキストとして追加
        simplePdfBody.forEach(line => {
            simplePdf.text(line, 10, simpleYPos);
            simpleYPos += 7; // 行の高さ
            // ページ下部に近づいたら新しいページを追加
            if (simpleYPos > simplePdf.internal.pageSize.getHeight() - 20) {
                simplePdf.addPage();
                simpleYPos = 10; // 新しいページの開始Y座標
                simplePdf.setFont('NotoSansJP', 'bold'); // 新しいページでも太字を維持
                simplePdf.setFontSize(12); // フォントサイズを維持
            }
        });
