...
async function generateSetlistPdf() {
    ...
    for (const slot of setlistSlots) {
        if (slot.classList.contains('setlist-item')) {
            ...
            if (isAlbum1) {
                ...
                const noString = `${currentNo}.`; // '1.' または '10.'
                // 1桁No.ならスペース2つ ('1.  '), 2桁No.ならスペース1つ ('10. ')
    ...
    try {
        ...
        // ★★★ 修正: 曲間を広げるための計算 (ボトムマージン 25mm 適用) ★★★
        const availableHeight = pageHeight - detailedYPos - bottomMargin; // 下部マージンを考慮
        const numDetailedRows = tableBody.length;
        let detailedRowHeight = 4; // 最小の行の高さ (mm)

        if (numDetailedRows > 0) {
            const calculatedDetailedRowHeight = availableHeight / numDetailedRows;
            // 最小値4mmを保証しつつ、割り当てられた高さを採用
        ...
        if (numSimpleRows > 0) {
            // ★★★ 修正: シンプルPDFの行間調整 (ボトムマージン 25mm 適用) ★★★
            const simpleAvailableHeight = pageHeight - simpleTopMargin - bottomMargin - simpleHeaderHeight; // 下部マージンを考慮
