...
function getCurrentState() {
  const setlistState = Array.from(setlist.children).map(item => {
      ...
      const hasCheckbox = !!checkbox; // チェックボックスが存在するかどうか
      const isShort = hasCheckbox ? checkbox.checked : false;
      const albumClass = Array.from(item.classList).find(className => className.startsWith('album'));
      return { name: songName, short: isShort, hasCheckbox: hasCheckbox, albumClass: albumClass }; // hasCheckbox を追加
  });
          ...
          .then((snapshot) => {
              ...
              if (state && state.setlist) {
                  ...
                  state.setlist.forEach(itemData => {
                      ...
                      if (songInfoDiv && itemData.hasCheckbox) { // hasCheckbox が true ならチェックボックスを追加
                          const checkbox = document.createElement('input');
                          checkbox.type = 'checkbox';
                          checkbox.checked = !!itemData.short; // short の状態を復元
