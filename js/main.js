...
function handleTouchStart(event) {
    ...
    if (event.touches.length === 1) {
        ...
        if (setlist.contains(touchedElement) && touchedElement.classList.contains('setlist-item')) {
            ...
            // _originalItemData は handleDrop で必要になるため、ここで取得して保存
            originalSetlistSlot._originalItemData = getSlotItemData(originalSetlistSlot);
        ...
        // ドラッグ開始の setTimeout ロジックは削除します。
        // ドラッグは handleTouchMove での移動によってのみ開始されるようにします。
    }
}
        ...
        if (!isDragging) { // まだドラッグが開始されていない場合
            ...
            if (deltaX > TAP_MOVE_THRESHOLD || deltaY > TAP_MOVE_THRESHOLD) {
                ...
                if (elementToDrag && draggingItemId) {
                    ...
                    // --- ⭐︎ ここを修正します ⭐︎ ---
                    // createTouchDraggedClone に渡す初期座標を調整
                    // クローンの左上隅がタッチ開始要素の左上隅と同じ位置に来るように計算
                    const rect = elementToDrag.getBoundingClientRect();
                    const cloneInitialX = rect.left;
                    const cloneInitialY = rect.top;

                    createTouchDraggedClone(elementToDrag, cloneInitialX, cloneInitialY, draggingItemId);
                    // --- ⭐︎ 修正ここまで ⭐︎ ---
