...
let currentTouchDraggedOriginalElement = null; 
...
const originalAlbumMap = new Map(); // 各アイテムの元のアルバムIDを保持
const maxSongs = 26; // セットリストの最大曲数

// --- DOM要素参照（初期化が必要なもの） ---
// DOMContentLoadedの後に値が代入されます
...
// --- DOM要素参照（初期化済み） ---
const menu = document.getElementById("menu"); // constで定義済み
const menuButton = document.getElementById("menuButton"); // constで定義済み
...
// (以降、album1ItemIds, specialEffectOptions, チューニングオプションなどの定数が続きます)
...
/**
 * 日本語の読み仮名を正規化し、ソート・グルーピング用の基本文字（清音・大文字）に変換する。
 * 例: ギャンブル -> カ, あっ -> ア
 * @param {string} reading - data-kana-sortの値
 * @returns {string} 正規化された基本文字
 */
function normalizeReadingForGrouping(reading) {
    if (!reading) return '';
    
    // 1. 全角カナへ統一 (半角カナを全角に変換)
    let normalized = reading.normalize('NFKC'); 

    // 2. 拗音・促音を基本文字に変換 (例: ァィゥェォャュョッ -> アエイウエオヤユヨツ)
    normalized = normalized.replace(/[ァィゥェォャュョッ]/g, (match) => {
        const map = {'ァ': 'ア', 'ィ': 'イ', 'ゥ': 'ウ', 'ェ': 'エ', 'ォ': 'オ', 'ャ': 'ヤ', 'ュ': 'ユ', 'ョ': 'ヨ', 'ッ': 'ツ'};
        return map[match] || match;
    });

    // 3. 濁点・半濁点を除去し、清音の基本行文字に変換
    normalized = normalized
        .replace(/[ガギグゲゴ]/g, 'カ')
        .replace(/[ザジズゼゾ]/g, 'サ')
        .replace(/[ダヂヅデド]/g, 'タ')
        .replace(/[バビブベボパピプペポ]/g, 'ハ');
    
    return normalized.toUpperCase();
}

// -----------------------------------------------------------------------------
// --- ビュー切り替えロジックの定義 (initializeNameOrderView の修正) ---
// -----------------------------------------------------------------------------
...
function initializeNameOrderView() {
    console.log("[initializeNameOrderView] Starting initialization using data-kana-sort/data-alpha-sort...");
    ...
    // 1. 全曲アイテムを収集し、ソートキーとグループ種別を決定する
    const allItems = Array.from(albumList.querySelectorAll(".item"));
    
    const processedItems = allItems.map(item => {
        const kanaSort = item.dataset.kanaSort;
        const alphaSort = item.dataset.alphaSort;
        
        let sortKey = '';
        let groupType = ''; // 'KANA', 'ALPHA', 'ETC'
        
        if (kanaSort) {
            sortKey = normalizeReadingForGrouping(kanaSort); // 日本語ソートキーの正規化
            groupType = 'KANA';
        } else if (alphaSort) {
            sortKey = alphaSort.toUpperCase();
            groupType = 'ALPHA';
        } else {
            // data-kana-sortもdata-alpha-sortも記載なしの場合
            sortKey = item.dataset.songName ? item.dataset.songName.toUpperCase() : 'zz_etc'; 
            groupType = 'ETC';
        }
        
        return {
            element: item,
            sortKey: sortKey, // グループ分けに使用するキー (正規化済み)
            groupType: groupType
        };
    });
    ...
    const sortedItems = processedItems.sort((a, b) => {
        // ETCアイテムは必ず最後に並ぶようにする
        if (a.groupType === 'ETC' && b.groupType !== 'ETC') return 1;
        if (a.groupType !== 'ETC' && b.groupType === 'ETC') return -1;
        
        // KANAとALPHAはそれぞれのロケールでソート
        return a.sortKey.localeCompare(b.sortKey, 'ja', { sensitivity: 'base' });
    });

    // 2. グループ化の定義 (新しいデータ属性に対応)
    const groups = {
        'AIUEO': [], 'KAKIKUKEO': [], 'SASISHISO': [], 'TATITUTETO': [], 'NANINUNENO': [], 
        'HAHIHUHEHO': [], 'MAMIMUMEMO': [], 'YAYUYO': [], 'RARIRURERO': [], 'WAWOUN': [], 
        'ETC_JP': [], // data-kana-sortを持つアイテムで分類できなかったもの
        
        'ABCDE': [], 'FGHIJ': [], 'KLMNO': [], 'PQRST': [], 'UVWXYZ': [], 
        'ETC_EN': [], // data-alpha-sortを持つアイテムで分類できなかったもの
        
        'ETC_NONE': [] // どちらのデータ属性も持たないアイテム
    };

    // 3. アイテムをグループに分類する
    sortedItems.forEach(data => {
        const firstChar = data.sortKey.charAt(0);
        
        if (data.groupType === 'ETC') {
            groups.ETC_NONE.push(data.element);
            return;
        }

        const charCode = firstChar.charCodeAt(0);
        
        if (data.groupType === 'ALPHA') {
            // --- ALPHAグループ ---
        ...
        } else if (data.groupType === 'KANA') {
            // --- KANAグループ (正規化された文字で判定) ---
            const jpChar = firstChar;
            
            if ('アイウエオ'.includes(jpChar)) groups.AIUEO.push(data.element);
            else if ('カ'.includes(jpChar)) groups.KAKIKUKEO.push(data.element);
            else if ('サ'.includes(jpChar)) groups.SASISHISO.push(data.element);
            else if ('タ'.includes(jpChar)) groups.TATITUTETO.push(data.element);
            else if ('ナニヌネノ'.includes(jpChar)) groups.NANINUNENO.push(data.element);
            else if ('ハ'.includes(jpChar)) groups.HAHIHUHEHO.push(data.element);
    ...
    // 4. ナビゲーションタブの生成と描画
    const navUl = nameOrderNav;
    navUl.innerHTML = '';
    navUl.classList.add('name-order-nav');
    ...
    const navItems = [
        ...
        { key: 'ETC_NONE', label: '記号・数字・無記' },
        { key: 'ETC_JP', label: 'その他(カナ)' }, 
        { key: 'ETC_EN', label: 'その他(英数)' }
    ];
