...
function handleDrop(event) {
  ...
  if (!isMovingInSetlist && setlist.children.length < maxSongs) {
      ...
      // ★ draggingItem のクラス情報を保持したまま移動 ★
      const shortClassExists = draggingItem.classList.contains("short");
      const songInfoElement = draggingItem.querySelector(".song-info");
      let checkboxChecked = false;
      if (songInfoElement) {
          const checkboxElement = songInfoElement.querySelector("input[type='checkbox']");
          if (checkboxElement) {
      ...
      tempItem.className = draggingItem.className;
      tempItem.draggable = true;
      tempItem.innerHTML = draggingItem.innerHTML;

      if (shortClassExists && tempItem.querySelector(".song-info") && !tempItem.querySelector(".song-info input[type='checkbox']")) {
          const songInfoDiv = tempItem.querySelector(".song-info");
          const checkbox = document.createElement("input");
          checkbox.type = "checkbox";
      ...
      } else if (shortClassExists && tempItem.querySelector(".song-info input[type='checkbox']")) {
          tempItem.querySelector(".song-info input[type='checkbox']").checked = checkboxChecked;
      }

      draggingItem.replaceWith(tempItem);
      draggingItem = tempItem; // draggingItem を新しい要素に更新
  }
  finishDragging();
}
