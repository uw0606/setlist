...
let initialTouchX, initialTouchY; // グローバル変数
...
function handleTouchStart(e) {
    ...
    touchTimeout = setTimeout(() => {
        // 長押しが検出された場合のみドラッグを開始
        // ここで `e.preventDefault()` を呼ぶことでスクロールを停止する
        // dragstartイベントが発行される前にこれが行われることが重要
        // Chromeなどのブラウザは、touchstartでpreventDefault()がないと、
        // touchmoveで一定量移動するとスクロールに切り替わる
        
        // **ここではe.preventDefault()を呼ばず、dragstartで呼ぶようにするか、
        // touchmoveのDRAG_THRESHOLDを超えたときにスクロールを許可するロジックを優先する**
        // もしくは、後続のhandleTouchMoveでスクロールを禁止するロジックを調整する

        // isDragging = true; // ここで true にするのは早すぎる可能性があるので、dragstart イベント内で設定する方が良い
        ...
        createTouchDraggedClone(touchedItem);
        
        // touchmoveイベントでスクロールを抑制するためにisDraggingをtrueにする
        isDragging = true; 

        draggingItemId = itemId; // グローバル変数にアイテムIDを保存
        // ここで dragstart イベントを手動で発火させることもできる
        // const dragStartEvent = new Event('dragstart', { bubbles: true, cancelable: true });
        // dragStartEvent.dataTransfer = new DataTransfer(); // ダミーのdataTransfer
        // dragStartEvent.dataTransfer.setData('text/plain', draggingItemId);
        // touchedItem.dispatchEvent(dragStartEvent);
...
function handleTouchMove(e) {
    ...
    if (isDragging) {
        ...
        updateTouchDraggedClonePosition(e.touches[0].clientX, e.touches[0].clientY);
        // ドロップゾーンの判定ロジック
...
function handleDoubleClick(e) {
    ...
    if (itemElement.closest('.album-content') && !itemElement.classList.contains('setlist-slot')) {
        ...
        if (nextEmptySlotIndex !== -1) {
            ...
            itemElement.style.display = 'none'; // アルバムメニューからアイテムを隠す
    ...
    // ダブルクリックされた要素がセットリストにある場合
        ...
        if (clearSlotContent(slotIndex)) { // clearSlotContent はアイテムを削除する関数と仮定
            ...
            if (albumItem) {
                albumItem.style.display = ''; // displayスタイルをリセットして表示
