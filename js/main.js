...
function handleTouchEnd(event) {
    ...
    if (isDragging) {
        ...
        if (endTouch) {
            ...
            dropTargetSlot = targetElement ? targetElement.closest('.setlist-slot') : null;
            console.log("[handleTouchEnd] Detected drop target slot:", dropTargetSlot ? dropTargetSlot.dataset.slotIndex : "none");
        } else {
            console.warn("[handleTouchEnd] No changedTouches[0] on touch end. Cannot determine drop target.");
        }

        // ここで processDrop 関数を呼び出す
        // draggedElement として currentTouchDraggedClone を渡す
        // originalSetlistSlot は既にグローバルで設定されている
        processDrop(currentTouchDraggedClone || initialTouchElement, dropTargetSlot, originalSetlistSlot); // currentTouchDraggedCloneがnullならinitialTouchElementを使う

        // ドラッグ終了後のクリーンアップ
        finishDragging(); // finishDraggingがcurrentTouchDraggedCloneの削除などを行う

        // showMessageBox は processDrop の内部で呼び出すか、
        // processDropの戻り値に応じてここで呼び出すように変更する
        // 現状、processDrop内でshowMessageBoxを呼んでいるため、ここでは不要。
        // displayMessage('曲の移動処理が完了しました。'); // もし一般的な成功メッセージが必要なら
    } else {
