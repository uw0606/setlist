...
const DOUBLE_TAP_DELAY = 350; // ダブルタップと認識する間隔（ミリ秒）を少し長くする
...
document.addEventListener('DOMContentLoaded', () => {
        ...
        itemElement.addEventListener('touchmove', (e) => {
            if (e.touches.length === 1) {
                ...
                if (deltaX > TAP_MOVE_THRESHOLD || deltaY > TAP_MOVE_THRESHOLD) {
                    ...
                    // console.log("[touchmove] Touch moved beyond threshold. Likely drag operation.");
                }
            }
        });
    ...
    setlist.querySelectorAll(".setlist-slot").forEach((slot, index) => {
        ...
        // ★修正3: スロット全体のクリックリスナーを調整し、曲の削除は行わないようにする★
        slot.addEventListener('click', (e) => {
            // クリックされたターゲットが setlist-item (またはその子要素) でないことを確認
            // setlist-item が存在しない（空のスロットである）ことを優先的にチェック
            if (!e.target.closest('.setlist-item')) {
                 ...
                 // ここで e.preventDefault() や e.stopPropagation() を行うと、
                 // スクロールや他のイベントに影響を与える可能性があるので、
                 // 必要がなければ行わない。誤削除の直接の原因にはならないはず。
            } else {
...
// removeSetlistItem 関数は別途定義されているものと仮定します。
// 例: function removeSetlistItem(itemElement) { /* ... 削除ロジック ... */ }
// この関数は、`itemElement` を DOM から削除し、必要に応じてデータ構造からも削除する責任があります。