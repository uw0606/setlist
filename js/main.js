...
function handleDrop(event) {
  ...
  if (!isMovingInSetlist && setlist.children.length < maxSongs) {
    ...
    songInfoDiv.textContent = draggingItem.textContent;

    const listItem = document.createElement("li");
    listItem.classList.add("setlist-item");
    if (draggingItem.classList.contains("album2")) { // アルバムクラスを引き継ぐ例 (あなたの実際のクラスに合わせてください)
      listItem.classList.add("album2");
    }
    ...
    draggingItem.replaceWith(listItem); // 古い draggingItem を新しいリストアイテムで置換
