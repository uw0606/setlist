...
function handleTouchStart(event) {
    ...
    // タッチされたのがドラッグハンドルではない場合、ダブルタップ検出以外では何もしない
    ...
    if (event.touches.length === 1 && isDragHandleTouch) { // ドラッグハンドルのタッチのみを処理
        ...
        // ドラッグハンドルに触れたら、即座にevent.preventDefault()を呼んでスクロールをブロックし、ドラッグを開始する
        // これにより、ドラッグハンドルを触った場合はスクロールを許容しない
        event.preventDefault();
        ...
        // 長押しは不要になるが、もし少しの遅延を設けたいならここでsetTimeoutを再導入
        // 例: setTimeout(() => { if (!isDragging) createCloneAndStartDrag(); }, 100);
        // 今回はドラッグハンドルに触れたら即座にドラッグ準備に入るため、不要
        // ただし、handleTouchMoveで距離判定は引き続き行う
    }
}
...
function handleTouchMove(event) {
    ...
    if (!isDragging) {
        // touchstart でドラッグハンドルが触れられたか、draggingItemId が設定されているかを確認
        // draggingItemId が null の場合は、ドラッグ対象要素が触られていない（＝スクロール優先）
        if (!draggingItemId) {
            return; // ドラッグ対象がなければスクロール優先
        }
        ...
        if (distance > DRAG_THRESHOLD) {
            ...
            // しきい値に達していない場合は何もしない = スクロールは発生しない（touchstartでpreventDefault済みのため）
            // ただし、これはドラッグハンドルに触れた場合のみ
            event.preventDefault(); // ドラッグハンドルを触った場合は、わずかな移動でもpreventDefaultを維持
            return;
        }
    }
