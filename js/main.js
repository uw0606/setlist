...
const DRAG_THRESHOLD = 15;   // ドラッグ開始とみなす最小移動距離 (ピクセル) - 現状は30ですが、長押しと組み合わせるなら短くしても良いでしょう
...
function handleTouchMove(event) {
    ...
    if (!isDragging) {
        ...
        if (deltaX > DRAG_THRESHOLD || deltaY > DRAG_THRESHOLD) {
            // ★修正: 閾値を超えたらタイマーをクリアして即ドラッグ開始
            if (touchTimeout) {
                clearTimeout(touchTimeout);
                touchTimeout = null;
                console.log("[handleTouchMove:Mobile] Drag threshold exceeded, clearing long press timer.");
            }

            isDragging = true;
            console.log("[handleTouchMove:Mobile] Dragging started (threshold exceeded).");
            ...
            // 閾値を超えていない場合は、スクロールを許可するために何もしない
            return;
        }
    }
