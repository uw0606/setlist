...
async function generateSetlistPdf() {
    ...
    try {
        ...
        const MAX_SONG_FONT_SIZE = 70; // 曲の最大フォントサイズ
        ...
        // ★修正1: 物理的な下マージンを確保するための新しい計算値 (20mm + 確保したい空き)
        const NEW_BOTTOM_MARGIN = 35; // ページ下部に約15mmの空きを作るために20mmから35mmに増加
        const simpleBottomMargin = 20; // 物理的な下マージン（ここでは使用しないが、念のため残す）
        ...
        if (numSimpleRows > 0) {
            // ★修正2: 利用可能な総高さを計算する際に、NEW_BOTTOM_MARGINを使用
            const simpleAvailableHeight = pageHeightSimple - simpleYPos - NEW_BOTTOM_MARGIN;
