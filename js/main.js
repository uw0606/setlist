    ...
    if (event.touches.length === 1) {
        ...
        touchTimeout = setTimeout(() => {
            ...
            if (!isDragging && draggingItemId && document.body.contains(touchedElement)) {
                // ロングプレスが成立した時点で、初めてドラッグを開始し、preventDefaultを呼ぶ
                // この時点では、すでにtouchmoveイベントが数回発生している可能性がありますが、
                // それらはpreventDefault()を呼ばずにスクロールを許可していたはずです。
                // ここでクローンを作成し、isDraggingをtrueにします。
                ...
                // Note: setTimeout内でのpreventDefault()は非同期のため効果がないことがほとんどです。
                // そのため、preventDefault()はtouchmoveがisDragging=trueになった時に呼び出すのが確実です。
            } else {
...
function handleTouchMove(event) {
    ...
    if (!isDragging) {
        ...
        // ここで event.preventDefault() を呼んではならない。
        // ロングプレスが成立して isDragging が true になるまで、スクロールは許可する。
        return;
    }

    // ドラッグが既に開始されている場合のみ、preventDefault() を呼び出す
    event.preventDefault(); // ドラッグ中は常にデフォルトのスクロールをキャンセルし、クローンを追従させる
...
function handleTouchEnd(event) {
    ...
    // これは短いタップまたはスクロールの意図だったと判断し、ドラッグ処理は行わない
    if (touchTimeout) {
        clearTimeout(touchTimeout);
        touchTimeout = null;
    ...
    // ドラッグが行われていなかった場合は、ここで処理を終了 (例えば、ロングプレス成立直前に指が離れた場合など)
    if (!isDragging) {
