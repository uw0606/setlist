...
// ドラッグ開始時にどのアイテムがドラッグされているかを追跡する変数
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ★修正1: enableDragAndDrop 関数に、ドラッグ開始/終了時の処理を追加するためのコールバックを追加（仮定）★
    // enableDragAndDrop の定義自体は変更されていませんが、内部で currentDraggedItem を設定/解除する想定です。
    // もし enableDragAndDrop 関数がここに定義されていない場合、その関数も提供してください。
    // 例:
    // function enableDragAndDrop(element, handle = element) {
    //     handle.addEventListener('dragstart', (e) => {
    //         e.dataTransfer.setData('text/plain', e.target.dataset.itemId);
    //         e.dataTransfer.effectAllowed = 'move';
    //         e.target.classList.add('dragging');
    //         currentDraggedItem = e.target; // ドラッグ開始時に設定
    //         console.log(`[DragStart] Item ${e.target.dataset.itemId} started dragging.`);
    //     });
    //     element.addEventListener('dragend', (e) => {
    //         e.target.classList.remove('dragging');
    //         currentDraggedItem = null; // ドラッグ終了時に解除
    //         console.log(`[DragEnd] Item ${e.target.dataset.itemId} ended dragging.`);
    //     });
    //     // ... その他の dragover, drop などのロジック ...
    // }
    ...
    const initializeSetlistItem = (itemElement) => {
        ...
        itemElement.addEventListener('touchmove', (e) => {
            if (e.touches.length === 1) {
                ...
                if (deltaX > TAP_MOVE_THRESHOLD || deltaY > TAP_MOVE_THRESHOLD) {
                    ...
                    // ドラッグ開始を認識したら、他のイベントが発火しないように防止
                    // e.preventDefault(); // これを有効にすると、タッチによるドラッグ&ドロップの挙動が変わる可能性があるので注意
                }
            }
        });
        ...
        itemElement.addEventListener('touchend', (e) => {
            const currentTime = new Date().getTime();
            // ドラッグ中でなく、かつ、規定時間内に2回目のタップがあり、指が移動していなければダブルタップ
            // さらに、現在ドラッグ中のアイテムがないことを確認 (これが非常に重要)
            ...
            initialTouchY = 0;
            isDraggingTouch = false; 
            // currentDraggedItem は enableDragAndDrop の dragend イベントでリセットされる想定
        });
    ...
    setlist.querySelectorAll(".setlist-slot").forEach((slot, index) => {
        ...
        // ★修正3: 空のスロットに対するクリック/タップ/ドラッグ時の削除機能を厳重に無効化★
        ...
        slot.addEventListener('touchstart', (e) => {
            // setlist-item がスロット内に既にある場合は、その itemElement の touchstart が処理するので無視
            // ドラッグ中のアイテムが存在する場合も、スロットのタッチイベントは無視
...
// removeSetlistItem 関数は引き続き別途定義されているものと仮定します。
// この関数は、引数として渡された 'itemElement' (必ず .setlist-item であることを想定) を削除することのみを責任とします。
// 例えば、以下のように実装されていることを想定しています。
// function removeSetlistItem(itemElement) { 
//     if (itemElement && itemElement.classList.contains('setlist-item')) {
//         const parentSlot = itemElement.closest('.setlist-slot');
//         itemElement.remove(); // setlist-item を削除
//         console.log(`[removeSetlistItem] Removed setlist item: ${itemElement.dataset.itemId}`);
//         // 必要であれば、親スロットが空になったことを示すクラスなどを追加
//         if (parentSlot) {
//             // 親スロットから不要なデータ属性やクラスを削除し、空の状態に戻す
//             parentSlot.removeAttribute('data-short');
//             parentSlot.removeAttribute('data-se-checked');
//             parentSlot.removeAttribute('data-drumsolo-checked');
//             parentSlot.classList.remove('short', 'se-active', 'drumsolo-active');
//             console.log(`[removeSetlistItem] Cleared classes/data from parent slot ${parentSlot.dataset.slotIndex}`);
//         }
//     } else {
//         console.warn("Attempted to remove non-setlist-item element:", itemElement);
//     }
// }

// enableDragAndDrop 関数もあなたの定義に依存します。
// 例:
// function enableDragAndDrop(element, handle = element) {
//     if (element.dataset.dndEnabled) return; // 二重初期化防止
//     element.dataset.dndEnabled = 'true';
//
//     // ドラッグ元として機能する場合
//     if (element.classList.contains('album-item') || element.classList.contains('setlist-item')) {
//         element.setAttribute('draggable', 'true');
//         handle.addEventListener('dragstart', (e) => {
//             e.dataTransfer.setData('text/plain', element.dataset.itemId);
//             e.dataTransfer.effectAllowed = 'move';
//             element.classList.add('dragging');
//             currentDraggedItem = element; // ドラッグ開始時に設定
//             console.log(`[DragStart] Item ${element.dataset.itemId} started dragging. Global flag set.`);
//         });
//         element.addEventListener('dragend', (e) => {
//             element.classList.remove('dragging');
//             currentDraggedItem = null; // ドラッグ終了時に解除
//             console.log(`[DragEnd] Item ended dragging. Global flag cleared.`);
//         });
//     }
//
//     // ドロップ先として機能する場合
...
//             e.preventDefault();
//             element.classList.remove('drag-over');
//             if (currentDraggedItem) { // グローバルな currentDraggedItem を使用
//                 const targetSlot = element;
//                 const sourceSlot = currentDraggedItem.closest('.setlist-slot');
//
//                 // 同じスロットへのドロップは無視
//                 if (sourceSlot && sourceSlot === targetSlot) {
//                     console.log("[Drop] Dropped on the same slot. No action.");
//                     return;
//                 }
//
//                 // ターゲットスロットに既にアイテムがある場合
//                 if (targetSlot.querySelector('.setlist-item')) {
//                     // アイテムを入れ替える、またはターゲットスロットのアイテムを移動させるなどのロジック
//                     // 例: 入れ替え (シンプル版)
//                     const existingItem = targetSlot.querySelector('.setlist-item');
//                     if (sourceSlot) {
//                         // 元のスロットに既存アイテムを移動
//                         sourceSlot.appendChild(existingItem);
//                         console.log(`[Drop] Swapped items. ${currentDraggedItem.dataset.itemId} moved to ${targetSlot.dataset.slotIndex}, ${existingItem.dataset.itemId} moved back to ${sourceSlot.dataset.slotIndex}`);
//                     } else {
//                         // アルバムからセットリストへのドロップで、ターゲットに既存アイテムがある場合
//                         // この場合は、既存アイテムを削除するか、ドロップを拒否するかの判断が必要
//                         console.warn("[Drop] Dropping album item onto occupied slot. Existing item removed.");
//                         existingItem.remove(); // 例として既存アイテムを削除
//                     }
//                     targetSlot.appendChild(currentDraggedItem);
//                 } else {
//                     // ターゲットスロットが空の場合
//                     targetSlot.appendChild(currentDraggedItem);
//                     console.log(`[Drop] Item ${currentDraggedItem.dataset.itemId} dropped into empty slot ${targetSlot.dataset.slotIndex}`);
//                     // 元のスロットがセットリストの一部だった場合、空にする
//                     if (sourceSlot) {
//                         console.log(`[Drop] Cleared source slot: ${sourceSlot.dataset.slotIndex}`);
//                         // ここで sourceSlot を空の状態に戻す追加のクリーンアップが必要な場合がある
//                         // 例: sourceSlot.innerHTML = ''; // または特定のDOM構造を再構築
//                         // データ属性やクラスのクリーンアップも必要に応じて
//                         sourceSlot.removeAttribute('data-short');
//                         sourceSlot.removeAttribute('data-se-checked');
//                         sourceSlot.removeAttribute('data-drumsolo-checked');
//                         sourceSlot.classList.remove('short', 'se-active', 'drumsolo-active');
//                     }
//                 }
//                 currentDraggedItem.classList.remove('dragging'); // classList.remove は dragend でも行われるが、念のため
//                 currentDraggedItem = null; // ドロップ完了でグローバルフラグをクリア
//             }
//         });
//     }
// }

// --- ドラッグ＆ドロップ関連のイベントハンドラ関数 ---

/**
 * ドラッグ開始ハンドラ。
 * @param {Event} e - dragstart イベントオブジェクト
 */
function handleDragStart(e) {
    ...
    // グローバル変数に現在ドラッグ中のアイテムを保持
    draggedElement = e.target;
    console.log(`[handleDragStart] Started dragging: ${draggedElement.dataset.itemId}`);
}

/**
 * ドラッグオーバーハンドラ。
 * @param {Event} e - dragover イベントオブジェクト
 */
function handleDragOver(e) {
    e.preventDefault(); // ドロップを許可するために必須
    e.dataTransfer.dropEffect = 'move'; // マウスカーソルを移動アイコンにする
    // ドロップターゲットにクラスを追加して視覚的に示す
    if (e.target.classList.contains('setlist-slot') && !e.target.classList.contains('drag-over')) {
        e.target.classList.add('drag-over');
    } else if (e.target.closest('.setlist-slot') && !e.target.closest('.setlist-slot').classList.contains('drag-over')) {
        e.target.closest('.setlist-slot').classList.add('drag-over');
    }
}

/**
 * ドロップハンドラ。
 * @param {Event} e - drop イベントオブジェクト
 */
function handleDrop(e) {
    e.preventDefault(); // デフォルトのドロップ動作を防ぐ
    const data = e.dataTransfer.getData('text/plain');
    const droppedItem = document.querySelector(`[data-item-id="${data}"]`);

    let targetSlot = e.target.closest('.setlist-slot'); // ドロップされたスロットを特定

    // 有効なターゲットスロットがない、またはドロップされたアイテムがない場合は何もしない
    if (!targetSlot || !droppedItem) {
        console.warn("[handleDrop] Invalid drop target or dropped item missing.");
        return;
    }

    // ドロップターゲットから 'drag-over' クラスを削除
    document.querySelectorAll('.drag-over').forEach(el => el.classList.remove('drag-over'));

    // ドロップされたアイテムがすでにセットリスト内にある場合の元のスロット
    const originalSlot = droppedItem.closest('.setlist-slot');

    // ★重要: ドロップ処理のロジック改善★
    // 1. ターゲットスロットに既にアイテムがある場合
    if (targetSlot.querySelector('.setlist-item')) {
        const existingItemInTarget = targetSlot.querySelector('.setlist-item');

        // 同じスロット内でドラッグ＆ドロップした場合 (何もしない)
        if (originalSlot && originalSlot === targetSlot) {
            console.log("[handleDrop] Dropped onto the same slot. No action.");
            return;
        }

        // 元のスロットが存在する場合 (セットリスト内での移動)
        if (originalSlot) {
            // 元のスロットに既存のアイテムを移動
            originalSlot.appendChild(existingItemInTarget);
            console.log(`[handleDrop] Swapped: ${droppedItem.dataset.itemId} to slot ${targetSlot.dataset.slotIndex}, ${existingItemInTarget.dataset.itemId} to slot ${originalSlot.dataset.slotIndex}`);
            
            // 元のスロットのデータ属性やクラスを、移動したアイテムが持っていたもので更新 (またはリセット)
            // ここで元のスロットをクリーンアップしないと、消えたスロットのデータが残る可能性あり
            resetSlotAttributes(originalSlot, existingItemInTarget); // 元のスロットのデータを更新/リセット
            
            // ターゲットスロットのデータをドロップしたアイテムのものに更新
            updateSlotAttributes(targetSlot, droppedItem);

        } else {
            // アルバムからセットリストへのドロップで、ターゲットに既存アイテムがある場合
            // このケースでは、既存アイテムを削除する（今回の要件にはないが、考慮）
            console.warn("[handleDrop] Dropping album item onto occupied setlist slot. Existing item will be removed.");
            existingItemInTarget.remove(); // 既存アイテムを削除
            // ターゲットスロットのデータ属性やクラスをクリーンアップしてから新しいアイテムを追加
            resetSlotAttributes(targetSlot);
            targetSlot.appendChild(droppedItem);
            updateSlotAttributes(targetSlot, droppedItem);
        }
    } else {
        // 2. ターゲットスロットが空の場合
        console.log(`[handleDrop] Dropped ${droppedItem.dataset.itemId} into empty slot ${targetSlot.dataset.slotIndex}`);
        targetSlot.appendChild(droppedItem);
        updateSlotAttributes(targetSlot, droppedItem);

        // 元のスロットがセットリストの一部だった場合、元のスロットを「空」の状態にクリーンアップする
        if (originalSlot) {
            console.log(`[handleDrop] Clearing original slot: ${originalSlot.dataset.slotIndex}`);
            // ★ここが最も重要: 元のスロットからアイテムが移動した後のクリーンアップ★
            // innerHTMLを空にするとスロットのDOM構造まで消える可能性があるので、
            // 子要素 (setlist-item) だけを削除し、スロット自体の属性をリセットする
            // originalSlot.innerHTML = ''; // これはNG、スロット自体が消える原因
            
            // 代わりに、スロット内の曲のチェックボックスの状態などをリセットする
            resetSlotAttributes(originalSlot); 
            
            // (もし必要なら)元のスロットから setlist-item を物理的に取り除く
            // これは droppedItem.remove() ではなく appendChild() が自動的に移動させるので通常は不要
        }
    }
    
    // ドラッグ中のアイテムのクラスを削除 (dragendでも行われるが念のため)
    droppedItem.classList.remove('dragging');
    draggedElement = null; // グローバルドラッグフラグをクリア
}

/**
 * ドラッグ進入ハンドラ。
 * @param {Event} e - dragenter イベントオブジェクト
 */
function handleDragEnter(e) {
    if (e.target.classList.contains('setlist-slot')) {
        e.target.classList.add('drag-over');
    } else if (e.target.closest('.setlist-slot')) {
        e.target.closest('.setlist-slot').classList.add('drag-over');
    }
}
...
/**
 * ドラッグ離脱ハンドラ。
 * @param {Event} e - dragleave イベントオブジェクト
 */
function handleDragLeave(e) {
    // 関連ターゲットがドロップゾーンの外にあることを確認してクラスを削除
    if (!e.relatedTarget || !e.relatedTarget.closest('.setlist-slot')) {
        e.target.classList.remove('drag-over');
    }
}

/**
 * ドラッグ終了ハンドラ (グローバルリスナー)。
 * @param {Event} e - dragend イベントオブジェクト
 */
function finishDragging(e) {
    document.querySelectorAll('.dragging').forEach(el => el.classList.remove('dragging'));
    document.querySelectorAll('.drag-over').forEach(el => el.classList.remove('drag-over'));
    draggedElement = null; // グローバルドラッグフラグをクリア
    console.log("[finishDragging] Drag operation finished. All drag classes removed.");
}

// --- タッチイベント関連のイベントハンドラ関数 ---
// (これらの関数は、enableDragAndDropの引数elementが'item'クラスを持つ場合にアタッチされます)

/**
 * タッチ開始ハンドラ。
 * @param {Event} e - touchstart イベントオブジェクト
 */
function handleTouchStart(e) {
    if (e.touches.length === 1) {
        ...
        touchTimeout = setTimeout(() => {
            isDraggingTouch = true; // 長押しでドラッグ開始とマーク
            draggedElement = e.target.closest('.item'); // タッチされたアイテムをドラッグアイテムとして設定
            if (draggedElement) {
                draggedElement.classList.add('dragging');
                console.log(`[handleTouchStart] Long press detected. Started dragging (touch): ${draggedElement.dataset.itemId}`);
                // preventDefaultでスクロールを防ぐ
                e.preventDefault(); 
            }
        }, 300); // 300ms 長押しでドラッグ開始
    }
    // ここでlastTapTimeを設定するのは、ダブルタップ検出のため
    const currentTime = new Date().getTime();
    if (currentTime - lastTapTime < DOUBLE_TAP_DELAY) {
        // ダブルタップ候補の場合、デフォルト動作を防ぐ (ズームなど)
        e.preventDefault(); 
        console.log("[handleTouchStart] Potential double tap. Preventing default.");
    }
    lastTapTime = currentTime;
}
...
/**
 * タッチ移動ハンドラ。
 * @param {Event} e - touchmove イベントオブジェクト
 */
function handleTouchMove(e) {
    if (e.touches.length === 1) {
        ...
        if (deltaX > TAP_MOVE_THRESHOLD || deltaY > TAP_MOVE_THRESHOLD) {
            // 指が一定距離以上動いたら、ダブルタップタイマーをクリアし、ドラッグモードに移行
            clearTimeout(touchTimeout);
            isDraggingTouch = true; // 移動を伴う場合はドラッグと判断

            // ドラッグ中のアイテムがあれば、視覚的なフィードバック
            if (draggedElement) {
                draggedElement.classList.add('dragging');
                // e.preventDefault(); // ここでpreventDefaultすると、ネイティブスクロールが阻害される可能性がある
                                   // ドラッグ中の要素の位置更新は別途実装
            }
            // console.log("[handleTouchMove] Touch moved. Likely drag operation.");
        }
        
        // もしドラッグモードに入っているなら、デフォルトのスクロールなどを防ぐ
        if (isDraggingTouch && draggedElement) {
            e.preventDefault(); // ドラッグ中の要素がスクロールによって動かないように
        }
    }
}
...
/**
 * タッチ終了ハンドラ。
 * @param {Event} e - touchend イベントオブジェクト
 */
function handleTouchEnd(e) {
    clearTimeout(touchTimeout); // 長押しタイマーをクリア

    // タッチによるドラッグ操作が完了した場合
    if (isDraggingTouch && draggedElement) {
        // ドロップ処理を実行 (draggedElement がドロップされた要素)
        // ここでは実際のDOM操作は行わず、handleDropと同様のロジックを呼び出すか、
        // もしくはドロップターゲットの特定とDOM操作は別途行う必要があります。
        // 現在のenableDragAndDropのdropリスナーはPCのドラッグ＆ドロップに最適化されています。
        // モバイルのタッチドラッグ＆ドロップのドロップ処理はもう少し複雑です。
        
        // ここでの最もシンプルな方法は、最後に触れたセットリストスロットにドロップさせることです。
        const targetElement = document.elementFromPoint(initialTouchX, initialTouchY); // ドロップした位置の要素を取得
        let targetSlot = targetElement ? targetElement.closest('.setlist-slot') : null;

        if (targetSlot && draggedElement) {
            // handleDrop のロジックを再利用
            console.log(`[handleTouchEnd] Touch drag ended. Attempting to drop ${draggedElement.dataset.itemId} to slot ${targetSlot.dataset.slotIndex}.`);
            
            // ドロップ時の処理を実行
            // handleDrop関数が e.dataTransfer.getData() に依存しないように調整が必要
            // または、この場で直接ドロップロジックを記述
            
            const originalSlot = draggedElement.closest('.setlist-slot');

            if (targetSlot.querySelector('.setlist-item')) {
                const existingItemInTarget = targetSlot.querySelector('.setlist-item');
                if (originalSlot && originalSlot === targetSlot) {
                    console.log("[handleTouchEnd] Dropped onto the same slot (touch). No action.");
                } else if (originalSlot) {
                    originalSlot.appendChild(existingItemInTarget);
                    resetSlotAttributes(originalSlot, existingItemInTarget);
                    targetSlot.appendChild(draggedElement);
                    updateSlotAttributes(targetSlot, draggedElement);
                } else {
                    // アルバムからセットリストへ（既存アイテムを置き換える）
                    console.warn("[handleTouchEnd] Dropping album item onto occupied slot (touch). Existing item removed.");
                ...
                updateSlotAttributes(targetSlot, draggedElement);
                if (originalSlot) {
                    console.log(`[handleTouchEnd] Clearing original slot (touch): ${originalSlot.dataset.slotIndex}`);
                    resetSlotAttributes(originalSlot); 
                }
            }
        } else {
            // 有効なドロップターゲットが見つからない場合、元の場所に戻す（オプション）
            // 例: if (draggedElement && draggedElement.closest('.setlist-slot')) {
            //         // draggedElement.closest('.setlist-slot').appendChild(draggedElement);
            //     }
            console.log("[handleTouchEnd] No valid drop target found or dragged item missing (touch).");
        }
        ...
        draggedElement = null; // グローバルドラッグフラグをクリア
        isDraggingTouch = false;
        e.preventDefault(); // ドロップ後の不要なイベント防止
    } else {
        // ドラッグではない通常のタップが完了した場合 (ダブルタップ検出は別途 initializeSetlistItem で処理)
        // ダブルタップ検出のためのlastTapTimeリセットは既にinitializeSetlistItemのtouchendで行われる想定
        // ここではタッチ状態をリセットするのみ
        isDraggingTouch = false;
        draggedElement = null;
        console.log("[handleTouchEnd] Simple tap ended. Not a drag or double tap here.");
    }
    lastTapTime = 0; // グローバルなダブルタップタイマーをリセット
    initialTouchX = 0;
    initialTouchY = 0;
}

/**
 * スロットのデータ属性とクラスをリセットするヘルパー関数
 * @param {Element} slot - 対象のスロット要素
 * @param {Element} [itemToUpdateWith] - スロットが空ではなくなる場合に、そのアイテムのデータでスロットを更新する（オプション）
 */
function resetSlotAttributes(slot, itemToUpdateWith = null) {
    if (!slot) return;
    
    // スロット内のチェックボックスの状態をリセット
    slot.querySelectorAll('input[type="checkbox"]').forEach(checkbox => {
        checkbox.checked = false;
    });

    // スロットに関連するクラスとデータ属性を削除
    slot.classList.remove('short', 'se-active', 'drumsolo-active');
    slot.removeAttribute('data-short');
    slot.removeAttribute('data-se-checked');
    slot.removeAttribute('data-drumsolo-checked');
    
    console.log(`[resetSlotAttributes] Slot ${slot.dataset.slotIndex || 'N/A'} attributes reset.`);
}

/**
 * スロットのデータ属性を、そこに入ったアイテムのデータで更新するヘルパー関数
 * @param {Element} slot - 対象のスロット要素
 * @param {Element} item - スロットに挿入されたアイテム要素
 */
function function updateSlotAttributes(slot, item) {
    if (!slot || !item) return;

    // item.dataset から対応する情報を取得し、slot.dataset に設定
    // これらは通常、チェックボックスのクリックで設定されますが、ドラッグ＆ドロップでも整合性を保つために必要かもしれません
    // ただし、通常はアイテムの初期状態を反映するものではないため、この関数はあまり使わないかもしれません
    // setlist-item自体が持つべきデータ（曲名など）はsetlist-itemに持たせる
    // スロット固有のオプション（Short, SEなど）はスロット内のチェックボックスで制御される
    console.log(`[updateSlotAttributes] Slot ${slot.dataset.slotIndex} updated with item ${item.dataset.itemId} (no direct slot data update here).`);
}


/**
 * セットリストアイテムを削除する関数。
 * @param {Element} itemElement - 削除する setlist-item 要素
 */
function removeSetlistItem(itemElement) {
    if (!itemElement || !itemElement.classList.contains('setlist-item')) {
        console.warn("Attempted to remove non-setlist-item element or null element:", itemElement);
        return;
    }

    const parentSlot = itemElement.closest('.setlist-slot');
    if (parentSlot) {
        // ★最重要: setlist-item のみを削除し、スロット自体は削除しない★
        itemElement.remove();
        console.log(`[removeSetlistItem] Removed setlist item: ${itemElement.dataset.itemId} from slot ${parentSlot.dataset.slotIndex}`);

        // アイテム削除後、親スロットを空の状態にクリーンアップする
        resetSlotAttributes(parentSlot);
        // innerHTML = '' はスロット内のDOM構造を全て削除してしまうため、
        // スロット自体の削除につながる可能性があるので避ける。
        // 代わりに、元々あったチェックボックスやドラッグハンドルを再生成するロジックが必要になる場合もあるが、
        // 現状のHTML構造が setlist-item の有無だけで外観が変わるなら不要。
        // もしスロット内にデフォルトで表示される要素があるなら、それを再追加するロジックが必要。
    } else {
        // セットリストスロットに属さないアイテム（例：アルバムリスト内のアイテム）が削除されようとした場合
        itemElement.remove();
        console.log(`[removeSetlistItem] Removed album item: ${itemElement.dataset.itemId}`);
    }
}