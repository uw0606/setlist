...
function handleTouchStart(event) {
    ...
    event.preventDefault(); // ここは常に呼び出す
    ...
    if (event.touches.length === 1) {
        ...
        // ★★★ ここを修正 ★★★
        // handleTouchMoveでドラッグ開始を厳密に制御するため、
        // ここではロングプレスのタイマーは不要。
        // タッチ開始時に既存のタイマーがあればクリアするのみ。
        if (touchTimeout) {
            clearTimeout(touchTimeout);
            touchTimeout = null;
        }
...
function handleTouchMove(event) {
    ...
    if (!isDragging) {
        ...
        if (deltaX < DRAG_THRESHOLD && deltaY < DRAG_THRESHOLD) {
            return; // しきい値に達していない場合は、ドラッグ処理をしない
        }

        // しきい値を超えた場合、ロングプレスのタイマーがあればクリアする
        if (touchTimeout) {
            clearTimeout(touchTimeout);
            touchTimeout = null;
        ...
        const touchedElement = document.elementFromPoint(touchStartX, touchStartY); // タッチ開始時の要素を取得
        const isTouchedFromSetlist = setlist.contains(touchedElement) && touchedElement.classList.contains('setlist-item');

        // ★★★ ここから修正箇所 ★★★
        // isDraggingをtrueにし、クローンを作成する条件を緩和
        // アルバムアイテムからのドラッグ、またはセットリストアイテムからのドラッグのどちらでも開始できるようにする
        if (draggingItemId && document.body.contains(touchedElement)) {
            // セットリスト内のアイテムの場合、createTouchDraggedCloneで元のスロットを隠す
            // createTouchDraggedCloneはoriginalSetlistSlotを見て処理するので、ここで特別対応は不要
            ...
            // 要素が見つからないなど、不正な状態の場合はドラッグを開始しない
            console.warn("[handleTouchMove] Cannot initiate drag. touchedElement or draggingItemId invalid.");
            return;
        }
        // ★★★ ここまで修正箇所 ★★★
    }

    // ドラッグ中は常にデフォルトのスクロールをキャンセル
    event.preventDefault();
    ...
    rafId = requestAnimationFrame(() => {
        ...
        // ドロップゾーンが元のスロットと同じ場合はハイライトを解除
        // originalSetlistSlotがある（セットリストからのドラッグ）場合にのみこのロジックを適用
        if (originalSetlistSlot && newDropZone && newDropZone.dataset.slotIndex === originalSetlistSlot.dataset.slotIndex) {
            ...
            currentDropZone = null;
            rafId = null;
            return; // ここで処理を終了し、ドラッグオーバーの状態にしない
        }
