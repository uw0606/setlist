...
let longPressTimer = null; // ★変更: 長押し判定用のタイマー
const LONG_PRESS_DELAY = 500; // 長押しと判定するミリ秒
const DRAG_INIT_THRESHOLD = 10; // ★追加: ドラッグ開始と判定するための微小な移動閾値（長押し中に少し動いてもOKにするため）
...
function handleTouchStart(event) {
    ...
    if (isCheckboxClick) {
        ...
        isDragging = false;
        draggingElement = null;
        // チェックボックスクリック時はデフォルト動作を許可し、ドラッグを阻止
        return;
    }
    ...
    // ダブルタップ検出ロジック (長押しとは別で先に判定)
    if (tapLength < 300 && tapLength > 0) {
        if (longPressTimer) { clearTimeout(longPressTimer); longPressTimer = null; } // 長押しタイマーもキャンセル
        handleDoubleClick(event); // ダブルタップとして処理
        lastTapTime = 0;
        event.preventDefault(); // ダブルクリックが意図された場合はスクロールを防止
        console.log("[touchstart] Double tap detected. Handled by handleDoubleClick.");
        return;
    }
    lastTapTime = currentTime; // 次のタップのために時間を記録

    // 既存の長押しタイマーがあればクリア (複数回タッチ開始イベントが発火する可能性を考慮)
    if (longPressTimer) {
        clearTimeout(longPressTimer);
        longPressTimer = null;
    }
    ...
    longPressTimer = setTimeout(() => {
        // タイマーが完了したら長押しと判定し、ドラッグを開始する
        if (draggingElement && draggingItemId && !isDragging) {
            console.log("[touchstart:Mobile] Long press detected. Initiating drag.");
            createTouchDraggedClone(draggingElement, touchStartX, touchStartY, draggingItemId);
            isDragging = true;
            // 長押しでドラッグが開始されたら、以後のデフォルト動作（スクロール）を抑制
            // ただし、この preventDefault は handleTouchMove の初回で呼ばれるように調整することも可能
            // event.preventDefault(); // ここで呼び出すと、長押し確定前にスクロールが抑制される可能性
        }
    }, LONG_PRESS_DELAY);

    // ドラッグ情報を初期化
    isDragging = false; // まだドラッグは開始していない
    draggingItemId = touchedElement.dataset.itemId;
    draggingElement = touchedElement;

    touchStartX = event.touches[0].clientX;
    touchStartY = event.touches[0].clientY;

    console.log("[touchstart:Mobile] Touch detected. Long press timer started. Waiting for drag initiation or scroll.");
}
...
function handleTouchMove(event) {
    ...
    if (!touch || !draggingElement) { // draggingElement が設定されていない場合は何もしない (touchstart が適切に処理されなかった)
        return;
    }
    ...
    if (longPressTimer) {
        // 長押しタイマーがまだ動いている間に閾値を超えて移動した場合、スクロールの意図とみなし、ドラッグをキャンセル
        // Y方向への移動が X方向より大きい かつ DRAG_THRESHOLD を超えたら、スクロールを許可する
        // ただし、CSSで `touch-action: none;` が設定されている場合、ここではブラウザのスクロールは発生しない。
        // そのため、このブロックは主に `longPressTimer` のクリアとドラッグ開始の抑制が目的になる。
        if (deltaY > DRAG_INIT_THRESHOLD * 2 || deltaX > DRAG_INIT_THRESHOLD * 2) { // スクロール判定をやや厳しく
            clearTimeout(longPressTimer);
            longPressTimer = null;
            isDragging = false; // ドラッグ状態ではない
            console.log("[handleTouchMove] Movement detected before long press, cancelling drag initiation. Allowing scroll (if touch-action permits).");
            // ここでは event.preventDefault() を呼ばないことで、ブラウザのスクロールを許可する
            return;
        }
    }

    // ドラッグが開始されている、または長押しタイマーが完了した（isDraggingがtrueになった）場合
    if (isDragging && currentTouchDraggedClone) {
        // ドラッグ中は常にデフォルトのスクロールをキャンセルし、クローンを指に追従させる
        event.preventDefault();
        ...
        // ドロップゾーンのハイライト処理（既存のロジック）
        // ドロップゾーンの要素を正確に検出するために elementsFromPoint を使用
        const elementsAtPoint = document.elementsFromPoint(currentX, currentY);
        const newActiveTouchSlot = elementsAtPoint.find(el => el.classList.contains('setlist-slot'));

        if (newActiveTouchSlot !== activeTouchSlot) {
            if (activeTouchSlot) {
                activeTouchSlot.classList.remove('drag-over');
            }
            if (newActiveTouchSlot) {
                // 元のスロットの上にドラッグバックされた場合はハイライトしない
                if (originalSetlistSlot && newActiveTouchSlot.dataset.slotIndex === originalSetlistSlot.dataset.slotIndex) {
                    newActiveTouchSlot.classList.remove('drag-over'); // 念のため
                } else {
                    newActiveTouchSlot.classList.add('drag-over');
                }
            }
            activeTouchSlot = newActiveTouchSlot;
            currentDropZone = newActiveTouchSlot; // PCドラッグと共通の変数も更新
        }
    }
}
...
function handleTouchEnd(event) {
    if (longPressTimer) {
        clearTimeout(longPressTimer);
        longPressTimer = null;
        console.log("[touchend] Long press timer cleared. Drag did not initiate via long press.");
    }

    // ドラッグが行われていなかった場合は、ここで処理を終了 (スクロールや短いタップだった)
    if (!isDragging) {
        console.log("[touchend] Not in dragging state. Skipping drop processing.");
        draggingElement = null; // リセット
        return;
    }
...
function finishDragging() {
  ...
  if (longPressTimer) { // ★変更: touchTimeout -> longPressTimer
      clearTimeout(longPressTimer);
      longPressTimer = null;
  }
  if (rafId) {
...
function handleDoubleClick(event) {
  ...
  // ダブルタップ中に長押しタイマーが残っている場合があるのでクリア
  if (longPressTimer) {
      clearTimeout(longPressTimer);
      longPressTimer = null;
  }
