...
async function generateSetlistPdf() {
    ...
    try {
        ...
        if (numSimpleRows > 0) {
            ...
            // simpleYPos は既にヘッダーの下端+5mmの位置にある
            const simpleAvailableHeight = pageHeightSimple - simpleYPos - simpleBottomMargin;
            
            // 曲間優先ロジック: 仮想行数で除算し、最大限に広げる
            const LINE_COUNT_THRESHOLD = 10;
            // 10行以下の場合、実質的な行数を70%とみなして IDEAL_LINE_HEIGHT を増幅
            const VIRTUAL_ROW_DIVISOR = (numSimpleRows > LINE_COUNT_THRESHOLD) ? numSimpleRows : 
                                        numSimpleRows * 0.7; 

            // 1. 理想的な行間を計算（この値を dynamicLineHeight として採用）
            const IDEAL_LINE_HEIGHT = simpleAvailableHeight / VIRTUAL_ROW_DIVISOR;
            
            // 2. 行間を IDEAL_LINE_HEIGHT に設定 (MAX_LINE_HEIGHT_MMは不要だが、念のため300mmを論理的上限とする)
