...
function fillSlotWithItem(slotElement, songData) {
  ...
  const isCurrentlyChecked = songData.short; // Firebaseからロードされた、または移動前のチェック状態

  // ★ 修正点1: songData から isShortVersion プロパティを取得する
  // このプロパティが、その曲が「ショートバージョン」であるという属性情報
  const isSongActuallyShortVersion = songData.isShortVersion || false; // undefined の場合は false
  ...
  // ★ 修正点2: チェックボックスと (Short) ラベルを表示する条件を isSongActuallyShortVersion にする
  if (isSongActuallyShortVersion) { // この曲がショートバージョンであれば常にUI要素を表示
    const checkbox = document.createElement('input');
    checkbox.type = 'checkbox';
    // チェックボックスの初期状態は、songData から受け取ったチェック状態 (isCurrentlyChecked) に基づいて設定
  ...
  // ★ 修正点3: スロットに 'short' クラスを付与するロジックは、引き続き現在のチェック状態に合わせる
