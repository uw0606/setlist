    ...
    if (albumItemElement && !albumItemElement.classList.contains('setlist-slot')) {
        ...
        if (firstEmptySlot) {
            ...
            addSongToSlot(firstEmptySlot, songData.itemId, songData.name, {
                // Short/SE/Drumsoloのオプションの有無を songData から取得し、addSongToSlotが期待するプロパティ名で渡す
                isShortVersion: songData.hasShortOption, 
                hasSeOption: songData.hasSeOption,
                
                // ★★★ 修正箇所：ドラムソロオプションの有無を songData.hasDrumsoloOption から取得して渡す ★★★
                drumsoloOption: songData.hasDrumsoloOption, 
...
function addSongToSlot(slotElement, itemId, songName, options, albumClass) {
    ...
    // ★★★ 修正箇所（オプションの有無を示す属性名を変更）★★★
    // data-has-se-option に倣い、hasDrumsoloOption に変更
    // ただし、optionsオブジェクト内で drumsoloOption という名前を使っているため、ここではその名前を信用する
    // -> 念のため、hasDrumsoloOption に統一し、options.drumsoloOption を参照する形に修正します。
    slotElement.dataset.hasDrumsoloOption = options.drumsoloOption ? 'true' : 'false';
