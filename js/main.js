...
function handleTouchStart(event) {
    ...
    // ここにあった event.preventDefault(); を削除します。
    // スクロールを許可するため、ここではデフォルト動作を阻止しない。
    ...
    if (tapLength < 300 && tapLength > 0) {
        ...
        event.preventDefault(); // ダブルタップの場合は preventDefault でブラウザのズームなどを防ぐ
    ...
    if (event.touches.length === 1) {
        ...
        touchTimeout = setTimeout(() => {
            ...
            if (!isDragging && draggingItemId && document.body.contains(touchedElement)) {
                // ここでクローンを作成し、ドラッグを開始した時点で preventDefault() を呼び出す必要はなくなる
                // 代わりに touchmove で preventDefault() を呼び出すようにする
...
function handleTouchMove(event) {
    // まだドラッグが開始されていない（isDraggingがfalse）場合、かつ
    // touchTimeoutが存在する場合（ロングプレス待機中）は、
    // ここで preventDefault() を呼び出してスクロールを抑制しないようにします。
    // これにより、ロングプレスが確定する前に指を動かしてもスクロールが可能です。
    if (!isDragging) {
        // 短いスライドはスクロールとして処理されるように、ここでは何もしない
        return; 
    }

    // ドラッグが開始された後（isDraggingがtrueになった後）は、
    // 以降の移動でスクロールが起きないように preventDefault() を呼び出します。
    event.preventDefault(); 
