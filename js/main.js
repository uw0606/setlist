...
function handleDrop(event) {
  ...
  if (setlist.children.length < maxSongs) {
    ...
    // ドラッグ元のリストが存在し、かつそれがセットリスト自身ではない場合にのみ削除を行う
    if (originalList && originalList !== setlist) {
      originalList.removeChild(draggingItem);
      originalAlbumMap.delete(draggingItem); // マップからも削除
    }

    // セットリストに追加されたアイテムの originalAlbumMap の情報を削除
...
document.addEventListener("dblclick", (event) => {
  const item = event.target.closest(".item, .setlist-item");
  if (!item) return;
  event.preventDefault();
  event.stopPropagation();
  if (!!event.target.closest("#setlist")) {
    // セットリストからハンバーガーメニューに戻す処理 (修正)
    const songToRemove = item; // 現在ダブルクリックされたセットリストのアイテム
    setlist.removeChild(songToRemove); // セットリストから削除

    // 元のハンバーガーメニューのリストを探す
    const originalList = originalAlbumMap.get(songToRemove);
    if (originalList) {
      // テキストが一致するアイテムを元のリストに追加 (複製)
      const originalItemText = songToRemove.querySelector(".song-info")?.childNodes[0]?.textContent?.trim() || songToRemove.textContent.trim();
      const originalAlbumContent = originalList.closest('.album-content');
      if (originalAlbumContent) {
        const newItem = document.createElement('li');
        newItem.classList.add('item');
        newItem.draggable = true;
        newItem.textContent = originalItemText;
        const albumClass = originalAlbumContent.id.replace('album', 'album');
        newItem.classList.add(albumClass);
        originalList.appendChild(newItem);
        enableDragAndDrop(originalList); // イベントリスナーを再設定
      }
    }
  } else {
    // セットリストの一番下に追加 (既存の処理)
    if (setlist.children.length < maxSongs) {
      ...
      // ハンバーガーメニューからセットリストに追加されたアイテムの originalAlbumMap の情報を削除
...
function shareSetlist() {
  ...
  setlistRef.set(currentState)
      .then(() => {
          ...
          const shareLink = `<span class="math-inline">\{window\.location\.origin\}</span>{window.location.pathname}?shareId=${shareId}`;
