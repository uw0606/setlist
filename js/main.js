...
function handleTouchStart(event) {
    ...
    // ここでは event.preventDefault() をまだ呼ばない
    // ドラッグは touchmove で一定量移動したときに開始する
    ...
    if (event.touches.length === 1) {
        ...
        if (touchTimeout) { // 念のため残っているタイムアウトをクリア
            clearTimeout(touchTimeout);
            touchTimeout = null;
        }
...
function handleTouchMove(event) {
    ...
    if (!isDragging) {
        ...
        // ドラッグ閾値を超えていない場合は、スクロールを許可
        if (deltaX < DRAG_THRESHOLD && deltaY < DRAG_THRESHOLD) {
            return; 
        }

        // ドラッグ閾値を超えた場合
        // ここで、スクロールとドラッグのどちらを優先するかを判断
        // 垂直方向の移動が水平方向の移動より明らかに大きい場合は、スクロールと判断
        if (deltaY > deltaX * 1.5) { // 垂直方向の移動が水平方向の1.5倍以上であればスクロールとみなす
            console.log("[handleTouchMove] Vertical movement dominant. Allowing scroll.");
            return; // スクロールを許可し、preventDefault()を呼ばない
        }

        // ここに到達した場合は、ドラッグを開始すると判断
        if (!draggingItemId) {
            console.log("[handleTouchMove] No draggingItemId. Allowing scroll.");
            return;
        }
        
        const originalElement = setlist.contains(event.target.closest(".setlist-slot.setlist-item")) ? 
                                event.target.closest(".setlist-slot.setlist-item") : 
                                event.target.closest(".item");
        
        if (!originalElement) {
            console.warn("[handleTouchMove] Original element for drag not found after movement. Allowing scroll.");
            return;
        }

        // ドラッグ開始
        createTouchDraggedClone(originalElement, touchStartX, touchStartY, draggingItemId);
        isDragging = true;
        console.log("[handleTouchMove] Drag initiated after movement threshold and direction check.");
        event.preventDefault(); // ドラッグが始まったらスクロールを停止
        return; // 初回ドラッグ開始後は、以降の移動はisDraggingがtrueのブロックで処理
    }
...
function handleTouchEnd(event) {
    if (touchTimeout) { // ダブルタップでキャンセルされた可能性のあるタイムアウトをクリア
        clearTimeout(touchTimeout);
        touchTimeout = null;
    }
