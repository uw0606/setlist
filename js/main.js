...
const DRAG_THRESHOLD = 10; // ドラッグと判断する最小の移動距離（ピクセル）
...
function handleTouchStart(event) {
    ...
    if (tapLength < 300 && tapLength > 0) {
        ...
        // ダブルタップの場合は preventDefault でブラウザのズームなどを防ぐ（ここでは維持）
        event.preventDefault(); 
    ...
    if (event.touches.length === 1) {
        ...
        touchTimeout = setTimeout(() => {
            ...
            if (!isDragging && draggingItemId && document.body.contains(touchedElement)) {
                ...
                // ここで event.preventDefault(); を呼び出すことも可能だが、
                // handleTouchMove での距離チェックと組み合わせるのがより安全。
            } else {
    ...
    rafId = requestAnimationFrame(() => {
        ...
        if (!isDragging) {
            ...
            if (deltaX > DRAG_THRESHOLD || deltaY > DRAG_THRESHOLD) {
                ...
                console.log("[handleTouchMove] Movement beyond threshold, but not yet dragging. Allowing scroll.")
            }
