...
function handleTouchEnd(event) {
    ...
    // 現在のタッチ座標から要素を取得し、セットリストスロットを探す
    currentTouchDraggedClone.style.visibility = 'hidden';
    const elementsAtPoint = document.elementsFromPoint(touch.clientX, touch.clientY);
    currentTouchDraggedClone.style.visibility = 'visible'; // 元に戻す
    dropTargetSlot = elementsAtPoint.find(el => el.classList.contains('setlist-slot'));
    
    // ★ここから修正点: ドロップターゲットが見つからなかった場合のロジックを強化★
    if (!dropTargetSlot) {
        // ドロップされた座標がセットリストの範囲内か確認
        ...
        if (touch.clientX >= setlistRect.left && touch.clientX <= setlistRect.right &&
            ...
            // セットリスト領域内で直接スロットが見つからなかった場合、最も近いスロットを探す
            ...
            setlist.querySelectorAll('.setlist-slot').forEach(slot => {
                ...
                // すでにドラッグ中の要素の元のスロットは除外する（スワップ時に自分自身をターゲットにしないため）
    ...
    if (dropTargetSlot) {
        // ドロップターゲットが見つかった場合は、通常の processDrop を呼び出す
        // draggedElement には currentTouchDraggedClone を渡す
        processDrop(currentTouchDraggedClone, dropTargetSlot, originalSetlistSlot);
    } else {
        // ★ここから修正点: ドロップ失敗時の回復ロジックの再確認★
        console.warn("[touchend] Dropped outside a setlist slot (or closest not found). Reverting changes for original source.");
        if (originalSetlistSlot) {
            // セットリストからドラッグされたが、有効なドロップターゲットが見つからなかった場合
            // 元のスロットに保存しておいたデータを使い、曲を元に戻す
            if (originalSetlistSlot._originalItemData) {
                // `clearSlotContent` でスロットをクリアしつつ、イベントリスナーは維持
                clearSlotContent(setlist, originalSetlistSlot.dataset.slotIndex); 
                // `fillSlotWithItem` で元のデータをスロットに再描画
            ...
            // アルバムからドラッグされたが、有効なドロップターゲットが見つからなかった場合
            // クローンは削除されるので、元のアルバムアイテムを再表示する
            restoreToOriginalList(currentTouchDraggedClone); // currentTouchDraggedCloneはitemIdを持つ
