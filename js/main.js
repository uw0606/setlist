  ...
  if (!!event.target.closest("#setlist")) {
    ...
    document.querySelectorAll('.album-content').forEach(albumContent => {
      const originalSong = albumContent.querySelector(`.item[draggable="true"]`) ?
        Array.from(albumContent.querySelectorAll(`.item[draggable="true"]`)).find(el => el.textContent.trim() === songName) :
        Array.from(albumContent.querySelectorAll(`.item`)).find(el => el.textContent.trim() === songName);
