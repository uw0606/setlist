...
function finishDragging() {
  ...
  // originalSetlistSlot がプレースホルダー状態であれば、一時的なスタイルを解除し、可視性を確実に復元する
  if (originalSetlistSlot) { // originalSetlistSlotが設定されている場合
      originalSetlistSlot.classList.remove('placeholder-slot'); // プレースホルダー状態を解除
      // 重要：元のスロットがsetlist-itemであるか否かに関わらず、visibilityを確実に復元する
      // （アイテムが移動して空になった場合も考慮）
  ...
  draggingItemId = null; 
  isDragging = false;
  originalSetlistSlot = null; // リセットを確実にする
