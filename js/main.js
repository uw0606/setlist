...
function handleDoubleClick(event) {
    ...
    if (albumItemElement) {
        ...
        if (firstEmptySlot) {
            const songData = getSlotItemData(albumItemElement); // ここで allowMultiple も取得される
            if (!songData) {
            ...
            addSongToSlot(firstEmptySlot, songData.itemId, songData.name, {
                ...
                seChecked: false,
                drumsoloChecked: false,
                // ★追加: allowMultiple もoptionsに含める
                allowMultiple: songData.allowMultiple 
            }, songData.albumClass);
            showMessage("セットリストに曲を追加しました。", "success");
            hideSetlistItemsInMenu(); 
        } else {
    ...
    if (setlistItemElement) {
        // ★修正: allowMultiple の曲はダブルクリックで削除しない
        if (setlistItemElement.dataset.allowMultiple === 'true') {
            console.log("[handleDoubleClick] Double-clicked a 'free input' song. Not removing.");
            showMessage("この曲はセットリストから自動削除されません。", "info");
            return;
        }
...
function updateSlotContent(slotElement, songName, options) {
    ...
    if (slotElement.dataset.allowMultiple === 'true') {
        songNameSpan.contentEditable = true; // spanを直接編集可能にする
        songNameSpan.classList.add('editable-song-name'); // 編集可能なスタイルを追加することも可能

        // フォーカスが外れたときにdata-song-nameを更新
        songNameSpan.addEventListener('blur', () => {
            const newName = songNameSpan.textContent.trim();
            if (newName === "") { // 空の場合はデフォルト名に戻す
...
function addSongToSlot(slotElement, itemId, songName, options, albumClass) {
    ...
    const originalDraggedElement = currentPcDraggedElement || currentTouchDraggedClone || currentTouchDraggedOriginalElement;
