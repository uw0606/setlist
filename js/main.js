...
async function generateSetlistPdf() {
    ...
    // let currentItemNoShareable = 1; // 共有テキストロジックがないためコメントアウト

    // let shareableTextContent = ''; // 共有テキストロジックがないためコメントアウト
    // if (headerText) {
    ...
    try {
        ...
        // ページサイズ情報
        const pageHeightSimple = (new jsPDF('p', 'mm', 'a4')).internal.pageSize.getHeight(); 
        const pageWidthSimple = (new jsPDF('p', 'mm', 'a4')).internal.pageSize.getWidth();
        ...
        let calculatedHeaderFontSize = 30; // ヘッダーフォントサイズを計算用に初期化
        ...
        let isSimplePdfGeneration = true; // 計算ロジック内でのフラグ

        // --- フォントサイズと行間の計算ロジック（共通化） ---
        if (headerText) {
        ...
        let currentYPos = simpleTopMargin;

        // ヘッダーの描画
        if (headerText) {
            simplePdf.setFont('NotoSansJP', 'bold');
            simplePdf.setFontSize(calculatedHeaderFontSize); 
            simplePdf.setTextColor(0, 0, 0); // 黒
            simplePdf.text(headerText, simpleLeftMargin, currentYPos);
            currentYPos += (calculatedHeaderFontSize * 0.38) + SPACE_BETWEEN_HEADER_AND_BODY; 
        } 
        ...
        if (numSimpleRows > 0) {
            simplePdfBody.forEach(row => {
                ...
                simplePdf.text(prefix, simpleLeftMargin, currentYPos);
                ...
                simplePdf.text(content, contentXPos, currentYPos);
                
                currentYPos += dynamicLineHeight; 
            });
        }
