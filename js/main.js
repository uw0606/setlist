...
function handleTouchStart(event) {
    ...
    if (isCheckboxClick) {
        ...
        // チェックボックスクリックの場合、既存のドラッグ関連のIDをクリア
        draggingItemId = null;
        originalSetlistSlot = null;
        return;
    }
    ...
    if (event.touches.length === 1) {
        ...
        console.log("[touchstart:Mobile] Multiple touches detected. Resetting drag state.");
        isDragging = false;
        draggingItemId = null;
        originalSetlistSlot = null;
        if (touchTimeout) {
...
function handleTouchMove(event) {
    ...
    if (!isDragging) {
        ...
        if (distance > DRAG_THRESHOLD) {
            // ドラッグ開始時に改めて touchedElement を取得するのではなく、
            // handleTouchStart で設定された draggingItemId が有効かを確認する
            if (!draggingItemId) {
            ...
            // draggingItemId が有効であれば、元の要素を見つける試みを行う
            // アルバムアイテムかセットリストスロットかによって異なる
            ...
            if (!elementToClone) {
                 console.warn("[handleTouchMove] Drag threshold met, but elementToClone (original item) not found. Aborting drag start.");
                 return;
            }
