...
let database; // <-- 修正: この行は残しておきますが、重複宣言の原因となる場合は削除またはコメントアウトしてください。
              // ただし、もし `firebase-config.js` でグローバルに `database` を宣言していない場合は必要です。
              // 一般的には、DOMや他のライブラリがロードされてから初期化する方が安全です。
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Firebaseが利用可能になったら初期化処理を続ける
    // script.jsの先頭で`database`変数を定義し、`firebase-config.js`が読み込まれた後に`firebase_ready`イベントをディスパッチすることで連携
    if (typeof firebase !== 'undefined' && firebase.database) {
        // 修正: ここでの`let database;`の宣言は不要です。
        // グローバルスコープの`database`変数に代入します。
        database = firebase.database(); 
        ...
        console.warn("Firebase is not fully loaded. Waiting for 'firebase_ready' event.");
    }
    ...
    // スロットの初期生成はinitializeSetlistSlotsで行われる
    // enableDragAndDropはfillSlotWithItemとclearSlotContent内でも呼ばれる
    ...
    // 共有IDがあればセットリストをロード
    // Firebase初期化後に実行されるようにする
    if (database) { // databaseがDOMContentLoaded時点で利用可能であれば即実行
    ...
    } else { // databaseがまだ利用可能でなければ、firebase_readyイベントを購読
        document.addEventListener('firebase_ready', () => {
            loadSetlistState().then(() => {
                console.log("[DOMContentLoaded] loadSetlistState (after firebase_ready) finished. Performing final cleanup.");
                hideSetlistItemsInMenu();
            }).catch(error => {
                console.error("[DOMContentLoaded] Error during loadSetlistState (after firebase_ready):", error);
                hideSetlistItemsInMenu();
            });
        }, { once: true });
    }
