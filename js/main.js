...
function handleTouchStart(event) {
    ...
    if (!touchedElement) {
        console.warn("[touchstart:Mobile] No draggable item found on touch start. Allowing default behavior.");
        // ドラッグ可能な要素でなければ、スクロールを妨げない
        return;
    }

    // ★重要: ここにあった event.preventDefault() を完全に削除。
    // スクロールを許可するため、ドラッグ開始が確定するまではpreventDefaultを呼ばない。
    ...
    if (tapLength < 300 && tapLength > 0) {
        ...
        // ダブルタップ時はブラウザのデフォルト動作を防ぐ
        event.preventDefault(); 
        console.log("[touchstart] Double tap detected. Handled by handleDoubleClick. Default prevented.");
        return;
    }
    ...
    if (event.touches.length === 1) {
        ...
        touchTimeout = setTimeout(() => {
            ...
            if (!isDragging && draggingItemId && document.body.contains(touchedElement)) {
                // ★重要★ ドラッグ開始時にのみ event.preventDefault() を呼び出す
                // これにより、ロングプレスが確定してドラッグが始まるまではスクロールが許可される
                event.preventDefault(); 
...
function handleTouchMove(event) {
    ...
    event.preventDefault(); 
    console.log("[handleTouchMove] Dragging active, preventing default.");
...
function handleTouchEnd(event) {
    // isDragging が true でない場合は、ドラッグ操作が行われていないので、
    // touchTimeout が残っていてもそれは単なるタップ（またはロングプレス途中での指離し）と見なす
    if (!isDragging) {
        if (touchTimeout) {
            clearTimeout(touchTimeout);
            touchTimeout = null;
            console.log("[touchend] Touch was a tap/short press (timeout active). Clearing timeout and returning.");
            // 短いタップの場合は、デフォルト動作（例えばクリックイベントの発生）を妨げない
            return;
        }
