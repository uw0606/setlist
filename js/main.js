...
function getCurrentState() {
  const setlistState = Array.from(setlist.children).map(item => {
      ...
      const albumClass = Array.from(item.classList).find(className => className.startsWith('album')); // ★ 所属アルバムのクラスを取得
      return { name: songName, short: isShort, albumClass: albumClass }; // ★ albumClass を追加
  });
          ...
          .then((snapshot) => {
              ...
              if (state) {
                  ...
                  state.setlist.forEach(itemData => {
                      ...
                      const songInfo = document.createElement('div');
                      songInfo.classList.add('song-info');
                      songInfo.textContent = itemData.name;
                      if (itemData.short) {
                          const checkbox = document.createElement('input');
                          checkbox.type = 'checkbox';
                      ...
                      li.draggable = true; // ★ ドラッグ可能属性を設定
                      if (itemData.albumClass) {
                          li.classList.add(itemData.albumClass); // ★ アルバムのクラスを追加
                      }
                      setlist.appendChild(li);
                  });
                  enableDragAndDrop(setlist); // ★ ドラッグ＆ドロップを再度有効化
                    ...
                    // アルバムリストの内容を復元（必要であれば）
                    if (state.albums) {
                        for (const albumId in state.albums) {
                            const albumContent = document.getElementById(albumId);
                            if (albumContent) {
                                albumContent.innerHTML = state.albums[albumId].map(itemName => `<li class="item ${albumId}" draggable="true">${itemName}</li>`).join('');
