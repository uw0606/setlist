...
function handleTouchStart(event) {
    ...
    // ⭐︎ ここに条件を追加します ⭐︎
    // ドラッグ可能な要素（セットリストの曲またはアルバムの曲）がタッチされた場合のみpreventDefaultを呼び出す
    if (touchedElement) {
        event.preventDefault(); // これにより、スクロール可能な要素上でドラッグが意図される場合にのみスクロールを抑制する
    } else {
        // ドラッグ対象ではない（例: ハンバーガーメニューの空白、スクロールバー、空のスロットの背景など）
        // この場合はスクロールを妨げないよう、ここで処理を終了し、preventDefaultを呼ばない
        console.log("[touchstart:Mobile] Non-draggable element touched. Allowing default behavior.");
        return;
    }
    ...
    if (tapLength < 300 && tapLength > 0) {
        ...
        handleDoubleClick(event); // ダブルタップでpreventDefaultは内部で行われる
        lastTapTime = 0;
    ...
    const initializeSetlistItem = (itemElement) => {
        ...
        itemElement.addEventListener("dblclick", (e) => {
            ...
            if (!e.target.closest('.drag-handle')) {
                ...
                restoreToOriginalList(itemElement); // PCでのダブルクリックも restoreToOriginalList に変更
                e.stopPropagation(); 
                e.preventDefault(); 
            }
        }); 
        ...
        // スマートフォンでのダブルタップ検出ロジックを改善 (isDraggingTouch と currentDraggedItem の確認)
        itemElement.addEventListener('touchstart', (e) => {
            if (e.touches.length === 1) { 
                ...
                // スクロールと競合しないよう、ここでは preventDefault を削除。
                // handleTouchStart のメインロジックで制御する。
                if (currentTime - lastTapTime < DOUBLE_TAP_DELAY) {
                   // e.preventDefault(); // ここは削除
        ...
        itemElement.addEventListener('touchend', (e) => {
            ...
            if (!isDraggingTouch && !currentDraggedItem && (currentTime - lastTapTime < DOUBLE_TAP_DELAY && currentTime - lastTapTime > 0)) {
                ...
                if (!e.target.closest('.drag-handle')) {
                    console.log(`[touchend] Confirmed double tap for item: ${itemElement.dataset.itemId}. Restoring item to album.`);
                    restoreToOriginalList(itemElement); // ダブルタップでアルバムに戻す
                    e.stopPropagation(); 
                    e.preventDefault(); 
                    finishDragging(); // 処理後、グローバル状態をリセット
                } else {
...
function handleTouchStart(event) {
    ...
    if (event.touches.length === 1) {
        // touchedElement はすでに上で定義・チェック済み
        if (!touchedElement) { // 念のため再チェック（実際には上での return でここには来ないはずだが、冗長性のため残す）
            console.warn("[touchstart:Mobile] No draggable item found on touch start after initial check.");
            return;
        }
        ...
        touchTimeout = setTimeout(() => {
            ...
            // ここにあった event.preventDefault(); は削除済み
            if (!isDragging && draggingItemId && document.body.contains(touchedElement)) {
                createTouchDraggedClone(touchedElement, touchStartX, touchStartY, draggingItemId);
                isDragging = true;
            } else {
...
// finishDragging 関数も修正 (前回の指示で既に修正済みですが、念のため完全版を再掲)
/**
 * ドラッグ終了時のクリーンアップ
 */
function finishDragging() {
