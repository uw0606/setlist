...
function handleTouchStart(event) {
    ...
    if (event.touches.length === 1) {
        // ★修正点1: touchedElement の特定をより厳密にする★
        // タッチされた要素が '.item' (アルバムの曲) または '.setlist-item' (セットリストの曲) の親要素を探す
        // その上で、それが実際にドラッグ可能なアイテムコンテンツであることを確認
...
// handleTouchEnd 関数の修正（変更なしですが、再確認のためにここに含めます）
function handleTouchEnd(event) {
    ...
    if (dropTargetSlot) {
        ...
        // ★ここをもう一度確認。このelseブロックに確実に入っているか？★
