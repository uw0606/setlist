...
function changeAlbumView() {
    ...
    if (mode === 'name_order') {
        // ★修正点1: アルバムタブ全体を非表示にする
        if (albumList) albumList.classList.add('hidden');
        
        // ★修正点2: アルバムコンテンツの親を非表示
...
function buildNameOrderList() {
    ...
    const nameOrderNav = document.getElementById('nameOrderNav'); // ナビゲーション要素の取得
    
    if (!nameOrderContent || !nameOrderNav) return; 

    // 既にリストが構築されていればスキップ（または再構築）
    if (allSongData.length === 0) {
        
        // --- 1. すべての曲データを収集 ---
        document.querySelectorAll('.album-content-list .item').forEach(item => {
            // ... (曲データ収集ロジックは省略) ...
            // （元のコードブロックと同じ内容をここにコピーしてください）
            // ----------------------------------------------------
        ...
        allSongData.sort((a, b) => {
            return a.songName.localeCompare(b.songName, 'ja', { sensitivity: 'base' });
        });
    ...
    // --- 4. コンテンツの構築 ---
    // ※ナビゲーション生成ロジックは削除/スキップ

    // コンテンツを描画
    clearAndDrawNameOrderList();
    
    // 初期状態で最初のグループを開く（HTMLで静的に定義された最初のタブを使用）
    const firstNav = nameOrderNav.querySelector('.name-order-nav-item');
    if (firstNav) {
        // 最初のタブをクリックした状態を再現
        toggleNameOrderGroup(firstNav, firstNav.dataset.group);
    } else {
        console.warn("[buildNameOrderList] No navigation tabs were found in HTML.");
    }
...
/**
 * グループ化されたデータを元に、文字順リストのHTMLを描画する。
 *
 * (注意: この関数は buildNameOrderList 内で実行され、
 * グローバル変数 allSongData と nameOrderGroups に依存します)
 */
function clearAndDrawNameOrderList() {
    console.log("[clearAndDrawNameOrderList] Starting content drawing.");
    const nameOrderContent = document.getElementById('nameOrderContent');
    if (!nameOrderContent) return;
    ...
    for (const groupKey in nameOrderGroups) {
        const groupSongs = nameOrderGroups[groupKey];
        // 曲が存在しないグループのセクションも、タブ切り替えのターゲットとして作成します。

        const groupDiv = document.createElement('div');
        groupDiv.id = `name-group-${groupKey}`;
        // 初期状態では hidden クラスを付けて非表示にする (toggleNameOrderGroupで表示が切り替わる)
        groupDiv.classList.add('name-order-group-content', 'hidden'); 
        ...
        if (groupSongs.length > 0) {
            // 曲がある場合：リストアイテムを作成
            groupSongs.forEach(songData => {
                // 曲データオブジェクトからアイテム要素を作成 (createAlbumItemElementは既存関数に依存)
                const itemElement = createAlbumItemElement(songData); 
                
                // Drag and Drop イベントを有効化 (既存の関数に依存)
            ...
            emptyMessage.classList.add('name-order-empty-message');
            emptyMessage.textContent = 'このグループには曲がありません。';
            ul.appendChild(emptyMessage);
        }

        groupDiv.appendChild(ul);
        nameOrderContent.appendChild(groupDiv);
    }
    
    // 初期表示時に、メニュー内のアイテムの重複を隠す
    if (typeof hideSetlistItemsInMenu === 'function') {
        hideSetlistItemsInMenu(); 
    }
    console.log("[clearAndDrawNameOrderList] Content drawing completed.");
}


/**
 * 曲名の頭文字に基づいてグループに分類する。
 * (日本語の分類は簡易的なものに留めます)
 * @param {Array<Object>} songs - 曲データオブジェクトの配列。
...
function groupSongs(songs) {
    ...
    // 日本語の頭文字変換のための簡易マッピング (実際のソートロジックは localeCompare に依存)
    const japaneseMap = {
        ...
        // ひらがなをカタカナに変換してマッチさせるのが理想だが、ここでは簡単な対応
        // 実際には、日本語の曲名は 'AIUEO' にほとんど入る可能性があるため、この分類は概算です
    };
    
    songs.forEach(song => {
        const firstChar = song.songName.trim().charAt(0).toUpperCase();

        if (/[A-E]/.test(firstChar)) groups['ABCDE'].push(song);
        else if (/[F-J]/.test(firstChar)) groups['FGHIJ'].push(song);
        else if (/[K-O]/.test(firstChar)) groups['KLMNO'].push(song);
        else if (/[P-T]/.test(firstChar)) groups['PQRST'].push(song);
        else if (/[U-Z]/.test(firstChar)) groups['UVWXYZ'].push(song);
        else if (/[0-9\W]/.test(firstChar)) groups['ETC_NONE'].push(song); // 数字、記号など
        else {
            // 日本語の分類 (ここではカタカナのみをサポートする簡易版)
            const jpGroup = japaneseMap[firstChar];
            if (jpGroup) {
                groups[jpGroup].push(song);
            } else {
                // その他の日本語（漢字や特殊なひらがな）はまとめて AIUEO に
                groups['AIUEO'].push(song); 
            }
        }
    });
...
/**
 * 曲データオブジェクトから、アルバムリストで表示するアイテム要素を作成する。
 * (既存のアルバムリストアイテムと全く同じUIを生成する必要があります)
 * @param {Object} songData - 曲のメタデータ。
 * @returns {HTMLElement} 新しく作成された <li> 要素。
 */
