...
let sortOrderSelect = null; 
let albumViewContainer = null;
let nameOrderContainer = null;
let nameOrderNav = null;
...
/**
 * セットリストに既にある曲を、アルバム/文字順メニュー内で非表示にする。
 * この関数は、セットリストの変更時とメニュー表示切り替え時に呼び出される。
 */
function hideSetlistItemsInMenu() {
    console.log("[hideSetlistItemsInMenu] Updating menu visibility.");
    
    // 1. セットリスト内の曲名（キー）を収集
    const setlistSongNames = new Set();
    document.querySelectorAll('#setlist .setlist-item').forEach(slot => {
        const item = slot.querySelector('.item');
        if (item && item.dataset.songName) {
            setlistSongNames.add(item.dataset.songName.trim());
        }
    });

    // 2. メニュー内のすべての曲アイテムを対象にする
    // アルバムビュー (.album-content .item) と文字順ビュー (.name-order-group-content .item) の両方を含める
    const allMenuItems = document.querySelectorAll(
        '.album-content .item, .name-order-group-content .item'
    );
    
    // 3. 各アイテムの表示/非表示を切り替える
    allMenuItems.forEach(item => {
        const songName = item.dataset.songName ? item.dataset.songName.trim() : null;
        
        if (songName && setlistSongNames.has(songName)) {
            // セットリストに存在する場合、非表示
            item.classList.add('hidden-in-menu');
            
            // 非表示にする際は、D&Dの操作を受け付けないようにすることも検討 (ここではCSSで対応が一般的)
            // item.draggable = false;
        } else {
            // セットリストに存在しない場合、表示
            item.classList.remove('hidden-in-menu');
            // item.draggable = true;
        }
    });
    
    // 💡 注意: 
    // このロジックは、アルバムビューと文字順ビューの両方で、
    // **曲名 (`data-song-name`)** が一致するアイテムを非表示にします。
}
...
async function generateSetlistPdf() {
    ...
    try {
        ...
        // ★★★ 1. 生成日時の取得と整形（HH:mm を使用） ★★★
        const now = new Date();
        const dateOptions = { year: 'numeric', month: '2-digit', day: '2-digit' };
        const timeOptions = { hour: '2-digit', minute: '2-digit', hour12: false };
        const formattedDate = now.toLocaleDateString('ja-JP', dateOptions).replace(/\//g, '/').slice(0, -1); // YYYY/MM/DD
        
        // ★修正済み：時と分を取得し、分を削除する処理を削除
        const formattedTimeOnly = now.toLocaleTimeString('ja-JP', timeOptions); // HH:mm の形式が維持されます
        
        const generationTime = `${formattedDate} ${formattedTimeOnly}`;
        ...
        const topMargin = 20;
        const leftMargin = 10;
        const rightMargin = 10; // ★追加：時間表示のために右マージンを定義
        ...
        detailedPdf.setFontSize(timeFontSize);
        detailedPdf.setFont('NotoSansJP', 'normal');
        detailedPdf.setTextColor(100, 100, 100); 
        
        const timeXPos = pageWidth - rightMargin;
        const timeYPos = topMargin - 5; 
        detailedPdf.text(`生成日時: ${generationTime}`, timeXPos, timeYPos, { align: 'right' });
        detailedPdf.setTextColor(0, 0, 0); // 色を元に戻す
        ...
        const footerHeight = 10;                     // フッターの高さ (実際のマージン)
        const IDEAL_BOTTOM_GAP = 20;                 // 少ない曲の時に確保したい空き
        const MIN_REQUIRED_BOTTOM_MARGIN = 12;       // 曲が多い時に描画エリア計算に使う最小マージン
        ...
        let calculatedHeaderFontSize = 30; // 追加: ヘッダーフォントサイズを格納する変数

        let simpleYPos = simpleTopMargin;

        // ヘッダーの描画と高さの計算 (タイトル最大化ロジックを修正してフォントサイズを保持)
        let simpleHeaderHeight = 0;
        if (headerText) {
            calculatedHeaderFontSize = 30; // 30を初期値として使用
        ...
        if (numSimpleRows > 0) {
            ...
            if (numSimpleRows >= MARGIN_REDUCTION_THRESHOLD) {
                // 曲が多い場合: 描画エリアをページ下端に近づける (フッターに必要な最小マージンを使用)
                currentCalcBottomMargin = MIN_REQUIRED_BOTTOM_MARGIN;
            } else {
                // 曲が少ない場合: 確保したい空き (20mm) + フッター位置 (10mm) = 30mm を確保
        ...
        // ★ フッターの描画ロジックの修正 ★
        // ===================================
        const footerText = ""; // ★修正: フッターテキストを復元
        const footerFontSize = 10; 
        const footerBottomMargin = 10; // ★修正: ページ下端からの適切なマージン(mm)に修正
        ...
        // --- 3. シンプルなセットリストPDFの色反転版の生成 (追加ブロック) ---

        // シンプルPDFで計算された dynamicFontSize, dynamicLineHeight, calculatedHeaderFontSize を再利用
        ...
        console.log("[generateSetlistPdf] Inverse Simple PDF generated and downloaded:", inverseFilename);
...
/**
 * ソート選択に応じてアルバム順と文字順の表示を切り替える。
 */
function changeAlbumView() {
    if (!sortOrderSelect || !albumViewContainer || !nameOrderContainer) {
        console.error("[changeAlbumView] Required UI elements are missing.");
        return;
    }
    ...
    if (selectedValue === 'album') {
        ...
        // 既存のアルバム順の最初のアルバムを開く（必要に応じて）
        // 例: toggleAlbum(1); 
        
        console.log("[changeAlbumView] Switched to Album View.");
        
    } else if (selectedValue === 'name_order') {
        // 文字順を表示
        albumViewContainer.classList.add('hidden');
        nameOrderContainer.classList.remove('hidden');
        
        // 💡 文字順ビューに切り替えたら、データをグループ化して描画するロジックを呼び出す
        if (!nameOrderContainer.dataset.initialized) {
            initializeNameOrderView(); 
            nameOrderContainer.dataset.initialized = 'true';
        }
        
        // ★修正点: initializeNameOrderView内で既に最初のグループを表示しているので、
        // 初期化済みであれば、既存の選択状態を維持し、DOM操作を減らします。
        // 未初期化の場合は initializeNameOrderView() の中で最初のグループが開かれます。
        
        // 初期化済みの場合、現在の選択を再アクティブ化
        const currentActiveNav = document.querySelector('#nameOrderNav .name-order-nav-item.active');
        if (currentActiveNav && nameOrderContainer.dataset.initialized === 'true') {
             // アクティブなタブがあれば、それを再表示する（DOMの表示/非表示はCSSで制御）
             const currentGroupKey = currentActiveNav.dataset.group;
             toggleNameOrderGroup(currentActiveNav, currentGroupKey);
        } else {
             // まだアクティブなタブがない場合や初期化直後の場合、最初のグループを開く
             const firstNavItem = document.querySelector('#nameOrderNav .name-order-nav-item');
             if (firstNavItem) {
                 toggleNameOrderGroup(firstNavItem, firstNavItem.dataset.group);
             }
        }
        
        console.log("[changeAlbumView] Switched to Name Order View. Initializing content...");
    }
}

/**
 * 文字順ビューで特定のグループのコンテンツを表示する。
 * @param {HTMLElement} clickedElement - クリックされた LI 要素
 * @param {string} groupKey - グループキー (例: 'ABCDE', 'AIUEO')
 */
function toggleNameOrderGroup(clickedElement, groupKey) {
    const contentContainer = document.getElementById('nameOrderContent');
    
    // 既存のアクティブなナビゲーションを解除
    document.querySelectorAll('#nameOrderNav .name-order-nav-item').forEach(item => {
        item.classList.remove('active');
    });
    
    // クリックされたナビゲーションをアクティブにする
    clickedElement.classList.add('active');
    
    // 全てのグループコンテンツを非表示にする
    document.querySelectorAll('#nameOrderContent .name-order-group-content').forEach(content => {
        content.classList.add('hidden');
    });

    // 選択されたグループのコンテンツを表示する
    const targetContent = document.getElementById(`group-content-${groupKey}`);
    if (targetContent) {
        targetContent.classList.remove('hidden');
        console.log(`[toggleNameOrderGroup] Displaying group: ${groupKey}`);
    } else {
        console.warn(`[toggleNameOrderGroup] Content for group ${groupKey} not found.`);
    }
    
    // 💡 どちらに切り替えても、非表示状態を同期させる関数をここで実行するのが理想です
    // hideSetlistItemsInMenu(); 
}
...
/**
 * 曲名から読み仮名を取得するダミー関数。
 * 実際には、data属性から取得するか、外部ライブラリ（Furiganaなど）を使用します。
 * @param {string} songName - 曲のタイトル
 * @returns {string} 読み仮名（カタカナ）
 */
function getReading(songName) {
    // 💡 注意: 実際には、この曲名と読み仮名のマッピングをデータとして持つ必要があります。
    // ここでは、データセットの 'data-reading' 属性から取得するか、なければ空を返します。
    // DOMから取得するため、この関数はDOM要素を引数に取る方が実用的かもしれません。
    // 今回はデモとして、曲アイテムのDOM要素から直接データを取得する形で進めます。
    return ""; // 呼び出し元で処理します
}

// -----------------------------------------------------------------------------
// --- ビュー切り替えロジックの定義 (initializeNameOrderView の追加) ---
// -----------------------------------------------------------------------------

/**
 * 文字順ビューを初期化し、ナビゲーションと曲データを生成する。
 */
function initializeNameOrderView() {
    console.log("[initializeNameOrderView] Starting initialization...");

    if (!albumList) {
        console.error("Error: .album-list element not found.");
        return;
    }

    // 1. 全曲アイテムを収集し、読み仮名でソートする
    const allItems = Array.from(albumList.querySelectorAll(".item"));
    
    // 💡 読み仮名を取得するロジック（ここでは data-reading 属性を想定）
    const sortedItems = allItems.map(item => {
        // アルバムリストのアイテムは data-reading を持つことを前提とする
        const reading = item.dataset.reading || item.dataset.songName || "";
        return {
            element: item,
            reading: reading.toUpperCase() // 大文字に統一してソート・グループ化しやすくする
        };
    }).sort((a, b) => a.reading.localeCompare(b.reading, 'ja')); // 日本語ロケールでソート

    // 2. グループ化の定義
    // ここでは、アイウエオ順とABC順のナビゲーションを同時に生成します。
    const groups = {
        'AIUEO': [], 'KAKIKUKEO': [], 'SASISHISO': [], 'TATITUTETO': [], 'NANINUNENO': [], 
        'HAHIHUHEHO': [], 'MAMIMUMEMO': [], 'YAYUYO': [], 'RARIRURERO': [], 'WAWOUN': [], 
        'ETC_JP': [], // 記号や数字など
        
        'ABCDE': [], 'FGHIJ': [], 'KLMNO': [], 'PQRST': [], 'UVWXYZ': [], 'ETC_EN': []
    };

    // 3. アイテムをグループに分類する
    sortedItems.forEach(data => {
        const firstChar = data.reading.charAt(0);
        
        if (!firstChar) {
            groups.ETC_JP.push(data.element);
            return;
        }
        ...
        // --- 英語/記号 ---
        if ((charCode >= 65 && charCode <= 90) || (charCode >= 48 && charCode <= 57)) { // A-Z または 0-9
            const char = firstChar.toUpperCase();
            if (char >= 'A' && char <= 'E') groups.ABCDE.push(data.element);
            else if (char >= 'F' && char <= 'J') groups.FGHIJ.push(data.element);
            else if (char >= 'K' && char <= 'O') groups.KLMNO.push(data.element);
            else if (char >= 'P' && char <= 'T') groups.PQRST.push(data.element);
            else if (char >= 'U' && char <= 'Z') groups.UVWXYZ.push(data.element);
            else groups.ETC_EN.push(data.element);

        // --- 日本語 ---
        } else {
            const jpChar = firstChar;
            if ('アイウエオ'.includes(jpChar)) groups.AIUEO.push(data.element);
            else if ('カキクケコガギグゲゴ'.includes(jpChar)) groups.KAKIKUKEO.push(data.element);
            else if ('サシスセソザジズゼゾ'.includes(jpChar)) groups.SASISHISO.push(data.element);
            else if ('タチツテトダヂヅデド'.includes(jpChar)) groups.TATITUTETO.push(data.element);
            else if ('ナニヌネノ'.includes(jpChar)) groups.NANINUNENO.push(data.element);
            else if ('ハヒフヘホバビブベボパピプペポ'.includes(jpChar)) groups.HAHIHUHEHO.push(data.element);
            else if ('マミムメモ'.includes(jpChar)) groups.MAMIMUMEMO.push(data.element);
            else if ('ヤユヨ'.includes(jpChar)) groups.YAYUYO.push(data.element);
            else if ('ラリルレロ'.includes(jpChar)) groups.RARIRURERO.push(data.element);
            else if ('ワヲン'.includes(jpChar)) groups.WAWOUN.push(data.element);
            else groups.ETC_JP.push(data.element);
        }
    });
    
    // 4. ナビゲーションタブの生成と描画
    const navUl = nameOrderNav; // グローバル変数の #nameOrderNav (ul.album-list)
    navUl.innerHTML = '';
    navUl.classList.add('name-order-nav'); // 必要に応じてクラスを追加

    const navItems = [
        { key: 'AIUEO', label: 'ア・イ・ウ・エ・オ' },
        { key: 'KAKIKUKEO', label: 'カ行' },
        { key: 'SASISHISO', label: 'サ行' },
        { key: 'TATITUTETO', label: 'タ行' },
        { key: 'NANINUNENO', label: 'ナ行' },
        { key: 'HAHIHUHEHO', label: 'ハ行' },
        { key: 'MAMIMUMEMO', label: 'マ行' },
        { key: 'YAYUYO', label: 'ヤ・ユ・ヨ' },
        { key: 'RARIRURERO', label: 'ラ・リ・ル・レ・ロ' },
        { key: 'WAWOUN', label: 'ワ・ヲ・ン' },
        { key: 'ETC_JP', label: '記号・数字' },
        { key: 'ABCDE', label: 'A・B・C・D・E' },
        { key: 'FGHIJ', label: 'F・G・H・I・J' },
        { key: 'KLMNO', label: 'K・L・M・N・O' },
        { key: 'PQRST', label: 'P・Q・R・S・T' },
        { key: 'UVWXYZ', label: 'U・V・W・X・Y・Z' },
        { key: 'ETC_EN', label: 'その他英数' }
    ];

    navItems.forEach(nav => {
        // グループにアイテムが存在しない場合はスキップ
        if (groups[nav.key].length === 0) return;
        
        const li = document.createElement('li');
        li.textContent = nav.label;
        li.dataset.group = nav.key;
        li.classList.add('name-order-nav-item');
        
        // ナビゲーションクリック時のイベントリスナーを設定
        li.addEventListener('click', () => {
            // toggleNameOrderGroup(clickedElement, groupKey) を呼び出す
            toggleNameOrderGroup(li, nav.key);
        });
    ...
    // 5. コンテンツコンテナの準備
    // コンテンツ全体を保持する ul.album-content のようなラッパー
    let contentWrapper = document.getElementById('nameOrderContent');
    if (!contentWrapper) {
        contentWrapper = document.createElement('ul');
        contentWrapper.id = 'nameOrderContent';
        contentWrapper.classList.add('name-order-content');
        // 初期化時に一度だけ nameOrderContainer に追加
        nameOrderContainer.appendChild(contentWrapper); 
    }
    contentWrapper.innerHTML = ''; // コンテンツをクリア

    // 6. グループ化された曲リストを描画（初期はすべて非表示のコンテンツとしてDOMに保持）
    for (const key in groups) {
        if (groups[key].length === 0) continue;

        // ★修正: グループコンテンツ自体を <ul> 要素として作成
        const groupContent = document.createElement('ul');
        groupContent.id = `group-content-${key}`;
        // CSSが .name-order-content の代わりにこれを対象とするようにするか、
        // 既存のCSSと連携させるために .name-order-content クラスも付与します。
        groupContent.classList.add('name-order-group-content', 'name-order-content', 'hidden'); 
        
        groups[key].forEach(originalItem => {
            // ★ポイント: アルバムリストの既存のアイテム（li要素）をクローンして使用する
            const clonedItem = originalItem.cloneNode(true);
            
            enableDragAndDrop(clonedItem); 
            
            clonedItem.classList.add('name-order-item'); 
            
            // ★修正: ul要素の子としてli要素（clonedItem）を追加
            groupContent.appendChild(clonedItem);
        });
        
        // ★修正: nameOrderContainer (親のDIV) に直接追加
        nameOrderContainer.appendChild(groupContent);
    }
    
    console.log("[initializeNameOrderView] Initialization complete. Total groups:", Object.keys(groups).length);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    sortOrderSelect = document.getElementById('sortOrder'); // HTMLのIDに合わせて修正
    albumViewContainer = document.getElementById('albumViewContainer');
    nameOrderContainer = document.getElementById('nameOrderContainer');
    nameOrderNav = document.getElementById('nameOrderNav');
    ...
    loadSetlistState().then(() => {
        ...
        document.querySelectorAll('.setlist-slot').forEach(slot => {
            ...
            // CSSで対応済み、かつ pan-y が自動スクロールを妨害する可能性があるため
