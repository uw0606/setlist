...
function handleDrop(event) {
  ...
  if (setlist.children.length < maxSongs) {
      ...
      if (songInfo) {
          songInfo.style.paddingLeft = "30px";
          songInfo.style.marginLeft = "0"; // ★ 明示的に margin-left を 0 に設定
      }
...
function finishDragging() {
  ...
  const songInfo = draggingItem.querySelector(".song-info");
  if (songInfo) {
      songInfo.style.paddingLeft = "30px"; // ★ 適切な padding-left の値を設定
  }
  draggingItem = null;
}
...
function restoreToOriginalList(item) {
  ...
  if (originalList) {
      ...
      const alreadyExists = Array.from(originalList.children).some(child => child.textContent.trim() === itemText);

      if (!alreadyExists) {
          // 元のリスト内での元の位置に要素を挿入
          const originalIndex = Array.from(originalList.children).findIndex(child => child.textContent === itemText);
          if (originalIndex !== -1) {
