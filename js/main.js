...
function handleTouchEnd(event) {
  // ドラッグ操作自体が開始されなかった、または短すぎるタップだった場合
  if (!isDragging && draggingItemId) { // isDraggingがfalseでも、draggingItemIdがある場合は、わずかな動きでドラッグ判定になる前のタッチ終了の可能性がある
    console.log("[touchend:Mobile] Drag not initiated or short tap. Checking for pending placeholder/clone cleanup.");
    
    // originalSetlistSlot がプレースホルダーになっていれば元に戻す
    if (originalSetlistSlot && originalSetlistSlot.classList.contains('placeholder-slot')) {
      console.log("[touchend:Mobile] Restoring original slot from placeholder after non-drag/short tap.");
      // 元のアイテムデータを再構築してスロットに戻す
      const itemDataFromOriginalAlbum = getItemDataFromAlbum(draggingItemId);
      if (itemDataFromOriginalAlbum) {
          fillSlotWithItem(originalSetlistSlot, itemDataFromOriginalAlbum);
          originalSetlistSlot.classList.remove('placeholder-slot');
          // アルバムリストの元のアイテムを表示に戻す (隠されていた場合)
          const albumItemInMenu = document.querySelector(`.album-content .item[data-item-id="${draggingItemId}"]`);
          if (albumItemInMenu) {
              albumItemInMenu.style.display = '';
          }
      } else {
          console.warn("[touchend:Mobile] Could not find original item data to restore. Clearing slot.");
          clearSlotContent(originalSetlistSlot.parentNode, originalSetlistSlot.dataset.slotIndex);
          originalSetlistSlot.classList.remove('placeholder-slot');
      }
    }
    ...
    if (currentTouchDraggedClone && currentTouchDraggedClone.parentNode === document.body) {
        ...
        console.log("[touchend:Mobile] Removed temporary currentTouchDraggedClone after non-drag/short tap.");
    }
  ...
  setlist.querySelectorAll('.setlist-slot.drag-over').forEach(slot => { // drag-over も解除
  ...
  if (dropTargetSlot) {
      ...
      // セットリスト外にドロップされた場合 (モバイルでの削除/アルバムへの復元)
      console.warn("[touchend:Mobile] Dropped outside a setlist slot. Attempting to restore to original list or remove.");
      // originalSetlistSlot が存在する場合 (セットリストからのドラッグの場合)
      if (originalSetlistSlot) {
          // 元のスロットをクリアした状態なので、元のアイテムデータを取得して戻す
          const itemDataFromOriginalAlbum = getItemDataFromAlbum(draggingItemId);
          if (itemDataFromOriginalAlbum) {
              fillSlotWithItem(originalSetlistSlot, itemDataFromOriginalAlbum);
              originalSetlistSlot.classList.remove('placeholder-slot'); // プレースホルダー解除
          } else {
              // データが見つからなければ、元のスロットを空にする
              clearSlotContent(originalSetlistSlot.parentNode, originalSetlistSlot.dataset.slotIndex);
              originalSetlistSlot.classList.remove('placeholder-slot');
          }
          // アルバムリストに元のアイテムを表示に戻す
          const albumItemInMenu = document.querySelector(`.album-content .item[data-item-id="${draggingItemId}"]`);
          if (albumItemInMenu) {
              albumItemInMenu.style.display = '';
              console.log(`[touchend:Mobile] Original album item ${draggingItemId} displayed.`);
          }
      } else {
          // アルバムリストからドラッグしてセットリスト外にドロップした場合
          // 元のアルバムアイテムを表示に戻す
          const albumItemInMenu = document.querySelector(`.album-content .item[data-item-id="${draggingItemId}"]`);
          if (albumItemInMenu) {
              albumItemInMenu.style.display = '';
...
// ヘルパー関数: itemIdからアルバム内のオリジナルアイテムのデータを取得する
function getItemDataFromAlbum(itemId) {
    ...
    if (originalAlbumItem) {
        return {
            ...
            short: originalAlbumItem.classList.contains('short') || (originalAlbumItem.querySelector('input[type="checkbox"]') && originalAlbumItem.querySelector('input[type="checkbox"]').checked),
...
function processDrop(currentDraggedElement, dropTargetSlot, originalSetlistSlot = null) {
  ...
  // currentDraggedElement は PCなら元の要素、モバイルならクローン要素を指す
  ...
  const isDraggedFromSetlist = originalSetlistSlot !== null; // originalSetlistSlot が設定されていればセットリストからのドラッグ

  if (isDraggedFromSetlist) {
    console.log("[processDrop] Drag source is setlist item (reordering/removal).");

    if (dropTargetSlot && dropTargetSlot !== originalSetlistSlot) { 
      // 有効なドロップゾーン（元の場所以外）にドロップされた場合
      if (dropTargetSlot.classList.contains('setlist-item')) {
        console.log("[processDrop] Dropping onto occupied slot (swap scenario).");
        const tempItemDataToSwap = getSlotItemData(dropTargetSlot); // ドロップ先スロットの既存アイテムデータを取得

        // ドロップ先のアイテムを元のスロット（originalSetlistSlot）に戻す（入れ替え）
        if (tempItemDataToSwap) {
          fillSlotWithItem(originalSetlistSlot, tempItemDataToSwap);
          originalSetlistSlot.classList.remove('placeholder-slot'); // プレースホルダー解除
        } else {
          // 入れ替え対象のデータが取得できなかった場合、元のスロットをクリア
          console.warn("[processDrop] Failed to extract data for swap. Clearing original slot.");
        ...
        // 無効な場所、または元の場所に戻された場合
        console.log("[processDrop] Dropped outside a valid setlist slot or back to original position. Restoring item to original slot.");
    ...
    // draggingItemIdを使って元のアルバムアイテムを取得（PC/Mobile共通）
  ...
  // この関数内で finishDragging は呼ばない。handleTouchEnd/handleDrop から呼ばれる。
}
 ...
 * @param {Element} itemToRestore - セットリストから戻す、または削除する対象の要素（セットリストスロットまたはその内部のアイテム）
 */
function restoreToOriginalList(itemToRestore) {
    // まず、対象のアイテムのIDを取得
    const itemId = itemToRestore.dataset.itemId; // itemToRestore自体がitemIdを持つ場合
    let slotToClear = null;

    // itemToRestoreがセットリストのスロット自身か、スロット内の子要素かを判定し、クリアすべきスロットを特定
    if (itemToRestore.classList.contains('setlist-slot') && itemToRestore.classList.contains('setlist-item')) {
        slotToClear = itemToRestore;
    } else {
        // itemToRestoreが.itemクラスの要素（例えば.song-info-container内の要素）だった場合、親のスロットを探す
        slotToClear = itemToRestore.closest('.setlist-slot.setlist-item');
    }

    if (!slotToClear || !itemId) {
        console.warn(`[restoreToOriginalList] No valid slot or item ID found for restoration. Item:`, itemToRestore);
        ...
        return; // 有効なスロットが見つからなければ処理を中断
    }

    console.log(`[restoreToOriginalList] Attempting to restore item ID: ${itemId} from slot: ${slotToClear.dataset.slotIndex}`);
    ...
    clearSlotContent(setlist, slotToClear.dataset.slotIndex); // slotToClearのデータ属性からindexを取得して渡す
    ...
    // PCドラッグでセットリスト外にドロップされた場合、currentPcDraggedElement は元の要素なので、ここでは削除しない
    // ブラウザのネイティブなドラッグ終了イベントで適切に処理されることを想定
}
