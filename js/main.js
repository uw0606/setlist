...
function handleTouchEnd(event) {
    ...
    if (isDragging && currentTouchDraggedClone) { 
        ...
        // ここで finishDragging() を直接呼び出すことで、確実に状態をリセット
        finishDragging(); 
    }
    // handleTouchEnd の最後に finishDragging() を常に呼ぶのではなく、
    // 上記のように条件分岐内で呼び出すことで、processDrop が呼ばれた後のクリーンアップと
    // それ以外のケースでのクリーンアップを統合する。
    // processDrop の内部で finishDragging() を呼んでいることを前提とする。
}
