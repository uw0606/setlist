...
// ★★★ 修正: DOM要素の参照を初期化時は null に設定 ★★★
// document.getElementById() の呼び出しは、DOMContentLoaded で行う必要があります。
let setlist = null; // document.getElementById("setlist"); ではない
let menu = null; // document.getElementById("menu"); ではない
let menuButton = null; // document.getElementById("menuButton"); ではない
let albumList = null; // document.querySelector(".album-list"); ではない
...
// ★★★ 修正: lastScrollDirection もここで定義 ★★★
let rafId = null; // requestAnimationFrame のID
let lastScrollDirection = 0; // 最後に要求されたスクロール方向を保持 (autoScrollSetlist用)
...
function handleTouchMove(event) {
    if (!isDragging || !currentTouchDraggedClone) {
        ...
        // 💡 autoScrollSetlist(0) はグローバル変数 rafId が定義されている前提で動作します。
        if (typeof autoScrollSetlist === 'function') autoScrollSetlist(0);
    ...
    if (!touch) {
        ...
        if (typeof autoScrollSetlist === 'function') autoScrollSetlist(0); // タッチ情報がない場合は停止
        return;
    }
    ...
    // --- ★修正: モバイル自動スクロールロジックの判定基準をビューポートに変更★ ---
    if (typeof autoScrollSetlist === 'function' && typeof SCROLL_AREA_HEIGHT !== 'undefined') {
        let scrollDirection = 0;
        const viewHeight = window.innerHeight; // ビューポートの高さ

        // 上端スクロールエリア (画面上部 SCROLL_AREA_HEIGHT px)
        if (currentY < SCROLL_AREA_HEIGHT) {
            scrollDirection = -1; // 上へスクロール
        } 
        // 下端スクロールエリア (画面下部 SCROLL_AREA_HEIGHT px)
        ...
        // 💡 rafId がグローバル変数として定義されている必要があります。
        if (scrollDirection !== 0 && (typeof rafId === 'undefined' || rafId === null)) {
            // スクロールが必要で、まだアニメーションが開始されていない場合
            autoScrollSetlist(scrollDirection);
        } else if (scrollDirection === 0) {
            // スクロールが不要になった場合
            autoScrollSetlist(0); 
        ...
        console.warn("[handleTouchMove] autoScrollSetlist function or necessary constants are missing. Skipping auto-scroll.");
    }
    // --- ★自動スクロールロジックの修正ここまで★ ---
...
function autoScrollSetlist(direction) {
    ...
    // すでにアニメーションが動いている場合は何もしない
    // ただし、方向が変わる場合はアニメーションを再開する必要がありますが、
    // ここでは direction が 0 以外の場合に再帰ループを開始/維持します。
    if (rafId && lastScrollDirection === direction) {
        return;
    }
    
    // 既存のアニメーションをキャンセルして、新しい方向で再開するための準備
    if (rafId) {
        cancelAnimationFrame(rafId);
        rafId = null;
    }

    lastScrollDirection = direction; // 新しいグローバル変数またはローカル変数として定義してください
    ...
    const step = () => {
        ...
        // 次のフレームを要求 (rafId は step() の実行中に更新されるため、ここで再代入は不要)
...
function enableDragAndDrop(element) {
    ...
    if (element.classList.contains('item') || element.classList.contains('setlist-item')) {
        ...
        // ★★★ 修正: touchstart, touchmove, touchend, touchcancel のすべてに passive: false を適用 ★★★
        element.addEventListener("touchstart", handleTouchStart, { passive: false });
        element.addEventListener("touchmove", handleTouchMove, { passive: false });
        element.addEventListener("touchend", handleTouchEnd, { passive: false }); 
        element.addEventListener("touchcancel", handleTouchEnd, { passive: false }); // touchcancel も追加
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (setlistYear) {
        ...
        // ★修正: currentYear + 5 のロジックは正しい日付（2025年）基準で行います。
        // 現在のシステム時刻が2025年10月なので、このロジックは維持します。
    ...
    loadSetlistState().then(() => {
        ...
        // ★★★ 修正: スロットへの touch-action の JS設定を削除 ★★★
        // CSSで対応済み、または touch-action: auto で十分であるため、JSからの設定を削除
        document.querySelectorAll('.setlist-slot').forEach(slot => {
            slot.style.pointerEvents = 'auto'; // すべてのスロットを常に auto に
            // slot.style.touchAction = 'pan-y'; // ← この行を削除/コメントアウト
        });
    }).catch(error => {
