...
function handleTouchStart(event) {
    ...
    if (event.touches.length === 1) {
        ...
        // touchTimeoutはダブルタップ検出用のみとし、ドラッグ開始用としては使用しない
        if (touchTimeout) { clearTimeout(touchTimeout); touchTimeout = null; }
        console.log("[touchstart:Mobile] Touch detected. Waiting for movement for drag or double tap.");
    }
}
...
function handleTouchMove(event) {
    ...
    if (!isDragging) {
        ...
        // DRAG_THRESHOLD に達していない場合は、何もしない（ブラウザにスクロールの機会を与える）
        if (deltaX < DRAG_THRESHOLD && deltaY < DRAG_THRESHOLD) {
            // ここで event.preventDefault() を呼ばないことが重要！
            // touch-action: none; が設定されているのでブラウザのスクロールは起こらないが、
            // ドラッグ開始と誤認して preventDefault すること自体を防ぐ
            return; 
        }

        // ここに到達した場合は、DRAG_THRESHOLD を超えたと判断し、ドラッグを開始する
        if (!draggingElement || !draggingItemId) {
            console.warn("[handleTouchMove] No draggable element info. Allowing scroll.");
            // この場合もドラッグを開始できないので、event.preventDefault()は呼ばない
            return;
        }
        ...
        console.log("[handleTouchMove] Drag initiated after movement threshold.");
...
function handleTouchEnd(event) {
    // touchTimeout はダブルタップ検出用なので、ここでのドラッグ判定には影響しない
    // if (touchTimeout) { clearTimeout(touchTimeout); touchTimeout = null; } // 削除
