...
function handleTouchStart(event) {
    ...
    if (isCheckboxClick) {
        ...
        isDragging = false; // チェックボックスクリック時はドラッグ状態をリセット
        return; 
    }
    ...
    if (event.touches.length === 1) {
        ...
        // ★修正点1: ドラッグ候補として isDragging を true に設定★
        isDragging = true; // 仮にドラッグが始まる可能性を示す
        ...
        // touchTimeoutが完了しなかった場合、またはタッチがタップで終わった場合にisDraggingをリセットするための処理
        // handleTouchEndでisDragging=falseであれば、このsetTimeoutをクリアする
        touchTimeout = setTimeout(() => {
            if (!isDragging) { // ここに到達した時点でまだisDraggingがfalseなら、ドラッグは開始されなかったと判断
                 console.log("[touchstart:Mobile] Long press timeout reached, but drag was not initiated or already ended.");
                 // ここで isDragging を false に戻す必要はない。handleTouchEnd で最終的に処理されるため。
            } else { // timeoutが発火し、かつisDraggingがtrueの場合（つまりロングプレスでドラッグ開始）
                 // createTouchDraggedCloneは handleTouchMove に移したのでここでは呼ばない
                 console.log("[touchstart:Mobile] Long press timeout completed. Dragging should have started.");
            }
            touchTimeout = null;
        }, 600); 
    }
...
function handleTouchMove(event) {
    // ★修正点2: isDragging が false の場合のみ、ドラッグ開始を検知するロジックを実行★
    if (!isDragging && draggingItemId && event.touches.length === 1) {
        const currentX = event.touches[0].clientX;
        const currentY = event.touches[0].clientY;
        const dx = Math.abs(currentX - touchStartX);
        const dy = Math.abs(currentY - touchStartY);
        const dragThreshold = 5; // ★修正点3: しきい値を小さくしてみる (例: 5ピクセル)★

        if (dx > dragThreshold || dy > dragThreshold) {
            if (touchTimeout) { // ロングプレス待ちのsetTimeoutがあればクリア
                clearTimeout(touchTimeout);
                touchTimeout = null;
            }
            ...
            if (touchedElement) {
                event.preventDefault(); // ここでブラウザのスクロールを停止！
                createTouchDraggedClone(touchedElement, touchStartX, touchStartY, draggingItemId);
                isDragging = true; // ドラッグ開始
                console.log("[handleTouchMove] Dragging initiated by movement threshold. Prevented default.");
            } else {
                console.warn("[handleTouchMove] Touched element not found for dragging.");
                isDragging = false; // 無効な状態なのでドラッグしない
            }
        }
    }
    // ★修正点2 終了★

    // isDragging が false の場合は、以降のドラッグ処理は行わない
    if (!isDragging || !currentTouchDraggedClone) {
        // ここで既に isDragging が false であれば、スクロール等を許可するために何もしない
        return; 
    }

    event.preventDefault(); // ドラッグ中は常にデフォルト動作をキャンセル
...
function handleTouchEnd(event) {
    ...
    if (!isDragging) { 
        if (!isCheckboxClick) { 
            ...
            draggingItemId = null;
            originalSetlistSlot = null; // スロットもリセット
            currentTouchDraggedClone = null; // クローンもリセット
        } else {
