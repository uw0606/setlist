...
async function generateSetlistPdf() {
    ...
    // 共有テキストの変数定義はここでは省略 (元のコードにはあったが、ロジックはスキップされていたため)
    ...
    try {
        ...
        // PDFデータをBlobとして取得
        const detailedBlob = detailedPdf.output('blob');
        ...
        // PDFデータをBlobとして取得
        const simpleBlob = simplePdf.output('blob');
        ...
        // PDFデータをBlobとして取得
        const inverseBlob = inversePdf.output('blob');
        ...
        const allPdfData = [
            { filename: detailedFilename, blob: detailedBlob },
            { filename: simpleFilename, blob: simpleBlob },
            { filename: inverseFilename, blob: inverseBlob }
        ];

        if (isMobileDevice()) {
            // モバイルの場合: 選択肢を表示 (displayDownloadLinksは別途実装が必要です)
            console.log("[generateSetlistPdf] Mobile device detected. Displaying download options.");
            
            // displayDownloadLinks 関数が、allPdfDataを受け取り、ダウンロードリンクを表示することを想定しています。
            if (typeof displayDownloadLinks === 'function') {
                 displayDownloadLinks(allPdfData);
                 showMessage("PDF生成完了。ダウンロードリンクから選択してください。", "success");
            } else {
                 // displayDownloadLinksが未定義の場合のフォールバック (1つ目のファイルのみダウンロード)
                 downloadBlob(detailedBlob, detailedFilename);
                 console.warn("displayDownloadLinks function is not defined. Downloading only the detailed PDF.");
                 showMessage("モバイル環境ですが、選択機能が未実装のため、詳細PDFのみダウンロードします。", "warning");
            }

        } else {
            // PCの場合: 連続してダウンロードを実行
            console.log("[generateSetlistPdf] Desktop device detected. Starting bulk download.");
            
            // Blobを強制的にダウンロードする処理
            allPdfData.forEach(data => {
...
// ===================================
// ★ 必要な補助関数 (ファイルの末尾などに追加してください) ★
// ===================================

/**
 * Blobオブジェクトを指定されたファイル名で強制的にダウンロードする。
 * (jsPDFの save() が内部的に行っている処理に近い)
...
/**
 * 現在のデバイスがモバイルかどうかを判定する。
 */
function isMobileDevice() {
    const userAgent = navigator.userAgent || navigator.vendor || window.opera;
    // Android, iPhone, iPod, iPad (タブレットも含む) をチェック
    return /android|iphone|ipod|ipad|iemobile/i.test(userAgent);
}
