...
const maxSongs = 20; // セットリストの最大スロット数
...
// Firebaseのリアルタイムデータベースインスタンス
// この部分はfirebase-config.jsで初期化されることを想定し、
// ここでの`let database;`の宣言は削除しました。
// Firebase初期化後、`database = firebase.database();` が設定される必要があります。
// 'database'変数は、`DOMContentLoaded`または`firebase_ready`イベントリスナー内で初めて割り当てられます。
// --> この行を削除することで、グローバルな`database`が自動的に作成されるか、
//     またはfirebase-config.jsで作成された既存のグローバル`database`に値が代入されます。
// let database; // <-- この行を削除！


// セットリストスロットをクリアした際に、アルバムメニューの元のアイテムの表示状態を復元するためのマップ
 ...
 * 例: https://raw.githubusercontent.com/yWorks/jsPDF-AutoTable/master/examples/fonts/NotoSansJP-Regular-normal.js
 * https://raw.githubusercontent.com/yWorks/jsPDF-AutoTable/master/examples/fonts/NotoSansJP-Bold-bold.js
...
function registerJapaneseFont(doc) {
    ...
    //     doc.addFileToVFS('NotoSansJP-Bold.ttf', '...');
    //     doc.addFont('NotoSansJP-Bold.ttf', 'NotoSansJP', 'bold');
    // 注意: フォントデータは非常に大きくなるため、ローカルに配置するか、ビルドプロセスで適切に処理することをお勧めします。
...
function fillSlotWithItem(slot, itemData) {
    ...
    if (originalAlbumItemInMenu) {
        originalAlbumItemInMenu.style.visibility = 'hidden';
        originalAlbumMap.set(itemData.itemId, originalAlbumItemInMenu);
...
document.addEventListener('firebase_ready', () => {
    ...
    if (firebase && firebase.database) {
        // 修正: ここでの`let database;`の宣言は不要です。
        // グローバルスコープの`database`変数に代入します。
...
function toggleMenu() {
    ...
    console.log("[toggleMenu] Menu toggled. State:", menu.classList.contains('open') ? 'open' : 'closed');
}
...
function getCurrentState() {
    ...
    const originalAlbumMapSerial = {};
    originalAlbumMap.forEach((value, key) => {
        // HTMLElement は直接シリアライズできないので、itemId のみを保存
        // ロード時に itemId から HTMLElement を再取得する
        originalAlbumMapSerial[key] = true; // 存在することを示すフラグ
    });
    ...
    return {
        ...
        // originalAlbumMap はロード時に再構築するため、不要
        // 必要であれば、hideSetlistItemsInMenu() をロード後に呼び出す
    };
}
...
document.addEventListener('DOMContentLoaded', () => {
    console.log("[DOMContentLoaded] ページが読み込まれました。ドラッグアンドドロップと日付ピッカーを初期化します。");
    ...
    const updateDays = () => {
        if (!setlistYear || !setlistMonth || !setlistDay) {
            console.warn("[updateDays] 日付選択要素が見つかりません。日を更新できません。");
            return;
        }
        ...
        console.log(`[updateDays] ${year}年-${month}月の日が更新されました。最大日数: ${daysInMonth}`);
    };
    ...
    if (setlistYear) {
        ...
        console.log("[日付初期化] 年が生成されました。");
    } else {
        console.error("[日付初期化エラー] setlistYear 要素が見つかりません。");
    }
    ...
    if (setlistMonth) {
        ...
        console.log("[日付初期化] 月が生成されました。");
    } else {
        console.error("[日付初期化エラー] setlistMonth 要素が見つかりません。");
    }
    ...
    if (setlistYear && setlistMonth && setlistDay) {
        ...
        updateDays(); 
        setlistDay.value = today.getDate().toString().padStart(2, '0');
        console.log(`[DOMContentLoaded] セットリストの日付を本日 (${setlistYear.value}-${setlistMonth.value}-${setlistDay.value}) に設定しました。`);
    } else {
        console.warn("[DOMContentLoaded] 日付選択要素（年、月、日）が完全に見つかりません。日付の自動設定をスキップします。");
    }

    // Firebaseの初期化（前回の修正で問題なければこのまま）
    if (typeof firebase !== 'undefined' && firebase.database) {
        database = firebase.database(); 
        document.dispatchEvent(new Event('firebase_ready'));
    } else {
        console.warn("Firebaseが完全に読み込まれていません。「firebase_ready」イベントを待機します。");
    }

    // アルバムアイテムにドラッグ＆ドロップイベントを設定
    document.querySelectorAll(".album-content .item").forEach((item) => {
      enableDragAndDrop(item);
      console.log(`[DOMContentLoaded] アルバムアイテムにドラッグアンドドロップを有効にしました: ${item.dataset.itemId || 'N/A'}`);
    });
    ...
    setlist.querySelectorAll(".setlist-slot").forEach((slot, index) => {
      ...
      slot.addEventListener('click', (e) => {
          ...
          if (checkbox) {
              console.log("[slotClick] スロットリスナー経由でチェックボックスがクリックされました。");
              e.stopPropagation(); 
              ...
              console.log(`[slotClick] スロット ${slot.dataset.slotIndex} の ${optionType} ステータスが変更されました: ${checkbox.checked}`);
    ...
    console.log("[DOMContentLoaded] セットリストのスロットにドラッグアンドドロップを有効にしました。");

    // 共有IDがあればセットリストをロード
    if (database) {
        loadSetlistState().then(() => {
            console.log("[DOMContentLoaded] loadSetlistState が完了しました。最終的なクリーンアップを実行します。");
            hideSetlistItemsInMenu(); 
        }).catch(error => {
            console.error("[DOMContentLoaded] loadSetlistState 中にエラーが発生しました:", error);
            hideSetlistItemsInMenu();
        });
    } else {
        document.addEventListener('firebase_ready', () => {
            loadSetlistState().then(() => {
                console.log("[DOMContentLoaded] loadSetlistState (firebase_ready 後) が完了しました。最終的なクリーンアップを実行します。");
                hideSetlistItemsInMenu();
            }).catch(error => {
                console.error("[DOMContentLoaded] loadSetlistState (firebase_ready 後) 中にエラーが発生しました:", error);
                hideSetlistItemsInMenu();
            });
        }, { once: true });
    }
    
    // --- モーダル関連の要素取得とイベントリスナー設定 ---
    ...
    // **修正点1: closeModalButton の取得方法をより堅牢に**
    // pastSetlistsModal が存在する場合にのみ、その内部の .close-button を探す
    ...
    // **修正点2: close2025DetailModalButton の取得方法をより堅牢に**
    // year2025DetailModal が存在する場合にのみ、その内部の .close-button を探す
    ...
    if (openPastSetlistsModalButton && pastSetlistsModal) {
        openPastSetlistsModalButton.addEventListener('click', () => {
            ...
            console.log("[pastSetlists] 過去セットリストモーダルが開かれました。ハンバーガーメニューは閉じられました。");
        });
    } else {
        console.warn("[DOMContentLoaded] 「過去セットリストを開く」ボタンまたはモーダルが見つかりません。");
    }

    // 過去セットリストモーダルの閉じるボタンのイベントリスナー
    if (closeModalButton) { // closeModalButton が null でないことを確認
        closeModalButton.addEventListener('click', () => {
            pastSetlistsModal.style.display = 'none';
            console.log("[pastSetlists] 過去セットリストモーダルが閉じられました。");
            toggleMenu(); // ハンバーガーメニューを再度開く
            console.log("[pastSetlists] ハンバーガーメニューが開いた状態に戻されました。");
        });
    } else {
        console.warn("[DOMContentLoaded] '過去セットリスト'モーダルの閉じるボタンが見つかりませんでした。");
    }
    ...
    if (pastSetlistsModal) {
        pastSetlistsModal.addEventListener('click', (event) => {
            if (event.target === pastSetlistsModal) {
                pastSetlistsModal.style.display = 'none';
                console.log("[pastSetlists] 過去セットリストモーダルがオーバーレイクリックで閉じられました。");
                toggleMenu(); // ハンバーガーメニューを再度開く
                console.log("[pastSetlists] オーバーレイクリック後、ハンバーガーメニューが開いた状態に戻されました。");
            }
        });
    }
    ...
    pastYearItems.forEach(yearButton => {
        yearButton.addEventListener('click', (event) => {
            ...
            console.log(`[pastSetlists] 年がクリックされました: ${year}（モーダル内）。`);

            if (year === '2025') {
                pastSetlistsModal.style.display = 'none'; // 既存のモーダルを閉じる
                if (year2025DetailModal) {
                    year2025DetailModal.style.display = 'flex'; // 新しいモーダルを表示
                    console.log("[pastSetlists] 2025年詳細モーダルを表示します。");
                } else {
                    console.warn("[pastSetlists] 2025年詳細モーダル要素が見つかりません。");
                ...
                toggleMenu(); // ハンバーガーメニューを再度開く
                console.log("[pastSetlists] 年選択後、ハンバーガーメニューが開いた状態に戻されました。");
            }
        });
    });

    // 2025年詳細モーダルの閉じるボタンのイベントリスナー
    if (close2025DetailModalButton) { // close2025DetailModalButton が null でないことを確認
        close2025DetailModalButton.addEventListener('click', () => {
            year2025DetailModal.style.display = 'none'; // 詳細モーダルを非表示
            console.log("[pastSetlists] 2025年詳細モーダルが閉じられました。");
            toggleMenu(); // ハンバーガーメニューを再度開く
            console.log("[pastSetlists] 2025年詳細モーダルが閉じられた後、ハンバーガーメニューが開いた状態に戻されました。");
        });
    } else {
        console.warn("[DOMContentLoaded] '2025年詳細'モーダルの閉じるボタンが見つかりませんでした。");
    }
    ...
    if (year2025DetailModal) {
        year2025DetailModal.addEventListener('click', (event) => {
            if (event.target === year2025DetailModal) {
                ...
                console.log("[pastSetlists] 2025年詳細モーダルがオーバーレイクリックで閉じられました。");
                toggleMenu();
                console.log("[pastSetlists] 2025年詳細モーダルがオーバーレイクリックで閉じられた後、ハンバーガーメニューが開いた状態に戻されました。");
            }
        });
    }
    ...
    setlistLinks.forEach(link => {
        link.addEventListener('click', (event) => {
            ...
            toggleMenu(); // ハンバーガーメニューを再度開く
            console.log(`[pastSetlists] セットリストリンクがクリックされました: ${date} ${venue}。`);
        });
    });
});