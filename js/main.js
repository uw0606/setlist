...
function fillSlotWithItem(slotElement, itemData) {
    // 既存のコンテンツをクリア（slotElement.parentNode は setlist を指すことを想定）
    // clearSlotContent(slotElement.parentNode, slotElement.dataset.slotIndex); // この行は既に processDrop で呼ばれているはずなので、ここでは不要かも

    // slotElement が既に setlist-item クラスを持っている場合はクリアしない方が良い場合も
    // 例: スワップの際に既に setlist-item クラスが付いているスロットに fill する場合
    // ただし、clearSlotContent は「スロットを空にする」処理なので、適切に呼び出す場所を選ぶ必要があります。
    // 今回の processDrop のロジックでは、clearSlotContent が fillSlotWithItem の前に適切に呼び出されています。

    // まずはスロットを空にする
    while (slotElement.firstChild) {
        slotElement.removeChild(slotElement.firstChild);
    }
    slotElement.classList.remove('setlist-item', 'short', 'se-active', 'drumsolo-active'); // 不要なクラスも除去
    delete slotElement.dataset.itemId; // スロットのitemIdもクリア
    // 他のdata-*属性も必要に応じてクリア...

    const itemDiv = document.createElement('div');
    // ★重要★ 'item' クラスを追加することで、アルバムアイテムと同じようにドラッグソースになれるようにします。
    itemDiv.classList.add('item', 'setlist-item'); 
    itemDiv.dataset.itemId = itemData.itemId;
    itemDiv.dataset.songName = itemData.name;
    itemDiv.dataset.rGt = itemData.rGt || '';
    itemDiv.dataset.lGt = itemData.lGt || '';
    itemDiv.dataset.bass = itemData.bass || '';
    itemDiv.dataset.bpm = itemData.bpm || '';
    itemDiv.dataset.chorus = itemData.chorus || '';
    itemDiv.dataset.isShortVersion = itemData.short ? 'true' : 'false';
    itemDiv.dataset.hasSeOption = itemData.hasSeOption ? 'true' : 'false';
    itemDiv.dataset.seChecked = itemData.seChecked ? 'true' : 'false';
    itemDiv.dataset.hasDrumsoloOption = itemData.hasDrumsoloOption ? 'true' : 'false';
    itemDiv.dataset.drumsoloChecked = itemData.drumsoloChecked ? 'true' : 'false';

    if (itemData.short) {
        itemDiv.classList.add('short');
    }
    if (itemData.seChecked) {
        itemDiv.classList.add('se-active');
    }
    if (itemData.drumsoloChecked) {
        itemDiv.classList.add('drumsolo-active');
    }
    ...
    itemDiv.innerHTML = `
        ...
        <div class="item-options">
            <label class="option-label">
                <input type="checkbox" data-option-type="short" ${itemData.short ? 'checked' : ''}> Short
            </label>
            <label class="option-label">
                <input type="checkbox" data-option-type="se" ${itemData.seChecked ? 'checked' : ''} ${!itemData.hasSeOption ? 'disabled' : ''}> SE
            </label>
            <label class="option-label">
                <input type="checkbox" data-option-type="drumsolo" ${itemData.drumsoloChecked ? 'checked' : ''} ${!itemData.hasDrumsoloOption ? 'disabled' : ''}> ドラムソロ
            </label>
        </div>
    `;

    slotElement.appendChild(itemDiv);
    // スロット自体にも setlist-item クラスと itemId を設定
    slotElement.classList.add('setlist-item'); 
    slotElement.dataset.itemId = itemData.itemId; 

    // ★重要★: 新しくセットリストに追加されたアイテムもドラッグ＆ドロップのソースになれるようにする
    enableDragAndDrop(itemDiv); // itemDiv に enableDragAndDrop を適用
    
    console.log(`[fillSlotWithItem] Slot ${slotElement.dataset.slotIndex} filled with item ${itemData.itemId}`);
}

// clearSlotContent 関数の定義 (もし未定義であれば追加してください)
// この関数は processDrop や loadSetlistState で使用されています。
function clearSlotContent(parentSetlistElement, slotIndex) {
    const slotElement = parentSetlistElement.querySelector(`.setlist-slot[data-slot-index="${slotIndex}"]`);
    if (slotElement) {
        // スロット内のすべてのコンテンツを削除
        while (slotElement.firstChild) {
            slotElement.removeChild(slotElement.firstChild);
        }
        // setlist-item クラスと data-item-id 属性も削除して空の状態にする
        slotElement.classList.remove('setlist-item', 'short', 'se-active', 'drumsolo-active');
        delete slotElement.dataset.itemId;
        console.log(`[clearSlotContent] Slot ${slotIndex} cleared.`);
    }
}

// getSlotItemData 関数の定義 (もし未定義であれば追加してください)
// この関数は processDrop などでアイテムデータを取得するために使用されています。
function getSlotItemData(itemElement) {
    if (!itemElement || !itemElement.dataset) {
        console.warn("[getSlotItemData] Invalid element provided:", itemElement);
        return null;
    }

    const isSlot = itemElement.classList.contains('setlist-slot');
    const targetElement = isSlot ? itemElement.querySelector('.item') : itemElement;

    if (!targetElement || !targetElement.dataset) {
        // スロットが空の場合や、アイテムではない要素が渡された場合
        console.log("[getSlotItemData] No item found within slot or element is not an item itself.");
        return null;
    }

    const itemData = {
        itemId: targetElement.dataset.itemId,
        name: targetElement.dataset.songName || targetElement.querySelector('.item-name')?.textContent || '',
        rGt: targetElement.dataset.rGt || '',
        lGt: targetElement.dataset.lGt || '',
        bass: targetElement.dataset.bass || '',
        bpm: targetElement.dataset.bpm || '',
        chorus: targetElement.dataset.chorus || '',
        // オプション情報もデータセットから取得
        short: targetElement.dataset.isShortVersion === 'true',
        hasSeOption: targetElement.dataset.hasSeOption === 'true',
        seChecked: targetElement.dataset.seChecked === 'true',
        hasDrumsoloOption: targetElement.dataset.hasDrumsoloOption === 'true',
...
function processDrop(draggedElement, dropTargetSlot, originalSetlistSlot = null) {
    ...
    if (isDraggedFromSetlist) {
        // ★ここがセットリスト内でのドラッグ＆ドロップのロジックです★
        if (dropTargetSlot) {
            if (dropTargetSlot.classList.contains('setlist-item')) {
                // ドロップターゲットも曲で埋まっている場合（スワップ）
                ...
                fillSlotWithItem(dropTargetSlot, draggedItemData); // ターゲットスロットに移動
            }
        } else {
            // セットリスト外にドロップされた場合（セットリストから削除に近い挙動）
            console.log("[processDrop] Dropped outside setlist (from setlist). Doing nothing; will restore visually.");
            // このケースは handleDrop で処理されることが多いですが、念のため
        }
    } else {
        // アルバムからのドロップ処理（変更なし）
        if (dropTargetSlot) {
...
const DRAG_THRESHOLD = 50; // ドラッグと判断する移動のしきい値（ピクセル）
...
function handleTouchStart(event) {
    ...
    if (tapLength < 300 && tapLength > 0) {
        // ダブルタップの場合のみデフォルト動作をキャンセル (スクロールもブロック)
        event.preventDefault(); // ここはダブルタップの場合なのでブロックしてOK
        if (touchTimeout) {
            clearTimeout(touchTimeout);
            touchTimeout = null;
        }
    ...
    if (event.touches.length === 1) {
        ...
        // **重要**: ここで event.preventDefault() を呼ばないことで、タッチ開始直後からスクロールを許可します。

        isDragging = false; // タッチ開始時点ではまだドラッグ中ではない
        draggingItemId = touchedElement.dataset.itemId;
        initialTouchX = event.touches[0].clientX; // 初期座標を保存
        ...
        // touchTimeout による長押し判定は、移動距離判定と競合するため削除します。
        // これにより、すぐにスクロールが開始されるようになります。
        if (touchTimeout) {
            clearTimeout(touchTimeout);
            touchTimeout = null;
        }
...
function handleTouchMove(event) {
    if (event.touches.length !== 1) return; // シングルタッチのみを処理

    const currentX = event.touches[0].clientX;
    const currentY = event.touches[0].clientY;

    // まだドラッグが開始されていない場合、移動距離をチェック
    if (!isDragging) {
        ...
        if (distance > DRAG_THRESHOLD) {
            const touchedElement = event.target.closest(".setlist-slot.setlist-item") || event.target.closest(".item");
            // draggingItemId も有効なことを確認 (touchstart で設定されているはず)
            if (!touchedElement || !draggingItemId) {
                 console.warn("[handleTouchMove] Drag threshold met, but no valid element or itemId to drag. Aborting.");
                 // ドラッグを開始しない場合、スクロールはブロックしない
                 return;
            }
            // **ここからドラッグ開始と判断**
            isDragging = true;
            // クローン要素作成時に、元のタッチ開始座標と draggingItemId を渡す
            createTouchDraggedClone(touchedElement, initialTouchX, initialTouchY, draggingItemId);
            console.log("[handleTouchMove] Dragging started due to movement threshold.");
            // ドラッグ開始したら、それ以降のスクロールをブロック
            event.preventDefault(); // ドラッグが始まったらスクロールをブロック
        } else {
            // しきい値に達していない場合、スクロールを許可するために何もしない
            return; // ここで return することで、preventDefault() が呼ばれずスクロール可能
        }
    }

    // ドラッグが既に開始されている場合のみ、クローン要素の移動とドロップゾーンのハイライトを行う
    if (isDragging && currentTouchDraggedClone) {
        // ドラッグ中は常にデフォルトのスクロール動作をキャンセル
        event.preventDefault(); // ドラッグ中はスクロールをブロック
...
function handleTouchEnd(event) {
    ...
    if (touchTimeout) { // 念のため残しておくが、新しいロジックでは使われない可能性が高い
        clearTimeout(touchTimeout);
        touchTimeout = null;
    }
    ...
    if (!isDragging) { // ドラッグしていなかった場合
        if (!isCheckboxClick) { 
            ...
            // スクロールが意図しないブロックを解除するために、ここでfinishDraggingを呼ばない
        } else {
