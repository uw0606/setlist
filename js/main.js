...
function handleTouchStart(event) {
    ...
    if (tapLength < DOUBLE_TAP_DELAY && tapLength > 0) { // DOUBLE_TAP_DELAY を使用
        ...
        // handleDoubleClick は既に event.preventDefault() と event.stopPropagation() を含む
    ...
    if (event.touches.length === 1) {
        ...
        // ★修正: event.preventDefault() を handleTouchMove に移動するため、ここでは呼ばない★
        // touchTimeout = setTimeout(() => {
        //     if (!isDragging && draggingItemId && document.body.contains(touchedElement)) {
        //         event.preventDefault(); // ドラッグ開始時にスクロールを抑制
...
function handleTouchMove(event) {
    if (event.touches.length === 1) {
        ...
        if (deltaX > TAP_MOVE_THRESHOLD || deltaY > TAP_MOVE_THRESHOLD) {
            if (touchTimeout) { // timeoutが設定されていればクリア (ロングプレスを中断)
                clearTimeout(touchTimeout);
                touchTimeout = null;
            }
            if (!isDragging) { // まだドラッグが開始されていなければ開始
                const touchedElement = event.target.closest(".setlist-slot.setlist-item") || event.target.closest(".item");
                if (touchedElement && draggingItemId === touchedElement.dataset.itemId) {
                    event.preventDefault(); // ★ここに追加: ドラッグが確定したらスクロールを抑制★
                    createTouchDraggedClone(touchedElement, initialTouchX, initialTouchY, draggingItemId);
                    isDragging = true;
                    console.log("[handleTouchMove] Dragging initiated by movement. Preventing default.");
                }
            }
        }
...
function finishDragging() {
  ...
  if (originalSetlistSlot) {
      ...
      originalSetlistSlot.style.visibility = ''; // ⭐︎ ここが重要: 元のスロットの visibility を常に元に戻す ⭐︎
...
function handleDoubleClick(event) {
    ...
    finishDragging(); // 全ての handleDoubleClick 処理の最後に呼び出す
}
    ...
    const initializeSetlistItem = (itemElement) => {
        ...
        itemElement.addEventListener("dblclick", (e) => {
            ...
            if (!e.target.closest('.drag-handle')) {
                ...
                restoreToOriginalList(itemElement); // ★修正: removeSetlistItem から restoreToOriginalList に変更★
                e.stopPropagation(); 
                e.preventDefault(); 
            }
        }); 
        ...
        itemElement.addEventListener('touchend', (e) => {
            ...
            if (!isDraggingTouch && !currentDraggedItem && (currentTime - lastTapTime < DOUBLE_TAP_DELAY && currentTime - lastTapTime > 0)) {
                ...
                if (!e.target.closest('.drag-handle')) {
                    ...
                    e.stopPropagation(); 
                    e.preventDefault(); 
                    finishDragging(); // ★追加: 処理後、グローバル状態をリセット★
                } else {
    ...
    setlist.querySelectorAll(".setlist-slot").forEach((slot, index) => {
        ...
        slot.addEventListener('touchstart', (e) => {
            ...
            if (!e.target.closest('.setlist-item') && !currentDraggedItem) {
                ...
                initialTouchY = touch.clientY;
                isDraggingTouch = false; 
                e.preventDefault(); // 空のスロットでのタッチはデフォルト動作をキャンセル
