...
function buildNameOrderList() {
    ...
    // --- 3. グループに分割 ---
    // groupSongsは既に提供済みの修正版（日本語対応済み）を使用
...
function groupSongs(songs) {
    ...
    songs.forEach(song => {
    // 曲名の最初の文字を取得し、強制的に大文字にする (英字判定のため)
    const firstChar = song.songName.trim().charAt(0).toUpperCase(); // ★修正ポイント★
    ...
    // 2. 数字・記号の分類 (英字ではないが数字/記号であるもの)
    else if (/[0-9\W]/.test(firstChar)) groups['ETC_NONE'].push(song); 
    
    // 3. 日本語の分類 (英字でも数字/記号でもないもの)
    else {
        // 日本語の分類は、元の文字 (大文字化されていない状態) で行う必要があります。
        // firstChar は既に大文字化されているため、この後の日本語判定が失敗します。
        
        // 解決策: 判定用に元の文字を再取得
        const originalFirstChar = song.songName.trim().charAt(0);
        
        const jpGroup = japaneseMap[originalFirstChar];
        if (jpGroup) {
            groups[jpGroup].push(song);
        } else {
            // 漢字、特殊なひらがな/カタカナなどは AIUEO に
