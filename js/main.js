...
function handleTouchEnd(e) {
    ...
    if (currentDropZone) {
        ...
        // 有効なドロップゾーンがない場合（ドラッグ失敗時）、元の要素を元に戻す
        console.log("[touchend] No valid drop zone was found. Reverting original item to its source.");
        
        if (originalSetlistSlot) {
            // セットリストからドラッグ開始した場合
            console.log(`[touchend] Restoring original setlist slot ${originalSetlistSlot.dataset.slotIndex}.`);
            originalSetlistSlot.style.visibility = ''; // 可視状態に戻す
            originalSetlistSlot.classList.remove('placeholder-slot'); // プレースホルダークラスを削除
            
            // _originalItemData があればスロットの内容を復元
            if (originalSetlistSlot._originalItemData) {
                // 元々スロットにあったアイテムデータを取得し、スロットを埋め直す
                const itemDataToRestore = originalSetlistSlot._originalItemData;
                fillSlotWithItem(originalSetlistSlot.dataset.slotIndex, itemDataToRestore);
                delete originalSetlistSlot._originalItemData; // 復元後クリア
                console.log(`[touchend] Item ${itemDataToRestore.name} restored to slot ${originalSetlistSlot.dataset.slotIndex}`);
            } else {
                console.warn("[touchend] originalSetlistSlot._originalItemData was not found, slot might be cleared. Assuming it should remain empty or current content is correct.");
            }
        } else if (currentPcDraggedElement) {
            // アルバムリストからドラッグ開始した場合 (モバイルでも currentPcDraggedElement を使用)
            console.log(`[touchend] Restoring original album item visibility for: ${currentPcDraggedElement.dataset.itemId}.`);
            currentPcDraggedElement.style.visibility = ''; // 元のアルバムアイテムを可視に戻す
        } else {
            console.warn("[touchend] Neither originalSetlistSlot nor currentPcDraggedElement was set, unexpected state for drag revert.");
        }
        showMessage('ドラッグをキャンセルしました。元の位置に戻します。');
        finishDragging(); // ドロップ失敗時もクリーンアップ
    }
    // ここで finishDragging() を呼ぶことで、グローバル変数が常にリセットされる
    // processDrop() の中からも finishDragging() が呼ばれるため、重複を避けるために
    // この else ブロックの最後に移動しました。
}
