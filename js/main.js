...
function handleDragStart(event) {
  ...
  console.log(`[handleDragStart] Drag started for item: ${draggedItem.dataset.itemId}`);
}
...
function handleDrop(event) {
  ...
  const sourceSlot = draggedItem.closest(".setlist-slot");
  ...
  if (sourceSlot && targetSlot !== sourceSlot) {
    ...
    clearSlotContent(setlist, sourceSlot.dataset.slotIndex);
    clearSlotContent(setlist, targetSlot.dataset.slotIndex);

    if (targetData) fillSlotWithItem(sourceSlot, targetData);
    if (sourceData) fillSlotWithItem(targetSlot, sourceData);

    console.log(`[handleDrop] Swapped items between slot ${sourceSlot.dataset.slotIndex} and ${targetSlot.dataset.slotIndex}`);
  } else if (!targetSlot.classList.contains("setlist-item")) {
    const itemData = getSlotItemData(draggedItem);
    if (itemData) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  setlist.querySelectorAll(".setlist-slot").forEach((slot, index) => {
    if (!slot.dataset.slotIndex) slot.dataset.slotIndex = index.toString();
  ...
  // セットリストに既に入ってるアイテムにもドラッグ許可
  setlist.querySelectorAll(".setlist-slot.setlist-item").forEach((item) => {
    enableDragAndDrop(item);
  });
