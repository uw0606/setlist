...
function handleTouchStart(event) {
    ...
    // ドラッグ可能な要素がタッチされた場合のみ、以下のロジックに進む
    // そうでない場合（メニューの空白、スクロールが必要な領域など）はスクロールを許可するため、ここでreturnする
    if (!touchedElement) {
    ...
    if (event.touches.length === 1) {
        ...
        // ★修正: ロングプレスによるドラッグ開始の `setTimeout` を削除します。
        // ドラッグ開始は `handleTouchMove` の移動量に基づいてのみ行われるようにします。
        // これにより、ロングプレスがスクロールを妨げる可能性をなくします。
        // (以前のコードにあった setTimeout 部分を削除します)
    }
}
...
function handleTouchMove(event) {
    if (event.touches.length === 1) {
        ...
        // タップ移動閾値を超えたらドラッグ開始とみなす
        if (deltaX > TAP_MOVE_THRESHOLD || deltaY > TAP_MOVE_THRESHOLD) {
            // ロングプレス待機中のタイマーがあればクリアし、ロングプレスによるドラッグ開始をキャンセル
            if (touchTimeout) {
                clearTimeout(touchTimeout);
                touchTimeout = null;
            ...
            // まだドラッグが開始されていなければ、ここでドラッグを開始する
            if (!isDragging) {
                const touchedElement = event.target.closest(".setlist-slot.setlist-item") || event.target.closest(".item");
                
                // draggingItemId が handleTouchStart で設定済みであることを確認し、
                // 実際に移動した要素がドラッグ対象と一致する場合にドラッグを開始
                if (touchedElement && draggingItemId === touchedElement.dataset.itemId) {
                    event.preventDefault(); // ドラッグが確定したらスクロールを抑制する
        ...
        if (isDragging && currentTouchDraggedClone) {
            ...
            // ドラッグ中は常にpreventDefaultを呼び出し、スクロールを抑制
            event.preventDefault(); 
        }
    }
}
...
const TAP_MOVE_THRESHOLD = 100; // タップと認識する指の移動距離（ピクセル）
let initialTouchX = 0;
let initialTouchY = 0;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const initializeSetlistItem = (itemElement) => {
        ...
        itemElement.addEventListener("dblclick", (e) => {
            ...
            if (!e.target.closest('.drag-handle')) {
                console.log(`[dblclick] PC double-click detected for item: ${itemElement.dataset.itemId}. Restoring item.`);
        ...
        }, { passive: false }); // passive: false は維持 (handleTouchStart で preventDefault 呼ばれるため)
        ...
        itemElement.addEventListener('touchend', (e) => {
            ...
            // isDraggingTouch が false で、かつダブルタップとして認識された場合
            if (!isDraggingTouch && !currentDraggedItem && (currentTime - lastTapTime < DOUBLE_TAP_DELAY && currentTime - lastTapTime > 0)) {
                ...
                if (!e.target.closest('.drag-handle')) {
                    console.log(`[touchend] Confirmed double tap for item: ${itemElement.dataset.itemId}. Restoring item.`);
                    restoreToOriginalList(itemElement); 
                    e.stopPropagation(); 
                    e.preventDefault(); 
                    finishDragging(); 
    ...
    setlist.querySelectorAll(".setlist-slot").forEach((slot, index) => {
        ...
        slot.addEventListener('touchstart', (e) => {
            // 空スロットのタップで余計なイベントを防止（スクロール防止）
            if (!e.target.closest('.setlist-item') && !currentDraggedItem) {
                const touch = e.touches[0];
                initialTouchX = touch.clientX;
                initialTouchY = touch.clientY;
                isDraggingTouch = false; 
                // e.preventDefault(); // ★修正: 空のスロットでのタッチはデフォルト動作をキャンセルしない (スクロールを許可)★
                ...
                // チェックボックスの場合はデフォルト動作を許容
                console.log("[slot touchstart] Checkbox within slot touched. Allowing default.");
                return;
            }
        }, { passive: false }); // passive: false は維持 (handleTouchMove で preventDefault 呼ばれるため)
    ...
    // --- 日付ドロップダウン、モーダル、セットリストリンク関連の初期設定 ---
