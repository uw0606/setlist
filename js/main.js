 ...
 * @param {Element|null} dropTargetSlot 
 * @param {Element|null} originalSetlistSlot 
 */
...
function handleTouchStart(event) {
    ...
    if (tapLength < 300 && tapLength > 0) {
        ...
        // ダブルタップの場合はブラウザのズームなどを防ぐために preventDefault を呼ぶ
        event.preventDefault(); 
    ...
    if (event.touches.length === 1) {
        ...
        // ロングプレスでドラッグを開始するためのタイマー。
        // ここでは event.preventDefault() を呼ばないことで、スクロールを許可する。
        touchTimeout = setTimeout(() => {
            // ロングプレスが確定したら isDragging を true にする
...
function handleTouchMove(event) {
    ...
    rafId = requestAnimationFrame(() => {
        ...
        // まだドラッグが開始されていない（isDraggingがfalse）場合
        if (!isDragging) {
            // 移動距離が閾値を超えていたら、スクロール目的の移動と判断し、
            // ロングプレスのタイマーをクリアして、デフォルトのスクロールを許可する
