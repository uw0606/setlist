...
function processDrop(draggedElement, dropTargetSlot, originalSourceSlot) {
    ...
    if (originalSourceSlot) {
        ...
        if (dropTargetSlot.classList.contains('setlist-item')) {
            ...
            const targetAlbumClass = Array.from(dropTargetSlot.classList).find(cls => cls.startsWith('album'));
        ...
        if (dropTargetSlot.classList.contains('setlist-item')) {
            // 埋まっているスロットへのアルバムからのドロップは、現時点では許可しない（あるいは入れ替えロジックを実装）
            // 今回はシンプルにエラーメッセージ
...
function handleDoubleClick(event) {
    ...
    if (albumItemElement) {
        console.log("[handleDoubleClick] Double-clicked an album item. Adding to setlist.");
        // アルバムからの追加処理をここに実装
        // セットリストの最初の空きスロットを探す
        ...
        if (firstEmptySlot) {
            ...
            const albumClass = Array.from(albumItemElement.classList).find(cls => cls.startsWith('album'));
