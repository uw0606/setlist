...
function clearSlotContent(slotElement) {
    ...
    slotElement.className = 'setlist-slot'; // すべてのカスタムクラスを削除
    // datasetもクリア
    for (const key in slotElement.dataset) {
        delete slotElement.dataset[key];
    }
    console.log(`[clearSlotContent] Slot ${slotElement.dataset.slotIndex || 'N/A'} content cleared.`);
}
...
function handleTouchStart(event) {
    ...
    if (tapLength < 300 && tapLength > 0) { // 300ms 以内の2回目のタップ
        ...
        // ダブルタップはドラッグではないので、finishDraggingは呼ばない（isDragging=falseのまま）
        return; // ダブルタップ処理が実行されたら、ここで関数を終了
    }
    lastTapTime = currentTime; // 最初のタップ時刻を記録

    // --- 2. シングルタッチからの長押し（ドラッグ開始）処理 ---
    // ここで event.preventDefault() を呼び出すことで、
    // 長押しによるドラッグ開始時のスクロールやデフォルト動作を防ぐ
    event.preventDefault(); 
    console.log("[touchstart] Prevented default for potential long press drag.");
...
function handleTouchMove(event) {
    ...
    // ドラッグ中またはドラッグ開始前でも、デフォルトのスクロールなどを防ぐ
    event.preventDefault(); 
    ...
    rafId = requestAnimationFrame(() => {
        ...
        if (!isDragging) {
            // アルバムからのドラッグ（originalSetlistSlotがnull）の場合のみ、
            // 閾値を超えたら長押しタイマーをキャンセル
            if (!originalSetlistSlot) { 
                ...
                if (deltaX > DRAG_THRESHOLD || deltaY > DRAG_THRESHOLD) {
                    if (touchTimeout) {
                        ...
                        console.log("[handleTouchMove] Movement beyond threshold, cancelling long press timer for album item.");
                    }
                    // アルバムからのドラッグ開始失敗時はクリーンアップ
                    finishDragging();
                }
            }
            rafId = null;
