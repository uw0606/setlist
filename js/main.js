...
function handleTouchStart(event) {
    ...
    if (event.touches.length === 1) {
        ...
        event.preventDefault(); // ここで一旦preventDefaultすることで、長押しドラッグの際にスクロールを抑制

        isDragging = false; // 初期状態ではドラッグではない
        draggingItemId = touchedElement.dataset.itemId;

        // ★修正点1: モバイルのタッチドラッグ用に元の要素を保持する変数を導入
        if (setlist.contains(touchedElement) && touchedElement.classList.contains('setlist-item')) {
            ...
            currentTouchDraggedOriginalElement = touchedElement; // セットリストからのドラッグの場合も保持
            ...
            currentTouchDraggedOriginalElement = touchedElement; // ★アルバムアイテムをモバイルドラッグ用に保持
            currentPcDraggedElement = null; // PC用はnullにリセット（明確に分離）
        }
        ...
        touchTimeout = setTimeout(() => {
            if (draggingItemId && document.body.contains(touchedElement)) {
                ...
                if (currentTouchDraggedOriginalElement) {
                    // セットリスト内のアイテムの場合は、placeholder-slotクラスを追加して非表示にする
...
function handleTouchEnd(event) {
    ...
    if (isDragging && (deltaX < dragThreshold && deltaY < dragThreshold)) {
        ...
        // それ以外の長押しタップ（曲が入っているスロットやアルバムアイテム）の場合も、
        // 長押しタップではドロップ処理は行わず、単にクリーンアップだけを行う
...
function finishDragging(cancelled = false) {
    ...
    if (currentPcDraggedElement && !originalSetlistSlot) { // originalSetlistSlotがnullならアルバムアイテムからのドラッグ（PC）
    ...
    // ★修正点3: モバイルタッチドラッグの元のアルバムアイテムのvisibilityを適切に戻す
    if (currentTouchDraggedOriginalElement) {
        // セットリストからのドラッグ（placeholder-slotが設定されている）の場合
