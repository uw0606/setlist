...
function processDrop(draggedElement, dropTargetSlot, originalSetlistSlot = null) {
    ...
    if (isDraggedFromSetlist) {
        ...
        if (dropTargetSlot) {
            ...
            // セットリスト外にドロップされた場合（セットリストから）
            console.log("[processDrop] Dropped outside setlist (from setlist). Restoring original item.");
            // 元のスロットのコンテンツを復元 (非表示だったアイテムを再表示)
            if (originalSetlistSlot && originalSetlistSlot._originalItemData) {
                fillSlotWithItem(originalSetlistSlot, originalSetlistSlot._originalItemData);
            }
            // finishDragging で visibility がリセットされる
        }
    } else {
        // アルバムからのドラッグの場合
        if (dropTargetSlot) {
            if (dropTargetSlot.classList.contains('setlist-item')) {
                ...
                restoreToOriginalList(draggedElement); // アルバムアイテムを元のアルバムリストに戻す
                return;
            }
            ...
            // アルバムからセットリストに追加する前に、同じ曲がセットリストに既に存在しないか確認
            if (setlist.querySelector(`.setlist-slot.setlist-item[data-item-id="${itemId}"]`)) {
            ...
            fillSlotWithItem(dropTargetSlot, draggedItemData);
            // 元のアルバムアイテムは createTouchDraggedClone (スマホ) または handleDragStart (PC) で既に hidden になっている
            // finishDragging でまとめて visibility がリセットされ、その後 hideSetlistItemsInMenu で制御される
        } else {
...
function handleDrop(event) {
    ...
    let draggedItem;
    // ドロップされたアイテムがセットリストから来たものか、アルバムから来たものかを判断
    if (originalSetlistSlot && originalSetlistSlot.dataset.itemId === droppedItemId) {
        // セットリストからのドラッグの場合、originalSetlistSlot がそのまま draggedItem
    ...
    if (dropTargetSlot) {
        ...
        console.warn("[handleDrop] Dropped outside a setlist slot. Restoring to original list.");
        // processDrop が既に元のアイテムの復元ロジックを持っているので、そちらに任せる
        // セットリストからドラッグした場合は元の位置に戻り、アルバムからドラッグした場合はアルバムに戻る
        processDrop(draggedItem, null, originalSetlistSlot); // dropTargetSlot を null で渡す
    }
    finishDragging();
}
...
function handleTouchStart(event) {
    ...
    // チェックボックスクリックの場合、ネイティブ動作を許可し、ドラッグ処理はスキップ
    if (isCheckboxClick) {
    ...
    // ドラッグハンドルに触れていない場合は、スクロールを許可するためにここで終了
    // (ダブルタップ検出は後続のロジックで対応)
    if (!isDragHandleTouch) {
        console.log("[touchstart:Mobile] Not drag handle. Allowing native behavior (scroll).");
        // ダブルタップ検出ロジックはドラッグハンドル以外でも機能させる
    ...
    // ここからはドラッグハンドルに触れた場合の処理
    event.preventDefault(); // ドラッグハンドルに触れたら即座にスクロールをブロック

    // ダブルタップ検出ロジック（ドラッグハンドルに触れた場合）
    if (tapLength < 300 && tapLength > 0) {
        if (touchTimeout) {
            clearTimeout(touchTimeout);
            touchTimeout = null;
    ...
    if (event.touches.length === 1 && isDragHandleTouch) {
        ...
        if (!touchedElement) {
            console.warn("[touchstart:Mobile] Drag handle touched, but no draggable item parent found.");
            return;
        }
        console.log("[touchstart:Mobile] Drag handle touched. Initializing potential drag.");

        isDragging = false; // まだ移動がないのでfalse
...
function handleTouchMove(event) {
    ...
    if (!isDragging) {
        // draggingItemId が設定されていない（touchstartでドラッグ対象を認識していない）
        // またはタッチがドラッグハンドルでなかった場合は、ここでは何もしない。
        // （touchstartでpreventDefault()が呼ばれていない可能性があるため、スクロールを妨げない）
        if (!draggingItemId) {
            return; 
        }
        ...
        if (distance > DRAG_THRESHOLD) {
            ...
            if (!touchedElement) {
                 console.warn("[handleTouchMove] Drag threshold met, but initial touched element not found.");
                 draggingItemId = null; // エラー状態としてリセット
                 return;
            }
            // ここで初めて isDragging を true に設定
            isDragging = true;
            createTouchDraggedClone(touchedElement, initialTouchX, initialTouchY, draggingItemId);
            console.log("[handleTouchMove] Dragging started due to movement threshold (from drag handle).");
            event.preventDefault(); // ドラッグが開始されたらスクロールをブロック
        } else {
            // しきい値に達していないが、touchstartでpreventDefault()が呼ばれている可能性があるので、
            // ここでもスクロールを許容しないようにpreventDefault()を呼び出す。
            event.preventDefault(); 
            return;
        }
    }
    ...
    if (isDragging && currentTouchDraggedClone) {
        ...
        rafId = requestAnimationFrame(() => {
            ...
            // 元のスロットの上にドラッグオーバーした場合はハイライトしない
...
// enableDragAndDrop 内の touchstart リスナーも更新
// passive: false はそのまま維持してください
/*
    element.addEventListener("touchstart", handleTouchStart, { passive: false });
    element.addEventListener("touchmove", handleTouchMove, { passive: false }); // ここも passive: false が必要
*/
...
function createTouchDraggedClone(originalElement, initialX, initialY, itemIdToClone) {
    ...
    if (setlist.contains(originalElement) && originalElement.classList.contains('setlist-item')) {
        ...
        originalSetlistSlot.classList.add('placeholder-slot');
        originalElement.style.visibility = 'hidden'; // 元のセットリストアイテムを隠す
...
function finishDragging() {
    ...
    if (originalSetlistSlot && originalSetlistSlot.classList.contains('placeholder-slot')) {
        ...
        // ✅ 修正: originalSetlistSlot がアイテムを持っている場合のみ visibility を戻す
        if (originalSetlistSlot.classList.contains('setlist-item')) {
            ...
            // アイテムがクリアされている（移動または削除の場合）
            originalSetlistSlot.style.visibility = ''; // clearSlotContent で既に設定済みだが念のため
    ...
    // ✅ 修正: 全てのアルバムアイテムの visibility を一旦リセット
    // これにより、一時的に隠されていたアルバムアイテムが確実に表示状態に戻る
    document.querySelectorAll('.album-content .item').forEach(item => {
        item.style.visibility = '';
    });
    console.log("[finishDragging] All album items' visibility reset.");
    ...
    originalSetlistSlot = null; // 必ずリセット
 ...
 * @param {Event} event - イベントオブジェクト
 */
// handleDoubleClick 関数
function handleDoubleClick(event) {
    // isDragging が true の場合は、ドラッグ操作中なのでダブルクリック処理をスキップ
    if (isDragging) {
        console.log("[handleDoubleClick] Dragging is active, skipping double click.");
        // finishDraggingはドラッグ終了時に呼ばれるので、ここでは呼ばない
        return;
    }
    ...
    if (!item) {
        ...
        // finishDragging(); // ここでは呼ばない。isDraggingがfalseなので、通常のクリックイベントと競合しない
        return;
    }
    ...
    if (isInsideSetlist) {
        ...
        if (!setlist.querySelector(`.setlist-slot.setlist-item[data-item-id="${item.dataset.itemId}"]`)) {
            ...
            item.style.visibility = 'hidden'; // 元のアルバムアイテムを非表示にする
    ...
    // ダブルクリック処理が完了したら、isDraggingがtrueになることはないので、ここでfinishDraggingを呼ばない
    // finishDraggingはドラッグ終了時にのみ呼び出されるべき
}
