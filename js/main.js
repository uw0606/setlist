...
function handleTouchStart(event) {
    ...
    if (tapLength < 300 && tapLength > 0) {
        ...
        event.preventDefault(); // ブラウザのズームやスクロールを防ぐ
        event.stopPropagation(); // イベントのバブリングを停止 (重要)

        if (isInSetlist) {
            // ✅ セットリスト内のアイテムをダブルタップ -> ハンバーガーメニューへ戻す (モバイル)
            // この挙動は維持したいとのことなので、変更なし
            console.log("[touchstart] Double tap on setlist item. Moving to menu as requested.");
            restoreToOriginalList(touchedElement, false);
            showMessageBox('曲をセットリストから削除しました。');
        } else if (isInAlbumContent) {
            // ✅ ハンバーガーメニュー内のアイテムをダブルタップ -> セットリストへ追加 (モバイル)
    ...
    // --- 2. シングルタッチからの長押し（ドラッグ開始）処理 (ダブルタップでなかった場合のみ実行) ---
        ...
        if (isInSetlist || isInAlbumContent) { // いずれかの draggable item であればタイマーを設定
            ...
            touchTimeout = setTimeout(() => {
                ...
                if (!isDragging && draggingItemId && document.body.contains(initialTouchElement)) {
                    ...
                    // ドラッグ開始後も event.preventDefault() を継続的に呼び出すのは handleTouchMove に任せる
                } else {
