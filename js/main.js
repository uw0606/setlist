...
async function generateSetlistPdf() {
    ...
    const tableBody = [];
    // シンプルPDFのデータ格納形式をオブジェクトに変更 (曲かテキストか判別するため)
    const simplePdfBody = []; 
    ...
    for (const slot of setlistSlots) {
        if (slot.classList.contains('setlist-item')) {
            ...
            // シンプルPDFの行揃えロジック
            const basePadding = '    '; 
            let simpleDisplayLine = '';

            if (isAlbum1) {
                const extraPadding = '  '; 
                simpleDisplayLine = `${basePadding}${extraPadding}${titleText}${simpleEffectNote}`;
            } else {
                const currentNo = currentItemNoSimple++;
                const noString = `${currentNo}.`;
                const spaceCount = (currentNo < 10) ? '  ' : ' '; 
                simpleDisplayLine = `${noString}${spaceCount}${titleText}${simpleEffectNote}`;
            }
            // オブジェクトとして保存 (曲かどうかを判定するため)
            simplePdfBody.push({ text: simpleDisplayLine, isItem: true });
            ...
            if (textContent) {
                ...
                // オブジェクトとして保存 (曲ではないと判定するため)
                simplePdfBody.push({ text: textContent, isItem: false }); 
    ...
    try {
        ...
        // --- 2. シンプルなセットリストPDFの生成 (文字の縮小とマージン調整) ---
        ...
        // ★★★ 修正3: シンプルPDFのボトムマージンを20mmに設定 ★★★
        const simpleBottomMargin = 20; 
        const pageHeightSimple = simplePdf.internal.pageSize.getHeight(); 
        const pageWidthSimple = simplePdf.internal.pageSize.getWidth();

        // 描画可能幅 (左右20mmマージンを使用)
        const availableWidth = pageWidthSimple - simpleLeftMargin - simpleLeftMargin; // rightMarginもleftMarginと同じ20mmを使用

        const MAX_SONGS_PER_PAGE = 25; 
        const MIN_FONT_SIZE = 20; 
        
        // 行間計算で使用するボトムマージンは20mm
        const bottomMarginToUse = simpleBottomMargin;

        let simpleHeaderHeight = 0;
        if (headerText) {
            simpleHeaderHeight = (BASE_FONT_SIZE + 4) * 0.38 + BASE_FONT_SIZE * 0.7;
        }

        const numSimpleRows = simplePdfBody.length;
        let dynamicFontSize = BASE_FONT_SIZE;
        let dynamicLineHeight = BASE_FONT_SIZE * BASE_LINE_HEIGHT_RATIO;


        if (numSimpleRows > 0) {
            // ★★★ 修正4: 全行を考慮して、ボトムマージン20mmを活用したベースとなるフォントサイズと行間を計算 ★★★
            const simpleAvailableHeight = pageHeightSimple - simpleTopMargin - bottomMarginToUse - simpleHeaderHeight;
            const idealLineHeight = simpleAvailableHeight / numSimpleRows;
            const maxCompressionLineHeight = simpleAvailableHeight / MAX_SONGS_PER_PAGE;
            
            // 行間は、理想値と最大圧縮値の小さい方、かつベースの比率で決まる最低ラインより大きく
            dynamicLineHeight = Math.min(idealLineHeight, maxCompressionLineHeight);
            ...
            // 最終的な行間を再計算 (フォントサイズが決定したら、そのフォントサイズの比率で決まる)
            dynamicLineHeight = dynamicFontSize * BASE_LINE_HEIGHT_RATIO;
            
            let simpleYPos = simpleTopMargin;
            
            if (headerText) {
                simplePdf.setFontSize(BASE_FONT_SIZE + 4);
                simplePdf.setFont('NotoSansJP', 'bold');
                simplePdf.text(headerText, simpleLeftMargin, simpleYPos);
                // ヘッダー行の高さ + 行間の調整 (約5mm)
                simpleYPos += (BASE_FONT_SIZE + 4) * 0.38 + 5; 
            } 

            // ★★★ 修正5: 各行のフォントサイズを動的に調整するループ (短い曲はdynamicFontSizeを維持) ★★★
            simplePdfBody.forEach(row => {
                const line = row.text;
                let currentFontSize = dynamicFontSize; 
                
                simplePdf.setFont('NotoSansJP', 'bold');
                
                if (row.isItem) {
                    // 1. 全体の動的フォントサイズを一旦適用し、幅を測定
                    simplePdf.setFontSize(dynamicFontSize);
                    // getStringUnitWidthは単位ptなので、mmに変換するために (fontSize / scaleFactor)をかける
                    let textWidth = simplePdf.getStringUnitWidth(line) * dynamicFontSize / simplePdf.internal.scaleFactor;

                    // 2. 描画可能幅を超えているかチェック
                    if (textWidth > availableWidth) {
                        // 3. 収まるように必要な縮小率を計算
                        const scaleFactor = availableWidth / textWidth;
                        currentFontSize = dynamicFontSize * scaleFactor;
                        currentFontSize = Math.max(MIN_FONT_SIZE, currentFontSize); // 最小サイズは保証
                        
                        // 4. 縮小後のフォントサイズを適用して描画
                        simplePdf.setFontSize(currentFontSize);
                    } else {
                        // 収まっている場合は、動的ベースサイズをそのまま使用
                        simplePdf.setFontSize(dynamicFontSize);
                    }
                } else {
                    // テキストスロットは全体の動的フォントサイズをそのまま使用
                    simplePdf.setFontSize(dynamicFontSize);
                }
                
                simplePdf.text(line, simpleLeftMargin, simpleYPos);
                simpleYPos += dynamicLineHeight; // 行間は、全行が均等に収まるように計算されたdynamicLineHeightを使用
            });
        } else {
            let simpleYPos = simpleTopMargin;
            if (headerText) {
