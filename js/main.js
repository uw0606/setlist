...
function clearSlotContent(parentList, slotIndex) {
    ...
    if (slotToClear) {
        ...
        // データ属性を全て削除
        for (const key in slotToClear.dataset) {
            delete slotToClear.dataset[key];
        }
        slotToClear.dataset.slotIndex = slotIndex; // slotIndexは保持

        slotToClear.style.cssText = ''; // インラインスタイルをリセット
        slotToClear.style.visibility = ''; // 可視性をリセット

        // イベントリスナーを削除 (fillSlotWithItemで再追加されるため)
        ...
        delete slotToClear._originalItemData; // 内部データもクリア
...
/**
 * ドロップ処理の中核ロジック
 * @param {Element} draggedElement - ドラッグされた元の要素またはそのクローン
 * @param {Element|null} dropTargetSlot - ドロップされた先のセットリストスロット要素、またはnull (セットリスト外にドロップされた場合)
 * @param {Element|null} originalSetlistSlot - セットリストからドラッグされた場合の元のスロット要素、それ以外はnull
 */
function processDrop(draggedElement, dropTargetSlot, originalSetlistSlot = null) {
    ...
    // ドラッグされたアイテムのデータを取得
    let draggedItemData = originalSetlistSlot && originalSetlistSlot._originalItemData ?
        originalSetlistSlot._originalItemData :
        getSlotItemData(draggedElement);
    ...
    if (isDraggedFromSetlist) { // セットリストからのドラッグ
        if (dropTargetSlot) { // セットリスト内のスロットにドロップ
        ...
        } else { // セットリスト外にドロップ（セットリストから削除し、アルバムに戻す）
            console.log("[processDrop] Dropped outside setlist (from setlist). Clearing original slot and restoring to album.");
            clearSlotContent(setlist, originalSetlistSlot.dataset.slotIndex);
            restoreToOriginalList(draggedItemData); // ★修正点4: draggedItemData を渡す
        }
    } else { // アルバムからのドラッグ
        if (dropTargetSlot) { // セットリスト内のスロットにドロップ
            if (dropTargetSlot.classList.contains('setlist-item')) {
                showMessageBox('このスロットはすでに埋まっています。');
                // アルバムからのドラッグで埋まっているスロットにドロップした場合、元のアルバムアイテムを再表示
                restoreToOriginalList(draggedItemData); // ★修正点5: draggedItemData を渡す
                return;
            }
            ...
            if (currentSongCount >= maxSongs) {
                ...
                restoreToOriginalList(draggedItemData); // ★修正点6: draggedItemData を渡す
                return;
            }
            ...
            fillSlotWithItem(dropTargetSlot, draggedItemData);
            // アルバム内の元のアイテムは createTouchDraggedClone で既に隠されているので、ここでは何もしない
            // hideSetlistItemsInMenu() が finishDragging で呼ばれるので、そこで適切に処理される
        } else { // セットリスト外にドロップ（アルバムに戻す、または何もしない）
            console.log("[processDrop] Dropped album item outside setlist. Restoring to original list (if applicable).");
            restoreToOriginalList(draggedItemData); // ★修正点7: draggedItemData を渡す
        }
    }
}
...
function handleTouchStart(event) {
    ...
    if (event.touches.length === 1) {
        ...
        // ★修正点1: アルバムアイテムからのタッチドラッグも開始できるよう、無条件でpreventDefaultせず、
        // 長押しドラッグ準備を開始する
        // ここではまだpreventDefaultしないことで、初期のスクロールは可能にする
        // ドラッグが開始されたらhandleTouchMoveでpreventDefaultする

        isDragging = false; // 初期状態ではドラッグではない
        draggingItemId = touchedElement.dataset.itemId;

        // originalSetlistSlot の設定はそのまま
        if (setlist.contains(touchedElement) && touchedElement.classList.contains('setlist-item')) {
            ...
            currentPcDraggedElement = touchedElement; // アルバムアイテムを保持
        }
...
function handleTouchMove(event) {
    ...
    // ★修正点2: isDraggingがtrueの場合のみpreventDefaultを呼び出す
    if (isDragging) {
...
function handleTouchEnd(event) {
    ...
    const deltaX = Math.abs(currentX - touchStartX);
    const deltaY = Math.abs(currentY - touchStartY);
    // ★修正点3: ドラッグとみなす最小移動距離を少し緩和（15pxから10pxに）
    const dragThreshold = 10;

    // isDragging が true だが、指の移動が閾値未満の場合は、ドラッグ操作ではないと判断
    // この場合、`processDrop` を呼び出さず、単にクリーンアップだけを行う
    if (isDragging && (deltaX < dragThreshold && deltaY < dragThreshold)) {
        console.log("[touchend] Drag initiated but finger moved minimally. Treating as long-press tap, not a drag drop. Performing cleanup.");
        finishDragging(); // クリーンアップのみ
        return;
    }

    // isDragging が false の場合（ロングプレスにならなかった場合）
    if (!isDragging) {
...
function finishDragging() {
    ...
    if (currentPcDraggedElement) {
        ...
        // ★修正点4: originalSetlistSlot ではない、かつアルバムアイテムの場合にのみ visibility を戻す
        // originalSetlistSlot は自身が setlist-item であり、そこで visibility を扱うため
        if (!originalSetlistSlot || currentPcDraggedElement !== originalSetlistSlot) { // originalSetlistSlot と重複しない場合
             currentPcDraggedElement.style.visibility = ''; // PCドラッグの元の要素（アルバムアイテム）の表示を戻す
             console.log(`[finishDragging] Restored visibility for PC album item: ${currentPcDraggedElement.dataset.itemId || 'N/A'}`);
        }
    }
    ...
    if (originalSetlistSlot) {
        ...
        // originalSetlistSlot の visibility は、そのスロットがアイテムを保持しているかどうかに応じて
        // clearSlotContent または fillSlotWithItem によって適切に制御されるべき。
        // ここで無条件に `visibility = ''` にすると、移動後のスロットが隠れたままになる可能性があるので注意。
        // もし、セットリスト内の要素が移動後に「隠れたまま」になる問題があれば、ここに `originalSetlistSlot.style.visibility = '';` を追加。
        // しかし、`clearSlotContent` と `fillSlotWithItem` で処理されているはずなので、通常は不要。
        // もし移動元がクリアされずに隠れたままなら `originalSetlistSlot.style.visibility = '';` が必要になる。
        // 現在の `processDrop` では `clearSlotContent` が呼ばれるため、そちらでリセットされます。
        console.log(`[finishDragging] originalSetlistSlot ${originalSetlistSlot.dataset.slotIndex} state handled by processDrop/clearSlotContent.`);
        delete originalSetlistSlot._originalItemData;
    }

    // 全てのスロットからドラッグ関連クラスを削除
    ...
    currentPcDraggedElement = null; // ★リセットを確実に行う
    draggingItemId = null;
    isDragging = false;
    originalSetlistSlot = null;
    ...
    hideSetlistItemsInMenu(); // アルバムメニューのアイテム表示を更新
