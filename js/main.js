...
function shareSetlist() {
  // Firebase が利用可能か確認（ここで `database` が定義されている必要があります）
    ...
    .then(() => {
        ...
        if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(shareLink)
                .then(() => {
                    // ★ 修正点: クリップボードコピー成功時にメッセージを表示
                    showMessageBox('共有リンクをクリップボードにコピーしました！');
                    console.log(`[shareSetlist] Setlist saved. Share ID: ${shareId}, Link: ${shareLink}`);
                })
                .catch(err => {
                    console.error('[shareSetlist] クリップボードへのコピーに失敗しました:', err);
                    showMessageBox('共有リンクのコピーに失敗しました。');
                });
        } else {
            // フォールバック: navigator.clipboard が使えない場合
            ...
            // ★ 修正点: フォールバック時にもメッセージを表示
            showMessageBox('共有リンクをクリップボードにコピーしました！ (旧形式)');
