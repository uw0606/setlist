...
function handleTouchStart(event) {
    ...
    // ドラッグできるのは「.item」クラスの要素、または「.setlist-slot.setlist-item」クラスの要素のみ
    const touchedElement = event.target.closest(".item"); // アルバムアイテムの場合
    let isSetlistItem = false;

    // セットリスト内のスロットを触った場合、それが 'setlist-item' を持つ曲か確認
    if (!touchedElement) { // .item で見つからなかった場合、セットリストスロットの可能性をチェック
        const slot = event.target.closest(".setlist-slot");
        if (slot && slot.classList.contains('setlist-item')) {
            isSetlistItem = true;
            touchedElement = slot; // ドラッグ対象をこのセットリストアイテムスロットにする
        }
    }

    // ドラッグ対象（.item または .setlist-slot.setlist-item）ではない要素の場合、処理を終了
    if (!touchedElement || (!touchedElement.classList.contains('item') && !isSetlistItem)) {
        console.log("[touchstart:Mobile] Touched non-draggable/empty element. Allowing default behavior (e.g., scrolling).");
        return; // event.preventDefault() を呼ばない
    }
    ...
    if (event.touches.length === 1) {
        ...
        if (isSetlistItem) { // isSetlistItem が true なら、それはセットリスト内の曲
        ...
        touchTimeout = setTimeout(() => {
            ...
            if (elementToClone && draggingItemId) { // draggingItemId があることを改めて確認
