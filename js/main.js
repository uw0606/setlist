...
document.addEventListener("dblclick", (event) => {
  ...
  if (!!event.target.closest("#setlist")) {
    ...
    if (originalList && itemToRestore) {
      ...
      if (songInfo) {
        // ★ 追加：インラインスタイルを削除
        songInfo.removeAttribute('style');
      ...
      .then((snapshot) => {
        ...
        if (state && state.setlist) {
          ...
          state.setlist.forEach(itemData => {
            ...
            li.draggable = true; // ★ ここで draggable 属性を明示的に設定
            ...
            li.addEventListener("dragstart", (event) => {
              ...
              if (draggedItem) {
                ...
                if (!originalAlbumMap.has(draggingItemId)) {
                  ...
                  const originalListId = originalList ? originalList.id : null;
                  originalAlbumMap.set(draggingItemId, originalListId);
                  console.log("dragStart (shared) - itemId:", draggingItemId, "originalListId (newly set):", originalListId);
                } else {
                  console.log("dragStart (shared) - itemId:", draggingItemId, "originalListId (already exists):", originalAlbumMap.get(draggingItemId));
                }
            ...
            li.addEventListener("dragend", finishDragging);
          });
          // ★ セットリスト全体への dragover と drop のリスナーは一度だけ設定すれば良い
          if (!setlist.hasDragListeners) {
            setlist.addEventListener("dragover", handleDragOver);
            setlist.addEventListener("drop", handleDrop);
            setlist.hasDragListeners = true;
          }
        } else {
