...
function getSlotItemData(element) {
    ...
    let isOriginalShortVersion = false;
    ...
    if (isSetlistItem) {
        ...
        isCheckedShort = checkbox ? checkbox.checked : false;
        isOriginalShortVersion = element.dataset.isShortVersion === 'true';
    } else if (isAlbumItem) {
        songName = element.dataset.songName || element.textContent.trim();
        isOriginalShortVersion = element.classList.contains('short');
        isCheckedShort = false; 
    } else if (element.dataset.itemId) {
    ...
    return {
        ...
        isShortVersion: isOriginalShortVersion,
...
function clearSlotContent(parentList, slotIndex) {
    ...
    if (slotToClear) {
        ...
        slotToClear.classList.remove('setlist-item', 'item', 'short', 'placeholder-slot'); 
...
function fillSlotWithItem(slotElement, songData) {
  ...
  Array.from(slotElement.classList).forEach(cls => {
    if (cls.startsWith('album') || cls === 'setlist-item' || cls === 'item' || cls === 'short') {
      slotElement.classList.remove(cls);
    }
  });

  const shouldShowShortOption = songData.isShortVersion === true;
  ...
  slotElement.dataset.short = isCurrentlyChecked ? 'true' : 'false';
...
function processDrop(draggedElement, dropTargetSlot, originalSetlistSlot = null) {
    ...
    if (!itemId) {
        console.warn("[processDrop] No item ID found on dragged element. Aborting drop.");
        return;
    }

    let draggedItemData = getSlotItemData(draggedElement); 
    if (!draggedItemData || !draggedItemData.name) { 
        const originalAlbumItem = document.querySelector(`.album-content .item[data-item-id="${itemId}"]`);
        if (originalAlbumItem) {
            draggedItemData = getSlotItemData(originalAlbumItem); 
            ...
            console.error("[processDrop] Could not determine dragged item data. Aborting drop.");
            return;
        }
    }
    console.log("[processDrop] draggedItemData:", draggedItemData);

    const isDraggedFromSetlist = originalSetlistSlot && originalSetlistSlot._originalItemData && originalSetlistSlot._originalItemData.itemId === itemId;
    ...
    if (isDraggedFromSetlist) {
        if (dropTargetSlot) {
            ...
            if (targetSlotIndex === originalSlotIndex) {
                console.log("[processDrop] Dropped onto the same original slot. Restoring original visibility.");
                return; 
            }
            
            if (dropTargetSlot.classList.contains('setlist-item')) {
                const itemDataToSwap = getSlotItemData(dropTargetSlot); 
                console.log(`[processDrop] Swapping items. Original slot: ${originalSlotIndex}, Target slot: ${targetSlotIndex}`);
                fillSlotWithItem(dropTargetSlot, draggedItemData);
                fillSlotWithItem(originalSetlistSlot, itemDataToSwap);
            } else {
                console.log(`[processDrop] Moving item. Original slot: ${originalSlotIndex}, Target slot: ${targetSlotIndex}`);
                fillSlotWithItem(dropTargetSlot, draggedItemData);
                clearSlotContent(setlist, originalSlotIndex);
                console.log(`[processDrop] Original slot ${originalSlotIndex} content cleared by clearSlotContent.`);
            }
        } else {
            console.log("[processDrop] Dropped outside setlist. Restoring to original list.");
...
function handleTouchStart(event) {
    ...
    // ✅ クリックされた要素がチェックボックスであるかを判定する新しいロジック
    // event.target またはその最も近い祖先に input[type="checkbox"] が存在するかをチェック
    const closestCheckbox = event.target.closest('input[type="checkbox"]');
    const isCheckboxClick = closestCheckbox !== null;

    if (isCheckboxClick) { // ✅ チェックボックスがタッチされた場合
        ...
        // ここでは event.preventDefault() を**呼ばない**ことで、ネイティブなチェックボックスの動作を許可
        return; // これ以上、タッチ開始のロジックは実行しない
    }
    ...
    if (event.touches.length === 1) {
        ...
        // ✅ チェックボックスでなければ、デフォルト動作をキャンセルしてドラッグ処理へ進む
        event.preventDefault(); 
        ...
        touchTimeout = setTimeout(() => {
            if (!isDragging && draggingItemId && document.body.contains(touchedElement)) {
                ...
                } else if (touchedElement.classList.contains('item') && touchedElement.parentNode.classList.contains('album-content')) {
                    touchedElement.style.visibility = 'hidden'; 
                }
            } else {
...
function handleTouchEnd(event) {
    // ✅ タッチ終了時の要素がチェックボックスかどうかを判定
    // event.target.closest() を使って、より確実にチェックボックス要素を特定します
    const closestCheckbox = event.target.closest('input[type="checkbox"]');
    const isCheckboxClick = closestCheckbox !== null;

    // ✅ チェックボックスのタップでなければ event.preventDefault() を呼ぶ
    // これにより、チェックボックスのネイティブなクリック動作を妨げません
    if (!isCheckboxClick) {
        event.preventDefault();
    }
    ...
    // isDragging が false の場合でも、それがチェックボックスのクリックであれば
    // finishDragging() を呼ばないようにします。
    // finishDragging() はドラッグ操作が完了したときにのみ呼ぶべきです。
    if (!isDragging) {
        if (!isCheckboxClick) { // ✅ チェックボックスのクリックでなければ finishDragging を呼ぶ
            console.log("[touchend] Not dragging, finishing up.");
            finishDragging();
        } else {
...
function shareSetlist() {
  ...
  const setlistRef = database.ref('setlists').push(); // database は Firebase の初期化後に利用可能
    ...
    .then(() => {
        ...
        if (setlistItems.length > 0) {
            songListText = Array.from(setlistItems).map((slot, index) => {
                const songName = getSlotItemData(slot)?.name || ''; 
                const checkbox = slot.querySelector("input[type='checkbox']");
                return checkbox && checkbox.checked ? ` ${index + 1}. ${songName} (Short)` : ` ${index + 1}. ${songName}`;
            }).join("\n");
        }
  ...
  setlist.querySelectorAll(".setlist-slot").forEach((slot, index) => {
    ...
    slot.addEventListener('click', (e) => {
        ...
        if (checkbox) {
            ...
            // e.preventDefault(); // この行を削除またはコメントアウトします
            e.stopPropagation(); // チェックボックスのクリックがスロット全体に伝播するのを防ぎます
            
            // checkbox.checked = !checkbox.checked; // この行は削除します。ブラウザが自動でトグルします。
            
            // ブラウザがチェックボックスの状態を更新した後に、その状態に基づいてクラスとdatasetを更新
            // 注意: このイベントリスナーはクリックされた '後' に発火するので、
            // checkbox.checked は既に新しい状態を反映しています。
    ...
    // これにより、document全体ではなく、スロット内でのダブルクリックのみが処理されます
