...
function handleTouchStart(event) {
    ...
    if (tapLength < 300 && tapLength > 0) {
        ...
        // ダブルタップ時は preventDefault を呼び出し、スクロールなどを防ぐ
        event.preventDefault(); // ★追加: ダブルタップ時にはデフォルト動作を防止
        return;
    }
    ...
    if (event.touches.length === 1) {
        ...
        // ★修正: タッチ開始地点の座標を記録 (移動量判定のため)
        initialTouchX = event.touches[0].clientX;
        initialTouchY = event.touches[0].clientY;

        // ★修正: クローン作成時のオフセットは、実際にドラッグ開始する際に設定
        // touchStartX と touchStartY は handleTouchMove でクローン作成時に設定するように変更
        touchStartX = 0; // 初期化
        touchStartY = 0; // 初期化
