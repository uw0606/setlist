...
async function generateSetlistPdf() {
    ...
    try {
        ...
        // ★ フッターの描画ロジックの追加 ★
        // ===================================
        const footerText = "Generated by Setlist Maker (C) 2024"; // 任意のフッターテキスト
        const footerFontSize = 10; // フッターのフォントサイズ
        const footerBottomMargin = 30; // ページ下端からのマージン(mm)

        simplePdf.setFontSize(footerFontSize);
        simplePdf.setFont('NotoSansJP', 'normal'); // フッターは通常フォント
        
        // X座標: ページ幅の中心 (中央揃え)
        const footerXPos = pageWidthSimple / 2;
        
        // Y座標: ページ高から下マージンを引いた位置
