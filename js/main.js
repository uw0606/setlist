...
function handleTouchStart(event) {
    ...
    // ★重要: ドラッグ可能な要素がタッチされたら、まずデフォルトのスクロール動作を防ぐ
    // ただし、ダブルタップの場合は後続の処理でpreventDefaultが呼ばれるため、ここでは条件を調整
    // シングルタップ（長押し）の可能性が高い場合にのみpreventDefaultを呼ぶ
    if (event.touches.length === 1 && (isInSetlist || isInAlbumContent)) {
        // ダブルタップの可能性がある場合は、一旦preventDefaultしない
        // ダブルタップ処理内で改めてpreventDefaultする
        if (!(tapLength < 300 && tapLength > 0)) { // ダブルタップではない場合
             event.preventDefault(); // ここでスクロールを防ぐ
             console.log("[touchstart] Prevented default for potential drag.");
        }
    }
    ...
    if (tapLength < 300 && tapLength > 0) {
        ...
        // ダブルタップの場合は、ここで確実にpreventDefault()とstopPropagation()を呼ぶ
        event.preventDefault(); 
        ...
        if (isInSetlist || isInAlbumContent) {
            ...
            touchTimeout = setTimeout(() => {
                if (!isDragging && draggingItemId && document.body.contains(initialTouchElement)) {
                    ...
                    // ここでイベントのデフォルト動作を防ぎ続けるのは handleTouchMove に任せる
                } else {
...
function handleTouchMove(event) {
    ...
    // `rafId` の管理は `requestAnimationFrame` の呼び出し元で行う
    // ここで `cancelAnimationFrame` と `requestAnimationFrame` を直接入れ子にすると複雑になる
    // 通常、moveイベントごとに新しいRAFを要求し、古いものをキャンセルする形にする
    if (rafId !== null) {
        cancelAnimationFrame(rafId);
    }
    ...
    rafId = requestAnimationFrame(() => {
        ...
        // ★isDraggingがfalseの場合の処理を見直し。
        // ここでpreventDefault()を呼ばないことでスクロールを許可しているが、
        // ドラッグ開始を意図している場合はhandleTouchStartで早めにpreventDefault()すべき。
        // isDraggingがfalseの段階での閾値チェックは、ロングプレスをキャンセルする目的。
        if (!isDragging) {
            if (deltaX > DRAG_THRESHOLD || deltaY > DRAG_THRESHOLD) {
                ...
                // ここで `event.preventDefault()` を呼ばないことで、ブラウザはスクロールを継続できる。
                // ただし、handleTouchStartで既にpreventDefaultされている場合は、ここは影響しない。
            }
            rafId = null; // RAFをクリア
            return; // ドラッグが始まっていないので、これ以上処理しない
        }

        // isDraggingがtrueになったら、常にデフォルト動作を抑制し、ドラッグを優先する
        // ★このpreventDefault()は重要。handleTouchStartで既に呼ばれていることを前提としつつ、
        // move中も継続して呼ばれることでスクロールを抑制し続ける。
        // ただし、イベントリスナー側でpreventDefault()する方が確実。
        event.preventDefault(); // ここで呼ばれることを確認
...
function handleTouchEnd(event) {
    ...
    if (isDragging && currentTouchDraggedClone) { 
        ...
        console.log("[touchEnd] No active drag, cleaning up.");
        // ドラッグが開始されていなかった場合のクリーンアップ
        if (currentTouchDraggedClone) {
            currentTouchDraggedClone.remove();
            currentTouchDraggedClone = null;
        }
        if (originalSetlistSlot) {
             // originalSetlistSlot をリセット
             originalSetlistSlot = null;
        }
        if (currentDropZone) {
            currentDropZone.classList.remove('drag-over');
    ...
    initialTouchElement = null; // タッチ開始要素もリセット
}
