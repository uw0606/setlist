...
// 🎸 R.Gt / L.Gt のチューニングオプション
const customGtTuningOptions = [
    { value: '', label: 'R.Gt / L.Gt' }, // 元の表示はプルダウンの初期表示に設定（データ値は空）
    { value: 'REG', label: 'REG' },
...
const customChorusOptions = [
    { value: 'false', label: '-----' }, // 元の表示はプルダウンの初期表示に設定
    { value: '克・信', label: '克・信' }, 
    { value: '克', label: '克' },
    { value: '信', label: '信' }
];
...
function generateBpmOptions() {
    const options = [{ value: '', label: '-----' }]; // 元の表示はプルダウンの初期表示に設定
...
async function generateSetlistPdf() {
    ...
    for (const slot of setlistSlots) {
        if (slot.classList.contains('setlist-item')) {
            ...
            // ★★★ 修正箇所: シンプルPDFの曲名始まりを揃える（共鳴曲の前にさらに空白追加） ★★★
            // No.ありの曲（例: '1.  ' や '10. '）の曲名が始まる位置までの空白
            const basePadding = '    '; 

            if (isAlbum1) {
                // No.なしの曲（共鳴曲）の場合: ベースの空白に加えてさらに2スペースを追加
                const extraPadding = '  '; 
                simplePdfBody.push(`${basePadding}${extraPadding}${titleText}${simpleEffectNote}`);
            } else {
                // No.ありの曲の場合: No.と区切りをつけ、残りスペースを調整して曲名を揃える
                const currentNo = currentItemNoSimple++;
                const noString = `${currentNo}.`;
                // 1桁No.ならスペース2個 ('1.  '), 2桁No.ならスペース1個 ('10. ')
                const spaceCount = (currentNo < 10) ? '  ' : ' '; 
                simplePdfBody.push(`${noString}${spaceCount}${titleText}${simpleEffectNote}`);
            }

            // ★★★ 修正箇所: 共有テキストの曲名始まりを揃える（共鳴曲の前にさらに空白追加） ★★★
            if (isAlbum1) {
                // 共有テキストも同様に、ベースの4スペースに2スペース追加（合計6スペース）
                const extraPadding = '  ';
    ...
    try {
        ...
        detailedPdf.autoTable({
            ...
            // No.とタイトルはfontSizeを11に上書きし、その他は8を継承
            columnStyles: {
                0: { cellWidth: 10, halign: 'center', fontSize: 11 },   // No. (大きく)
                1: { cellWidth: 72, halign: 'left', fontSize: 11 },     // タイトル (大きく)
        ...
        const simpleLineHeightBase = simpleFontSize * 0.38; // 基準となる行間 (pt -> mm 換算)
        const simpleTopMargin = 30;
        const simpleLeftMargin = 20;
        // ★★★ 修正1: シンプルPDFのボトムマージンを 25mm に変更 ★★★
        const simpleBottomMargin = 20; 
        const pageHeightSimple = simplePdf.internal.pageSize.getHeight(); // ページの高さを再取得

        let simpleHeaderHeight = 0;
        if (headerText) {
            // ヘッダーテキスト行の高さ (mm)
            simpleHeaderHeight = (simpleFontSize + 4) * 0.38 + simpleFontSize * 0.7;
        }

        const numSimpleRows = simplePdfBody.length;
        if (numSimpleRows > 0) {
            // ★★★ 修正2: シンプルPDFの行間調整のための計算 (下部マージン 25mm 適用) ★★★
            // 下部マージン (25mm) を考慮して、利用可能な高さを算出
            const simpleAvailableHeight = pageHeightSimple - simpleTopMargin - simpleBottomMargin - simpleHeaderHeight;
            
            // 行数で割って均等な行間を確保。最小の行間 (simpleLineHeightBase) を保証。
            const dynamicLineHeight = Math.max(simpleLineHeightBase, simpleAvailableHeight / numSimpleRows);
...
function updateSlotContent(slotElement, songName, options) {
    ...
    if (slotElement.dataset.itemId === 'album1-custom') {
        ...
        // 🚨 前提: customGtTuningOptions, customBassTuningOptions, customBpmOptions, customChorusOptions は外部で定義済み
        
        // チューニング、BPM、コーラスのプルダウンを追加
        itemOptions.appendChild(createCustomSelect('rGt', customGtTuningOptions));
        itemOptions.appendChild(createCustomSelect('lGt', customGtTuningOptions));
    ...
    // options.rGt, options.lGt, options.bass, options.bpm は初期値 (''または 'false') 以外が選択されている場合に表示
    if (options.rGt && options.rGt !== customGtTuningOptions[0].value) infoParts.push(`R.Gt: ${options.rGt}`);
    if (options.lGt && options.lGt !== customGtTuningOptions[0].value) infoParts.push(`L.Gt: ${options.lGt}`);
