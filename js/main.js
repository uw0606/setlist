...
// main.js の handleDragStart 関数
function handleDragStart(e) {
    if (isTouchDevice && currentTouchDragElement) {
        e.preventDefault();
        return;
    }

    const target = e.target.closest('.item, .setlist-item');
    if (!target) return;

    currentPcDraggedElement = target; 
    console.log(`[dragstart:PC] Dragging from ${target.classList.contains('album') ? 'album' : 'setlist'}. Original item ${currentPcDraggedElement.dataset.itemId} is the currentPcDraggedElement.`);

    // ★修正点: effectAllowed を最初に設定する
    e.dataTransfer.effectAllowed = 'move'; 
    // e.dataTransfer.clearData(); は setData の直前でも機能するため、ここでは変更しない

    // ドラッグ元の情報をセット
    if (currentPcDraggedElement.classList.contains('setlist-item')) {
        e.dataTransfer.setData('text/plain', `setlist-slot-${currentPcDraggedElement.dataset.slotIndex}`);
        e.dataTransfer.setData('text/setlist-item-data', JSON.stringify(getSlotItemData(currentPcDraggedElement)));
    } else { // album item
        e.dataTransfer.setData('text/plain', currentPcDraggedElement.dataset.itemId);
        e.dataTransfer.setData('text/album-item-data', JSON.stringify(getSlotItemData(currentPcDraggedElement)));
    }
    console.log(`[dragstart] dataTransfer set with: ${e.dataTransfer.getData('text/plain')}`);
 ...
 * @param {DragEvent} [e] - dragend イベントオブジェクト (存在しない場合は undefined)
 */
function finishDragging(e) {
    // e が DragEvent オブジェクトの場合、その dropEffect を確認してキャンセルされたかを判断
    // e が undefined や boolean などの場合は、デフォルトで false (キャンセルされていない) とみなす
    const wasCancelled = e && e.dataTransfer ? e.dataTransfer.dropEffect === 'none' : false;
    
    console.log(`[finishDragging] Cleanup started. Was cancelled: ${wasCancelled}`); 
    ...
    // すべてのセットリストスロットのpointer-eventsを再評価
    document.querySelectorAll('.setlist-slot').forEach(slot => {
        slot.style.pointerEvents = 'auto';
        slot.style.touchAction = 'pan-y'; 
    });
