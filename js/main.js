...
function clearSlotContent(slotElement) {
    // console.log("[clearSlotContent] Clearing slot:", slotElement.dataset.slotIndex); // 過剰なログは削減

    // スロットが実際にコンテンツを持っているかを確認
    if (!slotElement.classList.contains('setlist-item')) {
        // console.warn(`[clearSlotContent] Attempted to clear an empty slot: ${slotElement.dataset.slotIndex}. No action taken.`); // 過剰なログは削減
    ...
    // イベントリスナーの削除 (DOMContentLoadedで追加されたもの)
    slotElement.removeEventListener('click', handleSlotClick); // handleSlotClickがどこかで定義されているなら
...
function processDrop(draggedElement, dropTargetSlot, originalSourceSlot) {
    ...
    const options = {
        ...
        rGt: draggedElement.dataset.rGt || '', // ★修正: 真偽値ではなく文字列として取得
        lGt: draggedElement.dataset.lGt || '', // ★修正: 真偽値ではなく文字列として取得
        bass: draggedElement.dataset.bass || '', // ★修正: 真偽値ではなく文字列として取得
        bpm: draggedElement.dataset.bpm || '',   // ★修正: 真偽値ではなく文字列として取得
        chorus: draggedElement.dataset.chorus || 'false', // ★修正: 真偽値ではなく文字列として取得 (or 'true'/'false'文字列)
...
function handleDoubleClick(event) {
    ...
    if (albumItemElement) {
        ...
        if (firstEmptySlot) {
            ...
            addSongToSlot(firstEmptySlot, songData.itemId, songData.name, {
                ...
                hasSeOption: songData.hasSeOption,
                drumsoloOption: songData.hasDrumsoloOption,
                rGt: songData.rGt, // ★修正: 文字列として渡す
                lGt: songData.lGt, // ★修正: 文字列として渡す
                bass: songData.bass, // ★修正: 文字列として渡す
                bpm: songData.bpm,   // ★修正: 文字列として渡す
                chorus: songData.chorus, // ★修正: 文字列として渡す
    ...
    if (itemId) {
        const currentSongData = getSlotItemData(setlistItemElement); // 現在の曲のデータを取得
        if (!currentSongData) {
            console.warn("[handleDoubleClick] Could not get song data from setlist item. Aborting.");
            return;
        }

        // isShortVersion, hasSeOption, drumsoloOption は、そのオプションが「存在しうるか」なのでトグルしない
        // short, seChecked, drumsoloChecked は「現在チェックされているか」なのでトグルする
        const togglableOptions = {
            isShortVersion: currentSongData.hasShortOption, // これは元のプロパティなのでトグルしない
            hasSeOption: currentSongData.hasSeOption, // これは元のプロパティなのでトグルしない
            drumsoloOption: currentSongData.hasDrumsoloOption, // これは元のプロパティなのでトグルしない
            rGt: currentSongData.rGt, // ★修正: トグルしない
            lGt: currentSongData.lGt, // ★修正: トグルしない
            bass: currentSongData.bass, // ★修正: トグルしない
            bpm: currentSongData.bpm, // ★修正: トグルしない
            chorus: currentSongData.chorus === 'true' ? 'false' : (currentSongData.chorus ? currentSongData.chorus : 'true'), // コーラスは'true'/'false'でトグル
            
            // チェックボックスの状態をトグル
            short: currentSongData.hasShortOption ? !currentSongData.short : false, // オプションがある場合のみトグル
            seChecked: currentSongData.hasSeOption ? !currentSongData.seChecked : false,
            drumsoloChecked: currentSongData.hasDrumsoloOption ? !currentSongData.drumsoloChecked : false
        };

        // オプション（チェックボックスで制御できるもの）が一つもない場合は、削除処理を行う
        if (!currentSongData.hasShortOption && !currentSongData.hasSeOption && !currentSongData.hasDrumsoloOption && 
            !currentSongData.rGt && !currentSongData.lGt && !currentSongData.bass && !currentSongData.bpm && currentSongData.chorus === 'false') {
            console.log("[handleDoubleClick] No options to toggle and no additional info. Restoring to original list.");
            restoreToOriginalList(setlistItemElement);
        } else {
            console.log("[handleDoubleClick] Toggling options.");
            // スロットの dataset を直接更新
            setlistItemElement.dataset.isShortVersion = togglableOptions.isShortVersion.toString();
            setlistItemElement.dataset.hasSeOption = togglableOptions.hasSeOption.toString();
            setlistItemElement.dataset.drumsoloOption = togglableOptions.drumsoloOption.toString();
            setlistItemElement.dataset.rGt = togglableOptions.rGt;
            setlistItemElement.dataset.lGt = togglableOptions.lGt;
            setlistItemElement.dataset.bass = togglableOptions.bass;
            setlistItemElement.dataset.bpm = togglableOptions.bpm;
            setlistItemElement.dataset.chorus = togglableOptions.chorus;

            // チェックボックスの状態も更新
            setlistItemElement.dataset.short = togglableOptions.short.toString();
            setlistItemElement.dataset.seChecked = togglableOptions.seChecked.toString();
            setlistItemElement.dataset.drumsoloChecked = togglableOptions.drumsoloChecked.toString();

            // スロットを再描画して変更を反映
            updateSlotContent(setlistItemElement, currentSongData.name, {
                isShortVersion: togglableOptions.isShortVersion,
                hasSeOption: togglableOptions.hasSeOption,
                drumsoloOption: togglableOptions.drumsoloOption,
                rGt: togglableOptions.rGt,
                lGt: togglableOptions.lGt,
                bass: togglableOptions.bass,
                bpm: togglableOptions.bpm,
                chorus: togglableOptions.chorus, // chorusは文字列
                // updateSlotContent に現在のチェック状態を渡す
                short: togglableOptions.short,
                seChecked: togglableOptions.seChecked,
                drumsoloChecked: togglableOptions.drumsoloChecked
            });
        }
    } else {
...
function addSongToSlot(slotElement, itemId, songName, options, albumClass) {
    ...
    // スロットにイベントリスナーを再設定 (clearSlotContentで削除済みなのでここで再追加)
    // enableDragAndDropでまとめて設定されている場合はここでの重複は避ける
    // ここでは、DOM要素が動的に変更された際に確実にイベントが紐づくように明示的に再アタッチ
    ...
    slotElement.addEventListener('touchemove', handleTouchMove, { passive: false });
    slotElement.addEventListener('touchend', handleTouchEnd, { passive: false });
    slotElement.addEventListener('touchcancel', handleTouchEnd, { passive: false });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll(".album-content .item").forEach(item => {
        ...
        // ダブルクリックリスナーは enableDragAndDrop 内で追加されるためここでの追加は不要
    });
    ...
    setlist.querySelectorAll(".setlist-slot").forEach((slot, index) => {
        ...
        // セットリストスロットにダブルクリックリスナーは enableDragAndDrop 内で追加済み
    });
