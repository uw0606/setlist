...
async function generateSetlistPdf() {
    ...
    const tableBody = [];
    // ★★★ 修正1: シンプルPDFのデータ格納形式をオブジェクトに変更 (曲かテキストか判別するため) ★★★
    const simplePdfBody = []; 
    ...
    try {
        ...
        // --- 2. シンプルなセットリストPDFの生成 (文字の縮小処理を追加) ---
        ...
        // ★★★ 修正2: 右マージンを左マージンと同じ20mmに設定 ★★★
        const simpleRightMargin = 20; 
        const simpleBottomMargin = 10; 
        ...
        // 詳細PDFで使われているbottomMargin (10mm) を、シンプルPDFの計算に使用
        const bottomMarginToUse = 10;
        ...
        if (numSimpleRows > 0) {
            // 全体調整ロジック：全行を考慮してベースとなるフォントサイズを計算
