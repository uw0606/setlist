...
let currentDraggingItem = null; // 現在ドラッグ中のアイテムを保持 (ドラッグ＆ドロップ関数から設定される想定)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const initializeSetlistItem = (itemElement) => {
        ...
        itemElement.addEventListener("dblclick", (e) => {
            ...
            if (!e.target.closest('.drag-handle')) {
                ...
                e.preventDefault(); // デフォルト動作を防止（テキスト選択など）
            }
        }); 
        ...
        itemElement.addEventListener('touchstart', (e) => {
            if (e.touches.length === 1) { // シングルタッチのみ対象
                ...
                isDraggingTouch = false; // 新しいタッチでドラッグフラグをリセット
        ...
        itemElement.addEventListener('touchend', (e) => {
            ...
            if (!isDraggingTouch && (currentTime - lastTapTime < DOUBLE_TAP_DELAY && currentTime - lastTapTime > 0)) {
            ...
            initialTouchX = 0; // タッチ開始座標をリセット
            initialTouchY = 0;
            isDraggingTouch = false; // ドラッグフラグをリセット
        });
    ...
    setlist.querySelectorAll(".setlist-slot").forEach((slot, index) => {
        ...
        // ★修正点1: 空のスロットに対するダブルタップ/クリック対策を強化★
        slot.addEventListener('click', (e) => {
            // クリックされたのが setlist-item (またはその子要素) でない場合のみ、ログを出す。
            // ここで削除処理は行わない。削除は setlist-item の dblclick/touchend イベントに任せる。
            if (!e.target.closest('.setlist-item')) {
                 ...
                 // 空スロットのクリック時にテキスト選択などを防ぎたい場合は preventDefault() を追加
                 // e.preventDefault();
            } else {
                console.log("[slotClick] Clicked on an item within the slot. Double tap or drag will handle it.");
            }
        });
        
        // ★修正点2: 空のスロットに対するタッチイベント（ドラッグ誤認識防止）★
        slot.addEventListener('touchstart', (e) => {
            // スロット内に setlist-item がない場合のみ、タッチ開始座標を記録
            // setlist-item がある場合は、itemElement の touchstart が処理する
            if (!e.target.closest('.setlist-item') && e.touches.length === 1) {
                const touch = e.touches[0];
                initialTouchX = touch.clientX;
                initialTouchY = touch.clientY;
                isDraggingTouch = false; // ドラッグフラグをリセット
                // console.log("[slot touchstart] Touch initiated on empty slot.");
            }
        }, { passive: false }); // preventDefault() を使うため passive: false を指定

        slot.addEventListener('touchmove', (e) => {
            // スロット内に setlist-item がない場合のみ、移動を検出
            if (!e.target.closest('.setlist-item') && e.touches.length === 1) {
                const touch = e.touches[0];
                const deltaX = Math.abs(touch.clientX - initialTouchX);
                const deltaY = Math.abs(touch.clientY - initialTouchY);
                if (deltaX > TAP_MOVE_THRESHOLD || deltaY > TAP_MOVE_THRESHOLD) {
                    isDraggingTouch = true;
                    // console.log("[slot touchmove] Touch moved on empty slot. Likely drag intent.");
                }
            }
        });

        slot.addEventListener('touchend', (e) => {
            // スロット内に setlist-item がない場合のみ処理
            if (!e.target.closest('.setlist-item')) {
                // touchstart/move でドラッグと判定された場合は、削除処理を行わない
                if (isDraggingTouch) {
                    console.log("[slot touchend] Drag operation on empty slot. No deletion.");
                    // ここで `e.preventDefault()` や `e.stopPropagation()` を入れると、
                    // スロットへの曲のドロップなどの他のイベントが妨げられる可能性があるため、
                    // 慎重に検討するか、enableDragAndDrop 関数内で処理するのが望ましい
                } else {
                    // ドラッグと判定されず、かつ setlist-item もない場合は、何もしない（削除しない）
                    console.log("[slot touchend] Tap on empty slot. No deletion.");
                }
            }
            // グローバルなタッチ状態をリセット
            lastTapTime = 0; 
        ...
        // ★重要: スロットに直接ダブルクリックリスナーを追加しない (setlist-item のみで処理) ★
        // slot.addEventListener("dblclick", someFunction); // このような行は引き続き削除された状態に
    });
            ...
            if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                mutation.addedNodes.forEach(node => {
                    if (node.nodeType === 1) { // 要素ノードであることを確認
                        ...
                        // setlist-slot 自体は既にDOMContentLoadedで enableDragAndDrop されているはず
                        // ただし、もし動的にスロット自体が追加される場合はここでの再初期化が必要
                        if (node.classList.contains('setlist-slot')) {
                             // enableDragAndDrop(node); // 既にDOMContentLoadedで処理されているため、通常は不要
