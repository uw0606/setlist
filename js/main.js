...
function changeAlbumView() {
    ...
    // ★★★ 追加要素の取得 ★★★
    // 過去のセットリストボタンのコンテナ（IDは適宜修正してください）
    ...
    // ボタンの元の親要素（アルバムビューのどこか、ここでは #albumViewContainer の直下を仮定）
    // 実際のHTML構造に合わせて、元の親要素を取得できるセレクタ/IDに修正してください。
    // 例: const albumViewParent = document.getElementById('albumViewFooter');
    const albumViewParent = document.getElementById('albumViewContainer'); 
    // ★★★ /追加要素の取得 ★★★
    ...
    if (mode === 'name_order') {
        ...
        // ★★★ 過去セットリストボタンを文字順ナビの下に移動 ★★★
        if (pastSetlistsButton && nameOrderNav) {
            // #nameOrderNav の親要素 (通常 #nameOrderContainer) の中に、#nameOrderNav の直後の兄弟として挿入
        ...
        // ★★★ 過去セットリストボタンを元の位置に戻す ★★★
        if (pastSetlistsButton && albumViewParent) {
            albumViewParent.appendChild(pastSetlistsButton);
            // 文字順用のスタイルを削除
