...
const TAP_MOVE_THRESHOLD = 10; // ドラッグ開始とみなす最小移動ピクセル数
...
function handleTouchStart(event) {
    ...
    // ドラッグ対象ではない要素の場合、ここで処理を終了し、イベントのデフォルト動作（スクロールなど）を妨げない。
    if (!touchedElement) {
    ...
    if (event.touches.length === 1) {
        ...
        // タッチ開始地点の座標を記録 (移動量判定のため)
        initialTouchX = event.touches[0].clientX;
        initialTouchY = event.touches[0].clientY;

        // クローン作成時のオフセットを記録（クローンが指の真下に来るようにするため）
        ...
        touchTimeout = setTimeout(() => {
            // 長押しが確定したらドラッグを開始
            isDragging = true;
            // event.preventDefault() は handleTouchMove で移動が検知されたときに呼ぶ
            // ここではクローンを作成する
            console.log("[touchstart:Mobile] Long press detected. Initiating drag.");

            const elementToClone = originalSetlistSlot || touchedElement; // セットリストからのドラッグならoriginalSetlistSlot、そうでなければtouchedElement
            if (elementToClone && draggingItemId) {
                ...
                console.warn("[touchstart:Mobile] Cannot create clone: elementToClone or draggingItemId is missing.");
...
function handleTouchMove(event) {
    // ⭐ここから修正⭐
    // ドラッグがまだ開始されていない場合（長押しが確定していない、または移動量が閾値以下）
    if (!isDragging) {
        ...
        // 長押しタイマーが設定されている場合、移動量が閾値を超えたらタイマーをクリアし、ドラッグをキャンセル
        // これにより、長押しする前にスクロールを開始した場合にドラッグが誤って開始されるのを防ぐ
        if (touchTimeout && (deltaX > TAP_MOVE_THRESHOLD || deltaY > TAP_MOVE_THRESHOLD)) {
            clearTimeout(touchTimeout);
            touchTimeout = null;
            console.log("[handleTouchMove] Moved before long press threshold. Cancelling drag initiation.");
            // ここで return することで preventDefault() を呼ばず、スクロールを許可
            return;
        }
        // 長押しタイマーがまだ動いていて、閾値内の移動であれば、スクロールを許可し続ける
        return;
    }
    // ⭐ここまで修正⭐

    // isDragging が true の場合（ドラッグ中）
    ...
    rafId = requestAnimationFrame(() => {
        ...
        currentTouchDraggedClone.style.left = (newX - touchStartX) + 'px';
        currentTouchDraggedClone.style.top = (newY - touchStartY) + 'px';
