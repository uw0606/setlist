...
function processDrop(draggedElement, dropTargetSlot, originalSourceSlot) {
    ...
    console.log("Dragged Element (original):", draggedElement); // ここで元の要素が来ていることを確認
    ...
    // アルバムクラスを正確に取得する (draggedElementのクラスリストから)
    ...
    // 元のスロットが存在する場合（セットリスト内からの移動または入れ替え）
    if (originalSourceSlot) {
        ...
        if (dropTargetSlot.classList.contains('setlist-item')) {
            ...
            // clearSlotContent(originalSourceSlot); // clearSlotContentはaddSongToSlot内部で呼ばれる
            ...
            // clearSlotContent(dropTargetSlot); // clearSlotContentはaddSongToSlot内部で呼ばれる
            ...
            showMessage("セットリスト内の曲を入れ替えました。", "success"); // メッセージを「入れ替え」に変更
            ...
            // clearSlotContent(originalSourceSlot); // addSongToSlot内部で呼ばれる
            addSongToSlot(dropTargetSlot, songData.itemId, songData.name, {
                isShortVersion: songData.hasShortOption,
                hasSeOption: songData.hasSeOption,
                drumsoloOption: songData.hasDrumsoloOption,
...
function handleDrop(event) {
    ...
    // const originalListId = event.dataTransfer.getData("text/originalListId"); // 現在は未使用
    ...
    if (originalSetlistSlot && originalSetlistSlot.classList.contains('setlist-item')) {
        // セットリスト内からのドラッグの場合、元のスロットがドラッグ元
        ...
        // アルバムからのドラッグの場合、アルバム内の元のアイテムを探す
        actualDraggedElement = document.querySelector(`.album-content .item[data-item-id="${droppedItemId}"]`);
        console.log(`[handleDrop] Identified album source item for drag: ${droppedItemId}`);
    }

    if (!actualDraggedElement) {
        console.error("[handleDrop] Could not identify the actual dragged element. Aborting.");
        finishDragging(true);
        return;
    }

    // processDrop を呼び出して実際のドロップロジックを実行
    // currentPcDraggedElement はアルバムからのドラッグ時にクローン作成に使われたものなので、
    // ここでは actualDraggedElement を渡すのが適切です。
    // originalSetlistSlot はセットリスト内からのドラッグの場合にのみ設定されます。
...
function enableDragAndDrop(element) {
    ...
    if (element.classList.contains('setlist-slot')) {
        // ★ここも重要: setlist-slot に dragover/drop の pointer-events が適用されているか確認
        // CSSで setlist-slot の pointer-events を 'auto' に設定してください。
        // もし setlist-item が入ったときに 'auto' に切り替えているなら、空のスロットは 'auto' にしておかないとドロップできません。
