...
function handleDragStart(event) {
    ...
    if (!originalElement) {
        console.warn("[dragstart:PC] No draggable item found. Preventing default behavior.");
...
function handleDragOver(event) {
    ...
    event.dataTransfer.dropEffect = "move"; // ドロップ時のカーソル表示
    ...
    if (targetSlot) {
        ...
        if (!targetSlot.classList.contains('setlist-item')) {
            ...
            targetSlot.style.touchAction = 'auto'; // 必要であれば
        }
    } else {
