...
async function generateSetlistPdf() {
    ...
    try {
        ...
        const topMargin = 20;
        const leftMargin = 10;
        const bottomMargin = 60; // 詳細PDFのボトムマージン
        ...
        if (numSimpleRows > 0) {
            // ★★★ 修正2: 行間とフォントサイズの動的調整ロジック ★★★

            // 1. 利用可能な総高さを算出 (ボトムマージン 25mm 適用)
            const simpleAvailableHeight = pageHeightSimple - simpleTopMargin - simpleBottomMargin - simpleHeaderHeight;
            
            // 2. 実際の曲数で割った場合の理想の行の高さ (曲数が少ないと広くなる)
            const idealLineHeight = simpleAvailableHeight / numSimpleRows;

            // 3. 最大許容曲数 (25曲) で割った場合の最小限の行の高さ
            //    -> これが、26曲以上を1ページに収めるために許容する最大圧縮率となる
            const maxCompressionLineHeight = simpleAvailableHeight / MAX_SONGS_PER_PAGE;
            
            // 4. 採用する行の高さ (idealLineHeight と maxCompressionLineHeight を比較)
            //    - 曲数が少ない: idealLineHeight が採用され、行間が広がる
            //    - 曲数が多い: maxCompressionLineHeight が採用され、25曲の高さに圧縮される
            let dynamicLineHeight = Math.min(idealLineHeight, maxCompressionLineHeight);

            // 5. 行の高さに基づいてフォントサイズを決定
            let dynamicFontSize = dynamicLineHeight / BASE_LINE_HEIGHT_RATIO;
            
            // 6. フォントサイズを最大(BASE_FONT_SIZE)と最小(MIN_FONT_SIZE)で制限
            dynamicFontSize = Math.min(BASE_FONT_SIZE, Math.max(MIN_FONT_SIZE, dynamicFontSize));

            // 7. 最終的な動的行の高さ (フォントサイズに合わせた最終行間)
            ...
            if (headerText) {
                ...
                simplePdf.setFontSize(dynamicFontSize); // ★★★ 修正3: 動的フォントサイズを適用 ★★★
            } else {
                simplePdf.setFontSize(dynamicFontSize); // ★★★ 修正3: 動的フォントサイズを適用 ★★★
            }
            ...
            simplePdfBody.forEach(line => {
                ...
                simpleYPos += dynamicLineHeight; // ★★★ 修正4: 計算した動的な行間を適用 ★★★
            });
        } else {
            let simpleYPos = simpleTopMargin;
            if (headerText) {
