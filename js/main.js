...
function handleTouchStart(event) {
    ...
    if (!touchedElement) {
        ...
        return; // ドラッグ対象ではない場合、ここで処理を終了し、preventDefaultを呼ばない
    }
    // ここではまだ event.preventDefault() を呼ばない！
    // スクロール開始とドラッグ開始を区別するため。
    // ドラッグが確定したら handleTouchMove で preventDefault する。
    ...
    if (event.touches.length === 1) {
        ...
        // 長押しタイムアウトは、ドラッグと判定される前にクリアされる可能性があるため、
        // ここでドラッグ開始の直接のトリガーにはしない。
        // 移動距離が閾値を超えた場合に isDragging を true にする。
        if (touchTimeout) { // 念のためクリア
            clearTimeout(touchTimeout);
            touchTimeout = null;
        }
        // 長押しでドラッグ開始の意図がある場合のみクローンを作成
        touchTimeout = setTimeout(() => {
            if (!isDragging && draggingItemId && document.body.contains(touchedElement)) {
                // ここでクローンを作成し、isDraggingをtrueにする
                createTouchDraggedClone(touchedElement, touchStartX, touchStartY, draggingItemId);
                isDragging = true;
                console.log("[touchstart:Mobile] Drag initiated by long press timeout.");
            }
            touchTimeout = null;
        }, 600); // 長押しと判定する時間
    }
...
function handleTouchMove(event) {
    // isDragging が true でなければ、まだドラッグ開始ではないとみなす
    if (!isDragging) {
        ...
        // 移動距離がしきい値を超えたらドラッグを開始
        if (diffX > touchMoveThreshold || diffY > touchMoveThreshold) {
            // 長押しタイマーがあればクリア（スクロール意図と判断されたため）
            if (touchTimeout) {
                clearTimeout(touchTimeout);
                touchTimeout = null;
            }

            // ドラッグする要素がすでに特定されていることを確認
            const touchedElement = document.elementFromPoint(touchStartX, touchStartY)?.closest(".setlist-slot.setlist-item") || 
                                 document.elementFromPoint(touchStartX, touchStartY)?.closest(".item");

            if (touchedElement && draggingItemId) {
                createTouchDraggedClone(touchedElement, touchStartX, touchStartY, draggingItemId);
                isDragging = true; // ドラッグ開始
                console.log("[handleTouchMove] Drag initiated by movement exceeding threshold.");
            } else {
                // ドラッグ対象がない場合、何もしないでスクロールを許可し続ける
                return; 
            }
        } else {
            // しきい値を超えていない場合は、スクロールを許可するためにここで処理を終了
            return; 
        }
    }

    // ここから先は isDragging が true の場合のみ実行される
    event.preventDefault(); // ドラッグが開始されたらスクロールを抑制

    if (!currentTouchDraggedClone) return; // isDragging が true でもクローンがない場合は異常
