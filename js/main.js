...
document.addEventListener("dblclick", (event) => {
  ...
  if (!!event.target.closest("#setlist")) {
    // 共有されたセットリストのアイテムをハンバーガーメニューに戻す処理
    const songToRemove = item;
    setlist.removeChild(songToRemove);

    const songName = songToRemove.querySelector(".song-info")?.childNodes[0]?.textContent?.trim() || songToRemove.textContent.trim();

    document.querySelectorAll('.album-content').forEach(albumContent => {
      const originalSong = albumContent.querySelector(`.item[draggable="true"].original-item`) ?
        Array.from(albumContent.querySelectorAll(`.item[draggable="true"].original-item`)).find(el => el.textContent.trim().toLowerCase() === songName.toLowerCase()) :
        Array.from(albumContent.querySelectorAll(`.item.original-item`)).find(el => el.textContent.trim().toLowerCase() === songName.toLowerCase());

      if (originalSong) {
        const clonedSong = originalSong.cloneNode(true);
        albumContent.appendChild(clonedSong);
        enableDragAndDrop(albumContent);
      }
    });
  } else if (item.classList.contains('original-item')) {
    // 共有前のハンバーガーメニューのアイテムをセットリストに追加する処理
    if (setlist.children.length < maxSongs) {
