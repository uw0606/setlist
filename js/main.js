...
let allSongData = []; 
let nameOrderGroups = {};
let isBuildingList = false; // ★リスト構築中フラグ★
...
function changeAlbumView() {
    ...
    // ★★★ 修正箇所: 過去セットリストボタンのコンテナを取得 ★★★
    // 💡 ボタン(id="openPastSetlistsModal")の親要素である div.menu-section を取得
    const pastSetlistsButtonContainer = document.querySelector('.menu-section'); 
    
    const nameOrderNav = document.getElementById('nameOrderNav'); 
    // 💡 アルバム順ビューで .menu-section が元々属している親要素 (ここでは #menuContainer を仮定)
    ...
    if (mode === 'name_order') {
        ...
        if (pastSetlistsButtonContainer && nameOrderNav) {
            // #nameOrderNav の親要素の中に、#nameOrderNav の直後の兄弟として挿入
            nameOrderNav.parentNode.insertBefore(pastSetlistsButtonContainer, nameOrderNav.nextSibling);
        ...
        if (pastSetlistsButtonContainer && menuContainer) {
            // 元の親要素の末尾に戻す
            menuContainer.appendChild(pastSetlistsButtonContainer);
