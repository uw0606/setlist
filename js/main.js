...
const database = firebase.database(); // Firebase Realtime Databaseの参照
...
function getCurrentState() {
    ...
    const setlistDate = (setlistYear && setlistMonth && setlistDay) ? 
                         `${setlistYear}-${setlistMonth}-${setlistDay}` : '';
...
function fillSlotWithItem(targetSlot, itemData, fromDrop = false) {
    if (!targetSlot) {
        console.error("[fillSlotWithItem] Target slot is null or undefined.");
        return;
    }
...
function createTextInputSlot(slot) {
    ...
    inputField.addEventListener('keypress', (e) => {
        if (e.key === 'Enter') {
            saveButton.click(); // 保存ボタンのクリックイベントをトリガー
        }
    });
    ...
    if (draggedItem.classList.contains('setlist-item') || draggedItem.classList.contains('setlist-slot-text')) {
        ...
        if (originalParent === targetParent && draggedItem !== targetSlot) {
            ...
            if (draggedItemData && draggedItemData.type === 'text') { // テキストアイテムの場合
                ...
                targetSlot.textContent = draggedItemData.textContent;
                targetSlot.querySelector('input[type="text"]').value = draggedItemData.textContent; // inputフィールドにも値を設定
                const saveBtn = targetSlot.querySelector('.save-text-button');
                if (saveBtn) saveBtn.click(); // 自動保存
            } else if (draggedItemData && draggedItemData.itemId) { // 曲アイテムの場合
                fillSlotWithItem(targetSlot, draggedItemData);
            }
            ...
            if (targetSlotData) {
                if (targetSlotData.type === 'text') {
                    ...
                    draggedItem.textContent = targetSlotData.textContent;
                    draggedItem.querySelector('input[type="text"]').value = targetSlotData.textContent; // inputフィールドにも値を設定
                    const saveBtn = draggedItem.querySelector('.save-text-button');
                    if (saveBtn) saveBtn.click(); // 自動保存
                } else if (targetSlotData.itemId) {
                    fillSlotWithItem(draggedItem, targetSlotData);
                }
            } else {
...
function handleTouchStart(e) {
    ...
    // 長押しでドラッグ開始
    if (!draggedItem.classList.contains('text-input-field')) { // テキスト入力中はドラッグしない
        touchTimeout = setTimeout(() => {
            ...
            if (targetSlot) {
                ...
                if (draggedItem.classList.contains('setlist-item') || draggedItem.classList.contains('setlist-slot-text')) {
                    ...
                    if (originalParent === targetParent && draggedItem !== targetSlot) {
                        ...
                        console.log(`[TouchEnd] Reordered items from ${originalIndex} to ${targetIndex}`);
                        showMessageBox("セットリストの順序を変更しました。");
                    } else if (draggedItem !== targetSlot) {
                        // 異なるスロット間で交換（今回は単純に上書き）
                        const draggedItemData = getSlotItemData(draggedItem);
                        ...
                        if (draggedItemData && draggedItemData.type === 'text') {
                            createTextInputSlot(targetSlot);
                            targetSlot.textContent = draggedItemData.textContent;
                            targetSlot.querySelector('input[type="text"]').value = draggedItemData.textContent;
                            const saveBtn = targetSlot.querySelector('.save-text-button');
                            if (saveBtn) saveBtn.click();
                        } else if (draggedItemData && draggedItemData.itemId) {
                            fillSlotWithItem(targetSlot, draggedItemData);
                        }
                        ...
                        if (targetSlotData) {
                            if (targetSlotData.type === 'text') {
                                createTextInputSlot(draggedItem);
                                draggedItem.textContent = targetSlotData.textContent;
                                draggedItem.querySelector('input[type="text"]').value = targetSlotData.textContent;
                                const saveBtn = draggedItem.querySelector('.save-text-button');
                                if (saveBtn) saveBtn.click();
                            } else if (targetSlotData.itemId) {
                                fillSlotWithItem(draggedItem, targetSlotData);
                        ...
                        showMessageBox("アイテムを交換しました。");
                        console.log("[TouchEnd] Swapped items between slots.");
                    }
                } else if (draggedItem.classList.contains('item')) {
                    // アルバムアイテムをセットリストスロットにドロップ
                    const itemId = draggedItem.dataset.itemId;
                    ...
                    if (targetSlot.classList.contains('setlist-item')) {
                        ...
                        if (existingItemInMenu) {
                            existingItemInMenu.style.visibility = '';
                            console.log(`[TouchEnd] Returned existing item ${existingItemId} to menu.`);
                        }
                    }
                    ...
                    showMessageBox(`${itemName} をセットリストに追加しました！`);
                    console.log(`[TouchEnd] Added album item ${itemName} to slot ${targetSlot.dataset.slotIndex}`);
                }
            } else {
...
function handleDoubleClickSlot(e) {
    ...
    if (targetSlot.classList.contains('setlist-item')) {
        ...
        if (originalItemInMenu) {
            originalItemInMenu.style.visibility = ''; // 非表示を解除
            console.log(`[handleDoubleClickSlot] Restored visibility of album item: ${itemIdToRemove}`);
        }
        showMessageBox("スロットをクリアしました。");
...
function toggleMenu() {
    if (menu && hamburgerMenu) {
        ...
        console.log(`[toggleMenu] Menu state toggled. Menu open: ${menu.classList.contains('open')}`);
    } else {
        console.warn("[toggleMenu] Menu or hamburgerMenu element not found.");
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (hamburgerMenu) {
        ...
        console.warn("[DOMContentLoaded] Hamburger menu button element not found.");
    }
