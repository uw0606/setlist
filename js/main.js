...
function getSlotItemData(element) {
    ...
    // オプションの有無は、datasetから取得し、存在しない場合はfalseとする
    // ★ここを修正します★
    ...
    if (isSetlistItem) {
        ...
        // セットリストアイテムの場合、チェックボックスの実際の状態を見る
        ...
        // アルバムアイテムの場合、HTMLから直接読み込む
        // ★この部分が特に重要です★
        hasShortOption = (element.dataset.isShortVersion === 'true');
        hasSeOption = (element.dataset.hasSeOption === 'true');
        hasDrumsoloOption = (element.dataset.hasDrumsoloOption === 'true');
        
        // アルバムアイテムからの追加時は、チェックボックスの初期状態はfalse
        ...
        // クローン要素の場合もdatasetからオプションの有無を読み込む
        hasShortOption = (element.dataset.isShortVersion === 'true');
        hasSeOption = (element.dataset.hasSeOption === 'true');
        hasDrumsoloOption = (element.dataset.drumsoloOption === 'true');
    } else {
...
function updateSlotContent(slotElement, songName, options) {
    ...
    // Short有無
    // options.isShortVersion が true ならチェックボックスを生成
    if (options.isShortVersion) { 
        ...
        const shortVersionCheckboxWrapper = createCheckboxWrapper('Short', options.short, (e) => { 
            // クリックされたら、dataset と class を更新
            slotElement.dataset.short = e.target.checked.toString();
            slotElement.classList.toggle('short', e.target.checked);
            
            // updateSlotContent を再帰的に呼び出してUIを更新
            // ただし、updateSlotContent の引数 `options` は常に最新の状態を反映するようにする
            // ★ここがShortチェックボックスが消える問題の主な修正点★
            // 現在のslotElementのdatasetから最新のオプション状態を取得し、それを元に再描画する
            const currentSongData = getSlotItemData(slotElement);
            if (currentSongData) {
                updateSlotContent(slotElement, currentSongData.name, currentSongData);
            }
        });
    ...
    // SE有無
    // options.hasSeOption が true ならチェックボックスを生成
    ...
    // ドラムソロ有無
    // options.hasDrumsoloOption が true ならチェックボックスを生成
    // ★ドラムソロが表示されない問題の主な修正点★
    if (options.hasDrumsoloOption) { 
        hasAnyCheckboxOption = true;
