...
function handleTouchStart(event) {
    ...
    if (event.touches.length === 1) {
        ...
        touchTimeout = setTimeout(() => {
            ...
            // ★ドラッグが実際に開始される直前に event.preventDefault() を呼び出す★
            ...
            if (draggingItemId && document.body.contains(touchedElement)) {
                ...
                console.warn("[touchstart:Mobile] Dragging not initiated after timeout (touchedElement missing or draggingItemId null).");
            }
...
function handleTouchEnd(event) {
    ...
    if (touchTimeout) {
        ...
        // ここで finishDragging() を呼ばないことで、isDragging=falseのまま終了し、
        // 意図しないドラッグ終了処理が行われないようにする
        return;
    }
