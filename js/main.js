...
// 新しい変数と定数を追加
let draggingElement = null; // ドラッグ中の元の要素を保持
let touchTimeout = null; // 長押し判定用タイマー
const LONG_PRESS_THRESHOLD = 500; // 長押しと判定する時間（ミリ秒）
const DRAG_THRESHOLD = 20; // ドラッグ開始判定のための移動距離（ピクセル）
...
function handleTouchStart(event) {
    ...
    if (isCheckboxClick) {
        ...
        lastTapTime = 0;
        if (touchTimeout) { clearTimeout(touchTimeout); touchTimeout = null; } // チェックボックスの場合はタイマーをクリア
        isDragging = false;
    ...
    if (!touchedElement) {
        console.warn("[touchstart:Mobile] No draggable item found on touch start. Allowing default scroll behavior.");
        if (touchTimeout) { clearTimeout(touchTimeout); touchTimeout = null; } // ドラッグ対象外の場合はタイマーをクリア
        isDragging = false; // 念のためリセット
        return; // ドラッグ対象外の要素に触れた場合は、すぐに処理を終了し、ブラウザのスクロールを完全に許可
    }

    // ダブルタップ検出ロジック (既存のロジックを維持)
    if (tapLength < 300 && tapLength > 0) {
        if (touchTimeout) { clearTimeout(touchTimeout); touchTimeout = null; } // ダブルタップの場合はタイマーをクリア
    ...
    if (event.touches.length === 1) {
        ...
        // 既存のタイマーをクリアし、長押し判定用の新しいタイマーを設定
        if (touchTimeout) { clearTimeout(touchTimeout); touchTimeout = null; }
        
        // ★★ 長押し判定ロジックの追加 ★★
        touchTimeout = setTimeout(() => {
            // LONG_PRESS_THRESHOLD 時間が経過しても指が離れていない場合
            // この時点ではまだドラッグを開始せず、handleTouchMoveで移動と組み合わせる
            console.log(`[touchstart:Mobile] Long press threshold reached. (Timeout finished)`);
            // ここで特にフラグは立てないが、touchTimeoutがnullでなくなったことが後続の判定に利用される
        }, LONG_PRESS_THRESHOLD);

        console.log("[touchstart:Mobile] Touch detected. Waiting for movement for drag or long press/double tap.");
    }
    // ここでは event.preventDefault() を呼ばない！
    // ブラウザのスクロールを初期段階でブロックしないことが重要
}
...
function handleTouchMove(event) {
    ...
    if (!touch) {
        ...
        if (isDragging) {
            ...
            draggingItemId = null; // リセット
        }
        // タッチが失われたら長押しタイマーもクリア
        if (touchTimeout) { clearTimeout(touchTimeout); touchTimeout = null; }
        return;
    }
    ...
    if (!isDragging) {
        ...
        // DRAG_THRESHOLD に達していない場合、または長押しタイマーがまだアクティブな場合
        // この状態ではブラウザのデフォルトスクロール動作を妨げない
        if (deltaX < DRAG_THRESHOLD && deltaY < DRAG_THRESHOLD) {
            // スクロールに近い短い移動の場合は、長押しタイマーをリセットしない
            // これにより「少し指が震えた」程度で長押しがキャンセルされないようにする
            return;
        }

        // DRAG_THRESHOLD を超える動きがあったら、長押しタイマーをクリア
        // これは「素早いフリック」や「意図的なスクロール」の場合に、ドラッグ判定をキャンセルするため
        if (touchTimeout) {
            clearTimeout(touchTimeout);
            touchTimeout = null;
            console.log("[handleTouchMove] Drag threshold exceeded before long press. Long press cancelled. Allowing scroll.");
            // この時点でドラッグは開始しないので、event.preventDefault() は呼ばない
            // スクロール動作を許可するために、ここで return する
            return;
        }

        // ここに到達した場合は、「DRAG_THRESHOLD を超えた」 **かつ** 「長押しタイマーが既に終了している」
        // つまり、意図的なドラッグ開始の条件が満たされたと判断する
        if (!draggingElement || !draggingItemId) {
            console.warn("[handleTouchMove] No draggable element info after long press. Allowing scroll.");
            return;
        }
        
        // ドラッグ開始の瞬間
        // ここでクローンを作成し、ドラッグモードに切り替える
        createTouchDraggedClone(draggingElement, touchStartX, touchStartY, draggingItemId);
        isDragging = true; // ドラッグ状態に設定
        console.log("[handleTouchMove] Drag initiated after movement threshold AND long press.");
        
        // ★重要: ここで初めて event.preventDefault() を呼び出すことで、
        // これ以降のデフォルトスクロール（ブラウザによる）を停止し、ドラッグに完全に切り替える
        event.preventDefault(); 
    }

    // ドラッグが既に開始されている場合（または今開始された場合）
    // ★重要: ドラッグ中は常にデフォルトのスクロールをキャンセルし、クローンを追従させる
    event.preventDefault(); 
...
function handleTouchEnd(event) {
    ...
    if (touchTimeout) {
        clearTimeout(touchTimeout);
        touchTimeout = null;
        console.log("[touchend] Long press timer cleared.");
    }
