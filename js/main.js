...
const DRAG_THRESHOLD = 30; // 10ピクセル以上動いたらドラッグと判定
...
function handleTouchMove(event) {
    ...
    if (!isDragging) {
        ...
        if (deltaX > DRAG_THRESHOLD || deltaY > DRAG_THRESHOLD) {
            const touchedElement = event.target.closest(".setlist-slot.setlist-item") || event.target.closest(".item");
            if (!touchedElement || touchedElement.dataset.itemId !== draggingItemId) {
                // dragItemIdがセットされていない、またはタッチ開始時の要素と異なる場合は何もしない
                // これにより、スクロール目的のタッチムーブは続行される
                console.log("[handleTouchMove] Not a draggable item or initial item mismatch. Allowing scroll.");
                return; 
            }
