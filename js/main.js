...
function processDrop(draggedElement, dropTargetSlot, originalSourceSlot) {
    ...
    // albumClassがnullishの場合、空文字列をデフォルト値として設定
    // これにより、addSongToSlotでalbumClassがnull/undefinedにならないようにする
    ...
    if (originalSourceSlot) {
        ...
        if (dropTargetSlot.classList.contains('setlist-item')) {
            ...
            }, targetSongData.albumClass); // targetSongData.albumClass を渡す
            ...
            addSongToSlot(dropTargetSlot, songData.itemId, songData.name, {
                ...
                seChecked: songData.seChecked,
                drumsoloChecked: songData.drumsoloChecked
            }, finalAlbumClass); // こちらはドラッグ元のアルバムクラスを使う
            ...
            originalSetlistSlot.style.visibility = 'visible'; // 隠していた元のスロットを再表示
            originalSetlistSlot.classList.remove('placeholder-slot'); // プレースホルダークラスも削除
...
function handleDrop(event) {
    ...
    if (!isDragging) {
        ...
        // ここでの finishDragging は不要。isDragging が false なのでドロップ処理は行わない
        return;
    }
    ...
    if (!droppedItemId) {
        ...
        finishDragging(true); // エラー時はキャンセルとしてクリーンアップ
        return;
    }
    ...
    if (originalSetlistSlot) {
        ...
        // アルバムからのドラッグの場合
        // albumList がグローバルで定義されていることを確認
        // albumList は通常、アルバムアイテムの親要素を指す変数です
        const albumList = document.querySelector('.album-list'); // または適切なアルバムコンテナのセレクタ
        if (!albumList) {
            console.error("[handleDrop] albumList element not found. Cannot find album item.");
            finishDragging(true);
            return;
        }
    ...
    if (!actualDraggedElement) {
        ...
        finishDragging(true); // 要素が特定できない場合はキャンセルとしてクリーンアップ
        return;
    }

    processDrop(actualDraggedElement, targetSlot, originalSetlistSlot);
    
    // ドロップ処理が完了したら、必ずクリーンアップ
    finishDragging(); // ★wasCancelled 引数は削除。成功も失敗もこの関数が呼ばれるため
}
