...
function handleTouchStart(event) {
    ...
    if (tapLength < 300 && tapLength > 0) {
        ...
        if (isInSetlist && touchedElement.classList.contains('setlist-item')) {
            ...
            if (!emptySlot) {
                ...
                touchedElement.style.visibility = 'hidden'; // アルバムリストの元のアイテムを非表示
        ...
        lastTapTime = 0;
        isDragging = false; // ダブルタップで処理が完結するため、ドラッグ状態は開始しない
        draggingItemId = null;
    ...
    // --- シングルタッチ/ロングプレスのドラッグ開始処理 ---
    if (event.touches.length === 1) {
        isDragging = false; // タッチ開始時点ではドラッグ中ではない

        if (isInSetlist && touchedElement.classList.contains('setlist-item')) {
            // ルール2: セットリスト内でのドラッグを許可 (モバイル)
            originalSetlistSlot = touchedElement;
            console.log(`[touchstart] Touch on setlist item. Setting long press timer for internal setlist drag.`);
            // 移動開始座標を記録
            touchStartX = event.touches[0].clientX;
            touchStartY = event.touches[0].clientY;

            if (touchTimeout) { clearTimeout(touchTimeout); touchTimeout = null; } // 既存のタイマーがあればクリア
            touchTimeout = setTimeout(() => {
                if (!isDragging && draggingItemId && document.body.contains(initialTouchElement)) {
                    ...
                    console.warn("[touchstart] Dragging not initiated from setlist item after timeout (condition unmet).");
                }
                touchTimeout = null;
            }, 600); // 600ms でロングプレスと判定
        } else if (isInAlbumContent && touchedElement.classList.contains('item')) {
            // ルール1: ハンバーガーメニューからのドラッグを許可 (モバイル)
            originalSetlistSlot = null; // アルバムアイテムからのドラッグ (セットリストへの追加)
            console.log(`[touchstart] Touch on album item. Setting long press timer for drag to setlist.`);
            // 移動開始座標を記録
            touchStartX = event.touches[0].clientX;
            touchStartY = event.touches[0].clientY;
            ...
            touchTimeout = setTimeout(() => {
                if (!isDragging && draggingItemId && document.body.contains(initialTouchElement)) {
                    ...
                    console.warn("[touchstart] Dragging not initiated from album item after timeout (condition unmet).");
                }
                touchTimeout = null;
            }, 600); // 600ms でロングプレスと判定
        } else {
...
function handleDoubleClick(event) {
    ...
    // モバイルのタッチイベントから呼ばれた場合、既にhandleTouchStartで処理されているため重複を避ける
    // ただし、PCの場合はdblclickイベントが直接発火するため、以下のロジックは必要
    
    event.preventDefault(); // デフォルト動作を防ぐ
    event.stopPropagation(); // イベントのバブリングを停止
    console.log(`[handleDoubleClick] Double click on item ID: ${item.dataset.itemId}`);

    const isInsideSetlist = setlist.contains(item) && item.classList.contains('setlist-item');
    const isMobileDevice = /Mobi|Android/i.test(navigator.userAgent);

    if (isMobileDevice && event.type === 'dblclick') {
        // モバイルデバイスでdblclickイベントが発火した場合 (仮想的なもの)。
        // 実際のモバイル操作はhandleTouchStartのダブルタップで処理されるので、
        // ここでは通常何もしないか、あるいはログを出す程度にする。
        // もしPCのような物理的なダブルクリックをモバイルでも検出したい場合はこの限りではないが、
        // 通常のモバイルUIではダブルタップを使う。
    ...
    if (isInsideSetlist) {
        // ルール3: セットリストからハンバーガーメニューに戻す (PCダブルクリック)
        console.log("[handleDoubleClick] Item is in setlist. Restoring to original list by double click (PC).");
        restoreToOriginalList(item); // セットリストからアイテムを削除し、アルバムに戻す
        showMessageBox('曲をセットリストから削除しました。');
    } else {
        // ルール1: アルバムからセットリストへ追加 (PCダブルクリック)
        console.log("[handleDoubleClick] Item is in album list. Attempting to add to setlist by double click (PC).");
        const emptySlot = Array.from(setlist.children).find(slot => !slot.classList.contains('setlist-item'));
