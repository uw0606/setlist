...
async function generateSetlistPdf() {
    ...
    try {
        ...
        // ★修正ポイント1: BASE_FONT_SIZEを100ptに戻す (行間優先のため)
        const BASE_FONT_SIZE = 100; 
        const MAX_SONG_FONT_SIZE = 60; // 曲の最大フォントサイズを60ptに設定
        ...
        const simpleBottomMargin = 20; // マージンを調整
        const SPACE_BETWEEN_HEADER_AND_BODY = 5; // マージンを調整
        ...
        // --- 1. 曲の配置に必要なスペースとサイズを予測（ヘッダーサイズ決定のため） ---
        if (numSimpleRows > 0) {
            // ヘッダーの高さを考慮して、曲が使える高さを予測
            const tempAvailableHeight = pageHeightSimple - simpleTopMargin - simpleBottomMargin - (headerText ? 35 : 0);
            
            // 行間調整ロジック（予測用）
            // ★修正ポイント2: MIN_LINE_HEIGHT_FACTORを0.6に戻す (最大の広げ幅)
            const MIN_LINE_HEIGHT_FACTOR = 0.6; 
            const MAX_LINE_HEIGHT_FACTOR = 1.0; 
            const LINE_COUNT_START = 6;  
            const LINE_COUNT_END = 15;   
            ...
            const IDEAL_LINE_HEIGHT = tempAvailableHeight / VIRTUAL_ROW_DIVISOR;
            let tempDynamicFontSize = IDEAL_LINE_HEIGHT / BASE_LINE_HEIGHT_RATIO;
            
            // ★修正ポイント3: 予測フォントサイズに上限を適用
            tempDynamicFontSize = Math.min(MAX_SONG_FONT_SIZE, tempDynamicFontSize);
            
            dynamicFontSize = Math.max(MIN_FONT_SIZE, tempDynamicFontSize);
        }

        // ヘッダーの描画と高さの計算
        let simpleHeaderHeight = 0;
        if (headerText) {
            // ★修正ポイント4: ヘッダーサイズの初期値を、予測した曲のサイズを基準にする
            let headerFontSize = dynamicFontSize + 4; // 曲のフォントサイズより4pt大きくする
            ...
            // ★28ptの最大値制限は削除し、幅に収まる限り大きく表示されるようにする
            
            // ヘッダー描画
            simplePdf.setFontSize(headerFontSize); 
            simplePdf.text(headerText, simpleLeftMargin, simpleYPos);
            
            simpleHeaderHeight = headerFontSize * 0.38; // ヘッダーテキストの高さ自体
