...
let touchMoved = false; // touchstartからtouchendまでの間に指が動いたかどうか
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const initializeSetlistItem = (itemElement) => {
        ...
        // ★修正1: 通常のPCからのダブルクリック（dblclickイベント）を再度有効化★
        itemElement.addEventListener("dblclick", (e) => {
            // イベントのターゲットがドラッグハンドルでなければ、削除を試みる
            if (!e.target.closest('.drag-handle')) {
                console.log(`[dblclick] PC double-click detected for item: ${itemElement.dataset.itemId}`);
                removeSetlistItem(itemElement);
                e.stopPropagation(); // 重複するイベント発火を防ぐ
            }
        }); 
        console.log(`[initializeSetlistItem] Added dblclick listener to setlist item: ${itemElement.dataset.itemId}`);

        // ★修正2: スマートフォンでのダブルタップ検出ロジックを改善★
        itemElement.addEventListener('touchstart', (e) => {
            if (e.touches.length === 1) { // シングルタッチのみ対象
                const currentTime = new Date().getTime();
                touchMoved = false; // タッチ開始時に移動フラグをリセット

                if (currentTime - lastTapTime < DOUBLE_TAP_DELAY) {
                    // ダブルタップ候補の場合、すぐにremoveSetlistItemを呼び出さず、
                    // touchendで最終判断するため、ここでは何もしない。
                    // ただし、デフォルトのズームなどを防ぐためにpreventDefault()は必要。
                    e.preventDefault(); 
                    console.log("[touchstart] Potential double tap initiated.");
                }
        ...
        itemElement.addEventListener('touchmove', (e) => {
            // 指がわずかでも動いたら、ダブルタップではなくドラッグと判断
            touchMoved = true;
            // e.preventDefault(); // ここでpreventDefaultするとスクロールなども妨げる可能性があるので注意
        });

        itemElement.addEventListener('touchend', (e) => {
            const currentTime = new Date().getTime();
            // ドラッグ中でなく、かつ、規定時間内に2回目のタップがあり、指が移動していなければダブルタップ
            if (!touchMoved && (currentTime - lastTapTime < DOUBLE_TAP_DELAY && currentTime - lastTapTime > 0)) {
                // ここでhandleDoubleClickを直接呼び出す代わりに、removeSetlistItemを呼び出す
                // event.target.closest('.drag-handle') でドラッグハンドルをタップしたか確認
                if (!e.target.closest('.drag-handle')) {
                    console.log(`[touchend] Confirmed double tap for item: ${itemElement.dataset.itemId}`);
                    removeSetlistItem(itemElement);
                    e.stopPropagation(); // イベント伝播を停止
                    e.preventDefault(); // デフォルト動作を再度防止
                }
            }
            lastTapTime = 0; // ダブルタップ検出のためのタイマーをリセット
            touchMoved = false; // 移動フラグもリセット
        });
    ...
    setlist.querySelectorAll(".setlist-slot").forEach((slot, index) => {
        ...
        // ★修正3: スロット全体のクリックリスナーを調整★
        // 曲と曲の間（空のスロット）をタップしても何も起こらないようにする
        slot.addEventListener('click', (e) => {
            // クリックされたのがスロット自体、または直接の子要素（テキストノードなど）で、
            // かつ setlist-item やチェックボックスのようなインタラクティブな要素でない場合
            if (e.target === slot || e.target.nodeType === 3) { 
                console.log("[slotClick] Clicked on slot background/empty space. No action.");
                // 必要であればここで preventDefault や stopPropagation を追加
                // e.preventDefault(); // これを入れるとスクロールが妨げられる可能性があるので慎重に
                // e.stopPropagation(); // これを入れると親要素のクリックイベントなども止まる
            }
        });
    });
