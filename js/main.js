...
function handleTouchStart(event) {
    ...
    if (tapLength < 300 && tapLength > 0) {
        event.preventDefault(); // ダブルタップの場合はブロック
        if (touchTimeout) {
            clearTimeout(touchTimeout);
            touchTimeout = null;
        }
    ...
    if (event.touches.length === 1) {
        ...
        // ★★★ 修正点1: ドラッグ対象要素を触ったらすぐにpreventDefaultを呼び出す ★★★
        // これにより、タッチ開始直後からブラウザのスクロールをブロックします。
        event.preventDefault(); 
        console.log("[touchstart:Mobile] Touched draggable element. Preventing default (scroll) immediately.");

        isDragging = false; // まだドラッグ中ではない
...
function handleTouchMove(event) {
    ...
    if (!isDragging) {
        ...
        if (distance > DRAG_THRESHOLD) {
            ...
            if (!touchedElement || !draggingItemId) {
                 ...
                 // ★修正点2: ここでドラッグを開始しない場合でも、touchstartでブロックしたpreventDefaultを解除する必要はありません。
                 // なぜなら、touchstartでブロックした時点でスクロールは停止しており、
                 // ここでreturnしてもスクロールが再開されることはないためです。
                 // ユーザーが指を離せば、touchendでクリーンアップされます。
                 return;
            }
            isDragging = true;
            createTouchDraggedClone(touchedElement, initialTouchX, initialTouchY, draggingItemId);
            console.log("[handleTouchMove] Dragging started due to movement threshold. Preventing default (scroll).");
            // ドラッグ開始したら、以降は常にpreventDefault()を呼び続けます。
            // touchstartですでに呼ばれているため、ここでの呼び出しは二重になりますが、
            // 安全のために残しておきます。
            event.preventDefault(); 
        } else {
            // ★★★ 修正点3: しきい値に達していない場合は、event.preventDefault()を呼ばない ★★★
            // しかし、touchstartで既にpreventDefault()を呼んでいるため、ここは何もしません。
            // スクロールはtouchstartで既にブロックされているからです。
            return; 
        }
    }
    ...
    if (isDragging && currentTouchDraggedClone) {
        ...
        event.preventDefault(); // ブラウザにスクロールさせない
        // ... (以降のクローン移動とドロップゾーンハイライトのロジックは変更なし)
        if (rafId !== null) {
...
function createTouchDraggedClone(originalElement, initialX, initialY, itemIdToClone) {
    ...
    if (setlist.contains(originalElement) && originalElement.classList.contains('setlist-item')) {
        ...
        // --- ★ここが最も重要です★ ---
        // 元の要素を「空」の状態に戻すために、clearSlotContent を呼び出します。
        // clearSlotContent は、クラス、データ属性、innerHTML をすべてクリアします。
        // これにより、元の位置が確実に空っぽの「プレースホルダー」になります。
        clearSlotContent(setlist, originalSetlistSlot.dataset.slotIndex); 
        // ただし、clearSlotContent は draggable = false にするので、一時的に draggable を true に戻す
        // または、ドラッグ終了時に改めて設定し直す。
        // ここでは、一時的なプレースホルダーとして使うので、draggableの設定は不要です。
        
        originalSetlistSlot.classList.add('placeholder-slot'); // プレースホルダー用のクラスを追加
