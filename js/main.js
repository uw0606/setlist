...
function handleTouchStart(event) {
    ...
    if (event.touches.length === 1) {
        ...
        // ★★★ 重要な変更点 ★★★
        // セットリスト内のアイテム（setlist-slot.setlist-item）の場合にのみ、長押しドラッグを許可する
        const isInsideSetlist = setlist.contains(touchedElement) && touchedElement.classList.contains('setlist-item');
        ...
        if (isInsideSetlist) {
            ...
            touchTimeout = setTimeout(() => {
                if (draggingItemId && document.body.contains(touchedElement)) {
                    ...
                    console.log("[touchstart:Mobile] Dragging initiated after timeout for setlist item.");
                } else {
            ...
            // ハンバーガーメニューからのアイテム（.item）の場合、長押しではドラッグを開始しない
            console.log("[touchstart:Mobile] Album item touched. Long press drag disabled for album items. Only double click/tap is allowed for adding.");
            // この場合、touchTimeoutをセットしないため、長押しは無視される
            originalSetlistSlot = null;
            currentPcDraggedElement = touchedElement; // ダブルクリック時の restoreToOriginalList のために保持
        }
    }
}
...
function handleTouchEnd(event) {
    ...
    // ★★★ 重要な修正点1: isDragging が true でも、実際にドラッグがあったかを確認する ★★★
    // ここで、指がほとんど動かなかった（＝長押しタップだった）場合はドラッグとみなさない
    const touch = event.changedTouches[0];
    const currentX = touch.clientX;
    const currentY = touch.clientY;
    const deltaX = Math.abs(currentX - touchStartX);
    const deltaY = Math.abs(currentY - touchStartY);
    const dragThreshold = 15; // ドラッグとみなす最小移動距離（ピクセル単位）

    // isDragging が true だが、指の移動が閾値未満の場合は、ドラッグ操作ではないと判断
    if (isDragging && (deltaX < dragThreshold && deltaY < dragThreshold)) {
        console.log("[touchend] Drag initiated but finger moved minimally. Treating as long-press tap, not a drag drop.");
        // この場合はドロップ処理を行わず、単にクリーンアップだけを行う
        finishDragging();
        return;
    }

    // ★★★ 重要な修正点2: isDragging が false の場合は、何もしない（長押しタップではない） ★★★
    // （既存ロジックだが、再確認のため）
    if (!isDragging) {
    ...
    // ドロップターゲットはセットリストスロットか、またはセットリスト外（アルバムに戻す場合）
    ...
    // processDrop には、ドラッグされた要素（クローン）とドロップターゲットを渡す
    // originalSetlistSlot はセットリストからのドラッグの場合にのみ設定されている
...
function finishDragging() {
    ...
    if (currentPcDraggedElement) {
        ...
        // PCドラッグの場合、元の要素がアルバムアイテムなら表示を戻す
        if (!setlist.contains(currentPcDraggedElement)) { // アルバムアイテムの場合
    ...
    if (originalSetlistSlot) {
        ...
        // ★修正点8: originalSetlistSlot の visibility を常に元に戻す
...
function handleDoubleClick(event) {
    ...
    // finishDragging(); // ここも削除（if/elseブロックの外にあった場合）
    // 必要であれば、個別の処理の最後に hideSetlistItemsInMenu() を呼び出すことで、
    // アルバムメニューの表示状態を適切に更新できます。
    hideSetlistItemsInMenu(); // ここで呼び出すのが適切です
}
