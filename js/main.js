    ...
    if (event.touches.length === 1) {
        ...
        touchTimeout = setTimeout(() => {
            ...
            if (draggingItemId && document.body.contains(touchedElement)) {
                ...
                // ★★★ 修正箇所: セットリスト全体の touch-action を 'none' に設定 ★★★
                // これにより、曲を掴んでいる間はブラウザのデフォルトの縦スクロールを完全に阻止します。
                if (setlist) {
                    setlist.style.touchAction = 'none';
                    console.log("[touchstart:Mobile] Setlist touch-action set to none to enable auto-scroll.");
                }
                // -------------------------------------------------------------
                ...
                document.querySelectorAll('.setlist-slot').forEach(slot => {
                    ...
                    // touch-action: pan-y; は不要になりますが、念のため残しておきます。
                    // しかし、親の setlist が none なので、この設定は無効になります。
...
function handleTouchEnd(event) {
    ...
    // ★★★ 修正箇所: セットリストの touch-action を元に戻す ★★★
    // ドラッグ中に 'none' に設定された touch-action を 'unset' に戻し、
    // 通常のユーザーによるスクロールを許可します。
    if (setlist) {
        setlist.style.touchAction = 'unset'; 
        console.log("[touchend] Setlist touch-action reset to unset.");
    }
    // ★★★ 修正ここまで ★★★
 ...
 * @param {number} scrollDirection - スクロール方向 (1:下, -1:上, 0:停止)
 */
function autoScrollSetlist(scrollDirection) {
    ...
    if (setlist) {
        ...
        const newScroll = currentScroll + (SCROLL_SPEED * scrollDirection);

        if (newScroll < 0) {
            setlist.scrollTop = 0;
            autoScrollSetlist(0); // スクロール限界で停止
            return;
        }
    ...
    // 次のフレームを要求し、rafIdを更新
    rafId = requestAnimationFrame(() => autoScrollSetlist(scrollDirection));
}
