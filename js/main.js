...
function handleTouchEnd(event) {
    ...
    if (isDragging && (deltaX < dragThreshold && deltaY < dragThreshold)) {
        ...
        if (potentialDropTargetSlot && !potentialDropTargetSlot.classList.contains('setlist-item')) {
            ...
            isDragging = false; // 強制的にドラッグ状態をリセット
            // ここで finishDragging(true) を呼ぶことで、クローンと元のアイテムの表示が適切に戻る
            finishDragging(true); 
            event.preventDefault(); // ここでも念のためデフォルト動作を防止
            return; // ここで処理を完全に中断
        }
    ...
    // ★★★ 重要な修正点6: processDrop を呼び出す前に、ドロップターゲットの妥当性を最終確認 ★★★
    // ドラッグ＆ドロップで空のスロットにドロップされる場合以外は processDrop を呼び出さない
    if (dropTargetSlot && !originalSetlistSlot && !dropTargetSlot.classList.contains('setlist-item')) {
        // アルバムからのドラッグで、空のスロットにドロップする場合のみ processDrop を実行
        processDrop(currentTouchDraggedClone, dropTargetSlot, originalSetlistSlot);
        finishDragging(); // ドロップが成功した場合はキャンセルフラグなし
    } else if (dropTargetSlot && originalSetlistSlot) {
        // セットリスト内でのドラッグ（入れ替え）の場合のみ processDrop を実行
        processDrop(currentTouchDraggedClone, dropTargetSlot, originalSetlistSlot);
        finishDragging(); // ドロップが成功した場合はキャンセルフラグなし
    } else {
        // それ以外の不適切なドロップ（空のスロットへの長押しタップなど、すでに handledTouchEndで処理済みの場合もここに来る）
        console.log("[touchend] Invalid drop scenario or already handled. Performing cleanup as cancelled.");
        finishDragging(true); // キャンセル扱いとしてクリーンアップ
    }
}
