...
async function generateSetlistPdf() {
    ...
    const tableHeaders = [
        "No.", "タイトル", "R.Gt(克哉)", "L.Gt(彰)", "Bass(信)人", "BPM", "コーラス"
    ];
    ...
    for (const slot of setlistSlots) {
        if (slot.classList.contains('setlist-item')) {
            ...
            if (songData) {
                ...
                // --- シンプルPDF用の行の生成 (ここを修正) ---
                if (isAlbum1) {
                    // album1の曲の場合、No.なしで少しインデントしたタイトル
                    // 必要に応じて、冒頭のスペースの数を調整してインデントを変えられます
                    simplePdfBody.push(`    ${titleText}`); // 4つの半角スペースでインデント
                } else {
                    // album1以外の曲の場合、No.とタイトル
    ...
    try {
        ...
        if (headerText) {
            ...
            detailedPdf.text(headerText, 10, detailedYPos);
            detailedYPos += 10;
        }
        ...
        // ★ここに遅延を追加します（500ミリ秒 = 0.5秒）★
        // これにより、ブラウザが最初のPDFダウンロード処理を完了する時間を与えます。
        ...
        .then((snapshot) => {
          ...
          if (state && state.setlist) {
            ...
            if (state.setlistDate && setlistYear && setlistMonth && setlistDay) {
                const dateParts = state.setlistDate.split('-'); // YYYY-MM-DD を分割
                if (dateParts.length === 3) {
                    setlistYear.value = dateParts[0];
                    setlistMonth.value = dateParts[1];
                    
                    // 月を設定した後、updateDaysを呼び出して日を更新
                    // updateDays関数がこのスコープで利用可能であることを前提とします
                    if (typeof updateDays === 'function') {
                        ...
                        // ここで手動で日を生成するフォールバックロジックを追加することも可能ですが、
                        // updateDays関数を適切に定義・利用することを強く推奨します。
                    }
                    // 日の選択肢が更新された後で、日付を設定
                ...
                // 日付がなかったり要素が見つからなかったりする場合は、ドロップダウンをデフォルト値に戻す
