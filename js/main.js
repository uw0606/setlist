          ...
          .then((snapshot) => {
              ...
              if (state && state.setlist) {
                  ...
                  state.setlist.forEach(itemData => {
                      ...
                      li.innerHTML = `<div class="song-info">${itemData.name}</div>`; // まず曲名のみ追加

                      if (itemData.short) {
                          const checkbox = document.createElement('input');
                          checkbox.type = 'checkbox';
                          checkbox.checked = true;
                          li.querySelector('.song-info').appendChild(checkbox);
                          const label = document.createElement('span');
                          label.textContent = '(Short)';
                          li.querySelector('.song-info').appendChild(label);
                          li.classList.add('short'); // short クラスを付与
                      } else {
                          const checkbox = document.createElement('input');
                          checkbox.type = 'checkbox';
                          li.querySelector('.song-info').appendChild(checkbox);
                      ...
                      const checkboxElement = li.querySelector('.song-info input[type="checkbox"]');
                      if (checkboxElement) {
