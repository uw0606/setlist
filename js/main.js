...
document.addEventListener('DOMContentLoaded', () => {
    console.log("[DOMContentLoaded] Page loaded. Initializing drag and drop and date pickers and modals.");
    ...
    // --- モーダル関連の初期設定 ---
    const open2025ModalButton = document.getElementById('open2025ModalButton'); // モーダルを開くボタン (HTMLで追加が必要)
    ...
    if (open2025ModalButton && year2025DetailModal && close2025DetailModalButton) {
        // 2025年セットリスト詳細モーダルを開く処理
        open2025ModalButton.addEventListener('click', () => {
            year2025DetailModal.classList.add('active'); // activeクラスを追加して表示
        ...
        year2025DetailModal.addEventListener('click', (event) => {
            // イベントのターゲットがモーダルオーバーレイ自体であるかを確認（コンテンツ内クリックでは閉じない）
        ...
        console.warn("[Modal Init] One or more elements for 2025 detail modal not found. Check IDs: 'open2025ModalButton', 'year2025DetailModal', 'close2025DetailModalButton'.");
    }

    // モーダル内の setlist-link のクリックハンドラ (共有IDのロードとモーダルクローズ)
    // この処理は、既存の `setlist-link` へのリスナーに追加・統合されます。
    // `loadSetlistState` はURLから `shareId` を読み込むため、
    // ここでURLを更新してから `loadSetlistState` を呼び出す形が適切です。
            ...
            if (shareIdMatch) {
                ...
                loadSetlistState().then(() => {
                    ...
                    if (year2025DetailModal) {
                        year2025DetailModal.classList.remove('active');
                        console.log("[Modal] 2025 Detail Modal Closed after setlist load.");
                    }
                }).catch(error => {
    ...
    loadSetlistState().then(() => {
      ...
      hideSetlistItemsInMenu(); // メニュー内のアルバムアイテムの表示を更新
    }).catch(error => {
