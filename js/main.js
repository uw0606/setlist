...
function handleDragStart(event) {
    ...
    // ★重要: dataTransferにitemIdを確実にセットする
    event.dataTransfer.setData("text/plain", itemId);
    // event.dataTransfer.setData("text/originalListId", currentPcDraggedElement.dataset.originalListId || ''); // これは現在未使用なので維持
...
function handleDragOver(event) {
    event.preventDefault(); // ★重要: ドロップを許可するために必要
    ...
    if (originalSourceSlot) {
        ...
        if (dropTargetSlot.classList.contains('setlist-item')) {
            ...
            addSongToSlot(originalSourceSlot, targetSongData.itemId, targetSongData.name, {
                ...
                bass: targetId.bass, // <--- ★修正の可能性あり：targetSongData.bass に変更
        ...
        // 移動または入れ替え後、元のスロットをクリア
        // ここでのclearSlotContentは不要です。addSongToSlot内で既にクリアされています。
        // もし、元のスロットを「空のスロット」にするのであれば、`clearSlotContent(originalSourceSlot);` を呼び出す必要があります。
        // ただし、入れ替えの場合はこのままでは元のスロットにターゲットの曲が入るので、クリアはしません。
        // 移動の場合のみ元のスロットをクリアする必要があるため、条件分岐が必要です。
        // 今回はシンプルに、入れ替え後のoriginalSourceSlotは新しい曲が既に入っているのでそのまま、
        // 移動の場合はaddSongToSlotが元のスロットをクリアするように修正します。
        // (現状のaddSongToSlotは渡されたslotElementをクリアしてからコンテンツを入れるので、問題ありません)
    } else {
...
function handleDrop(event) {
    ...
    // セットリスト内からのドラッグの場合
    if (originalSetlistSlot) { // originalSetlistSlot がセットされていればセットリストからのドラッグ
        ...
        // アルバムからのドラッグの場合
        // albumList.querySelector を使うことで、アルバムメニュー内の元の要素を確実に取得
    ...
    // originalSetlistSlot はセットリスト内からのドラッグの場合にのみ null 以外になります。
