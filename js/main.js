 ...
 * @param {Element|null} dropTargetSlot - ドロップされた先のセットリストスロット要素、またはnull
...
function processDrop(draggedElement, dropTargetSlot, originalSetlistSlot = null) {
    ...
    if (isDraggedFromSetlist) {
        if (dropTargetSlot) {
            ...
            console.log("[processDrop] Dropped outside setlist (from setlist). Clearing original slot.");
            clearSlotContent(setlist, originalSetlistSlot.dataset.slotIndex);
            restoreToOriginalList(draggedElement); // アルバムにアイテムを戻す
        }
    } else { // アルバムからのドラッグ
        if (dropTargetSlot) {
            if (dropTargetSlot.classList.contains('setlist-item')) {
            ...
            fillSlotWithItem(dropTargetSlot, draggedItemData);
            document.querySelector(`.album-content .item[data-item-id="${itemId}"]`).style.visibility = 'hidden';
        } else {
    ...
    if (event.touches.length === 1) {
        ...
        touchTimeout = setTimeout(() => {
            if (!isDragging && draggingItemId && document.body.contains(touchedElement)) {
                event.preventDefault(); // ロングプレスによるスクロールを防止
                ...
                console.warn("[touchstart:Mobile] Dragging not initiated after timeout (already dragging or element removed).");
            }
            touchTimeout = null;
...
function handleTouchEnd(event) {
    ...
    if (!isDragging) {
        // ドラッグ状態ではないが、チェックボックスがクリックされた場合はそのまま終了
...
function createTouchDraggedClone(originalElement, initialX, initialY, itemIdToClone) {
    ...
    // クラスとデータ属性のコピー
    // datasetループでほとんどコピーされるが、念のため主要なクラスも明示的にコピー
...
function finishDragging() {
    ...
    if (currentPcDraggedElement && currentPcDraggedElement.classList.contains("dragging")) {
    ...
    if (originalSetlistSlot) {
        ...
        if (originalSetlistSlot.classList.contains('setlist-item')) {
            originalSetlistSlot.style.visibility = ''; // アイテムが残っている場合のみ可視に戻す
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- ドラッグ＆ドロップ関連の初期設定 ---
    document.querySelectorAll(".album-content .item").forEach(enableDragAndDrop);
    setlist.querySelectorAll(".setlist-slot").forEach((slot, index) => {
        if (!slot.dataset.slotIndex) { // slotIndexが未設定の場合のみ設定
