...
function handleTouchStart(event) {
    event.preventDefault(); // デフォルトのブラウザ操作（ズーム、スクロールなど）を防止
...
function updateSlotContent(slotElement, songName, options) {
    ...
    if (slotElement.dataset.itemId === 'album1-custom') {
        ...
        // ★★★ ここに dblclick イベントリスナーを追加します ★★★
        customNameInput.addEventListener('dblclick', (e) => {
            e.stopPropagation(); // 親要素へのイベント伝播を停止
            console.log("[custom-song-input] dblclick on input. Stopped propagation to prevent removal.");
        });
