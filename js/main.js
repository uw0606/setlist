...
function handleDoubleClick(event) {
    ...
    // ダブルクリックされたアイテムがセットリストのスロット内の曲であるかを確認
    // PCの場合とタッチデバイスの場合でイベントターゲットのパスが異なるため、両方を考慮
    let itemElement = event.target.closest('.setlist-slot.setlist-item');

    // itemElement が null の場合、それは有効なセットリストアイテムではない
    if (!itemElement) {
        console.log("[handleDoubleClick] No valid setlist item found for double click. Event target was:", event.target);
        return; // 有効なアイテムでなければ処理を中断
    }

    const slotIndex = itemElement.dataset.slotIndex;
    const itemId = itemElement.dataset.itemId;
    console.log(`[handleDoubleClick] Double-clicked item: ID=${itemId}, Slot Index=${slotIndex}`);

    if (itemId) {
        // 現在の曲のデータを取得
        const songName = itemElement.dataset.songName;
        const isShortVersion = itemElement.dataset.isShortVersion === 'true';
        const hasSeOption = itemElement.dataset.hasSeOption === 'true';
        const drumSoloOption = itemElement.dataset.drumsoloOption === 'true';
        const hasRgt = itemElement.dataset.rGt === 'true';
        const hasLgt = itemElement.dataset.lGt === 'true';
        const hasBass = itemElement.dataset.bass === 'true';
        const hasBpm = itemElement.dataset.bpm === 'true';
        const hasChorus = itemElement.dataset.chorus === 'true';

        // 特定のキーを押しながらダブルクリックされた場合 (PCのみ想定)
        if ((event.ctrlKey || event.metaKey) && !isTouchDevice) {
            console.log("[handleDoubleClick] Ctrl/Meta key pressed. Restoring to original list.");
            restoreToOriginalList(itemElement);
        } else {
            console.log("[handleDoubleClick] Toggling options.");
            // 短縮版、SEオプション、ドラムソロ、楽器有無、BPM、コーラスのいずれかがある場合にトグル処理
            if (isShortVersion || hasSeOption || drumSoloOption || hasRgt || hasLgt || hasBass || hasBpm || hasChorus) {
                // ここでオプションをトグルするロジックを実装
                // 例: 短縮版のトグル
                if (itemElement.dataset.isShortVersion !== undefined) {
                    itemElement.dataset.isShortVersion = (!isShortVersion).toString();
                    console.log(`[handleDoubleClick] Toggled short version to ${itemElement.dataset.isShortVersion}`);
                }
                // SEオプションのトグル
                if (itemElement.dataset.hasSeOption !== undefined) {
                    itemElement.dataset.hasSeOption = (!hasSeOption).toString();
                    console.log(`[handleDoubleClick] Toggled SE option to ${itemElement.dataset.hasSeOption}`);
                }
                // ドラムソロオプションのトグル
                if (itemElement.dataset.drumsoloOption !== undefined) {
                    itemElement.dataset.drumsoloOption = (!drumSoloOption).toString();
                    console.log(`[handleDoubleClick] Toggled drum solo option to ${itemElement.dataset.drumsoloOption}`);
                }
                // RGTのトグル
                if (itemElement.dataset.rGt !== undefined) {
                    itemElement.dataset.rGt = (!hasRgt).toString();
                    console.log(`[handleDoubleClick] Toggled R-GT option to ${itemElement.dataset.rGt}`);
                }
                // LGTのトグル
                if (itemElement.dataset.lGt !== undefined) {
                    itemElement.dataset.lGt = (!hasLgt).toString();
                    console.log(`[handleDoubleClick] Toggled L-GT option to ${itemElement.dataset.lGt}`);
                }
                // BASSのトグル
                if (itemElement.dataset.bass !== undefined) {
                    itemElement.dataset.bass = (!hasBass).toString();
                    console.log(`[handleDoubleClick] Toggled Bass option to ${itemElement.dataset.bass}`);
                }
                // BPMのトグル
                if (itemElement.dataset.bpm !== undefined) {
                    itemElement.dataset.bpm = (!hasBpm).toString();
                    console.log(`[handleDoubleClick] Toggled BPM option to ${itemElement.dataset.bpm}`);
                }
                // Chorusのトグル
                if (itemElement.dataset.chorus !== undefined) {
                    itemElement.dataset.chorus = (!hasChorus).toString();
                    console.log(`[handleDoubleClick] Toggled Chorus option to ${itemElement.dataset.chorus}`);
                }

                // スロットを再描画して変更を反映
                updateSlotContent(itemElement, songName, {
                    isShortVersion: itemElement.dataset.isShortVersion === 'true',
                    hasSeOption: itemElement.dataset.hasSeOption === 'true',
                    drumsoloOption: itemElement.dataset.drumsoloOption === 'true',
                    rGt: itemElement.dataset.rGt === 'true',
                    lGt: itemElement.dataset.lGt === 'true',
                    bass: itemElement.dataset.bass === 'true',
                    bpm: itemElement.dataset.bpm === 'true',
                    chorus: itemElement.dataset.chorus === 'true'
                });
            } else {
                // オプションがない場合はアルバムに戻す
