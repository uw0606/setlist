...
function createTouchDraggedClone(originalElement, initialX, initialY, itemIdToClone) {
    ...
    if (setlist.contains(originalElement) && originalElement.classList.contains('setlist-item')) {
        ...
        originalSetlistSlot.classList.add('placeholder-slot');
        // originalElement.style.visibility = 'hidden'; // ★削除★
        // 代わりに、CSSで placeholder-slot に opacity などを使って視覚的に隠す
        console.log(`[createTouchDraggedClone] Original setlist slot ${originalSetlistSlot.dataset.slotIndex} marked as placeholder. Visibility now controlled by CSS.`);
    } else {
        // アルバムリストからのアイテム
...
function finishDragging() {
  ...
  if (originalSetlistSlot && originalSetlistSlot.classList.contains('placeholder-slot')) {
      ...
      // ★修正: ここで visibility をリセットする代わりに、CSSで placeholder-slot の opacity を元に戻すか、
      // slotがセットリストから削除された場合は clearSlotContent が visibility をリセットする
      // ここでは特別な visibility リセットは不要になる場合が多い
      // if (originalSetlistSlot.classList.contains('setlist-item')) {
      //     originalSetlistSlot.style.visibility = ''; // この行はCSSで制御するため不要になる
      ...
      // }
      console.log(`[finishDragging] Removed 'placeholder-slot' class from ${originalSetlistSlot.dataset.slotIndex}.`);
