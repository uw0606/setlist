...
function handleTouchStart(event) {
    ...
    const isInAlbumContent = touchedElement.closest('.album-content'); // または .menu .item のように具体的なセレクタ
    ...
    if (tapLength < 300 && tapLength > 0) {
        ...
        event.preventDefault(); // ダブルタップの場合は、ブラウザのズームやスクロールを防ぐ
        ...
        if (isInSetlist && touchedElement.classList.contains('setlist-item')) {
            ...
            if (typeof handleMoveItemToMenu === 'function') { // 関数が定義されているか確認
                handleMoveItemToMenu(touchedElement); 
            } else {
                console.warn("handleMoveItemToMenu function not defined.");
            }
            ...
            if (typeof handleAddItemToSetlist === 'function') { // 関数が定義されているか確認
                handleAddItemToSetlist(touchedElement); 
            } else {
                console.warn("handleAddItemToSetlist function not defined.");
            }
        } else {
        ...
        lastTapTime = 0;
        // ダブルクリック処理が完了したので、ドラッグ状態は開始しない
        isDragging = false; 
        draggingItemId = null;
        currentTouchDraggedClone = null;
        originalSetlistSlot = null;
        currentDropZone = null;
        return; // ダブルタップで処理が完結するため、これ以上は処理しない
    }
    lastTapTime = currentTime; // 次のタップのための時間を記録

    // シングルタッチの場合のドラッグ開始処理
    if (event.touches.length === 1) {
        isDragging = false; // タッチ開始時点ではドラッグ中ではない

        // ドラッグ開始に必要な情報をセット
        if (isInSetlist && touchedElement.classList.contains('setlist-item')) {
            originalSetlistSlot = touchedElement; // セットリスト内移動の可能性がある
            console.log(`[touchstart] Touch on setlist item.`);
        } else if (isInAlbumContent && touchedElement.classList.contains('item')) {
            originalSetlistSlot = null; // アルバムアイテムからのドラッグ (セットリストへの追加)
            console.log(`[touchstart] Touch on album item.`);
        } else {
            console.warn("[touchstart] Touched element is not a recognized draggable item for long press. Aborting.");
        ...
        // ここでロングプレスのタイマーを設定
        // セットリストからのメニューへのドラッグ（長押し）は無効にするため、
        // isInSetlist && !isInAlbumContent の場合はタイマーを設定しない
        if (isInAlbumContent) {
            ...
            touchTimeout = setTimeout(() => {
                if (!isDragging && draggingItemId && body.contains(initialTouchElement)) {
                    // ここで event.preventDefault() を呼び出すことで、ロングプレスが確定した後のブラウザのスクロールを防ぐ
                    // ただし、タッチ移動が閾値を超えた場合にのみ preventDefault を呼ぶ方が自然なスクロール体験になるため、
                    // handleTouchMove に任せることも検討
                    // event.preventDefault(); // ここで呼ぶと、ロングプレスまでのスクロールもブロックされる
            ...
            // セットリストからメニューに戻す長押しはここでは許可しない (上記ifでAlbumContentではないことを確認)
            ...
            console.log("[touchstart] Unrecognized touch location, no long press timer set for drag.");
        }
    }
}
...
function handleTouchMove(event) {
    ...
    rafId = requestAnimationFrame(() => {
        ...
        if (!isDragging) {
            // 移動距離が閾値を超えていたら、スクロール目的の移動と判断し、ロングプレスのタイマーをクリア
            if (deltaX > DRAG_THRESHOLD || deltaY > DRAG_THRESHOLD) {
                ...
                // デフォルトのスクロール動作を許可するため、ここでは event.preventDefault() を呼ばない
            }
            rafId = null;
        ...
        if (!currentTouchDraggedClone) {
            console.warn("[handleTouchMove] Dragging clone is null, but isDragging is true.");
            rafId = null;
            return;
        }
...
function handleTouchEnd(event) {
    ...
    if (touchTimeout) {
        clearTimeout(touchTimeout);
        touchTimeout = null;
        console.log("[handleTouchEnd] Long press timer cleared because touch ended before drag initiated.");
    }

    if (isDragging) {
        event.preventDefault(); // ドラッグが完了した場合、デフォルト動作を抑制
        console.log("[handleTouchEnd] Dragging ended. Processing drop.");
        ...
        if (endTouch) {
            ...
            const dropTargetIsSetlistSlot = targetElement ? targetElement.closest('.setlist-slot') : null;
            ...
            if (draggedFromAlbum && dropTargetIsSetlistSlot) {
                ...
                if (typeof handleDroppedItem === 'function') {
                    handleDroppedItem(draggingItemId, dropTargetIsSetlistSlot); // 仮の関数名。実際のロジックを呼び出す
                }
                ...
                if (typeof handleDroppedItem === 'function') {
                    handleDroppedItem(draggingItemId, dropTargetIsSetlistSlot, originalSetlistSlot); // 仮の関数名、元のスロット情報も渡す
                }
                displayMessage('セットリスト内で曲を移動しました。');
            } else {
                // 無効なドロップ、またはセットリストからメニューへのドラッグ（これは長押しでは無効）
                console.log("[handleTouchEnd] Invalid drop target or operation. Item returned to original place if applicable.");
                if (draggedFromSetlist && originalSetlistSlot) {
                     // 元の要素を非表示にしていた場合は表示に戻す
        ...
        console.log("[handleTouchEnd] Tap/short press detected, not a drag. No further action for drag.");
        // ここでショートタップの処理が必要な場合は追加しない。
        // ダブルタップはhandleTouchStartで処理済みのため。
    }
}
