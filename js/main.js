...
async function generateSetlistPdf() {
    ...
    try {
        ...
        // ★★★ 修正3: rightMarginをleftMarginと同じ20mmに設定 ★★★
        ...
        if (numSimpleRows > 0) {
            // 全体調整ロジック (行の高さ、ベースフォントサイズ調整)
            const simpleAvailableHeight = pageHeightSimple - simpleTopMargin - bottomMargin - simpleHeaderHeight;
            ...
            simplePdfBody.forEach(row => {
                ...
                if (row.isItem) {
                    // ★★★ 修正4: 曲名が長い場合のみフォントサイズを縮小するロジック ★★★
                    
                    // 最初に計算された動的なフォントサイズを適用
                    simplePdf.setFontSize(dynamicFontSize);
                    
                    // 現在のフォントサイズでのテキスト幅を測定
                    ...
                    // テキストスロットは通常のフォントサイズを維持 (動的フォントサイズがない場合はBASE_FONT_SIZEを使用)
                    simplePdf.setFontSize(dynamicFontSize);
                }
