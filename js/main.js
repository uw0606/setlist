...
function handleTouchStart(event) {
    ...
    // ダブルタップの場合はpreventDefault()を呼ばない。ブラウザのdblclickイベントに任せるため。
    if (tapLength < 300 && tapLength > 0) {
        ...
        // event.preventDefault(); // ダブルタップではpreventDefault()を呼ばない
        event.stopPropagation(); // イベントの伝播は停止 (これは残す)
    ...
    if (event.touches.length === 1) {
        // isDragging = false; // ここではisDraggingをtrueにしない。タイマー発火時に設定する。
        ...
        if (isInSetlist || isInAlbumContent) {
            ...
            touchTimeout = setTimeout(() => {
                ...
                if (!isDragging && draggingItemId && document.body.contains(initialTouchElement) &&
                    deltaX <= DRAG_THRESHOLD && deltaY <= DRAG_THRESHOLD) { // 追加条件: 閾値を超えていないこと
    ...
    rafId = requestAnimationFrame(() => {
        ...
        if (!isDragging) {
            ...
            if (deltaX > DRAG_THRESHOLD || deltaY > DRAG_THRESHOLD) {
                ...
                // 閾値を超えて長押しがキャンセルされたら、ここでクリーンアップを実行
                // これにより、長押し失敗時の「固まり」を防ぎ、次の操作に備える
                finishDragging(); // isDraggingをfalseにし、クローンなどをクリーンアップ
            }
            rafId = null;
...
function enableDragAndDrop(element) {
    if (element.classList.contains('item')) {
        ...
        element.addEventListener("dragleave", handleDragLeave); // 追記された dragleave イベントリスナー
    }
}
...
async function generateSetlistPdf() {
  ...
  try {
    ...
    if (headerText) {
      ...
      detailedPdf.text(headerText, pageWidth / 2, detailedYPos + headerCellHeight / 2 + 0.5, {
        align: 'center',
        baseline: 'middle'
      });
    ...
    detailedPdf.autoTable({
      ...
      didDrawPage: function(data) {
        ...
        detailedPdf.text(str, detailedPdf.internal.pageSize.getWidth() - 10, detailedPdf.internal.pageSize.getHeight() - 10, {
          align: 'right'
        });
      },
    });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  setlistLinks.forEach(link => {
    link.addEventListener('click', (event) => {
      event.preventDefault();
      const shareIdToLoad = event.target.dataset.shareId;
      if (shareIdToLoad) {
        // 現在のURLからshareIdパラメータを削除し、新しいshareIdを追加
        const currentUrl = new URL(window.location.href);
        currentUrl.searchParams.set('shareId', shareIdToLoad);
        window.history.pushState({}, '', currentUrl.toString());

        year2025DetailModal.style.display = 'none'; // 詳細モーダルを閉じる
        // セットリストをロード
        loadSetlistState().then(() => {
          showMessageBox('セットリストをロードしました！');
          toggleMenu(); // ハンバーガーメニューを再度開く
          console.log(`[pastSetlists] Loaded setlist for shareId: ${shareIdToLoad}. Restored hamburger menu.`);
        }).catch(error => {
          console.error("[pastSetlists] Failed to load setlist from link:", error);
          showMessageBox('セットリストのロードに失敗しました。');
          toggleMenu(); // エラー時もメニューは開く
        });
      } else {
        console.warn("[pastSetlists] Setlist link clicked without a share ID.");
        showMessageBox('セットリストIDが見つかりません。');
      }
    });
  });
  ...
  if (menuButton && menu) {
    ...
    console.warn("[DOMContentLoaded] Hamburger menu button or menu element not found.");
  }
  // メニューオーバーレイをクリックで閉じる
  if (menuOverlay) {
    menuOverlay.addEventListener('click', toggleMenu);
  } else {
    console.warn("[DOMContentLoaded] Menu overlay element not found.");
  }
});