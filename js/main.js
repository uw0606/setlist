...
let initialTouchX = 0; // ★追加: タッチ開始時のX座標
let initialTouchY = 0; // ★追加: タッチ開始時のY座標
let touchStartX = 0;
let touchStartY = 0;
let lastTapTime = 0;
...
const LONG_PRESS_DELAY = 600; // 長押しと判断する時間（ミリ秒） - 例: 300ms
const DRAG_THRESHOLD = 10;   // ドラッグ開始とみなす最小移動距離 (ピクセル) - 現状は30ですが、長押しと組み合わせるなら短くしても良いでしょう
...
function handleTouchStart(event) {
    ...
    if (tapLength < 300 && tapLength > 0) {
        if (touchTimeout) { // 長押しタイマーがあればクリア
            clearTimeout(touchTimeout);
            touchTimeout = null;
        }
        handleDoubleClick(event);
        lastTapTime = 0;
        event.preventDefault(); // ダブルタップ時はpreventDefault
        isDragging = false;
        return;
    }
    lastTapTime = currentTime;

    // シングルタッチかつドラッグ可能な要素に触れた場合
    if (event.touches.length === 1) {
        console.log("[touchstart:Mobile] Touched draggable element (non-checkbox, single tap):", touchedElement);

        isDragging = false; // まだドラッグ開始としない
        ...
        // ★新規追加: 長押し判定のためのタイマーを開始
        // このタイマーが発火するまで、ドラッグは開始されない
        touchTimeout = setTimeout(() => {
            console.log("[touchstart:Mobile] Long press delay passed. Now ready for drag if moved.");
            // タイマーが発火したらnullにする (タイマー完了を示す)
            touchTimeout = null;
        }, LONG_PRESS_DELAY);
...
function handleTouchMove(event) {
    if (draggingItemId === null || event.touches.length > 1) {
        return;
    }
    ...
    if (!isDragging) {
        ...
        // ★★★ 変更箇所: 長押しタイマーが完了している AND 閾値を超えた場合のみドラッグ開始 ★★★
        if (touchTimeout === null && (deltaX > DRAG_THRESHOLD || deltaY > DRAG_THRESHOLD)) {
            isDragging = true;
            console.log("[handleTouchMove:Mobile] Dragging started (long press ready + threshold exceeded).");

            const touchedElement = event.target.closest(".setlist-slot.setlist-item") || event.target.closest(".item");
            if (touchedElement) {
                const rect = touchedElement.getBoundingClientRect();
                touchOffsetX = currentX - rect.left;
                touchOffsetY = currentY - rect.top;
                createTouchDraggedClone(touchedElement, currentX, currentY, draggingItemId);
                event.preventDefault(); // ドラッグ中はpreventDefault
            } else {
                console.warn("[handleTouchMove:Mobile] touchedElement not found for drag start. Cancelling drag.");
                isDragging = false;
                draggingItemId = null;
                return;
            }
        } else {
            // 長押しタイマーが完了していない、または閾値を超えていない場合は、スクロールを許可するために何もしない
            return;
        }
    }

    // isDragging が true の場合にのみ、クローンの移動と preventDefault を実行
    if (isDragging && currentTouchDraggedClone) {
        event.preventDefault(); // ドラッグ中は常に preventDefault を呼び出す
        ...
        rafId = requestAnimationFrame(() => {
            ...
            currentTouchDraggedClone.style.left = (currentX - touchOffsetX) + 'px';
            currentTouchDraggedClone.style.top = (currentY - touchOffsetY) + 'px';
...
function handleTouchEnd(event) {
    console.log("[handleTouchEnd:Mobile] Touch ended.");

    // ★変更なし: touchTimeout が残っていたらクリア
    // この変更点では touchTimeout が null になるのはタイマー完了時のみなので、
    // ここでクリアするのは主にユーザーが長押し時間内に指を離した場合。
    if (touchTimeout) {
        clearTimeout(touchTimeout);
        touchTimeout = null;
        console.log("[handleTouchEnd:Mobile] Long press timer cleared (touch ended before timer completion).");
    }
    ...
    if (isDragging && draggingItemId !== null) {
        if (currentDropZone) {
            const dropTarget = currentDropZone;
            processDrop(dropTarget, draggingItemId, originalSetlistSlot ? originalSetlistSlot._originalItemData : null);
            currentDropZone.classList.remove('drag-over');
        } else if (originalSetlistSlot) {
            const originalSlotIndex = originalSetlistSlot.dataset.slotIndex;
            const originalData = originalSetlistSlot._originalItemData;
            console.log(`[handleTouchEnd] Mobile: Item ${draggingItemId} was dragged but not dropped on a valid zone. Restoring to original slot ${originalSlotIndex}.`);
            clearSlotContent(setlist, originalSlotIndex);
            fillSlotWithItem(originalSetlistSlot, originalData);
        } else {
            restoreToOriginalList(draggingItemId);
            console.log(`[handleTouchEnd] Mobile: Item ${draggingItemId} was dragged from album but not dropped. Restoring in album menu.`);
        }
    } else {
        console.log("[handleTouchEnd:Mobile] Not a drag operation or item was null.");
    }
 ...
 * @param {HTMLElement} originalElement - ドラッグ元の要素
 * @param {number} initialX - タッチ開始時のクライアントX座標
 * @param {number} initialY - タッチ開始時のクライアントY座標
 * @param {string} itemIdToClone - ドラッグするアイテムのID (必ず指定される想定)
 */
function createTouchDraggedClone(originalElement, initialX, initialY, itemIdToClone) {
    if (currentTouchDraggedClone) {
        currentTouchDraggedClone.remove(); // 既存のクローンがあれば削除
        currentTouchDraggedClone = null;
    }

    // 元の要素が存在しない、またはDOMツリーにない場合は処理を中止
    ...
    currentTouchDraggedClone.classList.add("dragging", "touch-dragging-clone"); // ドラッグ中を示すクラスを追加
    currentTouchDraggedClone.style.display = 'block'; // 確実に表示状態に

    // 元の要素から必要なクラスをコピー
    // 短縮版、SEオプション、ドラムソロオプションのクラスをコピー
    ...
    if (originalElement.classList.contains('drumsolo-active')) { // ドラムソロに関するクラスを追加
        currentTouchDraggedClone.classList.add('drumsolo-active');
    }

    // 元の要素の全てのdata-*属性をクローンにコピー
    // これにより、item-id, is-short-version, has-se-option など全てのデータ属性がコピーされます。
    // その後の個別の dataset コピーは冗長になりますが、明確さのため一部残します。
    for (const key in originalElement.dataset) {
        currentTouchDraggedClone.dataset[key] = originalElement.dataset[key];
    }
    // ただし、itemIdToClone が明示的に渡されている場合はそれを優先
    currentTouchDraggedClone.dataset.itemId = itemIdToClone;

    // 特定のデータ属性を明示的にコピー（念のため、上記のループでコピーされますが、重要なものはここで確認できます）
    // drumsoloChecked は動的に更新される可能性があるので、明示的にコピーする意味があるかもしれません。
    ...
    if (originalElement.dataset.hasDrumsoloOption) { // ドラムソロに関するデータ属性を追加
        currentTouchDraggedClone.dataset.hasDrumsoloOption = originalElement.dataset.hasDrumsoloOption;
    }
    // drumsoloCheckedは、ドラッグ開始時の状態を正確に反映するためにも明示的にコピーが望ましい
    ...
    // 楽器パートやBPM、コーラスなどもここでコピー（forループで既にコピーされますが、明示的に）
    if (originalElement.dataset.rGt) { currentTouchDraggedClone.dataset.rGt = originalElement.dataset.rGt; }
    if (originalElement.dataset.lGt) { currentTouchDraggedClone.dataset.lGt = originalElement.dataset.lGt; }
    if (originalElement.dataset.bass) { currentTouchDraggedClone.dataset.bass = originalElement.dataset.bass; }
    if (originalElement.dataset.bpm) { currentTouchDraggedClone.dataset.bpm = originalElement.dataset.bpm; }
    if (originalElement.dataset.chorus) { currentTouchDraggedClone.dataset.chorus = originalElement.dataset.chorus; }

    // クローンをbodyに追加
    document.body.appendChild(currentTouchDraggedClone);

    // 元の要素がセットリスト内のアイテムだった場合の処理
    if (setlist.contains(originalElement) && originalElement.classList.contains('setlist-item')) {
        originalSetlistSlot = originalElement;  // 元のスロットを記録 (スワップや元に戻す際に使用)
        const originalItemData = getSlotItemData(originalElement); // 元のスロットのデータを取得
        if (originalItemData) {
            originalSetlistSlot._originalItemData = originalItemData; // 内部プロパティにデータを保存
        ...
        originalSetlistSlot.classList.add('placeholder-slot'); // 元のスロットをプレースホルダーとしてマーク
        ...
        // 元の要素がアルバムリストからのアイテムだった場合の処理
        originalElement.style.visibility = 'hidden'; // 元の要素を非表示にする
        originalSetlistSlot = null; // セットリストからのドラッグではないのでnull
        console.log(`[createTouchDraggedClone] Original album item ${originalElement.dataset.itemId} hidden.`);
    }

    // ドラッグ元のリストを記録 (アルバムリストからドラッグした場合の復元用)
    ...
    currentTouchDraggedClone.style.position = 'fixed'; // 画面に固定して移動
    currentTouchDraggedClone.style.zIndex = '10000'; // 他の要素より手前に表示
    currentTouchDraggedClone.style.width = rect.width + 'px'; // 元の要素の幅に合わせる
    currentTouchDraggedClone.style.height = rect.height + 'px'; // 元の要素の高さに合わせる
    ...
    currentTouchDraggedClone.style.pointerEvents = 'none'; // クローン自身がイベントを発生させないようにする
