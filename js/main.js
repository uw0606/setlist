...
function changeAlbumView() {
    ...
    if (mode === 'name_order') {
        ...
        // ★修正ポイント: アルバム順に戻る際はフラグをリセットし、ナビゲーションを有効に戻す★
...
function buildNameOrderList() {
    ...
    if (!nameOrderContent || !nameOrderNav) {
        // エラー時の処理を修正。構築フラグをリセット。
        isBuildingList = false; 
        return; 
    }

    // ★★★ 修正: データの再収集と初期化を強制的に行う ★★★
...
function clearAndDrawNameOrderList() {
    ...
    for (const groupKey in nameOrderGroups) {
        ...
        // 1. グループコンテナを作成 (ナビゲーションクリック時に表示/非表示を切り替えるターゲット)
        ...
        songs.forEach(songData => {
            // createAlbumItemElementは、既に提供済みのコードで定義済み
            const itemElement = createAlbumItemElement(songData);
            ul.appendChild(itemElement);
            
            // ★重要★ 生成したアイテムにD&Dイベントを登録
            // enableDragAndDropは既に提供済みのコードで定義済み
...
/**
 * 曲データのソートキーに基づいてグループに分類する。（データ属性を使用するように修正）
...
function groupSongs(songs) {
    ...
    songs.forEach(song => {
        let firstChar = ''; // グループ分けに使用するキーの最初の文字

        // 1. 英字ソートキー (alphaSort) があれば優先
        if (song.alphaSort) {
            firstChar = song.alphaSort.trim().charAt(0).toUpperCase();
        } 
        // 2. 日本語ソートキー (kanaSort) があれば次に優先
        else if (song.kanaSort) {
            firstChar = song.kanaSort.trim().charAt(0); // ひらがなで取得
        } 
        // 3. どちらもない場合は表示名 (songName)
        else {
            firstChar = song.songName.trim().charAt(0).toUpperCase(); // 英字判定のために大文字化
        }

        // --- 分類ロジック ---
        
        // 英字の分類 (alphaSortまたはsongNameの大文字)
        ...
        // 数字・記号の分類
        else if (/[0-9\W]/.test(firstChar)) groups['ETC_NONE'].push(song); 
        
        // 日本語の分類 (kanaSortまたはsongNameの最初の文字)
        else {
            // 日本語判定のため、ここでは大文字化されていない文字を使用
            const originalFirstChar = song.kanaSort ? song.kanaSort.trim().charAt(0) : song.songName.trim().charAt(0);

            const jpGroup = japaneseMap[originalFirstChar];
            if (jpGroup) {
 ...
 */
function toggleNameOrderGroup(navItem, groupKey) {
    console.log(`[TOGGLE DEBUG] Clicked group: ${groupKey}`); // ★デバッグログ追加★
    ...
    // 要素が見つからない場合のログを強化
    if (!contentToDisplay) {
        console.error(`[TOGGLE ERROR] Content element #name-group-${groupKey} not found!`); 
        return; 
    }
    ...
    if (contentToDisplay) {
        contentToDisplay.classList.remove('hidden');
        contentToDisplay.style.display = 'block'; // ★強制表示を追加★ (CSSの問題回避)
        navItem.classList.add('active');
    ...
    console.log(`[TOGGLE DEBUG] Displaying: #name-group-${groupKey}`); // ★デバッグログ追加★
}
