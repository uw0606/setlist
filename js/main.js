...
function processDrop(draggedElement, dropTargetSlot, originalSetlistSlot = null) {
    ...
    // draggedElement から itemId を安全に取得
    ...
    const draggedItemData = getSlotItemData(draggedElement);
    if (!draggedItemData) {
        console.error("[processDrop] Failed to get item data for draggedElement. Aborting processDrop.");
        return;
    }
    console.log("[processDrop] draggedItemData:", draggedItemData);

    const isDraggedFromSetlist = originalSetlistSlot !== null; // originalSetlistSlotがあればセットリストからのドラッグ
    console.log("[processDrop] isDraggedFromSetlist:", isDraggedFromSetlist);

    if (isDraggedFromSetlist) {
        // ⭐ 修正箇所: セットリストからドラッグされた場合のドロップターゲットがない場合
        if (dropTargetSlot) {
            // セットリスト内の別のスロットにドロップされた場合
            ...
            // セットリストからドラッグして、セットリスト外にドロップした場合
            // ⭐⭐⭐ ここが重要: スマホでのタッチドラッグの場合、意図せずセットリスト外にドロップされることが多い。
            // その場合、元のリストに戻すのではなく、単に「移動しなかった」と見なす。
            console.log("[processDrop] Dropped outside setlist (from setlist).");
            // `restoreToOriginalList(draggedElement)` をここでは呼ばない！
            // 代わりに、元のスロットの内容がそのまま残るようにする (visibility は finishDragging で戻る)
            console.log("[processDrop] Item was dragged from setlist and dropped outside. No action (will return to original setlist slot visually).");
            // ここで clearSlotContent を呼ばないことで、セットリストから消えるのを防ぐ
            // finishDragging() が呼ばれた時に originalSetlistSlot の visibility が元に戻るので、
            // 結果として元の位置に曲がそのまま残るように見えます。
        }
    } else { // アルバムからのドラッグ (このブロックはログから問題なさそうなので変更なし)
        if (dropTargetSlot) {
            if (dropTargetSlot.classList.contains('setlist-item')) {
