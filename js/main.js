...
function handleTouchStart(event) {
    ...
    // --- 既存のリセット処理 ---
    if (isDragging || currentTouchDraggedClone || currentDropZone || draggingItemId || originalSetlistSlot || touchTimeout) { // touchTimeoutもリセット対象に追加
        ...
        if (touchTimeout) { // 新しく追加: 既存のタイマーをクリア
    ...
    if (event.touches.length === 1) {
        ...
        touchTimeout = setTimeout(() => {
            // タイマーが発火した時点で、まだドラッグが開始されておらず、かつ指が大きく動いていないことを確認
            // (DRAG_THRESHOLD を超えていないことを、handleTouchMove側でより厳密にチェック)
            if (!isDragging && draggingItemId) { // draggingItemId がセットされていることを確認
                console.log("[touchstart:Mobile] Long press detected. Initiating drag.");
                // ドラッグを開始する要素を取得
                ...
                if (elementToClone && document.body.contains(elementToClone)) {
                    ...
                    // ここで event.preventDefault() を呼ぶことでスクロールをブロック
                    // ただし、touchstart内ではイベントオブジェクトが異なるため、
                    // handleTouchMoveで改めてpreventDefaultを呼ぶか、
                    // または、長押しでドラッグが開始されたことを示すフラグを立てて、
                    // handleTouchMoveの冒頭でpreventDefaultを呼ぶのがより安全です。
                    // 今回は、handleTouchMoveがドラッグの開始を最終的に制御する形を維持します。
                } else {
                    console.warn("[touchstart:Mobile] Long press detected but elementToClone not found. Aborting drag.");
...
function handleTouchMove(event) {
    ...
    // ★修正: ドラッグ開始ロジックを変更★
    // isDragging がまだ false の場合
    if (!isDragging) {
        // 長押しタイマーが設定されている場合、DRAG_THRESHOLD を超えたら長押しをキャンセル（スクロールを許可）
        if (touchTimeout) {
            if (distance > DRAG_THRESHOLD) {
                clearTimeout(touchTimeout);
                touchTimeout = null;
                console.log("[handleTouchMove] Cleared long press timeout due to movement exceeding threshold.");
                // スクロールを許可するために、ここではpreventDefault()を呼ばない
                return; // ドラッグは開始しない
            }
        }
        // 長押しタイマーがまだ発火していない、またはキャンセルされた場合は、ドラッグを開始しない
        // 長押しタイマーが発火して isDragging が true になるのを待つ
        return;
    }
