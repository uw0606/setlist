...
async function generateSetlistPdf() {
    ...
    for (const slot of setlistSlots) {
        if (slot.classList.contains('setlist-item')) {
            ...
            // ★★★ 修正箇所: シンプルPDFの曲名始まりを揃える ★★★
            // No.ありの曲のフォーマットを最大2桁番号（例: '10.'） + スペース1個分と定義し、
            // 全体で4文字分の幅('XX. ')を確保する。
            const paddingBase = '    '; 

            if (isAlbum1) {
                // No.なしの曲の場合: No.と区切りを空白で置き換えて曲名を揃える
                simplePdfBody.push(`${paddingBase}${titleText}${simpleEffectNote}`);
            } else {
                // No.ありの曲の場合: No.と区切りをつけ、残りスペースを調整
            ...
            // ★★★ 修正箇所: 共有テキストの曲名始まりを揃える ★★★
            if (isAlbum1) {
                // 共有テキストではタブやスペースがPDFと異なる表示になるため、ここでは4スペースで揃える
                shareableTextContent += `${paddingBase}${titleText}${simpleEffectNote}\n`; 
            } else {
