...
async function generateSetlistPdf() {
    ...
    try {
        ...
        // await new Promise(resolve => setTimeout(resolve, 500)); // ★この待機処理を削除（ダウンロードブロックの解除）
        ...
        if (numSimpleRows > 0) {
            // ★修正ポイント1: 行間調整ロジックの強化 (MAX_LINE_HEIGHT_MMをさらに大きく)
            ...
            // 2. 許容される最大の行間を計算 (A4の縦幅297mmを超える値に設定し、理想行間を確実に採用)
            const MAX_LINE_HEIGHT_MM = 300; // 50 -> 300 に変更
            
            // 3. 採用する行間を決定: 理想値が確実に優先される
