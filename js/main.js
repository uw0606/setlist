...
async function generateSetlistPdf() {
    ...
    try {
        ...
        const MIN_FONT_SIZE = 18; // 最小フォントサイズを設定 (見やすさを考慮し調整)
        const BASE_LINE_HEIGHT_RATIO = 0.45; 
        
        // ★修正1: マージンの設定
        const simpleTopMargin = 30; // 上マージン
        const simpleLeftMargin = 20; // 左マージン
        const simpleRightMargin = 20; // 右マージン
        const simpleBottomMargin = 20; // ボトムマージン (10mm -> 20mm に変更)
        ...
        // 最大行数 (A4縦でフォントサイズ20ptの場合、約30行程度が限界)
        const MAX_LINES_FOR_A4 = 30; 
        ...
        if (headerText) {
            // ヘッダーの高さ (ヘッダーフォントサイズ + 少しの余白)
            simpleHeaderHeight = (BASE_FONT_SIZE + 4) * 0.38 + 5; 
        }

        if (numSimpleRows > 0) {
            // ★修正2: 行数に基づいてフォントサイズと行間を調整し、紙面に収める
            const simpleAvailableHeight = pageHeightSimple - simpleTopMargin - simpleBottomMargin - simpleHeaderHeight;
            
            // 1. 理想的な行間を計算 (使える高さ / 総行数)
            const idealLineHeight = simpleAvailableHeight / numSimpleRows;
            
            // 2. 許容される最大の行間を計算 (ベースフォントサイズ26ptの時の行間)
            const maxLineHeight = BASE_FONT_SIZE * BASE_LINE_HEIGHT_RATIO; 
            
            // 3. 採用する行間を決定: 理想値と最大値の小さい方
            dynamicLineHeight = Math.min(idealLineHeight, maxLineHeight);
            
            // 4. 採用するフォントサイズを決定: 行間から逆算し、ベースサイズを上限、最小サイズを下限とする
