...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 新規作成されたスロットや既存のスロット全てに対してイベントを設定
    // セットリスト内の既存の項目（setlist-item）に対してもドラッグ＆ドロップを有効化
    // これにより、動的に追加されたsetlist-itemもドラッグ可能になります。
    // 注: setlist-item自体にドラッグ開始イベント、setlist-slotにドロップイベント
    const initializeSetlistItem = (itemElement) => {
        // ドラッグハンドルが存在する場合、ドラッグハンドルのみにドラッグイベントを設定
        // そうでない場合はアイテム全体に設定（アルバムリストのアイテムなど）
        ...
        if (dragHandle) {
            ...
            console.log(`[initializeSetlistItem] Enabled drag for album item: ${itemElement.dataset.itemId}`);
        }
        
        // ★修正点1: setlist-item に対して直接ダブルクリックリスナーを設定★
        ...
        // ★修正点2: setlist-item 内のチェックボックスにもイベントリスナーを設定★
        itemElement.querySelectorAll('input[type="checkbox"]').forEach(checkbox => {
            checkbox.addEventListener('click', (e) => {
                ...
                const slot = checkbox.closest('.setlist-slot'); // setlist-slot を取得
    ...
    setlist.querySelectorAll(".setlist-slot").forEach((slot, index) => {
        ...
        // ★修正点3: スロット全体のクリックリスナーからダブルクリック削除ロジックを削除★
        // チェックボックスの処理は setlist-item の初期化関数に移管するか、
        // もしくは slot.querySelector('input[type="checkbox"]') で明示的にターゲットを取得。
        // ここはチェックボックスクリック時の共通ロジックを維持。
        slot.addEventListener('click', (e) => {
            const checkbox = e.target.closest('input[type="checkbox"]');
            if (checkbox) {
                console.log("[slotClick] Checkbox clicked via slot listener.");
                e.stopPropagation(); // チェックボックスのクリックイベントがスロットに伝播しないように
                // チェックボックスの処理は initializeSetlistItem 内のリスナーに任せるため、ここでは不要
                // または、ここに共通のチェックボックス処理ロジックを記述することも可能です
                // （重複を避けるため、どちらか一方にまとめるのが良いでしょう）
            } else {
                // チェックボックス以外の部分をクリックした場合の処理 (現在は何もないが将来的に追加する可能性)
                console.log("[slotClick] Non-checkbox part of slot clicked.");
            }
        });
        
        // ★修正点4: setlist-slot から dblclick リスナーを削除！★
        // slot.addEventListener("dblclick", handleDoubleClick); // ← この行を削除またはコメントアウト
    });
        ...
        for (const mutation of mutationsList) {
            if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                mutation.addedNodes.forEach(node => {
                    // 追加されたノードが setlist-item であることを確認
                    if (node.nodeType === 1 && node.classList.contains('setlist-item')) {
                        initializeSetlistItem(node);
                        console.log(`[MutationObserver] Initialized newly added setlist item: ${node.dataset.itemId}`);
                    }
                    // 追加されたノードが setlist-slot かつ中に setlist-item がない場合
                    // または setlist-item が削除されて空のスロットに戻った場合など、
                    // スロットの状態変化にも対応できるよう、必要に応じて enableDragAndDrop(node) を再度呼び出す
                    if (node.nodeType === 1 && node.classList.contains('setlist-slot')) {
                        enableDragAndDrop(node); // スロットは常にドロップターゲット
    ...
    if (setlist) {
        ...
        console.error("[DOMContentLoaded] #setlist element not found, MutationObserver not initialized.");
    }
...
// handleDoubleClick 関数も修正が必要です
function handleDoubleClick(event) {
    // ダブルクリックされた要素が .setlist-item かどうかを確認
    // イベントのターゲットが .drag-handle ではないことを確認
    const setlistItem = event.target.closest('.setlist-item');
    if (setlistItem && !event.target.closest('.drag-handle')) {
        console.log("[handleDoubleClick] Double-clicked on a setlist item (not drag handle). Attempting to remove.");
        removeSetlistItem(setlistItem);
        // showMessageBox('曲を削除しました。'); // 必要であればメッセージを表示
    } else if (event.target.classList.contains('setlist-slot') && !setlistItem) {
        // 空のスロットをダブルクリックした場合の動作（現状は何もしない）
        console.log("[handleDoubleClick] Double-clicked on an empty setlist slot.");
    }
}