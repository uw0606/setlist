...
// Firebaseの初期化（このファイルでは定義しない。他の場所で定義されていることを前提とする）

/**
 * 特効のvalueから表示用のラベルを取得する。
 * @param {string} effectValue - 特効のデータ値 (例: 'fire', 'smoke')
 * @returns {string} 表示用のラベル (例: 'FF', '銀煙弾') または空白
 */
...
/**
 * すべてのアルバムアイテムのリストを保持。DOMContentLoadedで構築されます。
 * 形式: [{ id: 'item1', name: '曲名', albumClass: 'album1', element: HTMLElement }, ...]
 */
let allAlbumItems = []; 

/**
 * 文字順にソートされた曲を格納するオブジェクト。
 * key: カテゴリ名 (例: 'A-E', 'あ-お', '数字・記号')
 * value: そのカテゴリに属する曲オブジェクトの配列
 */
let sortedItemsByChar = {};
...
function clearSlotContent(slotElement) {
    ...
    // スロット自体が setlist-slot クラスを保持していることを確認
    slotElement.classList.remove('setlist-item', 'item');
    ...
    // ★修正：特効のデータ属性を削除に追加★
    delete slotElement.dataset.specialEffect; 
    
    // 関連するクラスを削除
    slotElement.classList.remove(
        'short', 'se-active', 'drumsolo-active'
    );
    ...
    // スロットが空になったら、D&Dを有効な状態に戻す（setlist-slotクラスは保持する）
    // ★修正：pointerEvents = 'none' は削除。空のスロットもドロップターゲットである必要があるため。
    // slotElement.style.pointerEvents = 'auto'; // (必要に応じて)
    slotElement.style.touchAction = 'auto';
    slotElement.style.visibility = 'visible';
    slotElement.classList.remove('placeholder-slot'); 
    
    // 念のためdraggable属性もリセット（デフォルトはfalseまたは未定義）
...
function restoreToOriginalList(setlistItem) {
    ...
    // この関数は、アルバムモード、文字順モードの両方でアイテムの visibility を適切に制御する役割を持つ。
 ...
 * この関数は、アルバムモード、文字順モードのどちらのアイテムリストに対しても適用される。
 */
function hideSetlistItemsInMenu() {
    console.log("[hideSetlistItemsInMenu] START: Hiding setlist items in album menu.");
    
    // 1. セットリスト内のアイテムID（重複不可なもののみ）を収集
    const setlistItemIds = new Set();
    document.querySelectorAll("#setlist .setlist-slot.setlist-item").forEach(slot => {
        const itemId = slot.dataset.itemId;
        // data-allow-multiple が "true" でない場合（重複不可）のみIDを収集
        if (itemId && slot.dataset.allowMultiple !== 'true') {
            setlistItemIds.add(itemId);
        }
    });

    // 2. アルバムメニュー内の全アイテムに対して可視性を設定
    // albumListはグローバル変数として定義されていると仮定
    albumList.querySelectorAll('.item').forEach(albumItem => {
        const itemId = albumItem.dataset.itemId;
        
        if (!itemId) return;

        // 自由入力曲（data-allow-multiple="true"）は、常に表示
        if (albumItem.dataset.allowMultiple === 'true') {
            albumItem.style.visibility = 'visible';
            return;
        }

        // 重複不可の曲の場合
        if (setlistItemIds.has(itemId)) {
            // セットリストに存在する場合 -> 非表示
            albumItem.style.visibility = 'hidden';
            // console.log(`[hideSetlistItemsInMenu] HIDDEN: ${itemId}`); // デバッグ用
        } else {
            // セットリストに存在しない場合 -> 表示
            albumItem.style.visibility = 'visible';
        }
    ...
    const currentSetlist = Array.from(document.querySelectorAll("#setlist .setlist-slot.setlist-item"))
        .map((slot, index) => {
            ...
            if (songData.specialEffect && typeof specialEffectOptions !== 'undefined') {
                ...
                // 特効が選択されており、かつそれが「特効なし」や空の値ではない場合のみ追加
            ...
            // --- コーラス ---
            // ★修正: chorusが'false'または空文字ではない場合、その値を表示する★
...
/**
 * 現在のアプリケーションの状態（セットリスト、メニューの開閉、開いているアルバム、日付、会場、**現在のソートモード**）を取得する。
 * @returns {object} 現在の状態オブジェクト
 */
function getCurrentState() {
    // 1. セットリストの状態を収集
    const setlistState = Array.from(setlist.children)
        // setlist-slotでsetlist-itemクラスを持つ要素のみを対象
        .filter(slot => slot.classList.contains('setlist-item')) 
        // getSlotItemDataの戻り値には、specialEffectを含むすべてのデータが含まれる
        .map(slot => getSlotItemData(slot));

    // 2. メニュー/アルバムの表示状態
    const menuOpen = menu.classList.contains('open');
    // 開いているアルバム（アルバムモード）または文字順モードでアクティブなコンテンツ（文字順モード）
    const openAlbums = Array.from(document.querySelectorAll('.album-content.active')).map(album => album.id);

    // 3. 現在のソートモードを取得 (HTMLのselect要素から)
    const sortOrderSelect = document.getElementById('sortOrder');
    const currentMode = sortOrderSelect ? sortOrderSelect.value : 'album'; // デフォルトは'album'

    // 4. originalAlbumMapをオブジェクトに変換 (MapはJSONに直接保存できないため)
    ...
    if (setlistYear && setlistMonth && setlistDay) {
        // 日付が選択されているかを確認してから結合
        if (setlistYear.value && setlistMonth.value && setlistDay.value) {
...
function handleDragStart(event) {
    ...
    // ★修正済み: dataTransfer.effectAllowed を設定
...
function handleDragEnter(event) {
    ...
    // ターゲットスロットが存在し、かつそれがドラッグ元のスロットではないことを確認
    if (targetSlot && !(originalSetlistSlot && targetSlot.dataset.slotIndex === originalSetlistSlot.dataset.slotIndex)) {
        // ドロップゾーンの視覚的なフィードバック (CSSで処理)
        targetSlot.classList.add('drag-over');
        
        // ★修正：pointerEventsの操作は削除。スロットは常にドロップターゲットとして機能すべき。
    }
}
...
function handleDragLeave(event) {
    ...
    // ターゲットスロットが存在する場合のみ処理
    if (targetSlot) {
        // relatedTarget (次にカーソルが乗る要素) がスロット自体またはその子要素ではない場合
        if (!event.relatedTarget || !targetSlot.contains(event.relatedTarget)) {
            // 視覚的なフィードバック (CSSクラス) を解除
            targetSlot.classList.remove('drag-over');
            
            // ★修正：pointerEventsの操作とcurrentDropZoneの管理を削除。
            // これらは他のイベントハンドラ (dragover, drop, dragend) で一貫して処理されるべきです。
        }
    }
}
...
function handleDragOver(event) {
    // 必須: これがないとドロップが許可されない
    event.preventDefault(); 
    event.dataTransfer.dropEffect = 'move'; // 移動として扱う（アルバムからの場合はコピーに相当するが、移動を優先）
    ...
    if (typeof autoScrollSetlist === 'function' && typeof SCROLL_AREA_HEIGHT !== 'undefined') {
        ...
        if (SETLIST_ELEMENT) {
            ...
            if (currentY < rect.top + SCROLL_AREA_HEIGHT && SETLIST_ELEMENT.scrollTop > 0) {
                scrollDirection = -1; // 上へスクロール
            } 
            // セットリストエリアの下端からSCROLL_AREA_HEIGHTまでの範囲
            else if (currentY > rect.bottom - SCROLL_AREA_HEIGHT && SETLIST_ELEMENT.scrollHeight > SETLIST_ELEMENT.clientHeight) {
            ...
            if (scrollDirection !== 0) {
                 // スクロール方向が変わった、またはスクロールが停止している場合のみ開始
                if (scrollDirection !== lastScrollDirection || rafId === null) {
    ...
    // ★修正：ハイライトの制御は handleDragEnter/Leave に任せ、ここでは DOM 操作を行わない。
    // targetSlotのハイライトは handleDragEnter が担当します。
}
...
function processDrop(draggedElement, dropTargetSlot, originalSourceSlot) {
    ...
    // アルバムクラスを取得。draggedElementがアルバムアイテムの場合はそのクラスを使用。
    // setlist-itemの場合は、getSlotItemDataから取得したsongData.albumClassが使われるべきだが、
    // ここでは念のためDOMからも取得を試みる。
    const albumClass = Array.from(draggedElement.classList).find(cls => cls.startsWith('album'));
    const finalAlbumClass = songData.albumClass || albumClass || ''; 
    ...
    if (originalSourceSlot && dropTargetSlot.dataset.slotIndex === originalSourceSlot.dataset.slotIndex) {
        ...
        // 元のスロットの visibility を戻す処理は handleDragEnd に任せる
    ...
    if (originalSourceSlot) { // セットリスト内からの移動または入れ替え
        ...
        if (dropTargetSlot.classList.contains('setlist-item')) {
            ...
            addSongToSlot(originalSourceSlot, targetSongData.itemId, targetSongData.name, {
                ...
                specialEffect: targetSongData.specialEffect // 修正済み
            ...
            addSongToSlot(dropTargetSlot, songData.itemId, songData.name, {
                ...
                seChecked: songData.seChecked,
                drumsoloChecked: songData.drumsoloChecked,
                specialEffect: songData.specialEffect // 修正済み
            }, finalAlbumClass);
            ...
            // 元のスロットをクリア (clearSlotContent が visibility をリセットする)
...
function handleDrop(event) {
    ...
    if (originalSetlistSlot) { // セットリスト内からのドラッグ
        ...
        // グローバル変数 albumList を使用する
        if (!albumList) { // albumList がグローバルで const albumList = document.querySelector(".album-list"); で定義されていることを前提とする
            console.error("[handleDrop] Global albumList element not found. Cannot find album item for " + droppedItemId);
            finishDragging(true);
            return;
        }
        // ドラッグ元のアルバムアイテムを特定
...
function handleTouchMove(event) {
    ...
    if (targetSlot) {
        // 現在のアクティブスロットとターゲットスロットが異なるか、またはアクティブスロットがない場合
        if (activeTouchSlot !== targetSlot) {
            // 以前のアクティブスロットのハイライトを解除
            if (activeTouchSlot) {
            ...
            if (!isSelfSlot) { 
                // 新しいスロットをハイライト
                targetSlot.classList.add('drag-over');
                activeTouchSlot = targetSlot; // 新しいアクティブスロットとして記録
            } else {
                activeTouchSlot = null; // 元のスロットに戻った場合はアクティブをリセット
            }
        }
    } else {
        // スロットの上に指がない場合、以前のアクティブスロットを解除
        if (activeTouchSlot) {
            activeTouchSlot.classList.remove('drag-over');
    ...
    // ★修正: handleTouchStartで設定済みのため、以下の繰り返しは削除
    // document.querySelectorAll('.setlist-slot').forEach(slot => { slot.classList.remove('drag-over'); });
}
...
function handleTouchEnd(event) {
    ...
    // touch-action のunsetロジックは handleTouchStart での設定削除に伴い、ここでも削除されています。

    const touch = event.changedTouches[0];
    const currentX = touch.clientX;
    const currentY = touch.clientY;
    
    // ドラッグ開始時の座標が設定されていない場合は異常終了
    if (typeof touchStartX === 'undefined' || typeof touchStartY === 'undefined') {
        console.warn("[touchend] touchStartX/Y not initialized. Treating as no drag.");
        return;
    }
    ...
    // --- 2. ドラッグは開始されたが、移動が最小限だった場合（ロングプレスと見なす） ---
    ...
    // ★修正：関数の末尾で重複して finishDragging を呼ばないようにする。
    // 各ブランチで確実に呼ばれているため、ここでの呼び出しは削除。
}
...
function finishDragging(wasCancelled = false) {
    // wasCancelledがイベントオブジェクトの場合があるため、論理値に変換
    const isCancelled = wasCancelled && wasCancelled !== 'false'; 

    console.log(`[finishDragging] Cleanup started. Was cancelled: ${isCancelled}`); 
    ...
    // アルバムアイテムの元の要素の可視性を戻す (キャンセル時のみ)
    if (currentTouchDraggedOriginalElement && !originalSetlistSlot && isCancelled) {
        currentTouchDraggedOriginalElement.style.visibility = 'visible';
        console.log(`[finishDragging] Album item ${currentTouchDraggedOriginalElement.dataset.itemId} visibility restored due to cancellation.`);
    }
    ...
    // すべてのドラッグオーバークラスをDOM全体から解除 (念のため)
    ...
    // ★修正済み: すべてのセットリストスロットのpointer-eventsを再評価★
    document.querySelectorAll('.setlist-slot').forEach(slot => {
    ...
    // アルバムメニューの表示を更新 (hideSetlistItemsInMenuが全てのvisibilityを再評価するため、個別の可視性復元は省略可能)
    hideSetlistItemsInMenu(); 
...
function handleDoubleClick(event) {
    ...
    if (setlistItemElement) {
        ...
        restoreToOriginalList(setlistItemElement); // この中で hideSetlistItemsInMenu と saveSetlistState が呼ばれる
        return; // 処理完了
    }
 ...
 * * グローバル変数: 
 * setlist (HTMLElement), rafId (number | null), 
 * lastScrollDirection (number), SCROLL_SPEED (number)
 * が適切に定義されている必要があります。
 */
function autoScrollSetlist(direction) {
    ...
    const step = () => {
        // setlist が null の場合は停止 (DOMがロードされていない場合のエラー回避)
        if (!setlist || typeof SCROLL_SPEED === 'undefined') {
            autoScrollSetlist(0);
            return;
        }
...
async function generateSetlistPdf() {
    ...
    // アルバム1のアイテムIDリスト（グローバル変数として定義されていると仮定）
    // const album1ItemIds = [...];
    ...
    for (const slot of setlistSlots) {
        if (slot.classList.contains('setlist-item')) {
            ...
            // album1ItemIds が未定義の場合に備えたガード (仮のリストを使用するか、グローバル定義に依存)
    ...
    try {
        ...
        // 動的マージン計算用の定数
        const footerText = "このフッターテキストはグローバル変数から復元される可能性があります。"; // ★修正: フッターテキストを復元 (グローバル変数がないため仮のテキスト)
 ...
 * (getSpecialEffectLabel関数がグローバルで定義されていることを前提とします)
 */
    ...
    setlistRef.set(currentState)
        .then(() => {
            ...
            currentState.setlist.forEach(songData => {
                ...
                // ★修正箇所: 特効の情報を追加 ★
                if (songData.specialEffect && typeof getSpecialEffectLabel === 'function') {
                    const specialEffectLabel = getSpecialEffectLabel(songData.specialEffect);
                    if (specialEffectLabel) {
                        titleText += ` [${specialEffectLabel}]`;
                    }
                }
            ...
            if (navigator.share) {
                ...
                // PC (Web Share API非対応): クリップボードへのコピー
                ...
                // document.execCommand('copy') は非推奨だが、互換性のために残す
...
function loadSetlistState() {
    return new Promise((resolve, reject) => {
        ...
        if (!shareId) {
            ...
            // ★修正: ロード完了後、UIの最終調整を行う関数を呼び出す
            if (typeof hideSetlistItemsInMenu === 'function') hideSetlistItemsInMenu(); 
            return resolve();
        }
            ...
            .then((snapshot) => {
                ...
                if (state && state.setlist) {
                    ...
                    state.setlist.forEach(itemData => {
                        ...
                        if (targetSlot) {
                            // fillSlotWithItem を使用してデータをスロットに設定（特効データも含まれる）
...
function updateSlotContent(slotElement, songName, options) {
    ...
    if (slotElement.dataset.itemId === 'album1-custom') {
        ...
        const createCustomSelect = (datasetKey, optionsArray) => {
            ...
            select.addEventListener('change', (e) => {
                ...
                // updateAdditionalInfoDisplay(slotElement); // saveSetlistState() や updateAdditionalInfoDisplay() を呼び出すのが理想
            });
    ...
    if (typeof specialEffectOptions !== 'undefined' && !isSpecialEffectExcluded) {
        ...
        // 初期値の設定
        const initialSpecialEffect = options.specialEffect || '';
        specialEffectSelect.value = initialSpecialEffect; 
        
        // ★修正: 初期値に基づいてクラスを設定
        if (initialSpecialEffect) {
            slotElement.classList.add(`fx-${initialSpecialEffect}`);
        }
    ...
    // R.Gtの表示チェック: customRGtTuningOptionsを使用 (前提: customRGtTuningOptions[0].value はデフォルト値)
    if (options.rGt && typeof customRGtTuningOptions !== 'undefined' && options.rGt !== customRGtTuningOptions[0].value) infoParts.push(`R.Gt: ${options.rGt}`);
    // L.Gtの表示チェック: customLGtTuningOptionsを使用
    if (options.lGt && typeof customLGtTuningOptions !== 'undefined' && options.lGt !== customLGtTuningOptions[0].value) infoParts.push(`L.Gt: ${options.lGt}`);
    
    // Bass、BPM、コーラスの表示チェック
    if (options.bass && typeof customBassTuningOptions !== 'undefined' && options.bass !== customBassTuningOptions[0].value) infoParts.push(`Ba: ${options.bass}`);
    if (options.bpm && typeof customBpmOptions !== 'undefined' && options.bpm !== customBpmOptions[0].value) infoParts.push(`BPM: ${options.bpm}`);
    if (options.chorus && typeof customChorusOptions !== 'undefined' && options.chorus !== customChorusOptions[0].value && options.chorus !== 'false') infoParts.push(`コーラス: ${options.chorus}`); 
...
function addSongToSlot(slotElement, itemId, songName, options, albumClass) {
    ...
    // ★★★ 特効の選択状態をデータ属性に保存 ★★★
    // ロード/移動時は options の値、アルバムからの追加時は初期値の '' を使用
...
function enableDragAndDrop(element) {
    ...
    if (element.classList.contains('item') || element.classList.contains('setlist-item')) {
        ...
        // ★★★ 修正箇所: touch-action を元に戻す ★★★
        // ドラッグ可能な要素（アルバムアイテムおよびセットリストアイテム）に設定が必要です。
    ...
    if (element.classList.contains('setlist-slot')) {
        ...
        // 空のスロットには touch-action は不要です
        // element.style.touchAction = 'pan-y'; // ← この行は削除したまま
    }
}
