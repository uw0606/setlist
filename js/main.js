...
function buildNameOrderList() {
    ...
    // --- 3. グループに分割 ---
    // groupSongs 関数は別途定義されている必要があります
    nameOrderGroups = groupSongs(allSongData);
    
    // --- 4. コンテンツの構築 ---
    // clearAndDrawNameOrderList 関数は別途定義されている必要があります
    // コンテンツを描画
    ...
    // ★★★ 修正箇所: 初期状態で曲がある最初のグループを開く ★★★
    const allNavItems = document.querySelectorAll('.name-order-nav-item');
    let firstNonEmptyGroupNav = null;

    // ナビゲーションをHTMLの出現順にチェック
    for (const navItem of allNavItems) {
        const groupKey = navItem.dataset.group;
        
        // そのグループに曲データが1曲以上あり、かつそのコンテンツがDOMに存在するかチェック
        // nameOrderGroups[groupKey] の存在チェックのみで良い（clearAndDrawNameOrderListで曲がないグループはスキップされるため）
        if (nameOrderGroups[groupKey] && nameOrderGroups[groupKey].length > 0) {
            firstNonEmptyGroupNav = navItem;
            break; // 最初に見つかった時点でループを終了
        }
    }

    if (firstNonEmptyGroupNav) {
        // 曲がある最初のタブをクリックした状態を再現
        // toggleNameOrderGroup 関数は別途定義されている必要があります
...
function toggleNameOrderGroup(navItem, groupKey) {
    ...
    // 要素が見つからない場合のログを強化 (ERRORからWARNINGに変更)
    if (!contentToDisplay) {
        console.warn(`[TOGGLE WARNING] Content element #name-group-${groupKey} not found! (これはそのグループに曲がない場合に予期されます)`); 
    }

    // 1. ナビゲーションの状態を更新 (コンテンツの有無に関わらず、クリックされたタブはアクティブにする)
    allNavItems.forEach(item => item.classList.remove('active'));
    navItem.classList.add('active'); 

    // 2. すべてのコンテンツを非表示にする
    allContents.forEach(content => {
        // hidden クラスと display: none の両方で確実に非表示にする
        content.classList.add('hidden');
        content.style.display = 'none'; 
    });
    
    // 3. 選択されたコンテンツが存在する場合のみ表示
    if (contentToDisplay) {
        contentToDisplay.classList.remove('hidden');
        ...
        console.log(`[TOGGLE DEBUG] Displaying: #name-group-${groupKey}`);
    } else {
        // コンテンツがない場合: 何も表示せず、空の状態にする（前のコンテンツは上記ステップ2で消えている）
        // 視覚的に「このタブは空である」とユーザーに伝わる
    }
}
