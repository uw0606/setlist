...
function groupSongs(songs) {
    ...
    songs.forEach(song => {
        // 曲名から最初の文字を取得 (大文字化は英字のみに影響)
        const firstChar = song.songName.trim().charAt(0); // 大文字化を一旦解除

        if (/[A-Za-z]/.test(firstChar)) {
            // 英字の場合のみ大文字にして、グループ分けを継続
            const upperChar = firstChar.toUpperCase();
            if (/[A-E]/.test(upperChar)) groups['ABCDE'].push(song);
            else if (/[F-J]/.test(upperChar)) groups['FGHIJ'].push(song);
            else if (/[K-O]/.test(upperChar)) groups['KLMNO'].push(song);
            else if (/[P-T]/.test(upperChar)) groups['PQRST'].push(song);
            else if (/[U-Z]/.test(upperChar)) groups['UVWXYZ'].push(song);
        }
        else if (/[0-9\W]/.test(firstChar)) {
            groups['ETC_NONE'].push(song); // 数字、記号など
        }
        else {
            // 日本語の分類
