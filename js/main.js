...
async function generateSetlistPdf() {
    ...
    try {
        ...
        // detailedPdf.save(detailedFilename); // ダウンロード処理
        ...
        // ★修正ポイント1: BASE_FONT_SIZEを十分に大きな値に戻し、曲間を優先
        ...
        const simpleBottomMargin = 20; // 15mm から 20mm に戻し、上下バランスを調整
        const SPACE_BETWEEN_HEADER_AND_BODY = 5; // 10mm から 5mm に戻し、ヘッダーと曲を近づける
        ...
        if (numSimpleRows > 0) {
            const tempAvailableHeight = pageHeightSimple - simpleTopMargin - simpleBottomMargin - (headerText ? 35 : 0); // ヘッダーを考慮して予測
            ...
            const tempDynamicFontSize = IDEAL_LINE_HEIGHT / BASE_LINE_HEIGHT_RATIO;
        ...
        if (headerText) {
            // ★修正ポイント3: ヘッダーサイズの初期値を、予測した曲のサイズを基準にする
        ...
        if (numSimpleRows > 0) {
            ...
            // 1. 理想的な行間を計算（この値を dynamicLineHeight として採用）
        ...
        // simplePdf.save(simpleFilename); // ダウンロード処理
