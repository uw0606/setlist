...
function handleTouchStart(event) {
    ...
    // ドラッグ関連のグローバル変数を関数の冒頭で確実にリセット
    // いずれかの条件で早期リターンする場合（またはドラッグを開始しない場合）は、
    // ドラッグ関連の状態をリセットすることで、前の操作の状態が残るのを防ぎます。
    // 特に isDragging が true の状態でタッチ開始された場合 (例: 不安定なタッチ操作後) に、
    // 確実にリセットされるようにします。
    ...
    if (event.touches.length === 1) {
        ...
        // 上の共通リセット処理で対応済みなので、ここでは追加の処理は不要です。
    }
}
...
function handleTouchMove(event) {
    ...
    if (!isDragging) {
        ...
        if (distance > DRAG_THRESHOLD) {
            // draggingItemId が存在し、かつ有効な要素が見つかるかを確認
            if (!draggingItemId) {
            ...
            // しきい値に達していない場合、スクロールを許可するために何もしません。
            // これにより、preventDefault() が呼ばれずスクロール可能です。
            return; 
        }
    }
...
function handleTouchEnd(event) {
    // ドラッグしていなかった場合（タッチ開始時にドラッグと判定されなかった場合）
    if (!isDragging) { 
        const closestCheckbox = event.target.closest('input[type="checkbox"]');
        if (closestCheckbox) { 
            ...
            // ドラッグしていなければ、finishDragging() を呼ばず、スクロールのブロックも解除しません。
        }
    ...
    if (!currentTouchDraggedClone) {
        ...
        finishDragging(); // 問題がある状態なので、クリーンアップだけは試みます。
        return;
    }
    ...
    // ドロップ処理を実行
    // currentTouchDraggedClone はクローンなので、実際のドロップ処理には draggingItemId と originalSetlistSlot を使用します。
