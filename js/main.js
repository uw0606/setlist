...
function processDrop(draggedElement, dropTargetSlot, originalSetlistSlot = null) {
    ...
    // draggedElementが有効なDOM要素か、itemIdを持っているか確認
    // currentTouchDraggedClone または originalSetlistSlot が渡されることを想定
    const itemId = draggedElement.dataset.itemId;
    if (!draggedElement || typeof itemId !== 'string' || itemId.trim() === '') {
        console.error("[processDrop] draggedElement or its itemId is invalid. Aborting processDrop.");
        // もし originalSetlistSlot があり、非表示になっていたら表示に戻す
        if (originalSetlistSlot) {
            originalSetlistSlot.classList.remove('placeholder-slot');
            originalSetlistSlot.style.visibility = '';
            // originalSetlistSlot._originalItemDataが存在するなら復元を試みる
            if (originalSetlistSlot._originalItemData) {
                fillSlotWithItem(originalSetlistSlot, originalSetlistSlot._originalItemData);
                console.log("[processDrop] Restored originalSetlistSlot after invalid draggedElement.");
            }
        }
        return;
    }
    ...
    if (originalSetlistSlot && originalSetlistSlot._originalItemData) {
        ...
        // アルバムからドラッグされた場合は、getSlotItemData で基本的なデータを取得し、オプション情報を付加
        ...
        if (originalAlbumItem) {
            draggedItemData.isShortVersion = originalAlbumItem.dataset.isShortVersion === 'true';
    ...
    if (!draggedItemData) {
        ...
        // draggedItemDataが取れない場合も、元のスロットをクリアしないよう復元を試みる
        if (originalSetlistSlot) {
            originalSetlistSlot.classList.remove('placeholder-slot');
            originalSetlistSlot.style.visibility = '';
            if (originalSetlistSlot._originalItemData) {
                 fillSlotWithItem(originalSetlistSlot, originalSetlistSlot._originalItemData);
                 console.log("[processDrop] Restored originalSetlistSlot due to missing draggedItemData.");
            }
        }
        return;
    }
    ...
    if (isDraggedFromSetlist) {
        ...
        if (dropTargetSlot) {
            ...
            // dropTargetSlotがnullの場合、元のスロットにアイテムを戻す (クリアしない)
            console.warn("[processDrop] Dropped outside a valid target slot (from setlist). Restoring item to original slot.");
            if (originalSetlistSlot && originalSetlistSlot._originalItemData) {
                // 非表示になった元のスロットをデータで埋め戻し、表示に戻す
                fillSlotWithItem(originalSetlistSlot, originalSetlistSlot._originalItemData); 
            } else {
                console.error("[processDrop] Cannot restore original setlist slot: originalSetlistSlot or _originalItemData is missing.");
                // ここで clearSlotContent は呼ばない。もし呼ばれると消える。
            }
        }
    } else {
        // ⭐ アルバムからのドラッグ ⭐
