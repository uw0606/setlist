...
function handleTouchStart(event) {
    ...
    if (tapLength < 300 && tapLength > 0) { // 300msはダブルタップ判定の一般的な閾値
        ...
        handleDoubleClick(event); // ダブルタップ処理（内部で preventDefault() を呼ぶ想定）
        lastTapTime = 0; // ダブルタップ後のリセット
        console.log("[touchstart] Double tap detected. Handled by handleDoubleClick.");
        return;
    }
    lastTapTime = currentTime; // 次のタップのための時刻を記録
    ...
    if (event.touches.length === 1) { // シングルタッチのみを処理
        ...
        // ドラッグ元がセットリスト内のアイテムの場合、そのスロットを記録しデータを保持
        ...
        // タッチ開始地点の絶対座標を記録（スクロール判定とクローン移動用）
        initialTouchX = event.touches[0].clientX;
        initialTouchY = event.touches[0].clientY;

        // クローン作成時のオフセットを記録（クローンが指の真下に来るようにするため）
        // これは、ドラッグ開始時にクローンの表示位置を調整するために使います
        ...
        // ⭐ここが重要: 長押しタイマーを設定する⭐
        // このタイマーが完了したらドラッグが開始されたとみなす
        if (touchTimeout) {
            clearTimeout(touchTimeout); // 既存のタイマーがあればクリア
        }
        touchTimeout = setTimeout(() => {
            // タイマーが完了した時点で、isDraggingをtrueにし、クローンを作成する
            isDragging = true;
            console.log("[touchstart:Mobile] Long press detected (timeout completed). Initiating drag.");

            const elementToClone = originalSetlistSlot || touchedElement; // セットリストからのドラッグなら元のスロット、そうでなければ触った要素
            if (elementToClone && draggingItemId) {
                // クローンの位置は、現在の指の位置 (event.touches[0].clientX/Y) と、
                // タッチ開始時に計算したオフセット (touchStartX/Y) を使って正確に配置する
