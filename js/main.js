    ...
    if (isDraggedFromSetlist) {
        ...
        if (dropTargetSlot) {
            if (dropTargetSlot.classList.contains('setlist-item')) {
                ...
                fillSlotWithItem(dropTargetSlot, draggedItemData); // ドロップ先にアイテムを入れる
            }
        } else {
            ...
            // finishDraggingでvisibilityがリセットされる
        }
    } else {
        // アルバムからのドラッグの場合
        if (dropTargetSlot) {
            ...
            fillSlotWithItem(dropTargetSlot, draggedItemData);
            // ここでアルバムアイテムを非表示にするのは不要 (touchstart/createTouchDraggedClone で既に隠しているため)
            // アルバムアイテムがまだ見えている場合は、`hideSetlistItemsInMenu()` で最終的に隠される
        } else {
            // セットリスト外にドロップされた場合（アルバムから）
...
function handleTouchStart(event) {
    ...
    // ドラッグハンドルに触れていない、かつチェックボックスクリックでもない場合は、
    // スクロールを許可するためにここで処理を終了し、preventDefault()を呼ばない
    const isDragHandleTouch = event.target.closest('.drag-handle') !== null;
    if (!isDragHandleTouch && !isCheckboxClick) {
    ...
    if (isCheckboxClick) {
        ...
        lastTapTime = 0; // チェックボックスクリック時はダブルタップ検出をリセット
        if (touchTimeout) {
            clearTimeout(touchTimeout);
            touchTimeout = null;
        }
        isDragging = false; // ドラッグ状態ではない
        return;
    }
    ...
    if (event.touches.length === 1 && isDragHandleTouch) {
        ...
        // ドラッグハンドルに触れたら、即座にevent.preventDefault()を呼んでスクロールをブロックし、
        // ドラッグ準備に入る（まだ isDragging は true にしない）
        event.preventDefault(); 
        ...
        if (setlist.contains(touchedElement) && touchedElement.classList.contains('setlist-item')) {
            ...
            originalSetlistSlot._originalItemData = getSlotItemData(originalSetlistSlot); // 元のデータを保持
        } else {
            originalSetlistSlot = null; // アルバムからのドラッグ
        }
        
        // 元のアルバムアイテムを一時的に非表示にする
        // これがないと、ドラッグ開始時に元のアルバムアイテムが見えたままになる可能性があります。
        if (!originalSetlistSlot) { // アルバムからのドラッグの場合のみ
            touchedElement.style.visibility = 'hidden';
            console.log(`[touchstart:Mobile] Hidden original album item: ${touchedElement.dataset.itemId}`);
        }
    }
}
...
function createTouchDraggedClone(originalElement, initialX, initialY, itemIdToClone) {
    ...
    // ✅ セットリスト内のアイテムだった場合、元の要素を隠す
    if (setlist.contains(originalElement) && originalElement.classList.contains('setlist-item')) {
        originalSetlistSlot = originalElement; 
        const originalItemData = getSlotItemData(originalElement);
        if (originalItemData) {
            originalSetlistSlot._originalItemData = originalItemData; // 元のデータを保持
        ...
        // アルバムリストからのアイテムの場合、既にtouchstartで隠しているが、念のため
        originalElement.style.visibility = 'hidden';
        originalSetlistSlot = null; // アルバムからのドラッグなのでoriginalSetlistSlotはnull
...
function finishDragging() {
    ...
    if (currentPcDraggedElement && setlist.contains(currentPcDraggedElement)) {
        ...
        // ここで visibility を直接リセットする必要はありません。
        // originalSetlistSlot の処理に任せます。
    ...
    if (originalSetlistSlot && originalSetlistSlot.classList.contains('placeholder-slot')) {
        ...
        // ✅ 修正: originalSetlistSlotがまだセットリストアイテムとして残っている場合、またはクリア済みの場合のvisibilityリセット
        // アイテムが残っているか（スワップやセットリスト外ドロップで復元された場合）
    ...
    // 全てのアルバムアイテムのvisibilityをリセット
    // これを呼ぶことで、一時的に隠されていたアルバムアイテムが再度表示されるのを防ぎ、
    // hideSetlistItemsInMenu()で必要なものだけ隠されるようにする。
    document.querySelectorAll('.album-content .item').forEach(item => {
        item.style.visibility = ''; // 全てのアルバムアイテムのvisibilityをリセット
    });
    ...
    setlist.querySelectorAll('.setlist-slot').forEach(slot => {
        ...
        slot.style.opacity = ''; // 必要であればopacityもリセット
    });
...
function handleDoubleClick(event) {
    ...
    if (isInsideSetlist) {
        ...
        restoreToOriginalList(item); // セットリストから削除し、アルバムに表示
    } else {
        ...
        if (!setlist.querySelector(`.setlist-slot.setlist-item[data-item-id="${item.dataset.itemId}"]`)) {
            ...
            showMessageBox('この曲はすでにセットリストにあります。'); // 既に存在する場合のメッセージ
        }
    }
    finishDragging(); // 操作完了後にクリーンアップを呼び出す
}
/*------------------------------------------------------------------------------------------------------------*/
