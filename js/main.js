...
function handleTouchStart(event) {
    ...
    // クリックされた要素がチェックボックス、またはその周囲（spanなど）であるかを判定
    const isCheckboxClick = event.target.type === 'checkbox' ||
                            (event.target.tagName === 'SPAN' && event.target.querySelector('input[type="checkbox"]')) ||
                            (event.target.classList.contains('setlist-item') && event.target.querySelector('input[type="checkbox"]') &&
                             (event.target.contains(event.touches[0].target) && event.touches[0].target.type === 'checkbox'));
    ...
    if (event.touches.length === 1) {
        ...
        event.preventDefault(); // ✅ チェックボックスでなければ、デフォルト動作をキャンセルしてドラッグ処理へ進む
