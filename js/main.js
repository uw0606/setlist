...
function groupSongs(songs) {
    ...
    const japaneseMap = {
        ...
        // ★修正点1: 拗音・濁音もAIUEOにフォールバックさせるため、ここでは定義しないか、既存の定義を信頼する
        // ただし、日本語ソートロジックのロバスト性を高めるため、以下の分類ロジックを修正します。
    };
    
    songs.forEach(song => {
        let firstChar = ''; 
        let groupKey = 'ETC_NONE'; // デフォルトはETC_NONE

        // 1. ソートキーの決定: alphaSort > kanaSort > songName の優先順位
        if (song.alphaSort) {
            firstChar = song.alphaSort.trim().charAt(0).toUpperCase();
        } else if (song.kanaSort) {
            // 日本語の場合は最初の文字をそのまま使用 (ひらがな・カタカナ)
            firstChar = song.kanaSort.trim().charAt(0);
        } else {
            // どちらもない場合は曲名（英字・数字はここで判定される可能性がある）
        ...
        // B. 日本語の分類 (ひらがな・カタカナ)
            ...
            // kanaSortの最初の文字を取得（ひらがなまたはカタカナ）
            const jpChar = song.kanaSort.trim().charAt(0);
            
            // マッピングからグループキーを取得
            const mappedGroup = japaneseMap[jpChar];
            
            if (mappedGroup) {
                groupKey = mappedGroup;
            } else if (/[ぁ-んァ-ヶ]/.test(jpChar)) { 
                // マッピングにないが、ひらがな・カタカナの場合はAIUEOにフォールバック
                // 例: ゃゅょ、っ、濁音・半濁音のソートキー
                groupKey = 'AIUEO';
            } else {
        ...
        // C. 数字・記号・フォールバック (英字・日本語以外)
            ...
            if (/[0-9\W]/.test(firstChar)) {
                ...
                groupKey = 'AIUEO'; // ★修正点2: 漢字などその他の文字をAIUEOに分類
            }
        }
