...
async function generateSetlistPdf() {
    ...
    try {
        ...
        // ヘッダーテキストの描画と高さの計算は下で動的に行います

        if (numSimpleRows > 0) {
            // ★修正ポイント1: 行間調整ロジックの強化（理想行間がより優先されるように）
            const simpleAvailableHeight = pageHeightSimple - simpleTopMargin - simpleBottomMargin;
            
            // ヘッダーの初期高さ（仮計算）：ヘッダーがないものとして行間を計算し、後でヘッダー高さを引く方法を採用
            // ただし、今回はヘッダーの描画を最初に行うため、シンプルに以下のロジックで対応します。

            // 1. 理想的な行間を計算 (使える高さ / 総行数)
            // ヘッダーが描画されると、その分高さを消費します。まずは、ヘッダーがない場合の理想値を出します。
            const IDEAL_LINE_HEIGHT_NO_HEADER = simpleAvailableHeight / numSimpleRows;
            
            // 2. 許容される最大の行間を計算 (ベースフォントサイズ26ptの時の行間)
            // ここを調整することで、曲数が少ない場合の最大行間を制御します。
            // A4縦全体に広げるため、MAX_LINE_HEIGHTを非常に大きく設定し、IDEAL_LINE_HEIGHTが優先されるようにします。
            const MAX_LINE_HEIGHT_MM = 50; 
            
            // 3. 採用する行間を決定: 理想値と最大値の小さい方。今回は理想値が優先されるはず。
            dynamicLineHeight = Math.min(IDEAL_LINE_HEIGHT_NO_HEADER, MAX_LINE_HEIGHT_MM);
            ...
            // ★修正ポイント2: ヘッダーの描画と自動縮小
            if (headerText) {
                let headerFontSize = dynamicFontSize + 4; // 曲名の基準より少し大きくする
                simplePdf.setFont('NotoSansJP', 'bold');
                
                // ヘッダーテキストの横幅をチェックし、必要に応じて縮小
                ...
                if (headerTextWidth > availableWidth) {
                    ...
                    // ヘッダーの最小サイズは曲名基準のMIN_FONT_SIZEとする
                ...
                // ヘッダー行の高さ + 行間の調整 (5mmの余白)
                simpleHeaderHeight = headerFontSize * 0.38 + 5; 
            ...
            simplePdfBody.forEach(row => {
                ...
                if (row.isItem) {
                    ...
                    // テキストスロットは全体の dynamicFontSize を使用
                    simplePdf.setFontSize(dynamicFontSize);
                }
