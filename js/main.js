...
const LONG_PRESS_DELAY = 200; // 長押しと判断する時間（ミリ秒） - 例: 300ms
const DRAG_THRESHOLD = 0;   // ドラッグ開始とみなす最小移動距離 (ピクセル) - 現状は30ですが、長押しと組み合わせるなら短くしても良いでしょう
const DRAG_READY_CLASS = 'drag-ready-highlight';
...
function handleTouchStart(event) {
    ...
    if (isCheckboxClick) {
        ...
        // ハイライトが残っている可能性があれば解除
        removeDragReadyHighlight();
        isDragging = false;
        return;
    }
    ...
    if (tapLength < 300 && tapLength > 0) {
        ...
        removeDragReadyHighlight(); // ダブルタップ時もハイライト解除
    ...
    if (event.touches.length === 1) {
        ...
        // ★重要: 現在のドラッグ対象要素を保存しておく（ハイライト用）
        currentTouchedDraggableElement = touchedElement; 
        ...
        // ★修正: 長押し判定のためのタイマーを開始。タイマーが発火したらハイライト。
        touchTimeout = setTimeout(() => {
            console.log("[touchstart:Mobile] Long press delay passed. Now ready for drag if moved.");
            touchTimeout = null; // タイマーが完了したことを示す
            // ★新規追加: ドラッグ準備完了の視覚的フィードバック
            if (currentTouchedDraggableElement) {
                currentTouchedDraggableElement.classList.add(DRAG_READY_CLASS);
                console.log("[touchstart:Mobile] Element highlighted for drag readiness.");
            }
        }, LONG_PRESS_DELAY);
        ...
        isDragging = false;
        draggingItemId = null;
        removeDragReadyHighlight(); // 複数指タッチ時もハイライト解除
    }
}
...
function handleTouchMove(event) {
    ...
    if (!isDragging) {
        ...
        // ★★★ 修正箇所: 長押しタイマーが完了している AND 閾値を超えた場合のみドラッグ開始 ★★★
        // これにより、スクロールが優先されます。
        if (touchTimeout === null && (deltaX > DRAG_THRESHOLD || deltaY > DRAG_THRESHOLD)) {
            ...
            // ここで event.preventDefault() を呼び出さないことが重要です
            return;
        }
    }
...
function handleTouchEnd(event) {
    ...
    // ★修正: touchTimeout が残っていても、ここでクリア
    if (touchTimeout) {
        clearTimeout(touchTimeout);
        touchTimeout = null;
        console.log("[handleTouchEnd:Mobile] Long press timer cleared (touch ended before completion or drag).");
    }
    // ★新規追加: タッチ終了時にハイライトを解除
    removeDragReadyHighlight();
    ...
    if (isDragging && draggingItemId !== null) {
        ...
        console.log("[handleTouchEnd:Mobile] Not a drag operation or item was null (likely a tap or brief touch).");
    }
...
// ドラッグ準備完了時にハイライトする要素を保持する変数
let currentTouchedDraggableElement = null;

/**
 * ドラッグ準備完了時のハイライトを解除する関数
 */
function removeDragReadyHighlight() {
    if (currentTouchedDraggableElement && currentTouchedDraggableElement.classList.contains(DRAG_READY_CLASS)) {
        currentTouchedDraggableElement.classList.remove(DRAG_READY_CLASS);
        console.log("[removeDragReadyHighlight] Highlight removed.");
    }
    currentTouchedDraggableElement = null; // リセット
}
