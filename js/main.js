...
function handleTouchStart(event) {
    ...
    if (isCheckboxClick) {
        ...
        // チェックボックスの動作を妨げないため、ここでは event.preventDefault() を呼ばない
        return; 
    }
    ...
    // ★重要★ ここにあった event.preventDefault() を削除しました。
    ...
    if (event.touches.length === 1) {
        ...
        touchTimeout = setTimeout(() => {
            ...
            if (!isDragging && draggingItemId && document.body.contains(touchedElement)) {
                ...
                isDragging = true;
                // ★重要★ ドラッグが開始される時点で初めて event.preventDefault() を呼び出す
...
// handleTouchMove 関数は前回の修正のままで問題ありません。
// isDragging が true の場合にのみ preventDefault を呼んでいます。
