...
// グローバル変数として定義（DOMContentLoadedの外で宣言）

const DOUBLE_TAP_DELAY = 300; // ダブルタップと認識する間隔（ミリ秒）
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // initializeSetlistItem 関数をDOMContentLoadedスコープ内で定義（もし外で定義されていない場合）
    ...
    const initializeSetlistItem = (itemElement) => {
        ...
        if (dragHandle) {
            enableDragAndDrop(itemElement, dragHandle); // ドラッグハンドルを渡す
            console.log(`[initializeSetlistItem] Enabled drag for setlist item with handle: ${itemElement.dataset.itemId}`);
        } else {
            enableDragAndDrop(itemElement); // ドラッグハンドルがない場合 (アルバムアイテムなど)
            console.log(`[initializeSetlistItem] Enabled drag for album item (no handle): ${itemElement.dataset.itemId}`);
        }
        
        // ★修正点1: タッチイベントによるダブルタップ検出と通常のダブルクリックリスナーの両方を設定★
        // 通常のPCからのダブルクリック
        itemElement.addEventListener("dblclick", (e) => handleDoubleClick(e, itemElement)); 
        console.log(`[initializeSetlistItem] Added dblclick listener to setlist item: ${itemElement.dataset.itemId}`);

        // スマートフォンでのダブルタップ検出（長押し・ドラッグとの競合対策）
        itemElement.addEventListener('touchstart', (e) => {
            if (e.touches.length === 1) { // シングルタッチのみ対象
                const currentTime = new Date().getTime();
                const tapLength = currentTime - lastTapTime;

                // ドラッグ開始を遅延させるための小さなタイマーを設定 (任意だが、より正確なダブルタップ検出に役立つ)
                // isDragging フラグが立っている間はダブルタップを無効化
                if (tapLength < DOUBLE_TAP_DELAY && tapLength > 0 && !isDragging) {
                    e.preventDefault(); // ダブルタップ時のデフォルト動作（ズームなど）を防ぐ
                    console.log(`[touchstart] Double tap detected for item: ${itemElement.dataset.itemId}`);
                    handleDoubleClick(e, itemElement); // ダブルタップで削除実行
                    lastTapTime = 0; // リセット
                } else {
        ...
        itemElement.addEventListener('touchmove', () => {
            // 指が動いたらドラッグ開始とみなし、ダブルタップ検出をリセット
            if (lastTapTime !== 0) {
                isDragging = true;
                lastTapTime = 0;
                console.log("[touchmove] Drag detected, resetting double tap timer.");
            }
        });
    ...
    setlist.querySelectorAll(".setlist-slot").forEach((slot, index) => {
        ...
        // ★修正点2: スロット全体のクリックリスナーからダブルクリック削除ロジックは引き続き削除★
        // チェックボックスの処理は setlist-item の初期化関数に移管
        slot.addEventListener('click', (e) => {
            // スロットの背景や余白をタップした場合、ドラッグ＆ドロップ以外の目的がない限り何もしない
            // `e.target` が `setlist-slot` 自身である、またはその直接の子要素（テキストノードなど）である場合
            // 曲の削除はsetlist-itemのdblclickリスナーに任せる
            if (e.target === slot || e.target.nodeType === 3) { // nodeType 3 はテキストノード
                console.log("[slotClick] Clicked on slot background/empty space. No action.");
            }
        });
    });
    ...
    // これにより、ドラッグ＆ドロップでセットリストに曲が追加された際に、
    // 新しい曲アイテムにも自動的にダブルクリックやドラッグイベントが設定されます。
    const observer = new MutationObserver((mutationsList) => {
        for (const mutation of mutationsList) {
            if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                mutation.addedNodes.forEach(node => {
                    if (node.nodeType === 1) { // 要素ノードであることを確認
                        ...
                        if (node.classList.contains('setlist-slot')) {
                            enableDragAndDrop(node); // スロットは常にドロップターゲットとして有効
                            // スロット内に既存の setlist-item がある場合も初期化
                            const existingItemInSlot = node.querySelector('.setlist-item');
                            if (existingItemInSlot) {
                                initializeSetlistItem(existingItemInSlot);
                            }
                            console.log(`[MutationObserver] Re-enabled drop zone for slot: ${node.dataset.slotIndex}`);
                        }
                    }
                });
            }
...
// handleDoubleClick 関数を修正
// event.currentTarget はイベントリスナーがアタッチされている要素を指す
function handleDoubleClick(event, targetElement) {
    // ダブルクリックされた要素が setlist-item かどうかを確認
    // event.target は実際にクリックされた要素、closest() で .setlist-item かどうかを判断
    const setlistItem = targetElement || event.currentTarget; // event.currentTargetを使用
    
    // ドラッグハンドル上でのダブルクリックは無視
    // event.target は実際にクリックされた要素なので、その親に .drag-handle がないか確認
    if (event.target.closest('.drag-handle')) {
        console.log("[handleDoubleClick] Double-clicked on drag handle. Ignoring delete.");
        return;
    }

    // setlist-item であり、かつイベントがドラッグ操作ではない場合のみ削除
    if (setlistItem && setlistItem.classList.contains('setlist-item')) {
        console.log(`[handleDoubleClick] Double-clicked on setlist item: ${setlistItem.dataset.itemId}. Attempting to remove.`);
        removeSetlistItem(setlistItem);
        // showMessageBox('曲を削除しました。'); // 必要であればメッセージを表示
    } else {
        console.log("[handleDoubleClick] Double-clicked on non-setlist-item or irrelevant area. No action.");
    }
}