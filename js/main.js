...
function handleTouchStart(event) {
    ...
    if (event.touches.length === 1) {
        ...
        // **重要**: ここで `event.preventDefault()` を呼ばない！
        // スクロール開始時点では許可するため。
        ...
        // **修正**: ここで setTimeout を使うロジックは、
        // 移動しきい値を使ったドラッグ開始と役割が重複するため削除します。
        // もし「長押しでもドラッグを開始したい」という要件が残るなら、
        // 別のロジック（例えば、移動しきい値到達前に長押し時間だけ満たしたらドラッグ開始）
        // を検討する必要がありますが、まずは移動でのドラッグ開始を優先します。
        if (touchTimeout) {
            clearTimeout(touchTimeout);
            touchTimeout = null;
        }
