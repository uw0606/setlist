...
document.addEventListener("dblclick", (event) => {
  ...
  if (!!event.target.closest("#setlist")) {
    ...
    document.querySelectorAll('.album-content').forEach(albumContent => {
      const originalSong = albumContent.querySelector(`.item[draggable="true"]`) ?
        Array.from(albumContent.querySelectorAll(`.item[draggable="true"]`)).find(el => el.textContent.trim().toLowerCase() === songName.toLowerCase()) :
        Array.from(albumContent.querySelectorAll(`.item`)).find(el => el.textContent.trim().toLowerCase() === songName.toLowerCase());
    ...
    // ハンバーガーメニューのアイテムをセットリストに追加する処理 (元の処理を維持)
    if (setlist.children.length < maxSongs) {
