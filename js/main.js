  ...
  const setlistState = Array.from(setlist.children)
    .map(slot => {
      if (slot.classList.contains('setlist-item')) {
        // ★変更: getSlotItemData が seChecked も返すのでそのまま利用
...
function createTouchDraggedClone(originalElement, initialX, initialY, itemIdToClone) {
    ...
    // ショートとSEのクラスもコピー
    if (originalElement.classList.contains('short')) { // ★変更: ショートクラスのコピー
        currentTouchDraggedClone.classList.add('short');
    }
    if (originalElement.classList.contains('se-active')) { // ★追加: SEアクティブクラスのコピー
    ...
    // ★変更: isShortVersion と hasSeOption のデータ属性をコピー
    ...
    if (originalElement.dataset.hasSeOption) { // ★追加
    ...
    .then(() => {
        ...
        if (setlistItems.length > 0) {
            songListText = Array.from(setlistItems).map((slot, index) => {
                const songData = getSlotItemData(slot); // ★変更: songData を取得
                let line = ` ${index + 1}. ${songData?.name || ''}`; 
                if (songData.short) { // ★変更: songData.short を参照
                    line += ' (Short)';
                }
                if (songData.seChecked) { // ★追加
                    line += ' (SE有り)';
                }
                return line;
            }).join("\n");
          ...
          if (state && state.setlist) {
            ...
            state.setlist.forEach(itemData => {
              ...
              if (targetSlot) {
                  ...
                  // ★変更: songData は getSlotItemData の形式に合わせているのでそのまま渡す
  ...
  setlist.querySelectorAll(".setlist-slot").forEach((slot, index) => {
    ...
    slot.addEventListener('click', (e) => {
        ...
        if (checkbox) {
            ...
            const optionType = checkbox.dataset.optionType; // ★追加: どのタイプのチェックボックスか取得
            ...
            } else if (optionType === 'se') { // ★追加: SEチェックボックスの場合の処理
