...
function handleTouchStart(event) {
    ...
    if (tapLength < 300 && tapLength > 0) {
        ...
        lastTapTime = 0;
        isDragging = false; // ドラッグ状態もリセット
        event.preventDefault(); // ダブルタップ時はデフォルトのスクロールなどを防ぐ
    ...
    if (event.touches.length === 1) {
        ...
        // ここで touchTimeout を設定し、ロングプレス後にドラッグを開始する
        touchTimeout = setTimeout(() => {
            ...
            if (!isDragging && draggingItemId && document.body.contains(touchedElement)) {
                ...
                isDragging = true;
                // ドラッグ開始時にのみ preventDefault() を呼び出すことで、
                // それまでのスクロールを許可しつつ、ドラッグ中のスクロールを禁止
                event.preventDefault(); // ここで初めて呼び出す
                console.log("[touchstart:Mobile] Dragging initiated after timeout and preventDefault called.");
            } else {
