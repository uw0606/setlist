...
function processDrop(draggedElement, dropTargetSlot, originalSetlistSlot = null) {
    ...
    if (dropTargetSlot) {
        ...
        if (isDraggedFromSetlist && dropTargetSlot.dataset.slotIndex === originalSetlistSlot.dataset.slotIndex) {
            ...
            // visibility hidden になっている元のスロットを再表示し、データを復元
            if (originalSetlistSlot) {
                // originalSetlistSlot._originalItemData を使ってデータを復元
                fillSlotWithItem(originalSetlistSlot, originalSetlistSlot._originalItemData); // ★ここで元のデータを復元
        ...
        if (isDraggedFromSetlist) {
            ...
            if (dropTargetSlot.classList.contains('setlist-item')) {
                ...
                if (targetSlotItemData) {
                    // clearSlotContent(setlist, originalSetlistSlot.dataset.slotIndex); // ここは不要、fillSlotWithItemで上書きされる
                    fillSlotWithItem(originalSetlistSlot, targetSlotItemData); // 元のスロットにターゲットのアイテムを入れる
                } else {
                     clearSlotContent(setlist, originalSetlistSlot.dataset.slotIndex); // ターゲットにデータがない場合は元をクリア
                }
            ...
            // アルバムリスト内の元のアイテムを非表示にする (createTouchDraggedCloneで既に処理されているはずだが念のため)
...
function handleTouchStart(event) {
    ...
    if (closestCheckbox) {
        ...
        isDragging = false; // チェックボックス操作時はドラッグ状態をリセット
        return;
    }
    ...
    // ここで event.preventDefault() を呼ばないことで、ブラウザのdblclickイベントに任せることができる。
    // ただし、モバイルではdblclickが発火しないことが多いため、ここではカスタムのダブルタップ検出を行う。
    // ダブルタップの場合はドラッグ開始処理を完全にスキップするため、ここでreturnする。
    if (tapLength < 300 && tapLength > 0) {
        ...
        lastTapTime = 0;
        isDragging = false; // ダブルタップ処理が完了したのでドラッグフラグはリセット
        draggingItemId = null;
        currentTouchDraggedClone = null;
        originalSetlistSlot = null;
        currentDropZone = null;
        finishDragging(); // ダブルタップ処理後もクリーンアップ
        return; // ダブルタップ処理が実行されたら、ここで関数を終了
    }
    lastTapTime = currentTime;

    // --- 2. シングルタッチからの長押し（ドラッグ開始）処理 ---
    // ここでevent.preventDefault()を呼ぶことで、長押しが開始される可能性がある場合は
    // その後のクリックイベントやデフォルトのスクロールを全て阻止する
    // ただし、セットリスト内のアイテムの場合、長押しドラッグを許可しつつ、意図しない「長押し削除」を防ぐ
    event.preventDefault(); 
    console.log("[touchstart] Prevented default for single touch (potential drag/long press).");
    ...
    if (event.touches.length === 1) {
        ...
        if (isInSetlist || isInAlbumContent) {
            // ここで originalSetlistSlot を設定し、元のデータを保存
            originalSetlistSlot = isInSetlist ? touchedElement : null;
            if (originalSetlistSlot) {
                // セットリストからのドラッグの場合、元のデータを一時的に保存
                originalSetlistSlot._originalItemData = getSlotItemData(originalSetlistSlot);
                console.log("[touchstart] Original setlist slot data saved for potential restore.");
            }
            
            console.log(`[touchstart] Setting long press timer for drag. From setlist: ${!!originalSetlistSlot}`);

            touchTimeout = setTimeout(() => {
                // タイマー発火時に、まだドラッグ中でなく、指が移動閾値を超えていない場合のみドラッグ開始
                const currentTouch = event.touches[0]; // setTimeout内ではevent.touchesが空になる可能性があるので注意
                // 代わりに、handleTouchMoveでtouchStartX, touchStartYと比較する
                const currentX = currentTouch ? currentTouch.clientX : touchStartX;
                const currentY = currentTouch ? currentTouch.clientY : touchStartY;
                ...
                if (!isDragging && draggingItemId && document.body.contains(initialTouchElement) && 
                    ...
                    console.warn("[touchstart] Dragging not initiated after timeout (condition unmet or already dragging, or moved too much).");
                    // 長押し失敗時はクリーンアップ
                    finishDragging();
                }
            ...
            // ドラッグ対象外の要素なら処理を終了
            finishDragging(); // ここでも念のためクリーンアップ
            return;
        }
    }
}
...
function handleTouchMove(event) {
    ...
    // isDraggingがfalseの間は、スクロールを防ぎつつ、長押しタイマーがキャンセルされる条件を監視する
    // isDraggingがtrueになったら、クローン移動とドロップゾーンハイライトに専念する
    event.preventDefault(); 
    ...
    rafId = requestAnimationFrame(() => {
        ...
        if (!isDragging) {
            ...
            // 閾値を超えたら長押しタイマーをキャンセルする
            if (!originalSetlistSlot) { // originalSetlistSlot が設定されていない (アルバムからのドラッグ)
                const deltaX = Math.abs(currentX - touchStartX);
                const deltaY = Math.abs(currentY - touchStartY);
        ...
        // ここでの判定は、originalSetlistSlotがまだvisible:hiddenの状態であることを前提とする
...
function handleTouchEnd(event) {
    if (touchTimeout) {
        clearTimeout(touchTimeout);
        touchTimeout = null;
        console.log("[handleTouchEnd] Cleared touchTimeout.");
    }

    if (!isDragging) {
        // ドラッグが開始されなかった場合（短タップや移動が閾値未満の場合）
        // ここでは finishDragging を呼ぶ必要はないかもしれないが、念のためクリーンアップ
        // ただし、ダブルタップ処理が優先されるため、このパスはあまり通らないはず
        console.log("[handleTouchEnd] isDragging is false. No active drag to finish.");
        finishDragging(false); // ドラッグは成功しなかった
        return;
    }

    // ドラッグがアクティブだった場合
    console.log("[handleTouchEnd] isDragging is true. Processing drop or finishing drag.");
    const touch = event.changedTouches[0];
    if (!touch || !currentTouchDraggedClone) {
        console.warn("[handleTouchEnd] No touch or clone found on touch end. Finishing drag unsafely.");
        finishDragging(false); // ドロップ失敗
        return;
    }
    ...
    const targetElement = document.elementFromPoint(currentX, currentY);
    const dropZone = targetElement ? targetElement.closest('.setlist-slot') : null;

    if (dropZone) {
        console.log("[handleTouchEnd] Dropped on a setlist slot.");
        processDrop(dropZone); // processDrop内でfinishDragging(true)が呼ばれる
    } else {
        console.log("[handleTouchEnd] Dropped outside setlist slots. Restoring original item.");
        // セットリスト外にドロップされた場合
        finishDragging(false); // ドロップ失敗
    }
}
