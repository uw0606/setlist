...
function getSlotItemData(element) {
    ...
    if (isSetlistItem) {
        ...
        // Song name can be directly from dataset for consistency or parsed from text node
        ...
        // Tunings, BPM, Chorus are always from dataset for setlist items
        ...
        // Album items don't have checked states initially
        isCheckedShort = false;
        isCheckedSe = false;
        isCheckedDrumsolo = false;

        // Tunings, BPM, Chorus are always from dataset for album items
        rGt = element.dataset.rGt || '';
        lGt = element.dataset.lGt || '';
        bass = element.dataset.bass || '';
        bpm = element.dataset.bpm || '';
    ...
    return {
        ...
        itemId: itemId,
        slotIndex: element.dataset.slotIndex, // Ensure slotIndex is captured if present
        rGt: rGt,
        lGt: lGt,
        bass: bass,
        bpm: bpm,
...
function clearSlotContent(parentList, slotIndex) {
    ...
    if (slotToClear) {
        ...
        // Remove all relevant data attributes
        ...
        slotToClear.style.cssText = ''; // Reset inline styles
        slotToClear.style.visibility = ''; // Ensure visibility is normal
        ...
        slotToClear.removeEventListener("dblclick", handleDoubleClick); // Also remove double click

        slotToClear.draggable = false; // Not draggable when empty
...
function restoreToOriginalList(elementToProcess, fromSetlistOnly = false) {
    ...
    if (!itemId) {
        ...
        // If it's a floating clone and has no item ID, just remove it
    ...
    const wasOriginallyInAlbum = albumItemInMenu && albumItemInMenu.style.visibility === 'hidden'; // Check if it was hidden by being in setlist

    if (albumItemInMenu && !fromSetlistOnly) { // If it's an album item and we're not restricting to setlist only
...
function fillSlotWithItem(slotElement, songData) {
  ...
  // Clear existing album-specific classes
  ...
  if (infoParts.length > 0) {
      ...
      additionalInfoDiv.style.display = ''; // Ensure it's visible if content exists
  } else {
      additionalInfoDiv.style.display = 'none'; // Hide if no info
  }
  ...
  slotElement.classList.add('setlist-item', 'item'); // Add base classes
  if (albumClass) {
    slotElement.classList.add(albumClass); // Add album specific class
  }
  ...
  slotElement.removeEventListener("dblclick", handleDoubleClick); // Ensure double click is removed too
...
function handleDragStart(event) {
  ...
  if (!originalElement || !originalElement.dataset.itemId) {
    ...
    event.preventDefault(); // Prevent default drag behavior if no valid item
    return;
  }
  ...
  if (setlist.contains(originalElement) && originalElement.classList.contains('setlist-item')) {
    ...
    // Dragging from album list
    originalSetlistSlot = null; 
    currentPcDraggedElement = originalElement; // For PC, this is the original album item
  ...
  // Add 'dragging' class to the original element for PC drag (if it stays in DOM)
  // For setlist items, it's the hidden originalSetlistSlot. For album items, it's currentPcDraggedElement.
...
function handleTouchMove(event) {
    ...
    rafId = requestAnimationFrame(() => {
        ...
        if (!touch) { // Sometimes touch can be null during rapid movements
            rafId = null;
            return;
        }
...
function handleTouchEnd(event) {
    // Checkbox click will have already returned in handleTouchStart
    // If touchTimeout is active, it means it was a tap or short press, not a drag.
    if (touchTimeout) {
        clearTimeout(touchTimeout);
        touchTimeout = null;
        console.log("[touchend] Touch was a tap/short press, not a drag. Clearing timeout and returning.");
        isDragging = false; // Ensure dragging flag is false
        // No need for finishDragging here as no drag was initiated
        return;
    }
...
function createTouchDraggedClone(originalElement, initialX, initialY, itemIdToClone) {
    ...
    if (setlist.contains(originalElement) && originalElement.classList.contains('setlist-item')) {
        ...
        // アルバムリストからのアイテムの場合
        // アルバムアイテム自体はDOMに残っており、隠す必要がない場合が多いが、
        // ドラッグ開始時に非表示にして、ドロップ後に再表示することで、移動感を出す
        // ただし、これはPCドラッグの挙動と合わせるため。モバイルではクローンがメインなので元の要素は隠さない方が自然な場合もある。
        // ここではPCの挙動に寄せる
...
// document.addEventListener("dblclick", handleDoubleClick); // 各要素にリスナーが追加されるため、このグローバルリスナーはコメントアウト
...
function enableDragAndDrop(element) {
    if (element.classList.contains('item')) {
        ...
        // ここでは dragstart, touchstart, touchmove, touchend, touchcancel, dblclick は追加しない
        // これらは fillSlotWithItem で slot が setlist-item になった時に追加されるべき
...
async function generateSetlistPdf() {
    ...
    try {
        ...
        detailedPdf.autoTable({
            ...
            styles: {
                font: 'NotoSansJP',
                fontStyle: 'normal', // Body content can be normal
                fontSize: 10,
                cellPadding: 2,
                lineColor: [0, 0, 0],
                lineWidth: 0.3,
                textColor: [0, 0, 0],
                valign: 'middle' // Vertically align text in middle of cell
            },
            headStyles: {
            ...
            // This function allows for custom row height calculation based on content
            // However, with autoTable, it typically handles this automatically unless specified
            // rowPageBreak: 'avoid' might be useful here for short rows
        });
        ...
        const simpleFontSize = 32; // さらに大きく
        ...
        simplePdfBody.forEach(line => {
            ...
            simpleYPos += simpleFontSize * 0.7; // 行の高さを調整
