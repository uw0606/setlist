...
function clearSlotContent(parentList, slotIndex) {
    console.log("[DEBUG] clearSlotContent fired for slotIndex:", slotIndex, "StackTrace:", new Error().stack);
...
function processDrop(draggedElement, dropTargetSlot, originalSetlistSlot = null) {
    console.log("[DEBUG] processDrop fired. draggedElement:", draggedElement, "dropTargetSlot:", dropTargetSlot, "originalSetlistSlot:", originalSetlistSlot);
...
// main.js の handleTouchStart 関数

const DOUBLE_TAP_DELAY = 300; // ダブルタップと認識する時間（ミリ秒）
const LONG_PRESS_DELAY = 500; // 長押しと認識する時間（ミリ秒）

function handleTouchStart(event) {
    console.log("[DEBUG] handleTouchStart fired. target:", event.target);

    // チェックボックスのクリックの場合は、長押しやドラッグの処理をスキップ
    const closestCheckbox = event.target.closest('input[type="checkbox"]');
    if (closestCheckbox) {
        console.log("[touchstart] Checkbox clicked. Skipping drag/double-tap logic.");
        isDragging = false; // 念のためドラッグ状態をリセット
        if (touchTimeout) {
    ...
    if (tapLength < DOUBLE_TAP_DELAY && tapLength > 0) {
        ...
        if (touchTimeout) { // 長押しタイマーが動いていたらクリア
            clearTimeout(touchTimeout);
            touchTimeout = null;
        }
        isDragging = false; // ダブルタップの場合はドラッグではない
        handleDoubleClick(event); // ダブルタップとして処理
        lastTapTime = 0; // ダブルタップ検出のためのタイマーをリセット
        return; // ダブルタップとして処理したので、これ以上は進まない
    } else {
        lastTapTime = currentTime;
    }

    // ここからが長押しドラッグの開始ロジック
    // 既存のisDraggingがtrueの場合、多重処理を避ける
    if (isDragging) {
        console.log("[touchstart] Already dragging, ignoring new touchstart.");
        return;
    }

    const targetElement = event.currentTarget; // element.addEventListener で設定された要素
    // 現在のイベントターゲットがセットリストスロット内のアイテム、またはアルバムアイテムであることを確認
    const setlistItem = targetElement.closest('.setlist-item');
    const albumItem = targetElement.closest('.album-content .item');

    let elementToDrag = null;
    let itemIdToClone = null;

    if (setlistItem) {
        elementToDrag = setlistItem;
        itemIdToClone = setlistItem.dataset.itemId;
        console.log(`[touchstart] Setlist item touch detected: ${itemIdToClone}`);
    } else if (albumItem) {
        elementToDrag = albumItem;
        itemIdToClone = albumItem.dataset.itemId;
        console.log(`[touchstart] Album item touch detected: ${itemIdToClone}`);
    } else {
        // ドラッグ対象外の要素がタッチされた場合
        console.log("[touchstart] Non-draggable element touched. Skipping long press detection.");
        if (touchTimeout) {
            clearTimeout(touchTimeout);
            touchTimeout = null;
        }
    ...
    touchTimeout = setTimeout(() => {
        ...
        // クローン作成とドラッグ開始
        createTouchDraggedClone(elementToDrag, event.touches[0].clientX, event.touches[0].clientY, itemIdToClone);
    }, LONG_PRESS_DELAY);

    // デフォルトのブラウザジェスチャー（スクロールなど）を防ぐ
    // ただし、passive: false が設定されていても、ブラウザが無視する可能性がある
    // event.preventDefault(); // ここで無条件に呼ぶと問題になる場合がある
    // 長押しが確定するまでpreventDefaultは遅らせるか、touch-action CSSプロパティで対応
}
...
function handleTouchMove(event) {
    ...
    rafId = requestAnimationFrame(() => {
        ...
        // 現在のクローンの中心点ではなく、複数の点でスロットをチェック
        // または、setlist-slot要素全体を走査して重なりを判定する

        // オプション1: 複数の点でのチェック（簡易的）
        // const pointsToCheck = [
        //     {x: newX, y: newY}, // 中心
        //     {x: newX - cloneRect.width/4, y: newY},
        //     {x: newX + cloneRect.width/4, y: newY},
        //     {x: newX, y: newY - cloneRect.height/4},
        //     {x: newX, y: newY + cloneRect.height/4}
        // ];
        // for (const p of pointsToCheck) {
        //     const el = document.elementFromPoint(p.x, p.y);
        //     if (el) {
        //         newDropZone = el.closest('.setlist-slot');
        //         if (newDropZone) break;
        //     }
        // }

        // オプション2: すべてのセットリストスロットを走査し、クローンとの重なりを判定（より確実）
        const setlistSlots = document.querySelectorAll('.setlist-slot');
        for (const slot of setlistSlots) {
            if (slot === originalSetlistSlot && originalSetlistSlot.classList.contains('placeholder-slot')) {
                // 元のスロットがプレースホルダー状態の場合は、ドロップゾーンとして扱わない
                continue;
            }
            const slotRect = slot.getBoundingClientRect();
            // クローンとスロットが重なっているか判定
            if (newX - cloneRect.width / 2 < slotRect.right &&
                newX + cloneRect.width / 2 > slotRect.left &&
                newY - cloneRect.height / 2 < slotRect.bottom &&
                newY + cloneRect.height / 2 > slotRect.top) {
                newDropZone = slot;
                break; // 最初に見つかった重なっているスロットをドロップゾーンとする
            }
        }
        // --- ★ここまで修正★ ---

        if (originalSetlistSlot && newDropZone && newDropZone.dataset.slotIndex === originalSetlistSlot.dataset.slotIndex) {
            // 元のスロットにドラッグバックした場合、ハイライトを解除
            if (currentDropZone) {
                currentDropZone.classList.remove('drag-over');
            }
            currentDropZone = null;
...
function handleTouchEnd(event) {
    ...
    // --- ★ここから修正★ ---
    // すべてのセットリストスロットを走査し、ドロップされた座標との重なりを判定
    const setlistSlots = document.querySelectorAll('.setlist-slot');
    for (const slot of setlistSlots) {
        if (slot === originalSetlistSlot && originalSetlistSlot.classList.contains('placeholder-slot')) {
            // 元のスロットがプレースホルダー状態の場合は、ドロップターゲットとして扱わない
            continue;
        }
        const slotRect = slot.getBoundingClientRect();
        if (dropX > slotRect.left && dropX < slotRect.right &&
            dropY > slotRect.top && dropY < slotRect.bottom) {
            dropTargetSlot = slot;
            break;
        }
    }
    ...
    // ドロップターゲットが元のスロット（非プレースホルダー状態）の場合、処理をスキップ
    if (dropTargetSlot === originalSetlistSlot && !originalSetlistSlot.classList.contains('placeholder-slot')) {
        console.log("[touchend] Dropped back to the original (non-placeholder) slot. No change needed.");
    } else {
...
function createTouchDraggedClone(originalElement, initialX, initialY, itemIdToClone) {
    console.log("[DEBUG] createTouchDraggedClone fired. originalElement:", originalElement);
    if (currentTouchDraggedClone) {
    ...
    if (setlist.contains(originalElement) && originalElement.classList.contains('setlist-item')) {
        ...
        originalSetlistSlot.classList.add('placeholder-slot');
        // ★ここを修正/追加★
        // 元のスロットの"中身"は残しつつ、見た目だけを一時的に隠す
        // originalElement.style.visibility = 'hidden'; // ← 完全に削除されていることを確認
        // 代わりに、元のスロットの中身を透明にするか、薄くするCSSクラスを適用する
        // または、一時的に中身を非表示にするが、スロット自体は残す
        const originalContent = originalElement.querySelector('.item-content'); // 例: スロット内の表示部分
        if (originalContent) {
            originalContent.style.opacity = '0'; // 中身だけ透明にする
            // originalContent.style.visibility = 'hidden'; // これでも良いが、opacityの方がCSSで制御しやすい
        }
        console.log(`[createTouchDraggedClone] Original setlist slot ${originalSetlistSlot.dataset.slotIndex} marked as placeholder and its content hidden.`);
    } else {
        // アルバムリストからのアイテム
...
console.trace("[DEBUG] createTouchDraggedClone call stack");
...
function finishDragging() {
  ...
  if (originalSetlistSlot && originalSetlistSlot.classList.contains('placeholder-slot')) {
      ...
      // ★ここを修正/追加★
      const originalContent = originalSetlistSlot.querySelector('.item-content'); // 例: スロット内の表示部分
      if (originalContent) {
          originalContent.style.opacity = ''; // 元のopacityに戻す
          // originalContent.style.visibility = ''; // 必要ならこれも戻す
      }
      console.log(`[finishDragging] Removed 'placeholder-slot' class from ${originalSetlistSlot.dataset.slotIndex} and restored content visibility.`);
  ...
  setlist.querySelectorAll('.setlist-slot').forEach(slot => {
    ...
    slot.style.opacity = ''; // 念のため、他の場所で設定された opacity をリセット
  });
  ...
  draggingItemId = null;
  isDragging = false;
  originalSetlistSlot = null; // ここで originalSetlistSlot をクリアすることが重要
...
// main.js の handleDoubleClick 関数

function handleDoubleClick(event) {
    // 既にドラッグ中の場合は何もしない (これは通常、上記修正で回避されるはずですが念のため)
    if (isDragging) {
        console.log("[handleDoubleClick] Dragging is active, ignoring double click.");
        return;
    }

    const slot = event.currentTarget; // ダブルクリックされたスロット
    console.log(`[handleDoubleClick] Double clicked on slot: ${slot.dataset.slotIndex}`);

    if (slot.classList.contains('setlist-item')) {
        // セットリストアイテムがダブルクリックされた場合のみ
        restoreToOriginalList(slot);
        console.log("[handleDoubleClick] Item restored to original list.");
    } else {
        console.log("[handleDoubleClick] Empty slot double clicked or non-setlist-item. No action.");
    }
    event.preventDefault(); // デフォルトのダブルクリック動作を防ぐ（テキスト選択など）
}
