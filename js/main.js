...
function handleTouchStart(event) {
    ...
    // ⭐修正1: ドラッグ対象ではない要素の場合、ここで処理を終了し、イベントのデフォルト動作（スクロールなど）を妨げない。
    // ハンバーガーメニューの空白部分や、セットリストの空スロットなどもこれに該当し、スクロール可能になる。
    if (!touchedElement) {
        console.log("[touchstart:Mobile] Touched non-draggable element. Allowing default behavior (e.g., scrolling).");
        return; // ここでは event.preventDefault() を呼ばない
    }

    // ダブルタップ検出ロジックはそのまま
    if (tapLength < 300 && tapLength > 0) { // 300msはダブルタップ判定の一般的な閾値
        if (touchTimeout) {
            clearTimeout(touchTimeout);
            touchTimeout = null;
        }
    ...
    if (event.touches.length === 1) {
        ...
        // ⭐修正2: タッチ開始地点の座標を記録 (移動量判定のため)
        initialTouchX = event.touches[0].clientX;
        initialTouchY = event.touches[0].clientY;

        // ⭐修正3: クローン作成時のオフセットを記録（クローンが指の真下に来るようにするため）
        const rect = touchedElement.getBoundingClientRect();
        touchStartX = event.touches[0].clientX - rect.left; // クローン内のXオフセット
        touchStartY = event.touches[0].clientY - rect.top;   // クローン内のYオフセット

        // ⭐修正4: setTimeout でのドラッグ開始ロジックを完全に削除
        // ドラッグは handleTouchMove での移動によってのみ開始されるようにする
    }
}
...
function handleTouchMove(event) {
    if (event.touches.length === 1) {
        ...
        if (!isDragging) { // まだドラッグが開始されていない場合
            ...
            // ⭐修正5: TAP_MOVE_THRESHOLD を超えたらドラッグ開始とみなす
            if (deltaX > TAP_MOVE_THRESHOLD || deltaY > TAP_MOVE_THRESHOLD) {
                ...
                if (elementToDrag && draggingItemId) {
                    ...
                    // クローン作成。クローンの左上隅が元の要素の左上隅と一致するように渡す
                    const rect = elementToDrag.getBoundingClientRect();
                    createTouchDraggedClone(elementToDrag, rect.left, rect.top, draggingItemId);
                    ...
                    // ここで return することで preventDefault() を呼ばない
                    return; 
                }
            } else {
        ...
        if (isDragging && currentTouchDraggedClone) {
            ...
            // ⭐修正6: クローンの位置を更新。指の絶対位置から、クローン作成時に計算したオフセットを引く
    ...
    // ⭐修正7: requestAnimationFrame が完了したら null に戻すのは、requestAnimationFrame のコールバック内で行う
    // rafId = null; // この行は削除またはコールバック内に移動
}
 ...
 */
function createTouchDraggedClone(originalElement, cloneLeft, cloneTop, itemIdToClone) { // 引数名を変更
    if (currentTouchDraggedClone) {
    ...
    // セットリスト内のアイテムだった場合、元のスロットを隠す (既存のロジックはそのまま)
    if (setlist.contains(originalElement) && originalElement.classList.contains('setlist-item')) {
        originalSetlistSlot = originalElement;
    ...
    // クローンの位置調整
    const rect = originalElement.getBoundingClientRect(); // ここでrectを取得
    currentTouchDraggedClone.style.position = 'fixed';
    currentTouchDraggedClone.style.zIndex = '10000';
    currentTouchDraggedClone.style.width = rect.width + 'px';  // 元要素の幅を使用
    currentTouchDraggedClone.style.height = rect.height + 'px'; // 元要素の高さを使用
    
    // ⭐修正8: 引数として受け取った cloneLeft, cloneTop をクローンの左上隅の座標として直接使用
    currentTouchDraggedClone.style.left = cloneLeft + 'px';
    currentTouchDraggedClone.style.top = cloneTop + 'px';
...
/**
 * 日のドロップダウンを更新する関数。
 * グローバルスコープに配置し、loadSetlistStateからもアクセス可能にする。
 */
const updateDays = () => {
    const setlistYear = document.getElementById('setlistYear');
    const setlistMonth = document.getElementById('setlistMonth');
          ...
          if (state && state.setlist) {
            ...
            if (state.setlistDate && setlistYear && setlistMonth && setlistDay) {
                ...
                if (dateParts.length === 3) {
                    ...
                    // updateDaysはグローバルスコープにあるため直接呼び出し
                    updateDays(); 
                ...
                // 日付がない場合、または要素が見つからない場合は今日の日付を設定
                console.log("[loadSetlistState] No date to restore or date select elements not found. Setting to today.");
                if (setlistYear && setlistMonth && setlistDay) {
                    ...
                    console.warn("[loadSetlistState] Date select elements not fully found for default date setting.");
                }
            }
            // 会場の復元
            if (setlistVenue) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const setlistYear = document.getElementById('setlistYear');
    const setlistMonth = document.getElementById('setlistMonth');
    // const setlistDay = document.getElementById('setlistDay'); // updateDays関数内で取得するため不要
    ...
    // デフォルトで今日の日付を選択状態にするロジックはloadSetlistStateに一本化されたため、ここから削除
