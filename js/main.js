...
function handleTouchStart(event) {
    ...
    // ⭐ 修正箇所: ドラッグ対象要素の特定をより厳密にする ⭐
    // ドラッグできるのは「.item」クラスの要素、または「.setlist-slot.setlist-item」クラスの要素のみです。
    let touchedElement = null; // まずは null で初期化

    // 1. アルバムの曲 (.item) をチェック
    const albumItem = event.target.closest(".item");
    if (albumItem) {
        touchedElement = albumItem;
    } else {
        // 2. セットリストのスロットをチェック (ただし、'setlist-item' クラスを持つものに限る)
    ...
    // ドラッグ対象ではない要素 (例: 空の setlist-slot、メニューの空白部分など) をタッチした場合は、
    ...
    // ダブルタップ検出ロジック (既存のまま)
    if (tapLength < 300 && tapLength > 0) { // 300ms はダブルタップ判定の一般的な閾値
        if (touchTimeout) {
            clearTimeout(touchTimeout);
            touchTimeout = null;
        }
        handleDoubleClick(event); // ダブルタップ処理
        lastTapTime = 0;
    ...
    if (event.touches.length === 1) { // シングルタッチのみを処理
        ...
        if (touchedElement.classList.contains('setlist-item')) { // isSetlistItem の代わりに直接クラスをチェック
