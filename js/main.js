...
function handleTouchMove(event) {
    if (event.touches.length !== 1) {
        ...
        isLongPressReady = false; // フラグもリセット
        return;
    }
    ...
    if (!isDragging) { // まだドラッグが開始されていない場合
        if (touchTimeout) { // 長押しタイマーがまだ動いている場合
            if (distance > DRAG_THRESHOLD) { // 長押し前に移動しすぎたらキャンセル
                clearTimeout(touchTimeout);
                touchTimeout = null;
                isLongPressReady = false; // フラグもリセット
                console.log("[handleTouchMove] Cleared long press timeout due to movement exceeding threshold before long press.");
                return; // ドラッグは開始しない（スクロールを許可）
            }
        }

        // ★追加: 長押し準備ができていれば、即座にドラッグを開始 (isDragging は touchstart で設定済み)
        if (isLongPressReady) {
            // isDragging は touchstart の setTimeout 内で既に true になっているはず
            // ここで改めてクローン作成は不要だが、念のため状態を確認
            if (!currentTouchDraggedClone && draggingItemId) { // クローンがまだないが、ドラッグアイテムがある場合
                console.warn("[handleTouchMove] Long press ready but clone not found. Attempting to create.");
                let elementToClone = null;
                if (originalSetlistSlot && originalSetlistSlot.dataset.itemId === draggingItemId) {
                    elementToClone = originalSetlistSlot;
                } else {
                ...
                if (elementToClone && document.body.contains(elementToClone)) {
                    createTouchDraggedClone(elementToClone, currentX, currentY, draggingItemId); // 現在の座標で作成
                    isDragging = true; // 念のため設定
                } else {
                    console.error("[handleTouchMove] Failed to create clone on move after long press ready.");
                    finishDragging(); // 問題発生のためクリーンアップ
                    return;
                }
            }

            if (isDragging && currentTouchDraggedClone) {
                event.preventDefault(); // 長押しでドラッグが確定したら、スクロールをブロック
                // 以降のクローン移動ロジックに進む
            } else {
                return; // 何らかの理由でドラッグが開始できない場合は処理しない
            }
        } else {
            // 長押し準備ができていない場合はスクロールを許可
            return;
        }
    }
