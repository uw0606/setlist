...
function getSlotItemData(element) {
    ...
    let hasDrumsoloOption = (element.dataset.drumsoloOption === 'true'); // `hasDrumsoloOption`に修正済み
    ...
    if (isSetlistItem) {
        // ★ここから修正★
        // セットリストアイテムの場合、まずカスタム入力の有無を確認する
        const customInput = element.querySelector('.custom-song-input');
        if (customInput) {
            // カスタム入力欄があれば、その値を取得する。値が空の場合はデフォルトに戻す。
            songName = customInput.value.trim() || '自由入力曲';
        } else {
            // カスタム入力欄がなければ、これまで通りデータ属性から取得
...
function updateSlotContent(slotElement, songName, options) {
    ...
    if (slotElement.dataset.itemId === 'album1-custom') {
        ...
        // 既存の曲名があれば、input要素のvalueとして設定
        // ただし、'自由入力曲'というデフォルト値は表示しない
        if (songName && songName !== '自由入力曲') {
            customNameInput.value = songName;
        }
        
        // 入力内容をデータ属性に保存するイベントリスナー
        ...
        // ダブルタップやドラッグと競合しないように、タッチイベントの伝播を停止
        customNameInput.addEventListener('touchstart', (e) => {
            e.stopPropagation();
        });
