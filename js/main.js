...
async function generateSetlistPdf() {
    ...
    try {
        ...
        // ★修正1: BASE_FONT_SIZEを100ptに戻す (行間優先のため)
        const BASE_FONT_SIZE = 100; 
        const MAX_SONG_FONT_SIZE = 60; // 曲の最大フォントサイズ（必要であればこの値を調整）
        const MAX_HEADER_FONT_SIZE = 40; // ★新規: タイトルの最大フォントサイズを40ptに設定
        ...
        const SPACE_BETWEEN_HEADER_AND_BODY = 5; // マージン調整
        ...
        if (headerText) {
            // ★修正2: ヘッダーサイズの初期値をMIN_FONT_SIZE + 4からスタート
            ...
            // ★修正3: MAX_HEADER_FONT_SIZE (40pt) を上限として適用し、大きく表示
            headerFontSize = Math.min(MAX_HEADER_FONT_SIZE, headerFontSize);
        ...
        if (numSimpleRows > 0) {
            ...
            // スムーズな行間推移ロジック
            // ★修正4: MIN_LINE_HEIGHT_FACTORを0.6に戻し、少ない曲数で最大の広げ幅を適用
