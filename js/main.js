...
const LONG_PRESS_DELAY = 300; // 長押しと判断する時間（ミリ秒） - 例: 300ms
...
function handleTouchStart(event) {
    ...
    if (event.touches.length === 1) {
        ...
        // このタイマーが発火するまでに一定距離以上動けば即ドラッグ、発火後に動けば長押しドラッグ
        touchTimeout = setTimeout(() => {
            console.log("[touchstart:Mobile] Long press delay passed. Ready for drag after move.");
            // ここで特に何かする必要はないが、フラグを立てるなどしても良い
            // 例: isLongPressReady = true;
        }, LONG_PRESS_DELAY);
...
function handleTouchEnd(event) {
    ...
    // ★新規追加: touchTimeout が残っていたらクリア
    if (touchTimeout) {
        clearTimeout(touchTimeout);
        touchTimeout = null;
        console.log("[handleTouchEnd:Mobile] Long press timer cleared.");
    }
