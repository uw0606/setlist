 ...
 */
// main.js の generateSetlistPdf 関数を一時的にこのコードに置き換えて試す
async function generateSetlistPdf() {
    showMessageBox("テストPDF生成中...");
    try {
        const { jsPDF } = window.jspdf;
        const pdf = new jsPDF('p', 'mm', 'a4');

        // ★重要: ここでフォントリストを確認！
        console.log("Registered Fonts:", pdf.getFontList()); 
        // この出力に 'jpfont' が含まれているか確認してください。

        pdf.setFont('jpfont', 'normal'); // これが機能するかどうかのテスト
        pdf.setFontSize(20);

        pdf.text('こんにちは、世界！', 10, 10); // 日本語テキスト
        pdf.text('This is a test.', 10, 30); // 英語テキスト

        pdf.save('test_japanese_pdf.pdf');
        showMessageBox("テストPDFを生成しました！");
    } catch (error) {
        console.error("テストPDF生成に失敗しました:", error);
        showMessageBox("テストPDF生成に失敗しました。コンソールを確認してください。");
    }
}
