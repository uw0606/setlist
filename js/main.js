...
function handleTouchStart(event) {
    ...
    if (closestCheckbox) {
        ...
        return; // チェックボックスの動作を妨げない
    }

    // タッチ開始時に最も近いドラッグ可能なアイテム（.item または .setlist-item）を取得
    const touchedElement = event.target.closest(".setlist-item") || event.target.closest(".item");
    if (!touchedElement) {
        console.warn("[touchstart] No draggable item found on touch start.");
        return;
    }

    initialTouchElement = touchedElement; // タッチ開始時の要素を記録
    draggingItemId = touchedElement.dataset.itemId;

    // ダブルタップ検出ロジック (既存を維持し、適切な場所で handleDoubleClick を呼び出す)
    if (tapLength < 300 && tapLength > 0) {
        if (touchTimeout) {
            clearTimeout(touchTimeout);
            touchTimeout = null;
        }
        event.preventDefault(); // ダブルタップの場合はブラウザのズームなどを防ぐ
        
        // ダブルクリックのイベント発生源によって処理を分岐
        const isInSetlist = setlist.contains(touchedElement);
        const isInAlbumContent = touchedElement.closest('.album-content'); 

        if (isInSetlist && touchedElement.classList.contains('setlist-item')) {
            // セットリスト内のアイテムをダブルクリック -> ハンバーガーメニューへ戻す
            console.log("[touchstart] Double tap on setlist item. Attempting to move to menu.");
            handleMoveItemToMenu(touchedElement); // セットリストからメニューへ移動する関数を呼び出す
        } else if (isInAlbumContent && touchedElement.classList.contains('item')) {
            // ハンバーガーメニュー内のアイテムをダブルクリック -> セットリストへ追加
            console.log("[touchstart] Double tap on album item. Attempting to add to setlist.");
            handleAddItemToSetlist(touchedElement); // メニューからセットリストへ追加する関数を呼び出す
        } else {
            console.warn("[touchstart] Double tap on unrecognized element. No action taken.");
        }
    ...
    if (event.touches.length === 1) {
        ...
        // タッチ開始要素がセットリスト内か、ハンバーガーメニュー内か判定
        const isInSetlist = setlist.contains(touchedElement);
        const isInAlbumContent = touchedElement.closest('.album-content');

        // ドラッグ開始に必要な情報をセット
        if (isInSetlist && touchedElement.classList.contains('setlist-item')) {
            originalSetlistSlot = touchedElement; // セットリスト内移動の可能性がある
            // 必要に応じてオリジナルのアイテムデータを保存 (getSlotItemData が定義されていれば)
            // originalSetlistSlot._originalItemData = getSlotItemData(originalSetlistSlot);
            console.log(`[touchstart] Dragging from setlist slot (potential setlist internal move).`);
        } else if (isInAlbumContent && touchedElement.classList.contains('item')) {
            originalSetlistSlot = null; // アルバムアイテムからのドラッグ (セットリストへの追加)
            console.log(`[touchstart] Dragging from album item (potential add to setlist).`);
        } else {
            console.warn("[touchstart] Touched element is not a recognized draggable item. Aborting.");
            return; // ドラッグ対象外の要素なら処理を終了
        }
        ...
        if (isInAlbumContent) {
            // ハンバーガーメニュー内のアイテムからのロングプレス -> セットリストへのドラッグを許可
            console.log("[touchstart] Album item touched. Setting long press timer for drag to setlist.");
            touchTimeout = setTimeout(() => {
                if (!isDragging && draggingItemId && document.body.contains(initialTouchElement)) {
                    ...
                    console.log("[touchstart] Dragging initiated from album item after timeout.");
                    // ドラッグ開始後はデフォルトのスクロールを抑制
                    event.preventDefault(); 
                } else {
                    console.warn("[touchstart] Dragging not initiated from album item after timeout.");
                }
                touchTimeout = null;
            }, 600); // 600ms でロングプレスと判定
        } else if (isInSetlist) {
            // セットリスト内のアイテムからのロングプレス -> セットリスト内でのドラッグを許可
            // セットリストからメニューに戻す長押しはここでは許可しない
            console.log("[touchstart] Setlist item touched. Setting long press timer for internal setlist drag.");
            touchTimeout = setTimeout(() => {
                if (!isDragging && draggingItemId && document.body.contains(initialTouchElement)) {
                    createTouchDraggedClone(initialTouchElement, touchStartX, touchStartY, draggingItemId);
                    isDragging = true; // ドラッグ開始！
                    console.log("[touchstart] Dragging initiated from setlist item (internal move) after timeout.");
                    // ドラッグ開始後はデフォルトのスクロールを抑制
                    event.preventDefault(); 
                } else {
                    console.warn("[touchstart] Dragging not initiated from setlist item after timeout.");
                }
                touchTimeout = null;
            }, 600);
        } else {
            // その他の場所でのタッチ開始はロングプレスドラッグを起動しない
            console.log("[touchstart] Unrecognized touch location, no long press timer set.");
        }
    }
}
...
/**
 * タッチ移動時の処理 (前回の修正を維持しつつ、ドラッグが始まっていない場合はスクロールを優先)
 */
function handleTouchMove(event) {
    ...
    rafId = requestAnimationFrame(() => {
        ...
        if (!isDragging) {
            // 移動距離が閾値を超えていたら、スクロール目的の移動と判断
            if (deltaX > DRAG_THRESHOLD || deltaY > DRAG_THRESHOLD) {
                if (touchTimeout) {
                    clearTimeout(touchTimeout);
                    touchTimeout = null; // ロングプレスをキャンセル
                    console.log("[handleTouchMove] Movement beyond threshold, cancelling long press for scroll.");
                }
                // この時点で event.preventDefault() は呼ばない。
                // ブラウザのネイティブスクロールが動作することを許可する。
            }
            rafId = null;
            return; // ドラッグが始まっていないので、これ以上処理しない
        }

        // ドラッグが開始された後 (isDraggingがtrue) は、
        // スクロールなどのデフォルト動作を抑制するために preventDefault() を呼び出す
        event.preventDefault(); 
        ...
        // ドロップゾーンとして有効なのは #setlist 内の .setlist-slot だけと仮定
...
function handleTouchEnd(event) {
    ...
    if (touchTimeout) {
        clearTimeout(touchTimeout);
        touchTimeout = null;
        console.log("[handleTouchEnd] Long press timer cleared because touch ended before drag.");
        // この時点で isDragging が false であれば、タップイベントだったと見なせる
        // しかし、ダブルクリックの判定を優先しているので、ここでは特別な処理は不要
    }

    if (isDragging) {
        event.preventDefault(); // ドラッグが完了した場合、デフォルト動作を抑制
        console.log("[handleTouchEnd] Dragging ended.");

        if (currentTouchDraggedClone) {
            currentTouchDraggedClone.remove(); // クローンを削除
            currentTouchDraggedClone = null;
        }
        if (currentDropZone) {
            currentDropZone.classList.remove('drag-over');
        ...
        if (endTouch) {
            ...
            // ドラッグ元とドロップ先の判断
            const draggedFromSetlist = originalSetlistSlot !== null;
            const draggedFromAlbum = !draggedFromSetlist && initialTouchElement && initialTouchElement.closest('.album-content');
            
            // ドロップ先がセットリストの有効なスロットか
            const dropTargetIsSetlistSlot = targetElement && targetElement.closest('.setlist-slot');

            if (draggedFromAlbum && dropTargetIsSetlistSlot) {
                // ハンバーガーメニューからのドラッグ＆ドロップ -> セットリストへ追加
                // このロジックはあなたの handleDrop で実装されているはず
                console.log("[handleTouchEnd] Album item dropped into setlist slot.");
                // 通常の handleDrop ロジックをトリガーするか、ここに直接処理を記述
                // 例: handleDrop(event, initialTouchElement.dataset.itemId, dropTargetIsSetlistSlot.dataset.slotIndex);
                // 仮で displayMessage を呼び出す
                displayMessage('曲をセットリストに追加しました。');
                
                // ここで実際にドラッグ＆ドロップで曲を追加する関数を呼び出す
                // 例: addSongToSetlist(draggingItemId, dropTargetIsSetlistSlot.dataset.slotIndex);
                // ただし、handleDrop関数にeventオブジェクトを渡す形式が多いので、それに合わせて調整してください。
                if (typeof handleDrop === 'function') {
                    // handleDrop関数に適切な引数を渡す必要があります
                    // 今回は仮で event オブジェクトを渡す形とします。
                    // handleDrop(event, draggingItemId, dropTargetIsSetlistSlot);
                    // より正確には、ドロップ時の詳細な情報を含む D&D Eventオブジェクトをシミュレートする必要がありますが、
                    // ここでは簡単な関数呼び出しに置き換えることを想定しています。
                    // 実際には、handleDropはdragover, dropイベントハンドラ内で呼ばれるので、
                    // ここで直接呼ぶのは適切ではないかもしれません。
                    // ドロップの処理を handleTouchEnd 内に直接書くか、新しいヘルパー関数を作る方が確実です。
                     handleDroppedItem(draggingItemId, dropTargetIsSetlistSlot); // 仮の関数名
                }

            } else if (draggedFromSetlist && dropTargetIsSetlistSlot) {
                // セットリスト内でのドラッグ＆ドロップ -> セットリスト内で移動
                // このロジックはあなたの handleDrop で実装されているはず
                console.log("[handleTouchEnd] Setlist item dropped within setlist.");
                // 例: moveSongWithinSetlist(originalSetlistSlot.dataset.slotIndex, dropTargetIsSetlistSlot.dataset.slotIndex);
                displayMessage('セットリスト内で曲を移動しました。');
                if (typeof handleDroppedItem === 'function') {
                    handleDroppedItem(draggingItemId, dropTargetIsSetlistSlot, originalSetlistSlot.dataset.slotIndex); // 仮の関数名、元のスロット情報も渡す
                }

            } else {
                // 不適切なドロップ、またはセットリストからメニューへのドラッグ（これは無効化したい）
                console.log("[handleTouchEnd] Invalid drop target or operation. Item returned to original place.");
                // 例えば、クローンが削除された後、元の位置に戻す処理が必要な場合
                if (draggedFromSetlist && originalSetlistSlot) {
                     // 元の要素を非表示にしていた場合は表示に戻すなど
                     originalSetlistSlot.style.opacity = '1';
                     originalSetlistSlot.classList.remove('dragging');
                }
                displayMessage('曲の移動はキャンセルされました。');
            }
        }
        ...
        initialTouchElement = null; // リセット
    } else {
        console.log("[handleTouchEnd] Tap/short press detected, not a drag.");
        // ここでショートタップの処理が必要な場合は追加
    }
}
