...
async function generateSetlistPdf() {
    ...
    // --- シンプルPDFのためのデータ再構築 ---
    // 連番を分離し、曲名部分と連番/パディング部分を分けて保存する
    ...
    try {
        ...
        if (numSimpleRows > 0) {
            // ★修正ポイント1: 行間調整ロジックの強化（少ない行でも広がるように）
            ...
            // A4縦全体に広げるため、MAX_LINE_HEIGHTを非常に大きく設定し、idealLineHeightが優先されるようにします。
            const MAX_LINE_HEIGHT_PIXEL = 50; // 最大行間を約50mmに設定（理想値優先のため）
            
            // 3. 採用する行間を決定: 理想値と最大値の小さい方。今回は理想値が優先されるはず。
            dynamicLineHeight = Math.min(idealLineHeight, MAX_LINE_HEIGHT_PIXEL);
