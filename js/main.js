...
// =============================================================================
// ★★★ UI操作 (アルバム/文字順ソート関連) - 修正済み ★★★
// =============================================================================

/**
 * アルバムの表示を切り替える。
 * ★修正: ハンバーガーメニューの開閉と、文字順ソートからの復帰に対応★
 * @param {number} albumNumber - 切り替えるアルバムのインデックス
 */
function toggleAlbum(albumNumber) {
    // グローバル変数 menu, menuButton, albumList, nameOrderList が外部で定義されている前提
    const targetContent = document.getElementById("album" + albumNumber);
    ...
    // 1. アルバムコンテンツの表示を切り替え
    document.querySelectorAll(".album-content").forEach(content => {
        if (content.id === "album" + albumNumber) {
            // アクティブな状態をトグル
            content.classList.toggle("active");
        } else {
            // 他のコンテンツは非アクティブに
    ...
    // 2. タブのアクティブ状態を切り替え (アルバムリスト内での表示用)
    document.querySelectorAll("#albumList li[class$='-tab']").forEach(tab => {
        const currentAlbumNum = tab.classList.value.match(/album(\d+)-tab/)?.[1];
        if (currentAlbumNum === String(albumNumber) && targetContent.classList.contains("active")) {
            // ターゲットのコンテンツがアクティブな場合のみタブもアクティブに
    ...
    // 3. ★メニューを開いている場合は閉じる★ (ハンバーガーメニューからの操作の場合)
    // 曲リストが表示された（=アクティブになった）ら、メニューを閉じる
    if (typeof menu !== 'undefined' && typeof menuButton !== 'undefined' && menu.classList.contains("open") && targetContent.classList.contains("active")) {
        menu.classList.remove("open");
        menuButton.classList.remove("open");
    }
    
    // 4. 文字順ソートが表示されていた場合、アルバム順に切り替える
    if (sortSelect && sortSelect.value !== 'album') {
    ...
    } else if (sortSelect && sortSelect.value === 'album') {
        // アルバム順が選択されている場合も、念のため表示状態を保証
        albumList.style.display = 'block';
        nameOrderList.style.display = 'none';
    }

    console.log(`[toggleAlbum] Album ${albumNumber} is now: ${targetContent.classList.contains('active') ? 'active' : 'inactive'}`);
}
 ...
 * ★修正: display: block が確実に適用されるように調整★
...
function toggleNameOrderContent(groupKey) {
    ...
    // 2. ターゲットのコンテンツとタブを表示/アクティブ化
    if (targetContent) {
        targetContent.style.display = 'block';
        if (targetTab) targetTab.classList.add('active');
    }
    
    console.log(`[toggleNameOrderContent] Switched to group: ${groupKey}`);
}
...
function generateNameOrderList() {
    ...
    // NAME_ORDER_GROUPSが定義されていることを前提とする
    NAME_ORDER_GROUPS.forEach(group => { 
        ...
        if (groupItems.length > 0) {
            ...
            tabLi.addEventListener('click', () => toggleNameOrderContent(groupKey)); 
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ★ グローバル変数 setlist, menu, menuButton が外部で定義されていることを前提とし、ここで取得・初期化
    ...
    if (sortSelect && albumList && nameOrderList) {
        ...
        sortSelect.addEventListener('change', (event) => {
            ...
            document.querySelectorAll('#albumList .album-content').forEach(c => c.classList.remove('active'));
            ...
            if (selectedOrder === 'album') {
                ...
                if(firstAlbumTab) {
                    ...
                    const albumNumber = firstAlbumTab.dataset.albumNumber || 1;
                    toggleAlbum(albumNumber); 
                }
