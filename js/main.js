...
function handleDragStart(event) {
    if (isDragging) return; // 既にドラッグ中なら無視
    ...
    draggingItemId = itemId; // ドラッグ中のアイテムIDをグローバルに保存

    // セットリスト内からのドラッグの場合、元のスロットを透明にする
    if (currentPcDraggedElement.classList.contains('setlist-item')) {
        originalSetlistSlot = currentPcDraggedElement; // originalSetlistSlotをセットリストアイテム自体に設定
        originalSetlistSlot.classList.add('placeholder-slot'); // 半透明のプレースホルダー
        originalSetlistSlot.style.visibility = 'hidden'; // 元のアイテムを非表示にする
    ...
    // dataTransferにデータをセット（この形式がdropイベントで取得されます）
    // ★ここが重要: itemid と originalListId を両方セットする★
    event.dataTransfer.setData("text/plain", itemId);
    event.dataTransfer.setData("text/originalListId", currentPcDraggedElement.dataset.originalListId || ''); 
    // console.log(`[dragstart] dataTransfer set with: ${itemId}`); // デバッグログは前回修正済み
}
...
function handleDrop(event) {
    ...
    console.log("[handleDrop] Drop event fired."); // ★ドロップイベント発火の確認ログ★
    ...
    const droppedItemId = event.dataTransfer.getData("text/plain"); // dragstartでセットしたデータを取得
    const originalListId = event.dataTransfer.getData("text/originalListId"); // 追加でセットしたデータを取得

    if (!droppedItemId) {
        console.error("[handleDrop] No item ID found in dataTransfer.");
        finishDragging(true);
        return;
    }
    ...
    const targetSlot = event.target.closest('.setlist-slot');
    if (!targetSlot) {
        console.warn("[handleDrop] No valid drop target slot found.");
        finishDragging(true);
        return;
    }
    ...
    // ドラッグ元がセットリスト内の場合（並び替え）
    if (originalSetlistSlot && originalSetlistSlot.classList.contains('setlist-item')) {
        const fromSlotIndex = parseInt(originalSetlistSlot.dataset.slotIndex);
        
        if (fromSlotIndex === targetSlotIndex) {
            console.log("[handleDrop] Dropped back into the same slot.");
            finishDragging(false); // キャンセルではないのでfalse
            return;
        }
        
        console.log(`[handleDrop] Reordering song from slot ${fromSlotIndex} to ${targetSlotIndex}.`);
        reorderSongInSetlist(fromSlotIndex, targetSlotIndex);

    } else { // ドラッグ元がアルバムの場合（新規追加）
        console.log(`[handleDrop] Adding new song ${droppedItemId} to slot ${targetSlotIndex}.`);
        // アルバムアイテムのデータを取得する関数を呼び出す
        const albumItem = document.querySelector(`.album-content .item[data-item-id="${droppedItemId}"]`);
        if (albumItem) {
            const songName = albumItem.dataset.songName; // またはalbumItem.textContent.trim()
            const songOptions = {
                albumId: albumItem.dataset.albumId, // アルバムIDを渡す
                // albumId がない場合は、itemId から抽出
                originalListId: albumItem.dataset.originalListId || droppedItemId.split('-')[0], 
                isShortVersion: albumItem.dataset.isShortVersion === 'true',
                hasSeOption: albumItem.dataset.hasSeOption === 'true',
                drumsoloOption: albumItem.dataset.drumsoloOption === 'true',
                rGt: albumItem.dataset.rGt || '',
                lGt: albumItem.dataset.lGt || ''
                // 他の必要なデータもここに追加
            };
            addSongToSlot(targetSlotIndex, songName, songOptions);
        } else {
            console.error(`[handleDrop] Album item with ID ${droppedItemId} not found.`);
        }
    }
    finishDragging(false); // ドロップが成功したのでキャンセルではない
}
