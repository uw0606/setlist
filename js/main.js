...
function initializeNameOrderView() {
    ...
    for (const key in groups) {
        ...
        groupContent.classList.add('name-order-group-content', 'hidden'); // デフォルト非表示
        
        groups[key].forEach(originalItem => {
            // ★ポイント: アルバムリストの既存のアイテムをクローンして使用する
            const clonedItem = originalItem.cloneNode(true);
            
            // クローンなので enableDragAndDrop を再設定
            // enableDragAndDrop(clonedItem); // ドラッグは元のアイテムで実行されるため不要な場合が多い
            // ただし、ドラッグ対象は .item なので、念のためD&Dを有効化しておきます。
            enableDragAndDrop(clonedItem); 
            
            // 文字順特有のクラスを追加（不要なパディング削除のためのCSSと連携）
            clonedItem.classList.add('name-order-item'); 
            
            groupContent.appendChild(clonedItem);
        });
        
        contentWrapper.appendChild(groupContent);
    }
