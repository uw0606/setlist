...
function handleTouchMove(event) {
    ...
    rafId = requestAnimationFrame(() => {
        ...
        // setlist内のすべてのスロットを取得
        const setlistSlots = Array.from(setlist.querySelectorAll('.setlist-slot'));
        let foundSlot = null;

        // タッチ座標が各スロットの境界内にあるかチェック
        for (const slot of setlistSlots) {
            const rect = slot.getBoundingClientRect();
            // スロットが非表示になっている場合はスキップ
            if (slot.style.visibility === 'hidden' && originalSetlistSlot !== slot) {
                continue;
            }
        ...
        newDropZone = foundSlot;

        // 元のスロットの上にいる場合はドロップゾーンにしない
...
function handleTouchEnd(event) {
    ...
    // --- 変更ここから ---
    // ドロップターゲットの検出を強化 (handleTouchMove と同様のロジック)
...
function finishDragging() {
  ...
  if (originalSetlistSlot && originalSetlistSlot.classList.contains('placeholder-slot')) {
      ...
      if (originalSetlistSlot.classList.contains('setlist-item')) { // ← この条件を維持
