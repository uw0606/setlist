...
function handleTouchStart(event) {
    ...
    // 要素がセットリスト内にあるか、アルバム（ハンバーガーメニュー）内にあるか
    ...
    if (tapLength < 300 && tapLength > 0) {
        ...
        // ダブルタップ処理が完了したので、ドラッグ関連の状態をリセットして関数を終了
        lastTapTime = 0;
        isDragging = false;
        draggingItemId = null;
    ...
    if (event.touches.length === 1) {
        ...
        // 既存のタイマーがあればクリア (ダブルタップでクリア済みだが念のため)
        if (touchTimeout) {
            clearTimeout(touchTimeout);
            touchTimeout = null;
        }
        ...
        // ここでロングプレスのタイマーを設定
        // セットリストからの長押しはD&Dのみ、アルバムからの長押しもD&Dのみ
        if (isInSetlist || isInAlbumContent) { // いずれかの draggable item であればタイマーを設定
            // originalSetlistSlot の設定はタイマーのコールバック内でなく、ここで確定させる
            ...
            touchTimeout = setTimeout(() => {
                ...
                // これがtrueの場合は、handleTouchMoveでDRAG_THRESHOLDを超えてisDraggingがtrueになった場合など。
                // ただし、今回はDRAG_THRESHOLDはisDraggingがfalseの場合にのみキャンセルされるので、
                // 基本的にはここでisDraggingがtrueになることはない想定。
                if (!isDragging && draggingItemId && document.body.contains(initialTouchElement)) {
                    ...
                    // event.preventDefault() は handleTouchMove に任せる
                } else {
