...
function createTouchDraggedClone(originalElement, initialX, initialY, itemIdToClone) {
    ...
    // ここで .setlist-item や .item など元の要素の重要なクラスを保持していることを確認
    // album1, album2 などのクラスも保持されるはずです。

    // ★修正ポイント1: クローンが常に曲名を表示するように調整
    // cloneNode(true) で内部構造はコピーされるはずですが、念のためテキストの色などを強制的に設定
    currentTouchDraggedClone.style.color = 'black'; // 曲名が見えるように色を設定（必要であれば）
    currentTouchDraggedClone.style.backgroundColor = 'white'; // 背景色を設定して見やすくする（必要であれば）
    currentTouchDraggedClone.style.border = '1px solid #ccc'; // 境界線をつけて見やすくする（必要であれば）
