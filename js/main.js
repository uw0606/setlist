...
function getSlotItemData(element) {
    ...
    // ★修正: updateSlotContent に合わせるためプロパティ名を変更
    let isShortVersion = false; 
    let hasSeOption = false;
    let drumsoloOption = false; // ★修正: updateSlotContent に合わせるためプロパティ名を変更
    ...
    if (isSetlistItem) {
        ...
        // ★修正: updateSlotContent に合わせるためプロパティ名を変更
        isShortVersion = element.dataset.isShortVersion === 'true';
        hasSeOption = element.dataset.hasSeOption === 'true';
        drumsoloOption = element.dataset.drumsoloOption === 'true'; // ★修正: updateSlotContent に合わせるためプロパティ名を変更
...
// Short有無
if (options.isShortVersion) { // この条件はそのまま
    const shortVersionCheckboxWrapper = createCheckboxWrapper('Short', options.short, (e) => {
        ...
        // ★修正: 最新のデータ状態をDOMから取得して再描画に使う
        const updatedSongData = getSlotItemData(slotElement); 
        updateSlotContent(slotElement, updatedSongData.name, updatedSongData); // 最新のデータを渡す
    });
...
// SE有無
if (options.hasSeOption) { // この条件はそのまま
...
// ドラムソロ有無
if (options.drumsoloOption) { // この条件はそのまま
