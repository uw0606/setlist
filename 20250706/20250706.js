...
async function generateSetlistPdf() {
    ...
    // PDF生成に必要なライブラリが読み込まれているか確認
    if (typeof window.jspdf === 'undefined' || typeof window.jspdf.jsPDF === 'undefined') {
        console.error("[generateSetlistPdf] jsPDF library is not loaded. Aborting PDF generation.");
        showMessage("PDFライブラリが読み込まれていません。開発者に連絡してください。", "error");
        return;
    }
    ...
    // --- フォント登録関数を定義 ---
    const registerJapaneseFont = (doc) => {
        // グローバル変数として定義されたフォントデータが存在するかチェック
        if (typeof NotoSansJP_normal_js_b64 === 'undefined') {
            console.error("NotoSansJP-normal.js font data not found. Aborting font registration.");
            showMessage("日本語フォントデータが見つかりません。", "error");
            throw new Error("Japanese font data not found."); // エラーをスローしてPDF生成を中断
        }
        
        doc.addFileToVFS('NotoSansJP-normal.ttf', NotoSansJP_normal_js_b64);
