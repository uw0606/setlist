...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (openPastSetlistsModalButton && pastSetlistsModal && closePastSetlistsModalButton) {
        openPastSetlistsModalButton.addEventListener('click', () => {
            ...
            loadPastSetlists(); // モーダルが開くときに一覧をロード
            console.log("[DOMContentLoaded] openPastSetlistsModalButton listener added. loadPastSetlists will be called.");
        });
        ...
        console.log("[DOMContentLoaded] PastSetlistsModal listeners added.");
    }
    ...
    // 「セットリストを保存」ボタン
    const saveSetlistButton = document.getElementById('saveSetlistButton');
    if (saveSetlistButton) {
        saveSetlistButton.addEventListener('click', saveSetlist);
        console.log("[DOMContentLoaded] saveSetlistButton listener added.");
    }
    ...
    loadSetlistState().then(() => {
        ...
        if (!hasShareOrPastId) {
            ...
            if (setlistYear && setlistMonth && setlistDay) {
                ...
                setlistMonth.value = "07"; // 月は '07' のように2桁文字列で
                updateDays(); // 月と年を設定後、日のオプションを更新
                setlistDay.value = "06"; // 日は '06' のように2桁文字列で
                console.log(`[DOMContentLoaded] Default date set to 2025/07/06.`);
            }
            ...
            if (setlistVenue) {
                ...
                console.log(`[DOMContentLoaded] Default venue set to 東京ドーム.`);
            }

            // 初期曲として配置したいアイテムのIDリスト
            ...
            initialItemIds.forEach((itemId, index) => {
                const slot = document.querySelector(`.setlist-slot[data-slot-index="${index}"]`);
                if (slot && !slot.classList.contains('setlist-item')) {
                    // HTML上の元のアルバムアイテム要素を探す
                    const originalAlbumItem = document.querySelector(`.album-content .item[data-item-id="${itemId}"]`);
                    
                    if (originalAlbumItem) {
                        // originalAlbumItem から必要なデータを取得
                        const songData = getSlotItemData(originalAlbumItem);
                        
                        // fillSlotWithItem に渡す前に、チェックボックスの初期状態をfalseに設定
                        // getSlotItemData は `data-short` などの属性があればそれを読み込むが、
                        // アルバムアイテムには通常チェックボックスの状態がないため、falseで上書きするのが安全
                        songData.short = false;
                        songData.seChecked = false;
                        songData.drumsoloChecked = false;

                        fillSlotWithItem(slot, songData);
                        console.log(`[DOMContentLoaded] Preloaded item ${songData.name} (ID: ${itemId}) into slot ${index}.`);
                    } else {
                        console.warn(`[DOMContentLoaded] Original album item with ID ${itemId} not found. Cannot preload.`);
                    }
                }
            });
        }
