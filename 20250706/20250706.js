...
async function generateSetlistPdf() {
    ...
    // --- フォント登録関数を定義 ---
    // この関数はgenerateSetlistPdfのスコープ内で定義されるため、ReferenceErrorは発生しない
    const registerJapaneseFont = (doc) => {
        // jsPDFのvfsにフォントファイルを登録
        if (typeof NotoSansJP_normal_js_b64 === 'undefined') {
            console.error("NotoSansJP-normal.js font data not found. PDF may not display Japanese correctly.");
            return;
        }
