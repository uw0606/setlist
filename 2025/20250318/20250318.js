    ...
    loadSetlistState().then(() => {
        ...
        if (!hasShareOrPastId) {
            ...
            if (setlistYear && setlistMonth && setlistDay) {
                ...
                updateDays();
                setlistDay.value = "18";
                console.log(`[DOMContentLoaded] Default date set to 2025/03/18.`);
            }
            if (setlistVenue) {
                setlistVenue.value = "Zepp Sapporo 1日目";
                console.log(`[DOMContentLoaded] Default venue set to Zepp Sapporo 1日目.`);
            }
            ...
            const initialItems = [
                ...
                { itemId: "album15-030", options: {} }, 
                { itemId: "album11-009", options: {} }, 
                { itemId: "album1-002", options: {} }, 
                { itemId: "album10-008", options: {} }, 
                { itemId: "album15-019", options: { short: true, seChecked: true } }, 
                { itemId: "album6-006", options: {} }, 
                { itemId: "album4-006", options: { short: true} }, 
                { itemId: "album2-009", options: {} }, 
                { itemId: "album10-009", options: {} }, 
                { itemId: "album13-010", options: {} }, 
                { itemId: "album1-022", options: {} }, 
                { itemId: "album8-009", options: {} }, 
                { itemId: "album12-011", options: { short: true } }, 
                { itemId: "album14-006", options: {} }, 
                { itemId: "album12-001", options: {} }, 
                { itemId: "album14-011", options: {} }, 
                { itemId: "album11-003", options: { short: true } }, 
            ];
            ...
            initialItems.forEach((item, index) => {
                ...
                if (slot && !slot.classList.contains('setlist-item')) {
                    ...
                    if (originalAlbumItem) {
                        // オプションの有無をアルバムアイテムから直接取得
                        // ★ここを修正しました★
                        const songOptions = {
                            isShortVersion: originalAlbumItem.dataset.isShortVersion === 'true' || item.options.short === true,
                            hasSeOption: originalAlbumItem.dataset.hasSeOption === 'true' || item.options.seChecked === true,
                            drumsoloOption: originalAlbumItem.dataset.drumsoloOption === 'true' || item.options.drumsoloChecked === true,
                            rGt: originalAlbumItem.dataset.rGt,
                            lGt: originalAlbumItem.dataset.lGt,
                            bass: originalAlbumItem.dataset.bass,
                            bpm: originalAlbumItem.dataset.bpm,
                            chorus: originalAlbumItem.dataset.chorus,
                        };

                        // 初期設定のオプションでチェック状態を上書き
                        const mergedOptions = {
                            ...songOptions,
                            short: item.options.short || false,
                            seChecked: item.options.seChecked || false,
                            drumsoloChecked: item.options.drumsoloChecked || false,
                            name: item.options.name || originalAlbumItem.dataset.songName,
                        };

                        const albumClass = originalAlbumItem.closest('.album-content').id;
                        addSongToSlot(slot, item.itemId, mergedOptions.name, mergedOptions, albumClass);
                        console.log(`[DOMContentLoaded] Preloaded item ${mergedOptions.name} (ID: ${item.itemId}) into slot ${index}.`);
                    } else {
