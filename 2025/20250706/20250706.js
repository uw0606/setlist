...
async function generateSetlistPdf() {
    ...
    try {
        ...
        if (headerText) {
            ...
            detailedPdf.text(headerText, pageWidth / 2, detailedYPos + headerCellHeight / 4 + 0.5, { align: 'center', baseline: 'middle' });
        ...
        simplePdfBody.forEach(line => {
            ...
            const bottomMarginThreshold = simpleFontSize + 2;
    ...
    loadSetlistState().then(() => {
        ...
        if (!hasShareOrPastId) {
            ...
            if (setlistYear && setlistMonth && setlistDay) {
                ...
                setlistMonth.value = "07";
                updateDays();
                setlistDay.value = "06";
                console.log(`[DOMContentLoaded] Default date set to 2025/07/06.`);
            }
            if (setlistVenue) {
                setlistVenue.value = "Zepp Fukuoka（デビュー20周年）";
                console.log(`[DOMContentLoaded] Default venue set to Zepp Fukuoka.`);
            }

            // 初期曲として配置したいアイテムのIDリストとオプション
            const initialItems = [
                { itemId: "album1-013", options: { drumsoloChecked: true } }, 

                { itemId: "album9-005", options: {} }, 
                { itemId: "album12-005", options: {} },
                { itemId: "album14-003", options: {} }, 
                { itemId: "album11-007", options: {} }, 
                { itemId: "album12-006", options: {} }, 
                { itemId: "album13-006", options: {} }, 

                { itemId: "album1-002", options: {} }, 

                { itemId: "album1-custom", options: { name: "ONIGIRI (feat. UVERworld & CHANMINA) [Remix]" } }, 
                { itemId: "album15-032", options: {} }, 
                { itemId: "album11-009", options: {} }, 
                { itemId: "album12-011", options: { short: true } }, 
                { itemId: "album4-006", options: { short: true } }, 
                { itemId: "album14-010", options: {} }, 
                ...
                // { itemId: "album1-005", options: { short: true } }, // 'CHANCE!'をShortにチェック
                // { itemId: "album3-002", options: {} },  // デフォルト
                // { itemId: "album1-custom", options: { name: "帰謬" } } // '自由入力曲'の初期値を"帰謬"にする
            ];
