    ...
    loadSetlistState().then(() => {
        ...
        if (!hasShareOrPastId) {
            ...
            if (setlistYear && setlistMonth && setlistDay) {
                ...
                updateDays();
                setlistDay.value = "22";
                console.log(`[DOMContentLoaded] Default date set to 2024/02/22.`);
            }
            if (setlistVenue) {
                setlistVenue.value = "Zepp Fukuoka (克哉生誕祭)";
                console.log(`[DOMContentLoaded] Default venue set to Zepp Fukuoka (克哉生誕祭).`);
            }

            // 初期曲として配置したいアイテムのIDリストとオプション
            const initialItems = [
                { itemId: "album1-012", options: { drumsoloChecked: true } }, 
