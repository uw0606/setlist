...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loadSetlistState().then(() => {
        ...
        if (!hasShareOrPastId) {
            ...
            const setlistVenue = document.getElementById('setlistVenue');
            if (setlistYear && setlistMonth && setlistDay) {
                setlistYear.value = "2024";
                setlistMonth.value = "02";
                updateDays();
                setlistDay.value = "06";
                console.log(`[DOMContentLoaded] Default date set to 2024/02/06.`);
            }
            if (setlistVenue) {
                setlistVenue.value = "KT Zepp Yokohama";
                console.log(`[DOMContentLoaded] Default venue set to KT Zepp Yokohama.`);
            }

            // 初期曲として配置したいアイテムのIDリストとオプション
            const initialItems = [
                { itemId: "album1-custom", options: { name: "DJ誠果" } }, 
                { itemId: "album1-009", options: { drumsoloChecked: true } }, 

                { itemId: "album9-004", options: { short: true } }, 
                { itemId: "album5-007", options: { short: true } },
                ...
                { itemId: "album11-013", options: {} }, 
                { itemId: "album15-027", options: {} }, 
                { itemId: "album15-022", options: {} }, 
                { itemId: "album15-003", options: {} }, 
